...
namespace TestHelper {
	...
	public class TestHelp {
		...
		public static bool binaryCopy(IDataAccess dest, DataTable tSource,
			UpdateType tipoCopia = UpdateType.insertAndUpdate) {
		...
		/// <param name="tipoCopia"></param>
		public static void binaryCopy(IDataAccess source, IDataAccess dest, string tableName, q filter,
			UpdateType tipoCopia) {
		...
		static string[] setRegistryTable = new[] {
			"registry", "registryreference", "registrydurc", "registrysorting", "registrytaxablestatus",
			"registryaddress"
		};

		static string[] setMandateTable = new[] {
			"mandate", "mandatedetail", "mandatesorting", "mandatecig", "mandateavcp", "mandateavcpdetail",
			"mandateattachment"
		};

		static string[] setEstimateTable = new[]
			{"estimate", "estimatedetail", "estimatesorting", "estimateattachment"};

		static string[] setInvoiceTable = new[]
			{"invoice", "invoicedetail", "invoicedeferred", "invoiceattachment", "invoicesorting", "ivaregister"};

		static string[] setItinerationTable = new[] {
			"itineration", "itinerationrefund", "itinerationattachment", "itinerationauthagency", "itinerationsorting",
			"itinerationflights",
		...
		static string[] setProfServiceTable = new[] {
			...
			"profserviceavcp", "profserviceavcpdetail", "profservicecig", "profservicerefund", "profservicesorting",
			"profservicetax"
		};
		...
		static string[] setPettyCashOperationTable = new[] {
			...
			"pettycashoperationitineration", "pettycashoperationprofservice", "pettycashoperationsorted",
		...
		/// <param name="tipoCopia"></param>
		public static bool binaryCopySet(IDataAccess dest, DataSet dSource,
			UpdateType tipoCopia = UpdateType.insertAndUpdate) {
			var res = true;
		...
		static string[] setExpenseTable = new[] {
			"expense", "expensevar", "expenseyear", "expenselast", "expensebill", "expensecasualcontract",
			"expenseclawback",
		...
		public void cancellaMovimentoEntrataSingolo(object idinc) {
			...
			foreach (string tableName in new string[] {"banktransaction", "expensetax"}) {
				testConn.DO_UPDATE(tableName, testConn.compile(q.eq("idinc", idinc)), new[] {"idexp"}, new[] {"null"},
					1);
			}

			testConn.DO_UPDATE("invoicedetail", testConn.compile(q.eq("idinc_taxable", idinc)), new[] {"idinc_taxable"},
				new[] {"null"}, 1);
			testConn.DO_UPDATE("invoicedetail", testConn.compile(q.eq("idinc_iva", idinc)), new[] {"idinc_iva"},
				new[] {"null"}, 1);
			testConn.DO_UPDATE("estimatedetail", testConn.compile(q.eq("idinc_taxable", idinc)),
				new[] {"idinc_taxable"}, new[] {"null"}, 1);
			testConn.DO_UPDATE("estimatedetail", testConn.compile(q.eq("idinc_iva", idinc)), new[] {"idinc_iva"},
				new[] {"null"}, 1);
			testConn.DO_UPDATE("import_flow", testConn.compile(q.eq("id_inc", idinc)), new[] {"id_inc"}, new[] {"null"},
				1);
		}
		...
		public DataSet getEpData(IDataAccess conn, DataRow document) {
			...
			ds.Tables.Add(conn.CreateTableByName("epexptotal", "*"));
			ds.Tables.Add(conn.CreateTableByName("epacctotal", "*"));
		...
		public bool getEpMovForDocument(DataSet D, IDataAccess Conn, string idrelated) {
			...
			foreach (string table in new List<string>() {"epexp", "epacc"}) {
				...
				DataTable ttotal = D.Tables[table + "total"];
				...
				string queryTotColumns = DataTools.aliasColumns(ttotal, ttotal.TableName);
				string queryTot = "select " + queryTotColumns +
				                  " from " + table +
				                  " join " + ttotal.TableName + " on " + ttotal.TableName + "." + idmov + " = " +
				                  table + "." + idmov +
				                  //" and " + tvar.TableName + ".yvar = " + QHS.quote(esercizio) +
				                  " and " + filterAlias;
				DataTools.MergeRows(Conn as DataAccess, ttotal, queryTot);
				...
				string queryYear = "select " + queryYearColumns +
				                   ...
				                   " and " + filterAlias +
				                   $" order by {tyear.TableName}.ayear";
				...
				string queryEntryDetailsAccrualsColumns =
					DataTools.aliasColumns(tEntryDetailAccrual, tEntryDetailAccrual.TableName);
				string queryEntryDetailsAccruals = "select " + queryEntryDetailsAccrualsColumns +
				                                   ...
				                                   " and " + tEntryDetail.TableName + ".yentry = " +
				                                   testConn.Security.GetEsercizio();
		...
		public bool binaryCopyEp(DataRow r, bool rebuildEP = true) {
			...
			testConn.SQLRunner("Alter table entrydetail disable trigger all ");
			testConn.SQLRunner("Alter table epaccyear disable trigger all ");
			testConn.SQLRunner("Alter table epexpyear disable trigger all ");
			...
			testConn.SQLRunner("Alter table entrydetail enable trigger all ");
			testConn.SQLRunner("Alter table epaccyear enable trigger all ");
			testConn.SQLRunner("Alter table epexpyear enable trigger all ");
		...
		ProcedureMessageCollection generaCancellaImpegniScritture(DataRow r, bool cancella, bool autoignore) {
			...
			if (metaTableName == "mandate" && !ds.Tables.Contains("mandatedetail"))
				throw new Exception("Dettaglio contratti passivi mancante nel ds");
			if (metaTableName == "estimate" && !ds.Tables.Contains("estimatedetail"))
				throw new Exception("Dettaglio contratti attivi mancante nel ds");
			if (metaTableName == "invoice" && !ds.Tables.Contains("invoicedetail"))
				throw new Exception("Dettaglio fatture mancante nel ds");
			if (metaTableName == "assetunload" && !ds.Tables.Contains("assetamortizationunloadview"))
				throw new Exception("assetamortizationunloadview mancante nel ds");
			if (metaTableName == "grantload" && !ds.Tables.Contains("assetgrant"))
				throw new Exception("assetgrant mancante nel ds");
			if (metaTableName == "grantload" && !ds.Tables.Contains("assetgrantdetail"))
				throw new Exception("assetgrantdetail mancante nel ds");
			if (metaTableName == "assetload" && !ds.Tables.Contains("assetacquireview"))
				throw new Exception("assetacquireview mancante nel ds");
			if (metaTableName == "provision" && !ds.Tables.Contains("provisiondetail"))
				throw new Exception("provisiondetail mancante nel ds");
			if (metaTableName == "assetload" && !ds.Tables.Contains("assetacquireview"))
				throw new Exception("assetacquireview mancante nel ds");
		...
		public bool checkScrittura(DataRow doc, string codiceConto = null, string codiceUPB = null,
			string codiceCausale = null, q filterDetail = null) {
		...
		public bool checkImpegnoBudget(DataRow doc, bool withChild = false, bool preImpegno = false,
			...
			q filter = withChild
				? q.like("idrelated", idDoc + "%") | q.eq("idrelated", idDoc)
				: q.eq("idrelated", idDoc);
		...
		public void checkImportoInizialeImpBudget(DataTable dtEpExp, object idepexp, int anno,
			decimal importoIniziale) {

			q filterEpacc = q.eq("idepexp", idepexp);
			var EpaccRows = dtEpExp._Filter(filterEpacc);
			Assert.AreEqual(1, EpaccRows.Length, $"Esiste l'accertamento di id {idepexp}");
			DataRow rEpexp = EpaccRows[0];
			q filterEpaccyear = q.eq("idepexp", EpaccRows[0]["idepexp"]) & q.eq("ayear", anno);
			DataTable tEpExpyear = testConn.readTable("epexpyear", filterEpaccyear);
			Assert.AreEqual(1, tEpExpyear.Rows.Count,
				$"L'impegno {rEpexp["yepexp"]}/{rEpexp["nepexp"]} nell'anno {anno} esiste");
			DataRow rEpExpYear = tEpExpyear.Rows[0];

			Assert.AreEqual(importoIniziale, rEpExpYear["amount"],
				$"L'impegno {rEpexp["yepexp"]}/{rEpexp["nepexp"]} nell'anno {anno} ha l'importo {importoIniziale}");


		}

		public void checkImportoInizialeAccBudget(DataTable dtEpacc, object idepacc, int anno,
			decimal importoIniziale) {

			q filterEpacc = q.eq("idepacc", idepacc);
			var EpaccRows = dtEpacc._Filter(filterEpacc);
			Assert.AreEqual(1, EpaccRows.Length, $"Esiste l'accertamento di id {idepacc}");
			DataRow rEpacc = EpaccRows[0];
			q filterEpaccyear = q.eq("idepacc", EpaccRows[0]["idepacc"]) & q.eq("ayear", anno);
			DataTable tEpaccyear = testConn.readTable("epaccyear", filterEpaccyear);
			Assert.AreEqual(1, tEpaccyear.Rows.Count,
				$"L'accertamento {rEpacc["yepacc"]}/{rEpacc["nepacc"]} nell'anno {anno} esiste");
			DataRow rEpaccYear = tEpaccyear.Rows[0];

			Assert.AreEqual(importoIniziale, rEpaccYear["amount"],
				$"L'accertamento {rEpacc["yepacc"]}/{rEpacc["nepacc"]} nell'anno {anno}  ha l'importo {importoIniziale}");


		}

		public void checkImportoInizialeAccBudget(DataTable dtEpacc, int annoMov, int numMov, int anno,
			decimal importoIniziale) {
			StringBuilder sb = new StringBuilder();
			q filterEpacc = q.eq("yepacc", annoMov) & q.eq("nepacc", numMov) & q.eq("nphase", 2);
			var EpaccRows = dtEpacc._Filter(filterEpacc);
			Assert.AreEqual(1, EpaccRows.Length, $"L'accertamento {annoMov}/{numMov} esiste");
			q filterEpaccyear = q.eq("idepacc", EpaccRows[0]["idepacc"]) & q.eq("ayear", anno);
			DataTable tEpaccyear = testConn.readTable("epaccyear", filterEpaccyear);
			Assert.AreEqual(1, tEpaccyear.Rows.Count, $"L'accertamento {annoMov}/{numMov} nell'anno {anno} esiste");
			DataRow rEpaccYear = tEpaccyear.Rows[0];
			Assert.AreEqual(importoIniziale, rEpaccYear["amount"],
				$"L'accertamento {annoMov}/{numMov} nell'anno {anno}  ha l'importo {importoIniziale}");

		}



		public void checkImportoInizialeImpBudget(DataTable dtEpexp, int annoMov, int numMov, int anno,
			decimal importoIniziale) {
			StringBuilder sb = new StringBuilder();
			q filterEpexp = q.eq("yepexp", annoMov) & q.eq("nepexp", numMov) & q.eq("nphase", 2);
			var EpaccRows = dtEpexp._Filter(filterEpexp);
			Assert.AreEqual(1, EpaccRows.Length, $"L'impegno {annoMov}/{numMov}  esiste");
			q filterEpaccyear = q.eq("idepexp", EpaccRows[0]["idepexp"]) & q.eq("ayear", anno);
			DataTable tEpexpyear = testConn.readTable("epexpyear", filterEpaccyear);
			Assert.AreEqual(1, tEpexpyear.Rows.Count, $"L'impegno {annoMov}/{numMov} nell'anno {anno} esiste");
			DataRow rEpexpYear = tEpexpyear.Rows[0];
			Assert.AreEqual(importoIniziale, rEpexpYear["amount"],
				$"L'impegno {annoMov}/{numMov} nell'anno {anno}  ha l'importo {importoIniziale}");
		}

		public void checkImportoTotaleVarAccBudget(DataTable dtEpacc, int annoMov, int numMov,
			decimal totaleVariazioni, int annoVar, int numVar = 0) {
			StringBuilder sb = new StringBuilder();
			q filterEpacc = q.eq("yepacc", annoMov) & q.eq("nepacc", numMov) & q.eq("nphase", 2);
			var rowSEpacc = dtEpacc._Filter(q.eq("nepacc", numMov) & q.eq("nphase", 2));
			Assert.AreEqual(1, rowSEpacc.Length, "Esiste un accertamento");
			q filterEpaccvar;
			if (numVar != 0) {
				filterEpaccvar = q.eq("idepacc", rowSEpacc[0]["idepacc"]) & q.eq("yvar", annoVar) &
				                 q.eq("nvar", numVar);
			}
			else {
				filterEpaccvar = q.eq("idepacc", rowSEpacc[0]["idepacc"]) & q.eq("yvar", annoVar);
			}
			DataTable tEpaccvar = testConn.readTable("epaccvar", filterEpaccvar);
			decimal totale = 0;
			if (totaleVariazioni != 0 && tEpaccvar.Rows.Count>0) {
				
				Assert.AreEqual(1, tEpaccvar.Rows.Count,
					$"Esiste una variazione per l'accertamento {annoMov}/{numMov} nell'anno {annoVar}");
				DataRow rEpaccVar = tEpaccvar.Rows[0];
				//SOMMO LA SOMMA DEGLI AMOUNT PER AVERE IL TOTALE
				totale = CfgFn.GetNoNullDecimal(rEpaccVar["amount"]);
				for (int i = 2; i <= 5; i++) {
					totale += CfgFn.GetNoNullDecimal(rEpaccVar["amount" + i.ToString()]);
				}
			}
			...
			//COSTRUISCO IL MESSAGGIO DA RESTITUIRE
		
			Assert.AreEqual(totaleVariazioni, totale, $"Importo totale variazione accertamento {annoMov}/{numMov} nell'anno {annoVar}");
		}

		public void checkImportoTotaleVarImpBudget(DataTable dtEpexp, int annoMov, int numMov,
			decimal totaleVariazioni, int annoVar, int numVar = 0) {
			StringBuilder sb = new StringBuilder();
			q filterEpexp = q.eq("yepexp", annoMov) & q.eq("nepxp", numMov) & q.eq("nphase", 2);
			var dataRowsEpexp = dtEpexp._Filter(filterEpexp);
			Assert.AreEqual(1, dataRowsEpexp.Length, "Esiste un impegno");
			q filterEpexpvar;
			if (numVar != 0) {
				filterEpexpvar = q.eq("idepexp", dataRowsEpexp[0]["idepexp"]) & q.eq("yvar", annoVar) &
				                 q.eq("nvar", numVar);
			}
			else {
				filterEpexpvar = q.eq("idepexp", dataRowsEpexp[0]["idepexp"]) & q.eq("yvar", annoVar);
			...
			DataTable tEpexpvar = testConn.readTable("epexpvar", filterEpexpvar);
			Assert.AreEqual(1, tEpexpvar.Rows.Count, $"Esiste una variazione per l'impegno {annoMov}/{numMov} nell'anno {annoVar}");
			DataRow rEpexpVar = tEpexpvar.Rows[0];

			//SOMMO LA SOMMA DEGLI AMOUNT PER AVERE IL TOTALE
			decimal totale = CfgFn.GetNoNullDecimal(rEpexpVar["amount"]);
			for (int i = 2; i <= 5; i++) {
				totale += CfgFn.GetNoNullDecimal(rEpexpVar["amount" + i.ToString()]);
			}

			//COSTRUISCO IL MESSAGGIO DA RESTITUIRE
			Assert.AreEqual(totaleVariazioni, totale, $"Importo totale variazione impegno {annoMov}/{numMov} nell'anno {annoVar}");
		}
	...
	public class testResponder : DefaultResponder {
		...
		public override bool getResponse(IWin32Window ctrl, string text, string caption, MessageBoxButtons btns,
			out DialogResult result) {
			messagesReceived.Add(text);
			if (responsesToGive.Count > 0) {
				result = responsesToGive[0];
				responsesToGive.RemoveAt(0);
