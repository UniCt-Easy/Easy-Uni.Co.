...
select 
	S.ESERCIZIO as  'anno',
	convert(int,replace(SD.NUMERO_SCRITTURA,'T','')) as  'numero',
	SD.NUMERO_MOVIMENTO	as  'ndett',
	S.TIPO_DOCUMENTO_AMM + S.ESERCIZIO_DOCUMENTO_AMM + S.NUMERO_DOCUMENTO_AMM as  'doc',
	NULL as  'datadoc',
	coalesce(SD.DATA_INIZIO_PERIODO,
				SD.DaTA_FINE_PERIODO,  
				CONVERT(datetime, '31-12-' + CONVERT(char(4),S.ESERCIZIO), 105))
				 as  'datacontabile',
...
and S.ATTIVA is null
order by S.esercizio,convert(int,replace(SD.NUMERO_SCRITTURA,'T','')) ,SD.numero_movimento
End
