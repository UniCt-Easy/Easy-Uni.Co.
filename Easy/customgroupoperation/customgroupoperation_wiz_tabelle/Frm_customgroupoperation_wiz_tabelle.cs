...
namespace customgroupoperation_wiz_tabelle//wizard_customgroupoperation_tabelle//
{
	...
	/// </summary>
	public class Frm_customgroupoperation_wiz_tabelle : System.Windows.Forms.Form
	{
		...
		void SetCustomObjectColumnCaptions(){
			foreach (DataColumn C in DS.customobject)
				C.Caption="";
			DS.customobject.objectnameColumn.Caption="Nome tabella/vista";
		}
		...
		bool CustomChangeTab(int oldTab, int newTab) {
			...
			if ((oldTab==1)&&(newTab==2)) {
				...
				if (SelectedTables==null || SelectedTables.Rows.Count==0){
					MessageBox.Show(this, "Non è stata selezionata alcuna tabella", "Errore");
					CanSave=false;
					return false;
				}
				CanSave=true;
			...
			foreach (Control Ctrl in grpOpTabelle.Controls){
				...
				foreach (DataRow GruppiRow in SelectedGroups.Rows){
					foreach (DataRow TableRow in SelectedTables.Rows){
						...
						// Se l'operazione e diversa dalla selezione e l'oggetto è una vista
						// passa avanti
		...
		private void btnNext_Click(object sender, System.EventArgs e) {
			if (tabController.SelectedIndex==tabController.TabPages.Count-1 && CanSave){
				if (ContaOperazioni()==0){
					MessageBox.Show(this, "E' necessario specificare almeno una operazione di tabella",
						"Errore");
					return;
				}
				SaveData();
