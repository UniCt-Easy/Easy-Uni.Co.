...
parentRow.extension = "tru";
				if (this.state.isSearchState()) {
					this.helpForm.filter($('#istanza_tru_seg_idreg_studenti'), null);
				} else {
					this.helpForm.filter($('#istanza_tru_seg_idreg_studenti'), this.q.eq('registry_active', 'Si'));
				}
				if (this.state.isSearchState()) {
					this.helpForm.filter($('#istanza_tru_seg_idreg_istituti'), null);
				} else {
					this.helpForm.filter($('#istanza_tru_seg_idreg_istituti'), this.q.eq('active', 'S'));
				}
				//beforeFillFilter
			...
			afterClear: function () {
				//parte sincrona
				this.enableControl($('#istanza_tru_seg_idreg_studenti'), true);
				this.helpForm.filter($('#istanza_tru_seg_idreg_studenti'), null);
				this.enableControl($('#istanza_tru_seg_iddidprog'), true);
				this.helpForm.filter($('#istanza_tru_seg_idreg_istituti'), null);
				this.enableControl($('#istanza_tru_seg_protnumero'), true);
				this.enableControl($('#istanza_tru_seg_protanno'), true);
				//afterClearin
			...
			afterFill: function () {
				this.enableControl($('#istanza_tru_seg_iddidprog'), false);
			...
			afterLink: function () {
				var self = this;
				this.state.addExtraEntity('istanza_tru');
				this.state.DS.tables.istanza.defaults({ 'aa': this.getAAByDate() });
				this.state.DS.tables.istanza.defaults({ 'data': new Date() });
				this.state.DS.tables.istanza.defaults({ 'extension': "tru" });
				this.state.DS.tables.istanza.defaults({ 'idistanzakind': 8 });
			...
			afterRowSelect: function (t, r) {
				...
				$('#istanza_tru_seg_idreg_studenti').prop("disabled", (this.state.isEditState() || this.haveChildren()) && this.state.currentRow.idreg_studenti);
				$('#istanza_tru_seg_idreg_studenti').prop("readonly", (this.state.isEditState() || this.haveChildren()) && this.state.currentRow.idreg_studenti);
				...
				if (t.name === "registrystudentiview" && r !== null) {
					...
					if (this.state.DS.tables.iscrizioneattiveviewdefaultview.rows.length)
						if (this.state.DS.tables.iscrizioneattiveviewdefaultview.rows[0].idreg !== r.idreg) {
							...
							$('#istanza_tru_seg_idiscrizione').val('');
						}
				}
