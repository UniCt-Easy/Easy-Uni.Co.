...
(function () {
    ...
    function metaPage_dichiar() {
		MetaPage.apply(this, ['dichiar', 'disabil_seg', false]);
        this.name = 'Dichiarazione di disabilità';
		this.defaultListType = 'disabil_seg';
        ...
        new MetaPage(),
        {
			...
			beforeFill: function () {
				...
				if (!parentRow.iddichiarkind)
					parentRow.iddichiarkind = 4;
				//beforeFillFilter
				
				//parte asincrona
				var def = appMeta.Deferred("beforeFill-dichiar_disabil_disabil_seg");
				var arraydef = [];
			...
			afterFill: function () {
				this.enableControl($('#dichiar_disabil_seg_protnumero'), false);
				this.enableControl($('#dichiar_disabil_seg_protanno'), false);
				//afterFillin
			...
			afterRowSelect: function (t, r) {
				var def = appMeta.Deferred("afterRowSelect-dichiar_disabil_disabil_seg");
				$('#dichiar_disabil_seg_idreg').prop("disabled", this.state.isEditState() || this.haveChildren());
				$('#dichiar_disabil_seg_idreg').prop("readonly", this.state.isEditState() || this.haveChildren());
				if (t.name === 'registrystudentiview' && r !== null)
					if (this.state.DS.tables['dichiar_disabil'].rows.length)
						this.state.DS.tables['dichiar_disabil'].rows[0].idreg = r.idreg;
				//afterRowSelectin
				return def.resolve();
			},
			...
			insertClick: function (that, grid) {
				if (!$('#dichiar_disabil_seg_idreg').val() && this.children.includes(grid.dataSourceName)) {
			...
			firebtnProtocol: function (that) {
				...
				var arrayTablesToProtocol = ['dichiar', 'dichiar_disabil'];
	...
	window.appMeta.addMetaPage('dichiar', 'disabil_seg', metaPage_dichiar);
