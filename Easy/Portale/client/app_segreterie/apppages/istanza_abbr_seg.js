...
;
				...
				//parte asincrona
				var def = appMeta.Deferred("afterGetFormData-istanza_abbr_abbr_seg");
				var arraydef = [];
				
				arraydef.push(this.manageistanza__abbr_seg_idcorsostudio());
			...
			beforeFill: function () {
				...
				if (this.state.isSearchState()) {
					this.helpForm.filter($('#istanza_abbr_seg_idreg_studenti'), null);
				} else {
					this.helpForm.filter($('#istanza_abbr_seg_idreg_studenti'), this.q.eq('registry_active', 'Si'));
				}
				//beforeFillFilter
				...
				if (dt.rows.length === 0) {
					...
					var defistanza_abbr = meta.getNewRow(parentRow.getRow(), dt, self.editType).then(
						function (currentRowabbr) {
							currentRowabbr.current.idistanzakind = 4;
			...
			afterClear: function () {
				//parte sincrona
				this.enableControl($('#istanza_abbr_seg_idreg_studenti'), true);
				this.helpForm.filter($('#istanza_abbr_seg_idreg_studenti'), null);
				this.enableControl($('#istanza_abbr_seg_idiscrizione'), true);
				this.enableControl($('#istanza_abbr_seg_iddidprog'), true);
				this.enableControl($('#istanza_abbr_seg_protnumero'), true);
				this.enableControl($('#istanza_abbr_seg_protanno'), true);
			...
			afterFill: function () {
				this.enableControl($('#istanza_abbr_seg_iddidprog'), false);
			...
			afterLink: function () {
				var self = this;
				this.state.addExtraEntity('istanza_abbr');
				this.state.DS.tables.istanza.defaults({ 'aa': this.getAAByDate() });
				this.state.DS.tables.istanza.defaults({ 'data': new Date() });
				this.state.DS.tables.istanza.defaults({ 'extension': "abbr" });
				this.state.DS.tables.istanza.defaults({ 'idistanzakind': 4 });
			...
			afterRowSelect: function (t, r) {
				var def = appMeta.Deferred("afterRowSelect-istanza_abbr_abbr_seg");
				$('#istanza_abbr_seg_idreg_studenti').prop("disabled", (this.state.isEditState() || this.haveChildren()) && this.state.currentRow.idreg_studenti);
				$('#istanza_abbr_seg_idreg_studenti').prop("readonly", (this.state.isEditState() || this.haveChildren()) && this.state.currentRow.idreg_studenti);
				if (t.name === 'registrystudentiview' && r !== null)
					if (this.state.DS.tables['istanza_abbr'].rows.length)
						this.state.DS.tables['istanza_abbr'].rows[0].idreg = r.idreg;
				if (t.name === "registrystudentiview" && r !== null) {
					...
					if (this.state.DS.tables.iscrizionedefaultview.rows.length)
						if (this.state.DS.tables.iscrizionedefaultview.rows[0].idreg !== r.idreg) {
							...
							$('#istanza_abbr_seg_idiscrizione').val('');
						}
				}
				$('#istanza_abbr_seg_idiscrizione').prop("disabled", (this.state.isEditState() || this.haveChildren()) && this.state.currentRow.idiscrizione);
				$('#istanza_abbr_seg_idiscrizione').prop("readonly", (this.state.isEditState() || this.haveChildren()) && this.state.currentRow.idiscrizione);
				if (t.name === 'iscrizionedefaultview' && r !== null)
					if (this.state.DS.tables['istanza_abbr'].rows.length)
						this.state.DS.tables['istanza_abbr'].rows[0].idiscrizione = r.idiscrizione;
				if (t.name === "iscrizionedefaultview" && r !== null) {
					...
					if (this.state.DS.tables.didprogdefaultview.rows.length)
						if (this.state.DS.tables.didprogdefaultview.rows[0].iddidprog !== r.iddidprog) {
							...
							$('#istanza_abbr_seg_iddidprog').val('');
						}
				}
				if (t.name === "registrystudentiview" && r !== null) {
					...
					if (this.state.DS.tables.iscrizionedefaultview_alias1.rows.length)
						if (this.state.DS.tables.iscrizionedefaultview_alias1.rows[0].idreg !== r.idreg) {
							...
							$('#istanza_abbr_seg_idiscrizione_from').val('');
						}
				}
				if (t.name === "iscrizionedefaultview" && r !== null) {
			...
			insertClick: function (that, grid) {
				if (!$('#istanza_abbr_seg_idreg_studenti').val() && this.children.includes(grid.dataSourceName)) {
					return this.showMessageOk('Prima devi selezionare un valore per il campo Studente');
				}
				if (!$('#istanza_abbr_seg_idiscrizione').val() && this.children.includes(grid.dataSourceName)) {
			...
			if (this.state.DS.tables.iscrizionedefaultview.rows.length) {
				...
				this.state.DS.tables.istanza_abbr.rows[0].iddidprog = this.state.DS.tables.iscrizionedefaultview.rows[0].iddidprog;
				this.state.DS.tables.istanza_abbr.rows[0].idcorsostudio = this.state.DS.tables.iscrizionedefaultview.rows[0].idcorsostudio;
			}
			return def.resolve();
			},

			manageistanza__abbr_seg_idcorsostudio: function () {
				var def = appMeta.Deferred("beforeFill-manageistanza__abbr_seg_idcorsostudio");
				var self = this;
