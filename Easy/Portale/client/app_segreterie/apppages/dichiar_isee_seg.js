...
(function () {
    ...
    function metaPage_dichiar() {
		MetaPage.apply(this, ['dichiar', 'isee_seg', false]);
        this.name = 'Attestazione ISEE';
		this.defaultListType = 'isee_seg';
    ...
    metaPage_dichiar.prototype = _.extend(
        new MetaPage(),
        {
			...
			beforeFill: function () {
				...
				//parte asincrona
				var def = appMeta.Deferred("beforeFill-dichiar_isee_isee_seg");
				var arraydef = [];
			...
			afterFill: function () {
				...
				this.enableControl($('#dichiar_isee_seg_protnumero'), false);
				//afterFillin
			...
			insertClick: function (that, grid) {
				if (!$('#dichiar_isee_seg_idreg').val() && this.children.includes(grid.dataSourceName)) {
	...
	window.appMeta.addMetaPage('dichiar', 'isee_seg', metaPage_dichiar);
