    ...
    metaPage_istanza.prototype = _.extend(
        new MetaPage(),
        {
			...
			afterGetFormData: function () {
				...
				//parte asincrona
				var def = appMeta.Deferred("afterGetFormData-istanza_imm_imm_seganagstu");
				var arraydef = [];
			...
			afterClear: function () {
				appMeta.metaModel.addNotEntityChild(this.getDataTable('istanza'), this.getDataTable('istanza_imm_alias1'));
				appMeta.metaModel.addNotEntityChild(this.getDataTable('istanza'), this.getDataTable('istanzadichiar'));
				appMeta.metaModel.addNotEntityChild(this.getDataTable('istanza'), this.getDataTable('nullaosta_imm_alias1'));
				appMeta.metaModel.addNotEntityChild(this.getDataTable('istanza'), this.getDataTable('diniego_alias1'));
				//afterClearin
			},
			...
			afterRowSelect: function (t, r) {
				...
				$('#istanza_imm_seganagstu_iddidprog').prop("disabled", this.state.isEditState() || this.haveChildren());
				$('#istanza_imm_seganagstu_iddidprog').prop("readonly", this.state.isEditState() || this.haveChildren());
				$('#istanza_imm_seganagstu_aa').prop("disabled", this.state.isEditState() || this.haveChildren());
				$('#istanza_imm_seganagstu_aa').prop("readonly", this.state.isEditState() || this.haveChildren());
				...
				if (t.name === "didprogdefaultview" && r !== null) {
					...
					var istanza_imm_seganagstu_iddidprogcurrCtrl = $('#istanza_imm_seganagstu_iddidprogcurr').data("customController");
					arraydef.push(istanza_imm_seganagstu_iddidprogcurrCtrl.filteredPreFillCombo(window.jsDataQuery.eq("iddidprog", r ? r.iddidprog : null), null, true)
						.then(function (dt) {
							if (self.state.currentRow && self.state.currentRow.iddidprogcurr)
								istanza_imm_seganagstu_iddidprogcurrCtrl.fillControl(null, self.state.currentRow.iddidprogcurr);
							return true;
						})
);
				}
			...
			afterActivation: function () {
				...
				if (parentRow.iddidprog) {
					appMeta.metaModel.cachedTable(this.getDataTable("didprogcurr"), false);
					var istanza_imm_seganagstu_iddidprogcurrCtrl = $('#istanza_imm_seganagstu_iddidprogcurr').data("customController");
					arraydef.push(istanza_imm_seganagstu_iddidprogcurrCtrl.filteredPreFillCombo(window.jsDataQuery.eq("iddidprog", parentRow.iddidprog), null, true));
				}
			...
			children: ['diniego_alias1', 'istanza_alias14', 'istanzadichiar', 'nullaosta_alias1'],
