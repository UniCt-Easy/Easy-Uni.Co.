  ...
  $.widget("jb.dragtable", {
		...
		_create: function() {
			...
			el.delegate(o.items, 'mousedown.' + self.widgetEventPrefix, function(e){
				...
				//if ($(e.target).attr('id') && ($(e.target).attr('id').toString().startsWith('icon_sort_id') ||
				//	$(e.target).attr('id').toString().startsWith('btngridexport_id') )) return;
				if ($(e.target).get(0).constructor.name !=='HTMLTableCellElement') return
				var $handle = $(this),
		...
		_mousemoveHandler: function( e ){
            ...
            $( document ).bind('mousemove.' + self.widgetEventPrefix, function( e ){
               ...
               if( e.pageX  < prevMouseX ){
                   ...
                   if( left  < threshold ) self._swapCol(self.startIndex-1);
