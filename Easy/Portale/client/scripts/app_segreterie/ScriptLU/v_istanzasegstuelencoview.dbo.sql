...
-- CREAZIONE VISTA istanzasegstuelencoview
IF EXISTS(select * from sysobjects where id = object_id(N'[dbo].[istanzasegstuelencoview]') and OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW [dbo].[istanzasegstuelencoview]
GO

CREATE VIEW [dbo].[istanzasegstuelencoview] AS 
SELECT  istanza.aa, istanza.ct AS istanza_ct, istanza.cu AS istanza_cu, istanza.data AS istanza_data, istanza.extension AS istanza_extension, istanza.idcorsostudio, istanza.iddidprog, istanza.idiscrizione AS istanza_idiscrizione, istanza.idistanza,
 [dbo].istanzakind.title AS istanzakind_title, istanza.idistanzakind,
