...
CREATE VIEW [dbo].[iscrizioneseganagstumastview] AS 
SELECT  iscrizione.aa, iscrizione.anno AS iscrizione_anno, iscrizione.ct AS iscrizione_ct, iscrizione.cu AS iscrizione_cu, iscrizione.data AS iscrizione_data, iscrizione.idcorsostudio,
 ...
 isnull('Master: ' + [dbo].didprog.title + '; ','') + ' ' + isnull('Master: ' + [dbo].didprog.aa + '; ','') + ' ' + isnull('Anno accademico: ' + iscrizione.aa + '; ','') as dropdown_title
FROM [dbo].iscrizione WITH (NOLOCK) 
LEFT OUTER JOIN [dbo].didprog WITH (NOLOCK) ON iscrizione.iddidprog = [dbo].didprog.iddidprog
WHERE  iscrizione.idcorsostudio  in (select cs.idcorsostudio from corsostudio cs where cs.idcorsostudiokind = 2 /*master*/) AND iscrizione.idcorsostudio IS NOT NULL  AND iscrizione.iddidprog IS NOT NULL  AND iscrizione.idiscrizione IS NOT NULL  AND iscrizione.idreg IS NOT NULL 
GO
