...
CREATE VIEW [assetdiaryorasegview] AS SELECT  assetdiaryora.amount AS assetdiaryora_amount, assetdiaryora.ct AS assetdiaryora_ct, assetdiaryora.cu AS assetdiaryora_cu, assetdiaryora.idassetdiary, assetdiaryora.idassetdiaryora, sal.start AS sal_start, sal.stop AS sal_stop, assetdiaryora.idsal AS assetdiaryora_idsal, assetdiaryora.idworkpackage, assetdiaryora.lt AS assetdiaryora_lt, assetdiaryora.lu AS assetdiaryora_lu, assetdiaryora.start AS assetdiaryora_start, assetdiaryora.stop AS assetdiaryora_stop FROM assetdiaryora WITH (NOLOCK)  LEFT OUTER JOIN sal WITH (NOLOCK) ON assetdiaryora.idsal = sal.idsal WHERE  assetdiaryora.idassetdiary IS NOT NULL  AND assetdiaryora.idassetdiaryora IS NOT NULL  AND assetdiaryora.idworkpackage IS NOT NULL 
GO

-- CREAZIONE VISTA assetdiaryseganagview
IF EXISTS(select * from sysobjects where id = object_id(N'[assetdiaryseganagview]') and OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW [assetdiaryseganagview]
GO

CREATE VIEW [assetdiaryseganagview] AS SELECT  assetdiary.ct AS assetdiary_ct, assetdiary.cu AS assetdiary_cu, assetdiary.idasset AS assetdiary_idasset, assetdiary.idassetdiary, assetacquire.description AS assetacquire_description, assetacquire.idinv AS assetacquire_idinv, asset.idasset AS asset_idasset, asset.idpiece AS asset_idpiece, inventory.description AS inventory_description, asset.ninventory AS asset_ninventory, asset.rfid AS asset_rfid, assetdiary.idpiece AS assetdiary_idpiece, progetto.titolobreve AS progetto_titolobreve, assetdiary.idprogetto, assetdiary.idreg AS assetdiary_idreg, workpackage.raggruppamento AS workpackage_raggruppamento, workpackage.title AS workpackage_title, assetdiary.idworkpackage, assetdiary.lt AS assetdiary_lt, assetdiary.lu AS assetdiary_lu, assetdiary.orepreventivate AS assetdiary_orepreventivate, isnull('Descrizione Descrizione: ' + assetacquire.description + '; ','') + ' ' + isnull('Class. Inv. Descrizione: ' + CAST( assetacquire.idinv AS NVARCHAR(64)) + '; ','') + ' ' + isnull('Descrizione Inventario: ' + inventory.description + '; ','') as dropdown_title FROM assetdiary WITH (NOLOCK)  LEFT OUTER JOIN asset WITH (NOLOCK) ON assetdiary.idasset = asset.idasset AND assetdiary.idpiece = asset.idpiece LEFT OUTER JOIN assetacquire WITH (NOLOCK) ON asset.nassetacquire = assetacquire.nassetacquire LEFT OUTER JOIN inventory WITH (NOLOCK) ON asset.idinventory = inventory.idinventory LEFT OUTER JOIN progetto WITH (NOLOCK) ON assetdiary.idprogetto = progetto.idprogetto LEFT OUTER JOIN workpackage WITH (NOLOCK) ON assetdiary.idworkpackage = workpackage.idworkpackage WHERE  assetdiary.idassetdiary IS NOT NULL  AND assetdiary.idworkpackage IS NOT NULL 
GO

-- CREAZIONE VISTA assetdiarysegview
IF EXISTS(select * from sysobjects where id = object_id(N'[assetdiarysegview]') and OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW [assetdiarysegview]
GO

CREATE VIEW [assetdiarysegview] AS SELECT  assetdiary.ct AS assetdiary_ct, assetdiary.cu AS assetdiary_cu, assetdiary.idasset AS assetdiary_idasset, assetdiary.idassetdiary, assetacquire.description AS assetacquire_description, assetacquire.idinv AS assetacquire_idinv, asset.idasset AS asset_idasset, asset.idpiece AS asset_idpiece, inventory.description AS inventory_description, asset.ninventory AS asset_ninventory, asset.rfid AS asset_rfid, assetdiary.idpiece, assetdiary.idprogetto AS assetdiary_idprogetto, [dbo].registry.title AS registry_title, assetdiary.idreg AS assetdiary_idreg, assetdiary.idworkpackage, assetdiary.lt AS assetdiary_lt, assetdiary.lu AS assetdiary_lu, assetdiary.orepreventivate AS assetdiary_orepreventivate, isnull('Descrizione Descrizione: ' + assetacquire.description + '; ','') + ' ' + isnull('Class. Inv. Descrizione: ' + CAST( assetacquire.idinv AS NVARCHAR(64)) + '; ','') + ' ' + isnull('Diario d''uso per : ' + CAST( asset.idasset AS NVARCHAR(64)) + '; ','') + ' ' + isnull('Diario d''uso per : ' + CAST( asset.idpiece AS NVARCHAR(64)) + '; ','') + ' ' + isnull('Diario d''uso per : ' + CAST( asset.ninventory AS NVARCHAR(64)) + '; ','') + ' ' + isnull('Diario d''uso per : ' + asset.rfid + '; ','') + ' ' + isnull('da parte di : ' + [dbo].registry.title + '; ','') + ' ' + isnull('per un totale di ore pari a : ' + CAST( assetdiary.orepreventivate AS NVARCHAR(64)) + '; ','') + ' ' + isnull('Descrizione Inventario: ' + inventory.description + '; ','') as dropdown_title FROM assetdiary WITH (NOLOCK)  LEFT OUTER JOIN asset WITH (NOLOCK) ON assetdiary.idasset = asset.idasset AND assetdiary.idpiece = asset.idpiece LEFT OUTER JOIN assetacquire WITH (NOLOCK) ON asset.nassetacquire = assetacquire.nassetacquire LEFT OUTER JOIN inventory WITH (NOLOCK) ON asset.idinventory = inventory.idinventory LEFT OUTER JOIN [dbo].registry WITH (NOLOCK) ON assetdiary.idreg = [dbo].registry.idreg WHERE  assetdiary.idassetdiary IS NOT NULL  AND assetdiary.idworkpackage IS NOT NULL 
GO
...
CREATE VIEW [flowchartsegview] AS SELECT  flowchart.address AS flowchart_address, flowchart.ayear AS flowchart_ayear, flowchart.cap AS flowchart_cap, flowchart.codeflowchart, flowchart.ct AS flowchart_ct, flowchart.cu AS flowchart_cu, flowchart.fax AS flowchart_fax, flowchart.idcity AS flowchart_idcity, flowchart.idflowchart, flowchart.idsor1 AS flowchart_idsor1, flowchart.idsor2 AS flowchart_idsor2, flowchart.idsor3 AS flowchart_idsor3, flowchart.location AS flowchart_location, flowchart.lt AS flowchart_lt, flowchart.lu AS flowchart_lu, flowchart.nlevel AS flowchart_nlevel, flowchartparent.codeflowchart AS flowchartparent_codeflowchart, flowchartparent.title AS flowchartparent_title, flowchart.paridflowchart, flowchart.phone AS flowchart_phone, flowchart.printingorder AS flowchart_printingorder, flowchart.title AS flowchart_title, isnull('Codice: ' + flowchart.codeflowchart + '; ','') + ' ' + isnull('Denominazione: ' + flowchart.title + '; ','') as dropdown_title FROM flowchart WITH (NOLOCK)  LEFT OUTER JOIN flowchart AS flowchartparent WITH (NOLOCK) ON flowchart.paridflowchart = flowchartparent.idflowchart WHERE  flowchart.idflowchart IS NOT NULL 
GO
...
CREATE VIEW [progettocostosegview] AS SELECT  progettocosto.amount AS progettocosto_amount, progettocosto.ct AS progettocosto_ct, progettocosto.cu AS progettocosto_cu, progettocosto.doc AS progettocosto_doc, progettocosto.docdate AS progettocosto_docdate, [dbo].contrattokind.title AS contrattokind_title, progettocosto.idcontrattokind AS progettocosto_idcontrattokind, expense.description AS expense_description, expense.ymov AS expense_ymov, expense.nmov AS expense_nmov, progettocosto.idexp, pettycash.description AS pettycash_description, pettycash.pettycode AS pettycash_pettycode, progettocosto.idpettycash, progettocosto.idprogetto, progettocosto.idprogettocosto, progettotipocosto.title AS progettotipocosto_title, progettocosto.idprogettotipocosto, entrydetail.description AS entrydetail_description, progettocosto.idrelated, rendicontattivitaprogetto.idprogetto AS rendicontattivitaprogetto_idprogetto, rendicontattivitaprogetto.idworkpackage AS rendicontattivitaprogetto_idworkpackage, rendicontattivitaprogetto.description AS rendicontattivitaprogetto_description, progettocosto.idrendicontattivitaprogetto, sal.start AS sal_start, sal.stop AS sal_stop, progettocosto.idsal AS progettocosto_idsal, workpackage.raggruppamento AS workpackage_raggruppamento, workpackage.title AS workpackage_title, progettocosto.idworkpackage, progettocosto.lt AS progettocosto_lt, progettocosto.lu AS progettocosto_lu, progettocosto.noperation AS progettocosto_noperation, progettocosto.yoperation AS progettocosto_yoperation, isnull('Voce di costo: ' + progettotipocosto.title + '; ','') + ' ' + isnull('Workpackage: ' + workpackage.raggruppamento + '; ','') + ' ' + isnull('Workpackage: ' + workpackage.title + '; ','') as dropdown_title FROM progettocosto WITH (NOLOCK)  LEFT OUTER JOIN [dbo].contrattokind WITH (NOLOCK) ON progettocosto.idcontrattokind = [dbo].contrattokind.idcontrattokind LEFT OUTER JOIN expense WITH (NOLOCK) ON progettocosto.idexp = expense.idexp LEFT OUTER JOIN pettycash WITH (NOLOCK) ON progettocosto.idpettycash = pettycash.idpettycash LEFT OUTER JOIN progettotipocosto WITH (NOLOCK) ON progettocosto.idprogettotipocosto = progettotipocosto.idprogettotipocosto LEFT OUTER JOIN entrydetail WITH (NOLOCK) ON progettocosto.idrelated = entrydetail.idrelated LEFT OUTER JOIN rendicontattivitaprogetto WITH (NOLOCK) ON progettocosto.idrendicontattivitaprogetto = rendicontattivitaprogetto.idrendicontattivitaprogetto LEFT OUTER JOIN sal WITH (NOLOCK) ON progettocosto.idsal = sal.idsal LEFT OUTER JOIN workpackage WITH (NOLOCK) ON progettocosto.idworkpackage = workpackage.idworkpackage WHERE  progettocosto.idprogetto IS NOT NULL  AND progettocosto.idprogettocosto IS NOT NULL  AND progettocosto.idprogettotipocosto IS NOT NULL  AND progettocosto.idworkpackage IS NOT NULL 
GO
...
CREATE VIEW [rendicontattivitaprogettoanagammview] AS SELECT  rendicontattivitaprogetto.ct AS rendicontattivitaprogetto_ct, rendicontattivitaprogetto.cu AS rendicontattivitaprogetto_cu, rendicontattivitaprogetto.datainizioprevista AS rendicontattivitaprogetto_datainizioprevista, rendicontattivitaprogetto.description AS rendicontattivitaprogetto_description, rendicontattivitaprogetto.iditineration AS rendicontattivitaprogetto_iditineration, progetto.titolobreve AS progetto_titolobreve, rendicontattivitaprogetto.idprogetto, rendicontattivitaprogetto.idreg AS rendicontattivitaprogetto_idreg, rendicontattivitaprogetto.idrendicontattivitaprogetto, workpackage.raggruppamento AS workpackage_raggruppamento, workpackage.title AS workpackage_title, rendicontattivitaprogetto.idworkpackage, rendicontattivitaprogetto.lt AS rendicontattivitaprogetto_lt, rendicontattivitaprogetto.lu AS rendicontattivitaprogetto_lu, rendicontattivitaprogetto.orepreventivate AS rendicontattivitaprogetto_orepreventivate, rendicontattivitaprogetto.stop AS rendicontattivitaprogetto_stop, isnull('Progetto: ' + progetto.titolobreve + '; ','') + ' ' + isnull('Workpackage: ' + workpackage.raggruppamento + '; ','') + ' ' + isnull('Workpackage: ' + workpackage.title + '; ','') + ' ' + isnull('Descrizione: ' + rendicontattivitaprogetto.description + '; ','') as dropdown_title FROM rendicontattivitaprogetto WITH (NOLOCK)  LEFT OUTER JOIN progetto WITH (NOLOCK) ON rendicontattivitaprogetto.idprogetto = progetto.idprogetto LEFT OUTER JOIN workpackage WITH (NOLOCK) ON rendicontattivitaprogetto.idworkpackage = workpackage.idworkpackage WHERE  rendicontattivitaprogetto.idprogetto IS NOT NULL  AND rendicontattivitaprogetto.idrendicontattivitaprogetto IS NOT NULL  AND rendicontattivitaprogetto.idworkpackage IS NOT NULL 
GO
...
CREATE VIEW [rendicontattivitaprogettoorasegview] AS SELECT  rendicontattivitaprogettoora.ct AS rendicontattivitaprogettoora_ct, rendicontattivitaprogettoora.cu AS rendicontattivitaprogettoora_cu, rendicontattivitaprogettoora.data AS rendicontattivitaprogettoora_data, rendicontattivitaprogettoora.idrendicontattivitaprogetto, rendicontattivitaprogettoora.idrendicontattivitaprogettoora, sal.start AS sal_start, sal.stop AS sal_stop, rendicontattivitaprogettoora.idsal AS rendicontattivitaprogettoora_idsal, rendicontattivitaprogettoora.idworkpackage, rendicontattivitaprogettoora.lt AS rendicontattivitaprogettoora_lt, rendicontattivitaprogettoora.lu AS rendicontattivitaprogettoora_lu, rendicontattivitaprogettoora.ore AS rendicontattivitaprogettoora_ore, isnull('del ' + CONVERT(VARCHAR, rendicontattivitaprogettoora.data, 103),'') + ' ' + isnull('Numero di ore: ' + CAST( rendicontattivitaprogettoora.ore AS NVARCHAR(64)) + '; ','') as dropdown_title FROM rendicontattivitaprogettoora WITH (NOLOCK)  LEFT OUTER JOIN sal WITH (NOLOCK) ON rendicontattivitaprogettoora.idsal = sal.idsal WHERE  rendicontattivitaprogettoora.idrendicontattivitaprogetto IS NOT NULL  AND rendicontattivitaprogettoora.idrendicontattivitaprogettoora IS NOT NULL  AND rendicontattivitaprogettoora.idworkpackage IS NOT NULL 
GO
...
CREATE VIEW [rendicontattivitaprogettosegview] AS SELECT  rendicontattivitaprogetto.ct AS rendicontattivitaprogetto_ct, rendicontattivitaprogetto.cu AS rendicontattivitaprogetto_cu, rendicontattivitaprogetto.datainizioprevista AS rendicontattivitaprogetto_datainizioprevista, rendicontattivitaprogetto.description, rendicontattivitaprogetto.iditineration AS rendicontattivitaprogetto_iditineration, rendicontattivitaprogetto.idprogetto, [dbo].registry.title AS registry_title, rendicontattivitaprogetto.idreg AS rendicontattivitaprogetto_idreg, rendicontattivitaprogetto.idrendicontattivitaprogetto, rendicontattivitaprogetto.idworkpackage, rendicontattivitaprogetto.lt AS rendicontattivitaprogetto_lt, rendicontattivitaprogetto.lu AS rendicontattivitaprogetto_lu, rendicontattivitaprogetto.orepreventivate AS rendicontattivitaprogetto_orepreventivate, rendicontattivitaprogetto.stop AS rendicontattivitaprogetto_stop, isnull('Descrizione: ' + rendicontattivitaprogetto.description + '; ','') as dropdown_title FROM rendicontattivitaprogetto WITH (NOLOCK)  LEFT OUTER JOIN [dbo].registry WITH (NOLOCK) ON rendicontattivitaprogetto.idreg = [dbo].registry.idreg WHERE  rendicontattivitaprogetto.idprogetto IS NOT NULL  AND rendicontattivitaprogetto.idrendicontattivitaprogetto IS NOT NULL  AND rendicontattivitaprogetto.idworkpackage IS NOT NULL 
GO
