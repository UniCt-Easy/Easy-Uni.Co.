...
namespace Backend.Controllers
{
    ...
    public class BesmartController : ApiController {
        ...
        public IHttpActionResult rendicontaDocenti(List<RendicontazioneDocente> rendicontazioneDocenteList)
        {
            ...
            {
                logErr = "Missing Token";
                common.logInfo("request", $"Besmart > rendicontaDocente > {logErr}");
            ...
            {
                common.logInfo("response", $"Besmart > rendicontaDocente > Missing Cache");
            ...
            {
                common.logInfo("response", $"Besmart > rendicontaDocente > Missing Token");
                return Content(HttpStatusCode.Unauthorized, LoginFailedStatus.BadCredential);
            }
            ...
            {
                logErr = "Nessuna RendicontazioneDocente";
                common.logInfo("request", $"Besmart > rendicontaDocente > {logErr}");
                return Content(HttpStatusCode.BadRequest, logErr);
            }

            if (rendicontazioneDocenteList.Count > 1)
            {
                logErr = $"RendicontazioneDocente deve contenere al massimo un elemento; ne sono presenti {rendicontazioneDocenteList.Count}";
                common.logInfo("request", $"Besmart > rendicontaDocente > {logErr}");
                return Content(HttpStatusCode.BadRequest, logErr);
            }

            RendicontazioneDocente rendicontazioneDocente = rendicontazioneDocenteList.First();
            ...
            //{
            //    logErr = $"RendicontazioneDocente deve avere una sola didatticaFrontale o una sola didatticaIntegrativa o una sola altraAttività; ne sono presenti: " +
            ...
            else
            {
                ...
                if (idreg > 0)
                {
                    ...
                    foreach (Erogazione di in rendicontazioneDocente.didatticheIntegrative)
                    {
                        ...
                        {
                            error += $" - CF '{CF}': didatticheIntegrative.erogazioneTipoAttivita too long (max 2048 char)\r\n";
                            isOk = false;
                        }
                    }
                    ...
                    foreach (Erogazione di in rendicontazioneDocente.altreAttivita)
                    {
                        if (di.erogazioneTipoAttivita.Length > 2048)
                        {
                            error += $" - CF '{CF}': altreAttivita.erogazioneTipoAttivita too long (max 2048 char)\r\n";
                            isOk = false;
                        }
                    }
                    ...
                    if (isOk)
                    {
						...
						foreach (DidatticaFrontale df in rendicontazioneDocente.didatticheFrontali)
                        {
                            string result = AddDidatticaFrontale(conn, df, idreg, out qryIns);
                        ...
                        // Se arrivo qui, è OK, vuol dire che i dati sono buoni
                        // Tuttavia possono esserci delle righe già scritte che non fanno scrivere nulla
                        // if (error == "")
            ...
            {
                common.logInfo("response", $"Besmart > rendicontaDocente > Error: {error}");
                return Content(HttpStatusCode.BadRequest, error);
            }

            common.logInfo("response", $"Besmart > rendicontaDocente > OK");
        ...
        public IHttpActionResult eliminaRendicontaDocenti(List<EliminaRendicontazioneDocente> eliminaRendicontazioneDocenteList)
        {
            ...
            if (eliminaRendicontazioneDocenteList.Count > 1)
            {
                logErr = $"RendicontazioneDocente deve contenere al massimo un elemento; ne sono presenti {eliminaRendicontazioneDocenteList.Count}";
                common.logInfo("request", $"Besmart > eliminaRendicontazioneDocente > {logErr}");
                return Content(HttpStatusCode.BadRequest, logErr);
            }
            ...
            if (!string.IsNullOrEmpty(error))
            {
                common.logInfo("response", $"Besmart > eliminaRendicontaDocente > Error: {error}");
                return Content(HttpStatusCode.BadRequest, error);
            }

            common.logInfo("response", $"Besmart > eliminaRendicontaDocente > OK");
            return Content(HttpStatusCode.OK, "OK");
        }

		private string AddDidattica(IDataAccess conn, string totalQry)
		{
			try
			{
				conn.SQLRunner(totalQry);
			}
			catch (Exception Ex)
			{
				return Ex.Message;
		...
		private string AddDidatticaFrontale(IDataAccess conn, DidatticaFrontale df, int idreg, out string qryIns)
        {
            qryIns = "";
        ...
        private string AddErogazione(IDataAccess conn, Erogazione di, int idreg, out string qryIns)
        {
            qryIns = "";
        ...
        private string DelDidatticaFrontale(IDataAccess conn, DelDidatticaFrontale df, int idreg)
        {
            try
            {
                ...
                if (cntConsolidamenti > 0)
                    return "Didattica frontale già consolidata";
		...
		private int GetSede(IDataAccess conn, ErogazioneLuogo erogazioneLuogo, int idreg)
		{
            ...
            if (idsede == 0)
            {
                ...
                if (idcity != 0)
                {
                    string qry = string.Format(InsSede, idcity, cap, title.Replace("'", "''"), idreg);
					DataTable newSede = conn.SQLRunner(qry);
                    if (newSede != null)
                    {
                        if (newSede.Rows != null)
                        {
		...
		private int GetRendicontAltroKind(IDataAccess conn, string tipoAttivita)
		{
            ...
            {
				string qry = string.Format(InsRendicontaltroKind, tipoAttivita.Replace("'", "''"));
				DataTable newRendicontAltroKind = conn.SQLRunner(qry);
				if (newRendicontAltroKind != null)
					if (newRendicontAltroKind.Rows != null)
