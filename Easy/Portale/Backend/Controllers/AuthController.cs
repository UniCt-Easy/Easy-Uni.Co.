...
namespace Backend.Controllers {
	...
	public class AuthController : ApiController, IAuthController {
		...
		public IHttpActionResult register(RegistrationFormData data) {
			if (data == null) {
		...
		public IHttpActionResult loginSSO(LoginFormDataSSO data) {
		...
		public IHttpActionResult login(LoginFormData data) {
			if (data?.userName == null) {
				return base.Content(HttpStatusCode.BadRequest, LoginFailedStatus.NoCredential);
			}
		...
		private IHttpActionResult _doLogin(String userName, String password, DateTime datacontabile, SessionInfoSSO sessionInfoSSO, int userkind) {
			...
			// 9.1 Calcola ruoli
			DataTable dtRoles = getRolesDataTable(datacontabile, sys_user, dispatcher);
            ...
            sys.Add("dbversion", dispatcher.conn.DO_READ_VALUE("updatedbversion", null, "max(versionname)"));
            sys.Add("backendversion", System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString());

            // 11. distruggo conenssione di sistema
            if (dispatcher != null) {
				dispatcher.Dispose();
			}
            ...
            var result = new JObject {
				...
				{"token", token},
				{"dtRoles", DataUtils.dataTableToJSon(dtRoles)},
        ...
        public IHttpActionResult nuovaPassword([Required] string token, [Required] string password ) {
            ...
            MetaExpression metaFilterByIdreference = MetaExpression.eq("idregistryreference", resetPasswordRow["idregistryreference"]);
	...
	public class TestController : ApiController {
		...
		public IHttpActionResult test_DeSerialize(JObject o) {
		...
		public IHttpActionResult test_read() {
			...
			//string filter = q.toSql(dispatcher.conn.GetQueryHelper(), dispatcher.conn);
			getData.GET_PRIMARY_TABLE(filterByEmail);
