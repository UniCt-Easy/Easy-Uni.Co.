...
[System.Xml.Serialization.XmlRoot("dsmeta_istanza_sosp_seg"),System.ComponentModel.Design.HelpKeyword("vs.data.DataSet")]
public partial class dsmeta_istanza_sosp_seg: DataSet {
	...
	[DebuggerNonUserCode,DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),Browsable(false)]
	public MetaTable istanza_sosp 		=> (MetaTable)Tables["istanza_sosp"];
...
[DebuggerNonUserCode]
public dsmeta_istanza_sosp_seg(){
	BeginInit();
	initClass();
	EndInit();
}
[DebuggerNonUserCode]
protected dsmeta_istanza_sosp_seg (SerializationInfo info,StreamingContext ctx):base(info,ctx) {}
[DebuggerNonUserCode]
private void initClass() {
	DataSetName = "dsmeta_istanza_sosp_seg";
	Prefix = "";
	Namespace = "http://tempuri.org/dsmeta_istanza_sosp_seg.xsd";
	...
	//////////////////// ISTANZA_SOSP /////////////////////////////////
	var tistanza_sosp= new MetaTable("istanza_sosp");
	tistanza_sosp.defineColumn("ct", typeof(DateTime),false);
	tistanza_sosp.defineColumn("cu", typeof(string),false);
	tistanza_sosp.defineColumn("idistanza", typeof(int),false);
	tistanza_sosp.defineColumn("idistanzakind", typeof(int),false);
	tistanza_sosp.defineColumn("idreg", typeof(int),false);
	tistanza_sosp.defineColumn("lt", typeof(DateTime),false);
	tistanza_sosp.defineColumn("lu", typeof(string),false);
	tistanza_sosp.defineColumn("motivo", typeof(string));
	tistanza_sosp.defineColumn("start", typeof(DateTime));
	tistanza_sosp.defineColumn("stop", typeof(DateTime));
	Tables.Add(tistanza_sosp);
	tistanza_sosp.defineKey("idistanza", "idistanzakind", "idreg");
	...
	cChild = new []{diniego_alias2.Columns["idistanza"], diniego_alias2.Columns["idistanzakind"], diniego_alias2.Columns["idreg"]};
	...
	cPar = new []{istanza.Columns["idistanza"], istanza.Columns["idistanzakind"], istanza.Columns["idreg_studenti"]};
	cChild = new []{istanza_sosp.Columns["idistanza"], istanza_sosp.Columns["idistanzakind"], istanza_sosp.Columns["idreg"]};
	Relations.Add(new DataRelation("FK_istanza_sosp_istanza_idistanza-idistanzakind-idreg",cPar,cChild,false));
