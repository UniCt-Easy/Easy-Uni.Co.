...
[System.Xml.Serialization.XmlRoot("dsmeta_dichiar_altre_seg"),System.ComponentModel.Design.HelpKeyword("vs.data.DataSet")]
public class dsmeta_dichiar_altre_seg: DataSet {
	...
	[DebuggerNonUserCode,DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),Browsable(false)]
	public MetaTable dichiar_altre 		=> (MetaTable)Tables["dichiar_altre"];
...
[DebuggerNonUserCode]
public dsmeta_dichiar_altre_seg(){
	BeginInit();
	initClass();
	EndInit();
}
[DebuggerNonUserCode]
protected dsmeta_dichiar_altre_seg (SerializationInfo info,StreamingContext ctx):base(info,ctx) {}
[DebuggerNonUserCode]
private void initClass() {
	DataSetName = "dsmeta_dichiar_altre_seg";
	Prefix = "";
	Namespace = "http://tempuri.org/dsmeta_dichiar_altre_seg.xsd";
	...
	//////////////////// DICHIAR_ALTRE /////////////////////////////////
	var tdichiar_altre= new MetaTable("dichiar_altre");
	tdichiar_altre.defineColumn("ct", typeof(DateTime),false);
	tdichiar_altre.defineColumn("cu", typeof(string),false);
	tdichiar_altre.defineColumn("iddichiar", typeof(int),false);
	tdichiar_altre.defineColumn("iddichiaraltrekind", typeof(int),false);
	tdichiar_altre.defineColumn("idreg", typeof(int),false);
	tdichiar_altre.defineColumn("lt", typeof(DateTime),false);
	tdichiar_altre.defineColumn("lu", typeof(string),false);
	Tables.Add(tdichiar_altre);
	tdichiar_altre.defineKey("iddichiar", "idreg");

	#endregion


	#region DataRelation creation
	var cPar = new []{dichiaraltrekind.Columns["iddichiaraltrekind"]};
	var cChild = new []{dichiar_altre.Columns["iddichiaraltrekind"]};
	Relations.Add(new DataRelation("FK_dichiar_altre_dichiaraltrekind_iddichiaraltrekind",cPar,cChild,false));
	...
	cChild = new []{dichiar_altre.Columns["iddichiar"], dichiar_altre.Columns["idreg"]};
	Relations.Add(new DataRelation("FK_dichiar_altre_dichiar_iddichiar-idreg-",cPar,cChild,false));
