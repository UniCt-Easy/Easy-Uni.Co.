...
{
    public class Meta_didprogaccesso : Meta_easydata 
	{
        public Meta_didprogaccesso(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "didprogaccesso") {
				Name = "Elenco delle didattiche programmate a cui il test di ingresso da acesso";
			...
			switch (edit_type) {
				case "default": {
						...
						if (R["iddidprog"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'test di ingresso' è obbligatorio";
							errfield = "iddidprog";
							return false;
						}
						if (R["iddidprog_acc"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'didattica programmata a cui il test di ingresso da l'accesso' è obbligatorio";
							errfield = "iddidprog_acc";
							return false;
						}
