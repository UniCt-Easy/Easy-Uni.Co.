...
namespace calcolocedolino {//calcolocedolino//
	...
	public class CalcoliPerLaGenerazione {
		...
		/// I cedolini già erogati rimangono inalterati.
		...
		/// <param name="rTuttiICedoliniErogati">cedolini già erogati</param>
		...
		/// già erogato e quanto si sarebbe dovuto erogare.
		/// </summary>
		/// <param name="esercizio">anno fiscale di cui si vogliono calcolare i cedolini</param>
		/// <param name="rCedoliniErogati">cedolini già erogati</param>
        ...
        /// e sulla periodicità desiderata dei cedolini.
        /// </summary>
        ...
        public static void calcolaDateInizioEFineDeiCedolini(
            ...
            if (((periodicita == 1) && (diffMesi == 0)) || (dataFine.Year - dataInizio.Year) > 0) {
                //Se non è un n. di mesi esatto oppure è a cavallo d'annno
		...
		/// e l'array dei cedolini già erogati, restituisce gli indici del primo e dell'ultimo
		...
		/// <param name="rCedoliniErogati">cedolini già erogati ordinati per datainizio</param>
		...
		/// Altrimenti se esiste un numero n di mesi multiplo della periodicità che, sommato alla data di inizio
		/// mi dà un giorno compreso tra l'1 ed il 12 gennaio dell'anno successivo all'esercizio,
		...
		/// se tale differenza è un numero intero; altrimenti restituisce 0.
		/// Nel caso di restituzione di un numero intero num varrà la seguente equazione:
		/// inizio.AddMonths(num) = fine.AddDays(1)
		/// In altri termini: 
		/// (1) se il inizio.Day=1 allora fine.Day è l'ultimo giorno del mese;
		/// (2) se invece inizio.Day è compreso tra 2 ed il penultimo giorno del mese di fine, 
		///			allora deve valere inizio.Day-1 = fine.Day;
		/// (3) se invece inizio.Day è uguale o maggiore dell'ultimo giorno del mese di fine 
		...
		///	Verrà restituito 0 ad esempio in questi casi:
		...
		///	
		///	Il flagCalcoloARitroso sarà posto a true se valgono le seguenti condizioni:
		///		il mese di inizio ha meno giorni del mese di fine;
		///		il giorno di inizio è l'ultimo del mese di inizio;
		///		il giorno di fine è compreso tra il penultimo giorno del mese di inizio ed il penultimo giorno del mese di fine
		///	
		///	Se numMesi è il valore restituito da questa funzione, allora:
		///	Se flagCalcoloARitroso è false allora le date di inizio vanno calcolate con la formula
		///		for (int i=0; i<numMesi; i++) dateInizio[i] = inizio.AddMonths(i);
		///	Se invece è true allora:
