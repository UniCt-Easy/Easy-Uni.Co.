		...
		public void GeneraScritture(string elencoCedolini){
			...
			foreach(DataRow CurrCedolino in tCedolino.Rows) {
				...
				if (idaccmot_main==DBNull.Value){
					MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stata impostata la causale per la prestazione.");
					return;
				}
                ...
                if ((idepexp == null || idepexp == DBNull.Value) && flagEpExp == "S" && esercizio > 2015) {
                    MetaFactory.factory.getSingleton<IMessageShower>().Show("Errore", "Non è stato trovato alcun impegno di budget per il contratto");
                    return;
                }
                ...
                foreach (DataRow Rit in tCedolinoRitenuta.Rows){
                    ...
                    if (idaccmotive_cost == DBNull.Value) {
                        ...
                        if ((idepexpContrib == null || idepexpContrib == DBNull.Value) && flagEpExp == "S" && esercizio > 2015) {
                            MetaFactory.factory.getSingleton<IMessageShower>().Show("Errore", "Non è stato trovato alcun impegno di budget per il contributo" + Rit["taxcode"].ToString());
                            return;
                        }
