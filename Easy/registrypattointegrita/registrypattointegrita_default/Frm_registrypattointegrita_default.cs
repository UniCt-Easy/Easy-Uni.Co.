...
namespace registrypattointegrita_default {
	public partial class Frm_registrypattointegrita_default : MetaDataForm {
        private MetaData Meta;
        CQueryHelper QHC;
        QueryHelper QHS;
        public IOpenFileDialog opendlg;

        public Frm_registrypattointegrita_default() {
			InitializeComponent();
        ...
        void AbilitaDisabilitaAllegati() {
            labPattoFileName.Text = "";
            if (Meta.IsEmpty) {
                btnAllegaPatto.Enabled = false;
                btnVisualizzaPatto.Enabled = false;
                btnRimuoviPatto.Enabled = false;
        ...
        void SalvaAllegato(string certification) {
            // Legge il file indicato dall'utente e lo scrive nel DB in 'pattointegritacertification' o in 'selfcertification'
            if (Meta.IsEmpty) return;
            if (!Meta.GetFormData(true)) return;
            ...
            DataRow Curr = HelpForm.GetLastSelected(DS.registrypattointegrita);
            if (Curr == null) return;
            FileStream FS;
            try {
        ...
        private void btnAllegaPatto_Click(object sender, EventArgs e) {
            SalvaAllegato("pattointegritacertification");
        }
        ...
        private void btnVisualizzaPatto_Click(object sender, EventArgs e) {
            VisualizzaAllegato("pattointegritacertification");
        }
