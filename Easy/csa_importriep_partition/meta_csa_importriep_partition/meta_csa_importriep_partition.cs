...
namespace meta_csa_importriep_partition {
    /// <summary>
    /// </summary>
    public class Meta_csa_importriep_partition :Meta_easydata {
        public Meta_csa_importriep_partition(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "csa_importriep_partition") {
        ...
        protected override Form GetForm(string FormName) {
            if (FormName == "detail") {
                Name = "Dettaglio";
                return MetaData.GetFormByDllName("csa_importriep_partition_detail");
                }
            if (FormName == "elenco") {
                Name = "Ripartizione unica righe di riepilogo";
                return MetaData.GetFormByDllName("csa_importriep_partition_elenco");
            }
            return null;
            }
        ...
        public override DataRow SelectOne(string ListingType, string filter, string searchtable, DataTable ToMerge) {
            if (ListingType == "default") {
                return base.SelectOne(ListingType, filter, "csa_importriep_partitionview", ToMerge);
            }
        ...
        public override bool IsValid(DataRow R, out string errmess, out string errfield) {
            ...
            if (CfgFn.GetNoNullDecimal(R["amount"]) == 0) {
                errmess = "Non è stato valorizzato l'importo";
                errfield = "amount";
                return false;
            }
            return true;
