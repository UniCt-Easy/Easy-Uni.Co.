...
namespace meta_mandate//meta_ordinegenerico//
{
	...
	public class Meta_mandate : Meta_easydata {
		...
		protected override void InsertCopyColumn(DataColumn C, DataRow Source, DataRow Dest) {
			if ((C.ColumnName == "yman") || (C.ColumnName == "nman") || C.ColumnName=="idmandatestatus"|| C.ColumnName == "cigcode") return;
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield){
		    ...
		    int flag = CfgFn.GetNoNullInt32( Conn.DO_READ_VALUE("mandatekind", QHS.CmpEq("idmankind", R["idmankind"]), "flag"));
      ...
      //                          "Occorre procedere con l'annullamento dei dettagli del contratto con apposizione della data di annullamento.";
            ...
            object multireg = Conn.DO_READ_VALUE("mandatekind", "(idmankind=" +
                            QueryCreator.quotedstrvalue(R["idmankind"], true) + ")", "multireg");
            if (multireg == null) multireg = "N";
            string smultireg = multireg.ToString().ToUpper();

            if ((CfgFn.GetNoNullInt32(R["idreg"]) == 0) && (smultireg != "S") && edit_type != "request" && LinkedForm != null && 
            ...
            if ((CfgFn.GetNoNullInt32(R["idreg"]) != 0) &&  edit_type != "request" && LinkedForm != null) {
                ...
                object registryclass = Conn.DO_READ_VALUE("registryclass", QHS.CmpEq("idregistryclass", anag["idregistryclass"]),
                                        "flaghuman");
            ...
            if ((R.RowState==DataRowState.Added) && (!RowChange.IsAutoIncrement(R.Table.Columns["nman"]))){
				int NPRESENT = Conn.RUN_SELECT_COUNT("mandate","(idmankind="+QueryCreator.quotedstrvalue(R["idmankind"],true)+")AND" + 
            ...
            if (R.Table.Columns.Contains("cigcode") && (R["cigcode"].ToString()!="")&&(R["cigcode"].ToString().Length != 10)) {
            ...
            if (R.Table.Columns.Contains("cigcode") && (R["cigcode"].ToString() != "") && !CfgFn.IsValidString(R["cigcode"].ToString()) ) {
            ...
            if ((R.Table.DataSet.Tables.Contains("mandatedetail"))
                ...
                && (R.Table.DataSet.Tables.Contains("mandatecig")) && (R.Table.DataSet.Tables["mandatecig"].Rows.Count > 0)

                && (cigChanged==false)

                )
            {
                string errore  = "Alcuni dettagli del Contratto Passivo sono stati modificati. Verificare se sia necessario modificare anche i Lotti del Bando." +"\r\n" +
                                 "1) 'OK' per effettuare la correzione " + "\r\n" + "2) 'Annulla' per proseguire ugualmente senza correggere i Lotti";
               
                if (this.ShowClientMsg(errore, "Attenzione!", MessageBoxButtons.OKCancel))
                {
