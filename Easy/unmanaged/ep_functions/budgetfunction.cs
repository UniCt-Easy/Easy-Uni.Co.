...
namespace ep_functions {
    ...
    public class BudgetFunction {
        ...
        static string[] DecomposeObjects(string S) {
            ...
            return S.Length == 0 ? null : S.Split('ß');
        }
        ...
        public static string getDocChildCondition(QueryHelper q, string idrelated) {
            return q.DoPar(q.AppOr(q.Like("idrelated", idrelated + "ß%"), q.CmpEq("idrelated", idrelated)));
            //"((idrelated LIKE " + q.quote(idrelated + "ß%") + ")" + " OR(idrelated = " + q.quote(idrelated) + "))";
        }

        public static string getDocChildCondition(string fieldBase, QueryHelper q, string idrelated) {
            return q.DoPar(q.AppOr(q.Like(fieldBase, idrelated + "ß%"), q.CmpEq(fieldBase, idrelated)));
            //"((idrelated LIKE " + q.quote(idrelated + "ß%") + ")" + " OR(idrelated = " + q.quote(idrelated) + "))";
        }
        ...
        public bool getEpMovForDocument(string table, string idrelated, int nPhase) {
            ...
            //"((idrelated LIKE " + QHS.quote(idrelated + "ß%") + ")"+ " OR(idrelated = " + QHS.quote(idrelated) + "))";
        ...
        /// <summary>
        /// Imposta i dati principali dell'impegno di budget, crea le righe se non esistono. La ricerca Ë fatta sulla base di idrelated
        /// </summary>
        /// <param name="description"></param>
        /// <param name="adate"></param>
        ...
        public DataRow SetEpExp(object idreg, object idman, decimal amount, object description, object adate,
            ...
            //Non cambia gli idparent quando gi‡ presenti nella riga
        ...
        public void updateAmounts(bool attivo) {
            ...
            foreach (DataRow e in D.Tables[table].Rows) {
                ...
                decimal[] currAmount = amountsFromRow(ey); //Sar‡ l'importo corrente
                decimal[] pagatoAnniPrecedenti = new decimal[] {0, 0, 0, 0, 0};
                int yepexp = CfgFn.GetNoNullInt32(e["y" + table]);
                if (yepexp < esercizio) {
                    //All'importo corrente va sottratto la differenza che c'Ë di base sul db, ossia il "pagato" anni precedenti
        ...
        public void updateEntryDetails() {
            foreach (DataRow entryDetail in D.Tables["entrydetail"].Select()) {
                //vede se Ë cambiato il conto di costo dell'impegno collegato
        ...
        //Cancella le fasi parent (in stato added) di cui esiste gi‡ la fase figlia collegata ad un altro preimpegno
        ...
        internal bool SaveAll(bool silent, bool chiediMovimentiParent, ep_poster posting) {
            ...
            //CopiaInfoParent(); //non c'Ë pi˘ questa
            ...
            //10434 lo commento perchË se no non genera le scritture nelle fatture collegate  a contratti passivi o attivi
