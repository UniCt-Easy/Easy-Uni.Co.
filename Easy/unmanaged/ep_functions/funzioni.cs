	...
	public class EP_Manager : IDisposable {
		...
		public bool abilitaScritture(DataRow r) {
			...
			if (r.Table.TableName == "mandate") {
				...
				return isManKindEpEnabled(r["idmankind"]);
			}
			...
			if (r.Table.TableName == "estimate") {
				...
				return isEstimKindEpEnabled(r["idestimkind"]);
			}
		...
		public void beforePost(bool forceDelete) {
			...
			if (!forceDelete) {
				...
				if (mainTable == "provision") {
					foreach (DataRow r in DS.Tables["provisiondetail"].Select(QHS.IsNotNull("idaccmotive"))) {
						_idrelatedToDelete.Add(BudgetFunction.GetIdForDocument(r));
					}
				}
			...
			if (mainTable == "provision" && forceDelete) {
				foreach (DataRow r in DS.Tables["provisiondetail"].Select(QHS.IsNotNull("idaccmotive"))) {
					_idrelatedToDelete.Add(BudgetFunction.GetIdForDocument(r));
				}
			}
		...
		private bool generaScrittureFattura(DataRow curr, BudgetFunction bf) {
			...
			if (!deferred) {
				...
				if (profService != null && profService.Rows.Count == 1) {
					...
					//object epkind = linkedProfServiceMain["epkind"];
					//if (epkind != null) {
					//    if (epkind.ToString().ToUpper() == "F")//fattura a ricevere
					//        isFattRic = true;
					//}
				}
			}
			...
			foreach (DataRow rInvDet in DS.Tables["invoicedetail"].Select(filterMainCurrent)) {
				...
				if (rInvDet["idestimkind"] != DBNull.Value) {
					...
					if (annoContratto < annoFattura) {
						string filtermand = QHS.CmpMulti(rInvDet, "idestimkind", "yestim", "nestim");
						filtermand = QHS.AppAnd(filtermand, QHS.CmpEq("rownum", rInvDet["estimrownum"]));
						object epkind = Conn.DO_READ_VALUE("estimatedetail", filtermand, "epkind");
						if (epkind != null) {
							if (epkind.ToString().ToUpper() == "F" || epkind.ToString().ToUpper() == "R") {
				...
				object idepexp = rInvDet["idepexp"];
				object idepacc = rInvDet["idepacc"];
				...
				if (importoDebito != 0) {
					...
					if ((idupbIva != DBNull.Value) &&
					    !(isIntraCom || applicaSplitPaymentCommerciale || isFlagReverseCharge)) {
						EP.EffettuaScritturaImpegnoBudget(idepcontext,
							imponibile - sconto,
				...
				foreach (DataRow rLiquidazione in tSpesa.Rows) {
					...
					switch (autokind) {
						...
						case 2: //Liquidazione ritenute
							...
							EP.EffettuaScritturaIdRelated("LIQRIT", importo, idAccRiten, rLiquidazione["idreg"],rLiquidazione["idupb"], null,idaccmotiveRiten, idrelDebit, descrdet);
							EP.EffettuaScritturaIdRelated("LIQRIT", importo, idaccPayment, rLiquidazione["idreg"],rLiquidazione["idupb"], null, idaccmotiveRiten, idrelated, descrdet);
							break;

						case 15: { /*manca causale*/
							//Liquidazione IVA Consolidata INTERNA,  ORMAI IN DISUSO 
							EP.EffettuaScritturaIdRelated("PAGAM", importo, idaccPayment, rLiquidazione["idreg"],rLiquidazione["idupb"], null, null, idrelated, descrdet);
							EP.EffettuaScrittura("PAGAM", importo, idAccIvaPaymentInternal, rLiquidazione["idreg"],rLiquidazione["idupb"], null, null, null, null, descrdet);
							break;
						}
						...
						case 12: { /*manca causale*/
							...
							EP.EffettuaScritturaIdRelated("PAGAM", importo, idaccPayment, rLiquidazione["idreg"],rLiquidazione["idupb"], null, null, idrelated, descrdet);
							var rIvaExpense = Conn.readObject("ivapayexpense", q.eq("idexp", rLiquidazione["idexp"]),"yivapay,nivapay");
							string idrelatedIvaPay = null;
							if (rIvaExpense!=null) idrelatedIvaPay =_composeObjects("ivapay", rIvaExpense["yivapay"], rIvaExpense["nivapay"]);
							EP.EffettuaScritturaIdRelated("PAGAM", importo, idAccIvaPayment, rLiquidazione["idreg"],rLiquidazione["idupb"], null, null, idrelatedIvaPay, descrdet);
							break;
						}
						...
						case 17: { /*manca causale*/
							...
							string idrelatedIvaPay = null;
							if (rIvaExpense!=null) idrelatedIvaPay =_composeObjects("ivapay", rIvaExpense["yivapay"], rIvaExpense["nivapay"]);
							EP.EffettuaScrittura("PAGAM", importo, idAccIvaPayment12, rLiquidazione["idreg"],rLiquidazione["idupb"], null, null, null, idrelatedIvaPay, descrdet);
							break;
						}
						...
						case 23: { /*manca causale*/
							...
							string idrelatedIvaPay = null;
							if (rIvaExpense!=null) idrelatedIvaPay =_composeObjects("ivapay", rIvaExpense["yivapay"], rIvaExpense["nivapay"]);
							EP.EffettuaScrittura("PAGAM", importo, idAccIvaPaymentSplit, rLiquidazione["idreg"],rLiquidazione["idupb"], null, null, null, idrelatedIvaPay, descrdet);
							break;
						}
						...
						case 4: {
							...
							object idser = Conn.DO_READ_VALUE("expenselast",QHS.CmpEq("idexp", rLiquidazione["idpayment"]), "idser");
							int idregPrincipale =  CfgFn.GetNoNullInt32(Conn.DO_READ_VALUE("expense",QHS.CmpEq("idexp", rLiquidazione["idpayment"]), "idreg"));//idreg movimento principale


							string idrelatedDebitoContoErario = idrelatedDebitFor( rLiquidazione["idpayment"],taxcode2); // cascon§2019§16§RITEN§17 

							TaxInfo tax2 = TaxInfo.getInfo(taxInfo, taxcode2);
							if (tax2 == null) { // (_tax.Select(QHC.CmpEq("taxcode", codiceriten)).Length == 0) 
								ShowMessage($"Il codice {taxcode2} non è il codice di una ritenuta esistente", "Errore");
								return false;
							}
							...
							if (motiveRow2 == null) { // (_tax.Select(QHC.CmpEq("taxcode", codiceriten)).Length == 0) 
								ShowMessage($"Il codice {tax2.taxRef()} non è il codice di una ritenuta configurata per l'EP.", "Errore");
								return false;
							}
							...
							if (idregPrincipale == idregAutomatismo) { //task 14931  Ritenute negative su compensi diversi da CSA
						...
						case 30: { //Nuovi riepiloghi CSA
							if (!generaScritturePagamentoRiepPartition(EP, rLiquidazione["idexp"],
								rLiquidazione["idreg"], false)) return false;
							break;
						}
						...
						default:
							...
							if (importoRecuperoIvaEstera != 0) {
								generaScritturaDebito(importoNettoDaIvaEstera, importoRecuperoIvaEstera,
									...
									rLiquidazione["idupb"],
									rLiquidazione["idexp"], idaccmotive, false, rLiquidazione["description"],rLiquidazione["idfin"],false);
							}
							else {
								...
								generaScritturaDebito(importoNettoDaSplitPayment,
									...
									rLiquidazione["idupb"],
									rLiquidazione["idexp"], idaccmotive, false, rLiquidazione["description"],rLiquidazione["idfin"],isPGiro);
							}
				...
				foreach (DataRow rVar in rVariazioni) {
					...
					switch (autokind) {
						...
						case 17: { /*manca causale*/
							...
							string idrelatedIvaPay = null;
							if (rIvaExpense!=null) idrelatedIvaPay =_composeObjects("ivapay", rIvaExpense["yivapay"], rIvaExpense["nivapay"]);
							EP.EffettuaScritturaIdRelated("PAGAM", importo, idAccIvaPayment12,rLiquidazione["idreg"], rLiquidazione["idupb"], null, null, idrelatedIvaPay, descrdet);
							break;
						}
						...
						case 23: { /*manca causale*/
							...
							string idrelatedIvaPay = null;
							if (rIvaExpense!=null) idrelatedIvaPay =_composeObjects("ivapay", rIvaExpense["yivapay"], rIvaExpense["nivapay"]);
							EP.EffettuaScritturaIdRelated("PAGAM", importo, idAccIvaPaymentSplit,rLiquidazione["idreg"], rLiquidazione["idupb"], null, null, idrelatedIvaPay, descrdet);
							break;
						}
						...
						case 4: {
							...
							string idrelatedDebitoContoErario = idrelatedDebitFor( rLiquidazione["idpayment"],taxcode3); // cascon§2019§16§RITEN§17 
						...
						case 30: { //Nuovi versamenti CSA
							if (!generaScritturePagamentoRiepPartition(EP, rLiquidazione["idexp"],
								rLiquidazione["idreg"], true)) return false;
							break;
						}
						...
						default: /*manca causale*/
							...
							if (importoRecuperoIvaEstera != 0) {
								generaScritturaDebito(importoNettoDaIvaEstera, quotaImportoRecuperoIvaEstera,
									idAccFornitore, idaccRecuperoSplitPayment, rLiquidazione["idreg"],
									rLiquidazione["idupb"],
									rLiquidazione["idexp"], null, true, rLiquidazione["description"],rLiquidazione["idfin"],false);
							}
							else {
								bool isPGiro= (CfgFn.GetNoNullInt32(rLiquidazione["finflag"])& 2)!=0;
								generaScritturaDebito(importoNettoDaSplitPayment,
									...
									idAccFornitore, idaccRecuperoSplitPayment, rLiquidazione["idreg"],
									rLiquidazione["idupb"],
									rLiquidazione["idexp"], null, true, rLiquidazione["description"],rLiquidazione["idfin"],isPGiro);
							}
		...
		bool spezzaScritturaCredito(Dictionary<int, importo_causale> amounts, decimal importoIncasso, object idAcc,
			...
			foreach (var idepinc in amounts.Keys) {
				...
				if (isVariazione) quota = -quota;
				importoIncasso -=
					quota; //Se importo negativo,  importo incasso è negativo, qui l'importo diminuisce ancor di più
				if (riproporziona) {
					somma -= importo;
		...
		Dictionary<int, importo_causale> ottieniImportiImpegnoDaTabellaIdrelated(DataTable t, string fieldIdEpExp,
			...
			foreach (var r in t.Select(QHC.IsNotNull(fieldIdEpExp))) {
				...
				int idepmov = CfgFn.GetNoNullInt32(r[fieldIdEpExp]);
		...
		void generaScritturaCredito(decimal importo,
			...
			if (pGiro) {
				...
				if (contiPrinc.Length > 0) {
					...
					object idIncFaseUno = Conn.DO_READ_VALUE("incomelink", QHS.AppAnd(QHS.CmpEq("idchild", idinc), QHS.CmpEq("nlevel",1)), "idparent");
			...
			EP.EffettuaScritturaIdRelated("INCAS", importoScrittura, idAccCredito, idReg, idUpb, null, idaccmotive,idrelated, description);
		}
		...
		/// <returns></returns>
		bool generaScritturaDebito(decimal importoNettoDaSplit, decimal importoIvaSplit /* importoRecuperoSplitPaymentCommerciale */,
			object idAccDebito, object idAccIvaSplit,
			object idReg, object idUpb, object idexp, object idaccmotive, bool isVariazione, object descrMov,object idfin, bool pGiro) {
			...
			if (dettFattura != null && dettFattura.Rows.Count > 0) {
				...
				EP.EffettuaScritturaImpegnoBudget("PAGAM", importoIvaSplit, idAccIvaSplit, idReg, "0001", null,
					idaccmotive, null,
					null,
			...
			if (contParcella != null && contParcella.Rows.Count > 0) {
				...
				if (parcella != null && parcella.Rows.Count > 0) {
					...
					if (dettFattura != null && dettFattura.Rows.Count > 0) {
						...
						foreach (DataRow rDett in dettFattura.Select(QHC.IsNotNull("idepexp"))) {
			...
			if (pGiro) {
				...
				if (contiPrinc.Length > 0) {
					...
					object idExpFaseUno = Conn.DO_READ_VALUE("expenselink", QHS.AppAnd(QHS.CmpEq("idchild", idexp), QHS.CmpEq("nlevel",1)), "idparent");
					//In fase di trasm. incasso sarà messo nell'idrelated della chiusura credito l'idinc dell'accertamento, in caso di autokind null e p.di giro
					object idIncLinked = Conn.DO_READ_VALUE("expense", QHS.CmpEq("idexp", idExpFaseUno), "idinc_linked");
					...
					switch (autokind) {
						case 7:
							...
							EP.EffettuaScritturaIdRelated("INCAS", importofornitore, idAccFornitore,
								rIncasso["idreg"], rIncasso["idupb"], null, idaccmotiveFps, "foeco§"+autocode, descrdet);
							break;
						case 15: {
						...
						case 4: {
							...
							object idser = Conn.DO_READ_VALUE("expenselast", QHS.CmpEq("idexp", rIncasso["idpayment"]), "idser");
							string idrelatedDebitoContoErario = "ritenuta§"+rIncasso["idpayment"].ToString() + "§" + codicerit.ToString();
						...
						case 31: { //Nuovi versamenti CSA
							if (!generaScrittureIncassoVerPartition(EP, rIncasso["idinc"], rIncasso["idreg"], false))
								return false;
							break;
						}

						case 30: { //Nuovi riepiloghi CSA
							if (!generaScrittureIncassoRiepPartition(EP, rIncasso["idinc"], rIncasso["idreg"], false))
								return false;
							break;
						}
						...
						default: {
							...
							generaScritturaCredito(importo,
								idAccCliente, rIncasso["idreg"],rIncasso["idupb"], rIncasso["idinc"], null, false, rIncasso["description"],rIncasso["idfin"],isPGiro );
							break;
						}
					}
				}
			...
			if (rVariazioni.Length > 0) {
				foreach (DataRow rVar in rVariazioni) {
					...
					switch (autokind) {
						...
						case 30: { //Nuovi versamenti CSA
							if (!generaScrittureIncassoRiepPartition(EP, rIncasso["idinc"], rIncasso["idreg"], true))
								return false;
							break;
						}

						case 31: { //Nuovi riepiloghi CSA
							if (!generaScrittureIncassoVerPartition(EP, rIncasso["idinc"], rIncasso["idreg"], true))
								return false;
							break;
						}
						...
						default: {
							...
							generaScritturaCredito(importo, idAccCliente, rIncasso["idreg"], rIncasso["idupb"],
								rIncasso["idinc"], null, importo < 0, rIncasso["description"],rIncasso["idfin"],isPGiro);
							break;
						}
					}
				}
		...
		private bool generaScrittureProvision(DataRow curr, BudgetFunction bf) {
			...
			if (idepexp == DBNull.Value) idepexp = curr["idepexp"];
			...
			if (getFaseAnticipoMissione(curr)) {
				...
				foreach (DataRow rSpesa in anticipi.Rows) {
					...
					EP.EffettuaScritturaImpegnoBudget(idepcontext, importospesa,
						...
						curr, idaccmotMain, idepexpRef, null, idrelatedRef, rSpesa["description"]);
			...
			if ((admincartotalcost != 0) && (rItinerationSetup["idaccmotive_admincar"] != DBNull.Value)
				...
				if (contiAc.Length > 0) {
					...
					EP.EffettuaScritturaImpegnoBudget(idepcontext, admincartotalcost,
						...
						curr, idaccmotMain, idepexpAdmin, null, idrelatedAdmin, "Rimborso mezzo amministrazione");
			...
			if ((owncartotalcost != 0) && (rItinerationSetup["idaccmotive_owncar"] != DBNull.Value)
				...
				if (contiOc.Length > 0) {
					...
					EP.EffettuaScritturaImpegnoBudget(idepcontext, owncartotalcost,
						...
						curr, idaccmotMain, idepexpOwn, null, idrelatedOwn, "Rimborso mezzo proprio");
			...
			if ((foottotalcost != 0) && (rItinerationSetup["idaccmotive_foot"] != DBNull.Value)) {
				...
				if (contiF.Length > 0) {
					...
					EP.EffettuaScritturaImpegnoBudget(idepcontext, foottotalcost,
						...
						curr, idaccmotMain, idepexpFoot, null, idrelatedFoot, "Rimborso spostamento a piedi");
...
//            int esercizio = CfgFn.GetNoNullInt32(meta.GetSys("esercizio"));
			...
			foreach (DataRow rit in wageadditiontax.Select()) {
				...
				object idaccmotiveCost = generaScritturaContributo(idepexp, amount, rit["taxcode"],
					curr["idupb"], curr["start"], curr["stop"], idaccmotMain, rit, curr["idser"], curr, doc.ToString());
				if (idaccmotiveCost == null) return false;
		...
		bool generaScrittureCedolino(DataRow curr, BudgetFunction bf) {
			...
			if (contiPrinc.Length == 0) {
				...
				EP.EffettuaScritturaImpegnoBudget(idepcontext, totale,
					...
					idreg, idupb,
					curr["start"], curr["stop"],
					curr, idaccmotMain, idepexp, null, idrelated, descr);
			}
		...
		object generaScritturaContributo(object idepexp, decimal importo, object taxcode, object idupb, object start,
			object stop,
			object idaccMotMain,
			DataRow ritRow, object idser, DataRow rowForIdSor = null, string description = "") {
			if (_teh == null) {
				_teh = new TaxEntryHelper(Conn);
			}
		...
		bool generaScrittureFondoEconomale(DataRow curr, BudgetFunction bf) {
			...
			if (idaccmotCost != DBNull.Value) {
				...
				if (contiCosto.Length > 0) {
					EP.EffettuaScritturaImpegnoBudget(idepcontextCosto, importo,
						contiCosto[0]["idacc"],
						idpettycashreg, idupb, curr["start"], curr["stop"], curr, idaccmotive, idepexp, null, idrelated,
						curr["description"]);
		...
		private bool generaScrittureLiquidazioneRitenute(DataRow curr, BudgetFunction bf) {
			...
			foreach (DataRow rDettaglioRitenuta in expenseTax.Rows) {
				...
				string idrelatedDebitoContoErario = "ritenuta§"+idexp.ToString() + "§" + rDettaglioRitenuta["taxcode"].ToString();
				...
				if (employtax != 0) {
					...
					foreach (DataRow currReg in regs.Rows) {
						...
						EP.EffettuaScritturaIdRelated("LIQRIT", amountToConsider, idAccRiten, currReg["idreg"], "0001", null,
			...
			foreach (DataRow rDettaglioCorrige in expenseTaxCorrige.Rows) {
				var rExpView = Conn.readObject("expenseview", QHS.CmpEq("idexp", rDettaglioCorrige["idexp"]),"idser,idupb,description");
				string idrelatedDebitoContoErario = "ritenuta§"+rDettaglioCorrige["idexp"].ToString() + "§" + rDettaglioCorrige["taxcode"].ToString();
				...
				if (admintax != 0) {
					if (idaccmotiveDebit == DBNull.Value) {
						...
						// Ciclo sui Contributi C/Ente                 
						var idexp = CfgFn.GetNoNullInt32(rDettaglioCorrige["idexp"]); //Movimento di spesa collegato alle ritenute, non della liquidazione
		...
		bool generaScrittureLiquidazioneIva(DataRow curr, BudgetFunction bf) {
			...
			foreach (DataRow rFattura in tableIva.Rows) {
				...
				if (ivaindetraibile_vendite != 0 && idepexp != DBNull.Value) {
					...
					if (ivaindetraibile_vendite != 0 && idepacc != DBNull.Value) {
						...
						//EP.EffettuaScrittura(idepcontext, ivaDetraibile, contoIvaCreditoDebito.ToString(), idreg,
						//	idupb_iva,
						//	null);
						if (istituzionale) {
							//Se istituzionale deve usare l'idrelated del documento e non del dettaglio
							EP.EffettuaScritturaIdRelated(idepcontext, ivaDetraibile, contoIvaCreditoDebito.ToString(),
								idreg, idupb_iva, null,
								null, $"inv§{rFattura["idinvkind"]}§{rFattura["yinv"]}§{rFattura["ninv"]}", null);
						}
						else {
							EP.EffettuaScritturaIdRelated(idepcontext, ivaDetraibile, contoIvaCreditoDebito.ToString(),
								idreg, idupb_iva, null,
			...
			EP.EffettuaScritturaIdRelated("LIQIVADEB", ivaDebito - ivaCredito, erarioContoIvaDebito, idregVersamento, idupb, null, null, idrelatedIvaPay, null);

			//EP.EffettuaScrittura("LIQIVADEB", ivaDebito12 - ivaCredito12, erarioContoIvaDebito12, idregVersamento,	idupb, null);
			EP.EffettuaScritturaIdRelated("LIQIVADEB", ivaDebito12 - ivaCredito12, erarioContoIvaDebito12, idregVersamento, idupb, null,null, idrelatedIvaPay, null);

			//EP.EffettuaScrittura("LIQIVADEB", ivaDebitoSplit, erarioContoIvaDebitoSplit, idregVersamento, idupb, null);
			EP.EffettuaScritturaIdRelated("LIQIVADEB", ivaDebitoSplit, erarioContoIvaDebitoSplit, idregVersamento, idupb, null,  null, idrelatedIvaPay, null);
		...
		private bool generaScrittureImportazioneCsa(DataRow curr, BudgetFunction bf) {
			...
			if (!usaContiPresentazioneDocumenti) {
				ShowMessage(
					"Attenzione, per far si che le scritture dei debiti e crediti si chiudano correttamente " +
					" è necessario configurare i conti transitori banca", "Avviso", false);
			}
				...
				foreach (DataRow rVer in T.Rows) {
					...
					foreach (InfoImpegno i in impegniBudget) {
						if (i.parIdExp == DBNull.Value && UsaImpegniDiBudget && esercizio > 2015 &&
							...
							ShowMessage(
								$"Non è stato trovato un impegno di budget per la riga di versamento n. {rVer["idver"]} id importazione {rVer["idcsa_import"]}",
								"Errore");
							return false;
						}
			...
			if (T != null) {
				...
				foreach (DataRow rVer in T.Rows) {
					...
					string idepcontext = "PRESTAZ";
					string idrelated = vecchiaGest ? null : BudgetFunction.GetIdForDocument(rVer);//+ "§" + rVer["ndetail"];
					// credito vs percipiente A DEBITO vs/conto erario, ma cambio il segno del primo dettaglio
					// il credito di solito va in AVERE in questi casi, cosi come il debito
					// essendo l'importo positivo vogliamo che il credito vada in DARE e quindi lo cambiamo di segno
		...
		private bool generaScritturePagamentoVerPartition(EP_functions EP, object idexp, object idregMovSpesa,
			bool Annullo) {
			...
			foreach (DataRow rVer in t.Rows) {
				...
				object doc = "Versamento, riga " + rVer["idver"] + "- Capitolo: " + rVer["capitolocsa"] +
				             "- Ruolo: " + rVer["ruolocsa"] + "- Matricola: " + rVer["matricola"];
				...
				if (rVer["flagclawback"].ToString().ToUpper() != "S" && idaccCost != DBNull.Value &&
				    originalAmount > 0) {
					List<InfoImpegno> impegniBudget = getAmountsForScrittureVer(rVer, rVer["idupb"], true);//idrelated di rVer
					foreach (InfoImpegno i in impegniBudget) {
				...
				if (rVer["flagclawback"].ToString().ToUpper() != "S" && idaccCost == DBNull.Value &&
					...
					foreach (InfoImpegno i in impegniBudget) {
						...
						EP.EffettuaScritturaIdRelated("PAGAM", quota * sign, idaccExpense, idregAgency, currUPB, null,
							null, i.idrelated,	//14908 Stiamo stabilendo di chiudere qui questo conto, e aprirlo in fase di elenco trasm. incasso
							$"Ritenuta positiva su riga versamento n.{rVer["idver"]}- Capitolo: {rVer["capitolocsa"]}- Ruolo: {rVer["ruolocsa"]}- Matricola: {rVer["matricola"]}");
				...
				ShowMessage(
					$"Ripartizione non trovata sulla riga di versamento {rVer["idver"]} (pagamento versamenti)");
				return false;
			}
		...
		//INCASSI VERSAMENTI 
		private bool generaScrittureIncassoVerPartition(EP_functions EP, object idinc, object idregIncasso,
			bool Annullo) {
			...
			foreach (DataRow rVer in t.Rows) {
				...
				object doc =
					$"Versamento, riga {rVer["idver"]}- Capitolo: {rVer["capitolocsa"]}- Ruolo: {rVer["ruolocsa"]}- Matricola: {rVer["matricola"]}";
				decimal quota = CfgFn.GetNoNullDecimal(rVer["quota"]);
				...
				if (rVer["flagclawback"].ToString().ToUpper() != "S" && idaccCost == DBNull.Value && amount > 0) {
					...
					if (EP.saldo != 0) {
						ShowMessage(
							$"Si è verificata una squadratura sulla riga di Versamento {rVer["idver"]} (ritenute positive)");
						return false;
					}
				...
				if (rVer["flagclawback"].ToString().ToUpper() != "S" && amount < 0) {
					...
					if (EP.saldo != 0) {
						ShowMessage("Si è verificata una squadratura sulla riga di Versamento " + rVer["idver"] +
						            " (ritenute e contributi negativi)");
						return false;
					}
				...
				ShowMessage(
					$"Ripartizione non trovata sulla riga di versamento {rVer["idver"]} (incasso versamenti)");
				return false;
			}
			...
			/*
			  //RITENUTE POSITIVE
			    if (rVer["flagclawback"].ToString().ToUpper() != "S" && idaccCost == DBNull.Value
			        ...
			        // il credito di solito va in AVERE in questi casi, cosi come il debito
			        // essendo l'importo positivo vogliamo che il credito vada in DARE e quindi lo cambiamo di segno
			        EP.EffettuaScritturaIdRelated("PAGAM", -quota*sign, idaccProceeds, idreg, idupb, null,null,null,
			            "Ritenuta positiva su riga versamento " + rVer["idver"] + "- Capitolo: " + rVer["capitolocsa"] + "- Ruolo: " + rVer["ruolocsa"] + "- Matricola: " + rVer["matricola"]);
			        EP.EffettuaScritturaIdRelated("PAGAM", quota*sign, idaccCredit, idregAgency, idupb, null,null,null,
			            "Ritenuta positiva su riga versamento " + rVer["idver"] + "- Capitolo: " + rVer["capitolocsa"] + "- Ruolo: " + rVer["ruolocsa"] + "- Matricola: " + rVer["matricola"]);
			        if (EP.saldo != 0) {
			            ShowMessage("Si è verificata una squadratura sulla riga di Versamento " + rVer["idver"] + " (ritenute positive)");
			            return false;
			        }
		...
		private bool generaScritturePagamentoRiepPartition(EP_functions EP, object idexp, object idregSpesa,
			bool Annullo) {
			string idrelatedTransitorio = "expense§" + idexp.ToString();
			object idaccPayment = _rConfig["idacc_bankpaydoc"];
			object idregCsa = _rConfig["idreg_csa"];
			...
			foreach (DataRow curr in t.Rows) {
				...
				if (curr["idacc"] != DBNull.Value && originalAmount > 0) {
					...
					if (EP.saldo != 0) {
						ShowMessage("Si è verificata una squadratura sulla riga di Riepilogo " + curr["idriep"] +
						            " (riepiloghi positivi)");
						return false;
					}
				...
				ShowMessage(
					$"Ripartizione non trovata sulla riga di Riepilogo {curr["idriep"]} (Pagamento riepiloghi)");
				return false;
			}
		...
		private bool generaScrittureIncassoRiepPartition(EP_functions EP, object idinc, object idregIncasso,
			bool Annullo) {
			string idrelatedTransitorio = "income§" + idinc.ToString();
			object idaccProceeds = _rConfig["idacc_bankprodoc"];
			object idregCsa = _rConfig["idreg_csa"];
			...
			foreach (DataRow curr in t.Rows) {
				...
				if (originalAmount < 0) {
					...
					foreach (InfoImpegno i in impegniBudget) {
						...
						if (EP.saldo != 0) {
							ShowMessage("Si è verificata una squadratura sulla riga di Riepilogo " +
							            curr["idriep"] + "- Ruolo: " + curr["ruolocsa"] + "- Matricola: " +
							            curr["matricola"] + " (lordi negativi)");
							return false;
						}
					}
				...
				ShowMessage($"Ripartizione non trovata sulla riga di Riepilogo {curr["idriep"]} (incasso riepiloghi)");
				return false;
		...
		bool generaAccertamentiRiscontiUnaFase(BudgetFunction bf, DataRow curr, int nphase) {
			...
			string tipo = kind == "D" ? "Def.Contributi" : "App.Risconti";
			object doc = $"{tipo} {curr["yload"]}";
			...
			foreach (DataRow rGrant in assetgrant.Rows) {
				...
				addImpegnoToDict(currEpAcc, idrel);
				somethingFound = true;
			}
		...
		bool generaImpegniProvisionUnaFase(BudgetFunction bf, DataRow curr, int nphase) {
			...
			//Considera tutte le righe senza la causale di ricavo
			foreach (DataRow rDettaglio in details.Select(QHC.IsNull("idaccmotive"))) {
				...
				if (detDate.Year > esercizio) {
					//object idepexp = getIdEpExpByIdRelated(idrelDetail, nphase);
					//if (idepexp!=DBNull.Value) bf.RemoveEpExp(idepexp);
					continue;
				}
		...
		bool generaAccertamentiUpbCommessaCompletata(BudgetFunction bf, DataRow curr, int nphase) {
			...
			//Accertamenti solo negli anni diversi dall'ultimo
			...
			if (costi > ricavi + riserve) {
				...
				//DataTable tCosti = ottieniCostiUPB(curr["idupb"]); //amount /  idreg
				...
				//var idreg = r["idreg"];
				//string idrel = BudgetFunction.ComposeObjects(new [] { mainidrel, idreg});
		...
		bool generaImpegniBuonoScaricoUnaFase(BudgetFunction bf, DataRow curr, int nphase) {
			...
			foreach (DataRow rAssAmm in assetamortizationunloadview.Rows) {
				...
				if ((idaccmotive == null) || (idaccmotive == DBNull.Value)) {
					...
					ShowMessage(
						"La causale " + textVar + inventoryamortization.ToString() + " non è stata impostata!",
						"Errore");
					return false;
				}
		...
		bool generaImpegniContrattoPassivoUnaFase(BudgetFunction bf, DataRow curr, int nphase) {
			...
			//task 10160 Non facciamo generare impegni per i dettagli annullati, che collegheremo al nuovo impegno
			if (yman == esercizio) {
			...
			foreach (DataRow rDettaglio in DS.Tables["mandatedetail"].Select(filterdetails)) {
				...
				var parentIdEpExp = getParentIdEpExp(curr, rDettaglio);
				...
				//Imposta l'impegno di budget relativo al dettaglio
				var currEpExp = bf.addEpExp(idregToConsider, curr["idman"], amount,
					rDettaglio["detaildescription"], dataImpegni,
					idacc, rDettaglio["idupb"], idrel, doc,
					curr["adate"],
					rDettaglio["competencystart"], rDettaglio["competencystop"], nphase, parentIdEpExp, idaccmotive);
				if (currEpExp == null) return false;
				addImpegnoToDict(currEpExp, idrel);
			...
			if (!somethingfound) {
				...
				if (!silent) {
					ShowMessage(
						$"Nessun dettaglio di competenza dell\'anno ha la causale EP quindi nessun {obj} è stato generato.",
						"Avviso", false);
				}
		...
		private bool generaImpegniProfessionale(BudgetFunction bf, DataRow curr, int nphase) {
			...
			decimal amount = CfgFn.GetNoNullDecimal(curr["totalcost"]);
			decimal sommaspese = 0;
		...
		public bool generaScrittureCommessaCompletata(DataRow curr, BudgetFunction bf) {
			...
			if (annoFine) {
				...
				DataTable tEntryDetailSource = ottieniRateiApertiProgettiInChiusura(curr["idupb"]);
				foreach (DataRow rRateo in tEntryDetailSource.Rows) {
					...
					EP.EffettuaScritturaImpegnoBudget(idepcontext, -rateo,
						...
						curr["idupb"],
						DBNull.Value, DBNull.Value,
						null, curr[campoCausaleRateoAttivo], null, rRateo["idepacc"], mainidrel);
				...
				if (costi > ricavi + riserve) {
					...
					foreach (DataRow r in tRicavi.Select()) {
						...
						EP.EffettuaScritturaImpegnoBudget(idepcontext, amount,
							...
							curr["idupb"],
							DBNull.Value, DBNull.Value,
							null, idaccmotive, null, getIdEpAccByIdRelated(idrel, 2), idrel);
		...
		private bool generaScrittureContrattoPassivo(DataRow curr, BudgetFunction bf) {
			...
			foreach (DataRow rmandet in manDet.Select(filterdetails)) {
				...
				object idaccmotive = rmandet["idaccmotive"];
				if (idaccmotive == DBNull.Value) {
					ShowMessage($"Attenzione, il dettaglio {rmandet["detaildescription"]} non ha la causale!");
					return false;
				}
			...
			foreach (var rAnnul in manDet.Select(filterdetails)) {
				...
				object idaccmotiveMainDebit2 = idAccMotiveDebit;
				if (idaccmotiveMainDebit2 == DBNull.Value) {
					idaccmotiveMainDebit2 = idaccmotiveAnnul;
				}
		...
		private bool generaScrittureRisconti(DataRow curr, BudgetFunction bf) {
			...
			DataRow mainEntry = EP.SetEntry(doc, dataAccertamenti, doc, dataAccertamenti,
				EP_functions.GetIdForDocument(curr));
			//CopySecurity(curr, mainEntry);
			EP.ClearDetails(mainEntry);
		...
		bool generaScrittureBuonoScarico(DataRow curr, BudgetFunction bf) {
			...
			if (assetamortizationrisconti.Rows.Count > 0) {
				...
				foreach (DataRow rAssAmm in assetamortizationrisconti.Rows) {
					...
					EP.EffettuaScritturaImpegnoBudget(idepcontext, importoAmm, idaccRisconto, idreg, idupb, rAssAmm,
						idaccmotiveRisconto,
						idepexp, null, idrel,
			...
			foreach (DataRow rAssAmm in assetamortizationunloadview.Rows) {
				...
				foreach (DataRow r in ContiSpesa) {
					...
					string what = ((isAmmortamento) ? "ammortamento " : "svalutazione ") +
					              ...
					              rAssAmm["ninventory"] + parte2
					              + rAssAmm["description"];
					EP.EffettuaScritturaImpegnoBudget(idepcontext, importoAmm, idacc, idreg, idupb, rAssAmm,
						idaccmotive,
				...
				if (valoreStorico > importoOriginale && idaccAssetLoadTrasferimento != null &&
					...
					foreach (DataRow conto in ContiTrasferimento) {
						EP.EffettuaScritturaSuddivisaImpegnoBudget(idepcontext, ammPregressi, conto["idacc"],
							idreg, idupb,
							null, null, null, idaccAssetLoadTrasferimento, null, null, null,
				...
				foreach (DataRow rAmm in tAmmortamento.Rows) {
					...
					if ((idaccmotiveunload == null) || (idaccmotiveunload == DBNull.Value)) {
						...
						ShowMessage("Cespite n." + rAsset["idasset"] + " / " + rAsset["idpiece"] +
						            "\nLa causale " + textVar + inventoryamortization.ToString() +
					...
					foreach (DataRow rr3 in ContiAmm) {
						...
						EP.EffettuaScritturaIdRelated(idepcontext, importoAmm, idaccamm, idreg, idupb, rAmm,
							idaccmotiveunload, null, what);
						//}
				...
				if (importoAttuale != 0) {
					...
					foreach (DataRow r in ContiSpesa) {
						...
						if (!EP.isCosto(idacc)) {
							...
							if (idepexp != DBNull.Value)
								EP.EffettuaScritturaImpegnoBudget(idepcontext, importoAttuale, idacc, idreg, idupb,
									rAsset, idaccmotive,
									idepexp, null, idrelatedEpExp,
									$"Scarico cespite {rAsset["inventory"]} {rAsset["ninventory"]} {parte}");
							else
								EP.EffettuaScritturaIdRelated(idepcontext, importoAttuale, idacc, idreg, idupb, rAsset,
									idaccmotive, idrelatedEpExp,
									$"Scarico cespite {rAsset["inventory"]} {rAsset["ninventory"]} {parte}");
						}
					}
					...
					if ((idaccRisconto != DBNull.Value) && (idaccRisconto != null)) {
						...
						EP.EffettuaScritturaImpegnoBudget(idepcontext, -importoAttuale, idaccRisconto, idreg, idupb,
							rAsset,
							idaccmotive, idepexp, null, idrel,
						...
						EP.EffettuaScritturaImpegnoBudget(idepcontext, importoAttuale, idaccReddito, idreg, idupb,
							rAsset,
							idaccmotive, null, idepaccReddito, idrelacc,
							$"Risconto Scarico cespite {rAsset["inventory"]} {rAsset["ninventory"]} {parte}");
			...
			foreach (DataRow rAssetAcquire in assetacquireview.Select()) {
				...
				if (valoreStorico > importoOriginale && idaccAssetLoadTrasferimento != null &&
					...
					foreach (DataRow conto in ContiTrasferimento) {
						EP.EffettuaScritturaSuddivisaImpegnoBudget(idepcontext, ammPregressi, conto["idacc"],
							idreg, idupb,
							null, null, null, idaccAssetLoadTrasferimento, null, null, null, idrelated + "§transfer",
				...
				if (caricoDAtrasferimento && idaccAssetLoadMotive != null) {
					...
					foreach (DataRow conto in contiScarico) {
						...
						EP.EffettuaScritturaSuddivisaImpegnoBudget(idepcontext, importoOriginale, conto["idacc"],
							idreg, idupb,
							null, null, null, idaccAssetLoadMotive, null, idepexp, null, idrelated + "§reload",
							rAssetAcquire["description"]);
					}
				...
				if (sconto > 0) {
					...
					EP.EffettuaScritturaSuddivisaImpegnoBudget(idepcontext, sconto, idaccImmobilizzazione, idreg, idupb,
						null, null, null, idaccmotiveImmobilizzazione, null, getIdEpExpByIdRelated(idrelDiscount, 2),
						null,
				...
				if (idaccAssetLoadMotive != null && idaccAssetLoadMotive != DBNull.Value) {
					...
					EP.EffettuaScritturaSuddivisaImpegnoBudget(idepcontext, importoOriginale, idaccImmobilizzazione,
						idreg, idupb,
						null, null, null, idaccmotiveImmobilizzazione, null, idepexp, null, idrelated + "§discount",
						rAssetAcquire["description"]);
					...
					if (idaccReddito != DBNull.Value) {
						if (isRicavo) {
							...
							EP.EffettuaScritturaSuddivisaImpegnoBudget(idepcontext, importoOriginale, idaccReddito,
								idreg, idupb,
								null, null, null, idaccAssetLoadMotive, null, null, idepacc, idrelated,
								rAssetAcquire["description"]);
						}
						else {
		...
		private bool generaScrittureContrattoAttivo(DataRow curr, BudgetFunction bf) {
			...
			if (yestim == esercizio) {
				...
				filterdetails = QHC.AppAnd(filterMainCurrent,
					QHC.DoPar(QHC.AppOr(QHC.IsNull("start"),
						QHC.DoPar(
							QHC.AppAnd(QHC.CmpGe("start", primoGennaio), QHC.CmpLe("start", trentunoDic))))
					));
	...
	public class EP_functions {
		...
		public void ClearDetails(DataRow mainEntry) {
			...
			DataTable Det = D.Tables["entrydetail"];
			foreach (DataRow R in Det.Select(QHC.CmpKey(mainEntry))) {
				var idacc = R["idacc"];
				if (R["idepexp"] != DBNull.Value) {
					int idepexp = CfgFn.GetNoNullInt32(R["idepexp"]);
