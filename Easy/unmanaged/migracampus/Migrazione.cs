...
namespace Install
{
	...
	public class Migrazione {
			...
			foreach (DataRow TabRow in ListaTabs.Select()) {
				...
				if (T.Columns.Count==0) {
					MessageBox.Show(form, "Non esiste la tabella "+
						T.TableName+"\npertanto non sar‡ generato lo script per tale tabella.", "ERRORE!");
					continue;
				} 
		...
		public static void CreaScriptPerViste(string filenameNonDbo, string filenameDbo, DataAccess Conn,
			...
			if (NDONE==NSP){
				...
				foreach (DataRow EL in SysObjects.Rows){
					int id = (int) EL["id"];
					if (id<0) continue;
					MessageBox.Show("La vista "+EL["name"].ToString()+" non Ë stata elaborata.");
			...
			foreach (DataRow rDepend in tDepend.Rows) {
				...
				switch (xtype) {
					...
					case "V ":
						...
						break;
					default:
						MessageBox.Show(form, xtype+" Ë un xtype non conosciuto");
						break;
				}
			}
			rView["dbo"] = isDbo ? "S" : "N";
		...
		private static bool creaEsitiDoveUnMovimentoBancarioUnMovimentoFinanziario(Form form, DataAccess destConn, 
				...
				if (MovFin.Length == 1) {
                    ...
                    if(importo > importomovfin) {
                        MessageBox.Show("Il movimento bancario " + tipo + " " +
                            ...
                            " pertanto non sar‡ possibile esitarlo totalmente");
			...
			return lanciaScript(form, destConn, bankTransaction, "Generazione Esiti dove c'Ë un mov. fin. per ogni mov. bancario");
		}
		...
		private static bool creaEsitiDoveImportoCorrMovFinIsImportoEsito(Form form, DataAccess destConn,
			...
			foreach(DataRow rEsito in tDettMovBanc.Select()) {
				...
				if (MovFin.Length > 0) {
					// Sotto Caso 2.1. Esiste un movimento finanziario il cui importo Ë pari a quello dell'esito
                    foreach(DataRow rMovFin in MovFin) {
                        ...
                        //Verifica di non averlo gi‡ utilizzato per un altro esito
			...
			return lanciaScript(form, destConn, bankTransaction, "Generazione Esiti dove c'Ë un mov. fin. con importo pari all'esito");
		}
		...
		private static bool creaEsitiDoveImportoNettoMovSpesaIsImportoEsito	(Form form, DataAccess destConn,
			...
			foreach(DataRow rEsito in tDettMovBanc.Select()) {
				...
				if (MovFin.Length > 0) {
					// Sotto Caso 2.2. (Solo Spesa) Esiste un movimento di spesa il cui importo netto Ë pari a quello dell'esito
					// Attenzione si passa al Sotto Caso 2.2. solo se il Sotto Caso 2.1. Ë fallito
					foreach(DataRow rMovFin in MovFin) {
						...
						// Se in TEMP Ë presente il movimento vuol dire che l'ho esitato completamente e quindi non vado avanti
						string filterTemp = "(idmov = " + QueryCreator.quotedstrvalue(rMovFin[idfieldmov_src], false)
							+ ")";
						DataRow [] rmovtemp = temp.Select(filterTemp);
						if ((rmovtemp.Length > 0) && (CfgFn.GetNoNullDecimal(rmovtemp[0]["residual"]) < importoEsito)) continue;
			...
			return lanciaScript(form, destConn, bankTransaction, "Generazione Esiti dove c'Ë un mov. di spesa con importo netto pari all'esito");
		}
		...
		private static bool creaEsitiDoveImportoRitMovSpesaIsImportoEsito (Form form, DataAccess destConn,
			...
			return lanciaScript(form, destConn, bankTransaction, "Generazione Esiti dove c'Ë un mov. di spesa con importo ritenute pari all'esito");
		}
			...
			foreach(string C in listaCampi) {
				switch(C) {
					...
					case "cf":
						idparam = "License_f";
                        paramvalue = rLicense[C].ToString();
						description = "Codice Fiscale Universit‡";
						break;
					case "p_iva":
						idparam = "License_P_Iva";
                        paramvalue = rLicense[C].ToString();
						description = "Partita Iva Universit‡";
						break;
				}
		...
		public static bool EffettuaControlli(Form form, DataAccess SourceConn){            
			...
			if (NPayNoNumOp>0) {
				if(MessageBox.Show(form,"Ci sono movimenti bancari senza numero operazione, che sar‡ quindi posto a zero. Procedo comunque? ","Avviso",
			...
			if (NDettPayNoNumOp>0) {
				if(MessageBox.Show(form,"Ci sono DETTAGLI di movimenti bancari senza numero operazione, che sar‡ quindi posto a zero. Procedo comunque? ","Avviso",
					MessageBoxButtons.YesNo)==DialogResult.No)
					return false;
			}
