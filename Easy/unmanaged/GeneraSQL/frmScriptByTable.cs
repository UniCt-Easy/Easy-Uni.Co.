	...
	public class frmScriptByTable : System.Windows.Forms.Form
	{
		...
		private bool Validazioni() {
			...
			if (txtOutputFile.Text.Trim() == "") {
				MetaFactory.factory.getSingleton<IMessageShower>().Show("Inserire il nome del file che verrà generato", "Attenzione",
					MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return false;
			}
		    ...
		    if (cboTable.Text.ToLower() == "audit" && radioButtonOnlyInsert.Checked == false) {
		        MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è ammesso creare degli script sulla tabella audit in update", "Errore");
                return false;
		    }
			return true;
		}
		...
		private void btnScriptSp_Click(object sender, System.EventArgs e) {
			...
			if (dr == DialogResult.OK) {
				...
				MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Non è stata specificata la cartella di destinazione");
			}
		}
