...
namespace generaSQL//GeneraSQL//
{
	...
	/// </summary>
	public class frmScriptByTable : MetaDataForm
	{
        CQueryHelper QHC;
        QueryHelper QHS;
		...
		private bool Validazioni() {
			if (cboTable.Text.Trim() == "") {
				show("Selezionare il nome della tabella / vista", "Attenzione",
		    ...
		    if (cboTable.Text.ToLower() == "audit" && radioButtonOnlyInsert.Checked == false) {
		        show("Non è ammesso creare degli script sulla tabella audit in update", "Errore");
                return false;
		    }
			return true;
		}
		...
		private void btnGenera_Click(object sender, System.EventArgs e) {
            ...
            //verifica che le tabelle siano tutte dbo o no
            int dbocount = 0;
            int nodbocount = 0;      
            ...
            if ( nTables > 1 && (dbocount > 0 && nodbocount > 0)) {
                show("Le tabelle devono avere lo stesso owner");   
                return;
			}
		    ...
		    foreach (string tableNameDirty in getTables()) {
				...
				try {
					...
					show("Errore eseguendo clear_table_info su '" + tableName + "'");
				}
                ...
                if (chkDati.Checked) {
                    ...
                    if (t.Rows.Count == 0) {
                        show("Nessuna riga trovata. Filtro:" + filter +
                ...
                string fNameNonDBO = txtOutputFile.Text.Trim();
                string fNameDBO;
                var parts = fNameNonDBO.Split('.');
                if (parts.Length > 1) {
	                parts[parts.Length - 2] += "_dbo";
	                fNameDBO = string.Join(".", parts);
                }
                else {
	                fNameDBO = fNameNonDBO + "_dbo";
                }
                GeneraSQL.GeneraStrutturaEDatiSplitted(/*true, */Conn, DS,fNameNonDBO,fNameDBO , chkAppend.Checked|| 
                                        (ntabella>1),
                    updateType, generationType, IsTable/*, chkCRLF.Checked*/);
                  
            }

            show("Script generato con successo.");
		...
		private void btnScriptSp_Click(object sender, System.EventArgs e) {
			...
			if (dr == DialogResult.OK) {
				...
				show(this, "Non è stata specificata la cartella di destinazione");
			}
		}
        ...
        private void btnTaxRate_Click(object sender, EventArgs e) {
            ...
            if (!GeneraSQL.generaTaxRateCity(Conn, txtOutputFile.Text.Trim(), filter)) {
                show(this, "Errore di generazione dello Script");
            }
            else {
                show(this, "Script generato con successo!");
            }
        ...
        private void btnAudit_Click(object sender, EventArgs e) {
            ...
            DataAccess.RUN_SELECT_INTO_TABLE(Conn, t, null, filter, null, true);
                if (t.Rows.Count == 0) {
                    show("Nessuna riga trovata nella tabella audit. Filtro:" + filter +
                        " Ultimo errore:" + Conn.LastError);
                }
                Conn.AddExtendedProperty( t);
            DS.Tables.Add(t);            
            ...
            DataAccess.RUN_SELECT_INTO_TABLE(Conn, t, null, filter, null, true);
            if (t.Rows.Count == 0) {
                show("Nessuna riga trovata nella tabella auditcheck. Filtro:" + filter +
                    " Ultimo errore:" + Conn.LastError);
            }
            Conn.AddExtendedProperty(t);
            DS.Tables.Add(t);
        ...
        private void btnDizionarioDati_Click(object sender, EventArgs e) {
            ...
            foreach (string tableNameDirty in tableLU) {
                ...
                show(@"Script generato in "+ txtOutputFile.Text.Trim(), @"Avviso");
            }
        }
