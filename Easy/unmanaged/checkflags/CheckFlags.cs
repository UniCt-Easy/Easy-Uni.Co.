	...
	public class CheckFlags
	{
		...
		public static bool GetFromInternet(DataRow R, DataAccess Conn){
			try {
                ...
                byte[] B = W.DownloadData(MetaData.errorLogUrl+"?c=" + SS2); //" "http://your-server/LiveLog/DoEasy.aspx
				string resp="";
		...
		public static void SendToInternet(DataAccess Conn, DataSet DS){
			try {
				...
				W.BaseAddress = MetaData.errorLogBaseAddress;// "http://your-server/LiveLog/";
                W.Headers.Add("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8");
                byte[] B = W.DownloadData(MetaData.errorLogUrl + "?d=" + SS); //"http://your-server/LiveLog/DoEasy.aspx
				string resp="";
				for (int i=0; i<B.Length;i++) resp=resp+ (char)B[i];
				resp= resp.Trim();
			}
