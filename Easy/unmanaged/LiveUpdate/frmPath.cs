...
;
		...
		private void cmdGenera_Click(object sender, System.EventArgs e) {
			if (txtDir.Text == "") {
				MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Specificare una directory.","XML Generator");
				return;
			}
			if (txtFile.Text == "") {
				MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Specificare un nome per il file XML da generare.","XML Generator");
				return;
			}
			if (!Directory.Exists(txtDir.Text)) {
				MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Impossibile trovare la directory " + txtDir.Text, "XML Generator");
				return;
			}
			...
			if (GenXML.GeneraFileXML(txtDir.Text, txtFile.Text, filter, null, false, out errori))
				MetaFactory.factory.getSingleton<IMessageShower>().Show("File XML generato con successo", "XML Generator",
					MessageBoxButtons.OK, MessageBoxIcon.Information);
			else
				MetaFactory.factory.getSingleton<IMessageShower>().Show("Sono stati riscontrati i seguenti errori nella generazione:\r"
