...
//		private MySQL Conn;
		...
		private void InitializeComponent()
		{
            ...
            this.label7.Text = "ATTENZIONE, il backup verr‡ salvato sul server";
		...
		private void btnEsegui_Click(object sender, System.EventArgs e) {
            ...
            if (MyConn == null) {
				MessageBox.Show("Impossibile procedere, la connessione non Ë attiva", "Attenzione",
					MessageBoxButtons.OK, MessageBoxIcon.Information);
				return;
			}
		...
		private void ImpostaGridDB(DataGrid G, DataTable T) {
			...
			//il primo bisogna valorizzarlo per far sapere che c'Ë un ordinamento
		...
		private bool CheckFile(string supporto,DataAccess Conn) {
			...
			DataTable T=Conn.SQLRunner(cmd);
			if (T!=null && T.Rows.Count > 0) {
				//la seconda colonna mi dice se Ë un file o directory
				if (T.Rows[0][1].ToString()=="1") {
					MessageBox.Show("Impossibile procedere, il supporto selezionato Ë una directory", "Attenzione",
						MessageBoxButtons.OK, MessageBoxIcon.Information);
					return false;
				}
				//la terza colonna mi dice se il path esiste
				if (T.Rows[0][2].ToString()=="0") {
					MessageBox.Show("Impossibile procedere, la directory contenente il file Ë inesistente", "Attenzione",
						MessageBoxButtons.OK, MessageBoxIcon.Information);
					return false;
				}
			}
		...
		private void btnEseguiRestore_Click(object sender, System.EventArgs e) {
			...
			try {
				...
				if (rdoDB.Checked) {
                    ...
                    //In questa modalit‡ Ë compito dell'utente fornire le opzioni esatte,
            ...
            MyConn.Close();
            MessageBox.Show(this, "Sar‡ necessario ora chiudere il programma.");
            Close();
            Application.Exit();
...
//				recovery);
		...
		/// in modalit‡ Database, altrimenti in modalit‡ periferica/file
		/// </summary>
