...
namespace ToolOnDemand//ToolOnDemand//
{
	...
	public class Frm_ToolOnDemand : System.Windows.Forms.Form
	{
		...
		private void btnFile_Click(object sender, System.EventArgs e) {
			...
			dir.Description="Specificare la cartella in cui esiste o verr‡ generato il file";
		...
		private bool IsValidMaster(DataRow R) {
			if (R["code"].ToString()=="") {
				ShowMsg("Il codice Ë obbligatorio");
				txtCodice.Focus();
				return false;
			}
			...
			if (Stato==eStato.INSERT) {
				...
				if (DS.update.Select(filter,null,DataViewRowState.CurrentRows).Length>0) {
					ShowMsg("Esiste gi‡ una riga con il codice "+R["code"].ToString());
					txtCodice.Focus();
					return false;
				}
			}
		...
		private bool IsValidDetail(DataRow R) {
			if (R["filename"].ToString()=="") {
				ShowMsg("Il nome del file Ë obbligatorio");
				txtNomeFile.Focus();
				return false;
			}
