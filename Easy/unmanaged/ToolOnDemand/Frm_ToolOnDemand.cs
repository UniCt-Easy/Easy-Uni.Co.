...
namespace ToolOnDemand//ToolOnDemand//
{
	...
	public class Frm_ToolOnDemand : System.Windows.Forms.Form
	{
		...
		private DialogResult ShowQuestion(string msg) {
			return MetaFactory.factory.getSingleton<IMessageShower>().Show(msg,"Domanda",
		...
		private bool Chiudi() {
			if (modified) {
				DialogResult res=MetaFactory.factory.getSingleton<IMessageShower>().Show("Ci sono modifche, vuoi salvare?","Attenzione",
					MessageBoxButtons.YesNoCancel,MessageBoxIcon.Question);
