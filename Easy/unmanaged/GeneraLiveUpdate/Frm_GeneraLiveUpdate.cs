...
namespace GeneraLiveUpdate //GeneraLiveUpdate//
{
	...
	/// </summary>
	public class frmGeneraLiveUpdate : Form {
		[STAThread]
		...
		private void caricaVersioni() {
			try {
				...
				var h = new Http(rempath, AppDomain.CurrentDomain.BaseDirectory);
		...
		private void popolaListView(Hashtable listafile) {
			...
			if (nonaggiornati.Items.Count > 0) {
				MessageBox.Show("Mancano file nella directory attuale, verificare attentamente prima di procedere",
					"Avviso");
			}
		...
		private void btnXMLFile_Click(object sender, EventArgs e) {
			...
			if (!GenXML.GeneraFileXML(m_LocalDir, m_XMLFile, m_Filter, skiplist, chkFiltraGiornalieri.Checked,
				...
				//else
				MessageBox.Show($"Sono stati riscontrati i seguenti errori nella generazione:\r\n{errori}",
		...
		private void menuElimina_Click(object sender, EventArgs e) {
			...
			if (tn.Level == 0) {
				if (MessageBox.Show("Cancello la cartella " + tn.Text + "?", "Conferma", MessageBoxButtons.OKCancel) !=
				...
				if (MessageBox.Show("Cancello il file " + tn.Text + "?", "Conferma", MessageBoxButtons.OKCancel) !=
				    DialogResult.OK) return;
				var tpar = tn.Parent;
				string dir = txtLocalSQL.Text + "\\" + tpar.Text;
