...
namespace EasyInstall
{
	...
	public class Iva
	{
		...
		public static void RifinisciDettagliFattura(Form form, DataAccess destConn){
			...
			//Chiede
			if (MetaFactory.factory.getSingleton<IMessageShower>().Show(form,"Si desidera rifinire i dettagli fattura dell'ultimo esercizio per farli quadrare approssimativamente "+
			...
			foreach(string spname in new string[]{
				...
				MetaFactory.factory.getSingleton<IMessageShower>().Show("Eseguo "+spname);
				destConn.CallSP(spname,new object[1]{esercmax},false,0);
			}
			MetaFactory.factory.getSingleton<IMessageShower>().Show("Rifinitura effettuata.");
		}
	}
}
