...
namespace Install
{
	...
	public class Classificazioni {
		...
		/// <returns>TRUE: Se l'esecuzione dello scrpit Ë corretta; ELSE: Se si Ë verificato un errore</returns>
			...
			foreach(DataRow rFinSorting in tFinSorting.Select()) {
				...
				if (ht[rFinSorting["idsor"]]==null) {
					MessageBox.Show("La classificazione ("+classificazione+")"+
										rFinSorting["idsor"].ToString()+" non Ë stata trovata nella tabella "+
										classificazione);
