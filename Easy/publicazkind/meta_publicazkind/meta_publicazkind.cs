...
namespace meta_publicazkind
{
    public class Meta_publicazkind : Meta_easydata 
	{
        public Meta_publicazkind(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "publicazkind") {
				Name = "Tipologie di pubblicazione ";
		...
		public override DataRow Get_New_Row(DataRow ParentRow, DataTable T) {
			...
			RowChange.MarkAsAutoincrement(T.Columns["idpublicazkind"], null, null, 0);
			RowChange.setMinimumTempValue(T.Columns["idpublicazkind"], 9990000);
			//$Get_New_Row$
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield) {
			...
			switch (edit_type) {
				case "default": {
						...
						if (R["subtitle"].ToString().Trim().Length > 256) {
							errmess = "Attenzione! Il campo 'Sotto-tipologia' può essere al massimo di 256 caratteri";
							errfield = "subtitle";
							return false;
						}
		...
		public override void DescribeColumns(DataTable T, string ListingType) {
			...
			switch (ListingType) {
				case "default": {
						DescribeAColumn(T, "idpublicazkind", "Codice", nPos++);
						DescribeAColumn(T, "title", "Tipologia", nPos++);
						DescribeAColumn(T, "subtitle", "Sotto-tipologia", nPos++);
