...
namespace registrycasellarioamministrativo_anagraficadetail {
    public partial class Frm_registrycasellarioamministrativo_anagraficadetail : MetaDataForm {

        public IOpenFileDialog opendlg;

        public Frm_registrycasellarioamministrativo_anagraficadetail() {
            InitializeComponent();
        ...
        void AbilitaDisabilitaAllegati() {
            ...
            if (Meta.IsEmpty) {
                btnAllegaCasellarioAmministrativo.Enabled = false;
                btnVisualizzaCasellarioAmministrativo.Enabled = false;
                btnRimuoviCasellarioAmministrativo.Enabled = false;

                return;
            }
            DataRow Curr = DS.registrycasellarioamministrativo.Rows[0];

            if (Curr["casellariocertification"] != DBNull.Value || Curr["idfilestorage"] != DBNull.Value) {
                btnAllegaCasellarioAmministrativo.Enabled = false;
                btnVisualizzaCasellarioAmministrativo.Enabled = true;
                btnRimuoviCasellarioAmministrativo.Enabled = true;
                ...
                btnAllegaCasellarioAmministrativo.Enabled = true;
                btnVisualizzaCasellarioAmministrativo.Enabled = false;
                btnRimuoviCasellarioAmministrativo.Enabled = false;
            }
        }
        ...
        void SalvaAllegato(string certification) {
            ...
            DataRow Curr = HelpForm.GetLastSelected(DS.registrycasellarioamministrativo);
            if (Curr == null) return;
        ...
        private void VisualizzaAllegato(string certification) {
            ...
            string prefix = "SWMORECASELLARIOA";
        ...
        private void btnAllegaDurc_Click(object sender, EventArgs e) {
            SalvaAllegato("casellariocertification");
        }

        private void btnVisualizzaDurc_Click(object sender, EventArgs e) {
            VisualizzaAllegato("casellariocertification");
        }

        private void btnRimuoviCasellarioAmministrativo_Click(object sender, EventArgs e) {
            if (DS.registrycasellarioamministrativo.Rows[0]["idfilestorage"] != DBNull.Value)
            {
                DS.registrycasellarioamministrativo.Rows[0]["idfilestorage"] = DBNull.Value;
            }
            else
            {
                DS.registrycasellarioamministrativo.Rows[0]["casellariocertification"] = DBNull.Value;
            }
            AbilitaDisabilitaAllegati();
        }
