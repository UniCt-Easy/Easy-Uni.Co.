...
{
	public partial class Frm_wageadditionattachment_detail : MetaDataForm
    {
        MetaData Meta;
        DataAccess Conn;
        CQueryHelper QHC;
        ...
        public Frm_wageadditionattachment_detail()
		{
			InitializeComponent();
        ...
        public void MetaData_AfterLink()
        {
            ...
            string filter_tipoallegato = qhs.AppAnd(qhs.CmpEq("active", "S"), qhs.BitSet("flag", 1));
            DataRow Dwageaddition = Meta.SourceRow.GetParentRow("wageaddition_wageadditionattachment"); 
            if ((Dwageaddition != null)){
                object idser = Dwageaddition["idser"];
                if ((idser != DBNull.Value) && (CfgFn.GetNoNullInt32(idser) != 0)){
                    filter_tipoallegato = qhs.AppAnd(filter_tipoallegato,
                        qhs.DoPar(qhs.AppOr(qhs.IsNull("idser"), qhs.CmpEq("idser", Dwageaddition["idser"])))
                    );
                }
            }
        ...
        public void MetaData_AfterFill(){
            DataRow R = DS.wageadditionattachment.Rows[0];
		...
		{
            if (Meta.IsEmpty) return;
            DataRow Curr = DS.wageadditionattachment.Rows[0];
