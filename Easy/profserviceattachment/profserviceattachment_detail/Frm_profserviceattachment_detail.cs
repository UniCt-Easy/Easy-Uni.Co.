...
{
	public partial class Frm_profserviceattachment_detail : MetaDataForm
    {
        MetaData Meta;
        DataAccess Conn;
        CQueryHelper QHC;
        ...
        public Frm_profserviceattachment_detail()
		{
			InitializeComponent();
        ...
        public void MetaData_AfterLink()
        {
            ...
            string filter_tipoallegato = qhs.AppAnd(qhs.CmpEq("active", "S"), qhs.BitSet("flag", 3));
            DataRow Dprofservice = Meta.SourceRow.GetParentRow("profservice_profserviceattachment");
            if ((Dprofservice != null)){
                object idser = Dprofservice["idser"];
                if ((idser != DBNull.Value) && (CfgFn.GetNoNullInt32(idser) != 0)){
                    filter_tipoallegato = qhs.AppAnd(filter_tipoallegato,
                        qhs.DoPar(qhs.AppOr(qhs.IsNull("idser"), qhs.CmpEq("idser", Dprofservice["idser"])))
                    );
                }
            }
        ...
        {
            DataRow R = DS.profserviceattachment.Rows[0];
		...
		{
            if (Meta.IsEmpty) return;
            DataRow Curr = DS.profserviceattachment.Rows[0];
