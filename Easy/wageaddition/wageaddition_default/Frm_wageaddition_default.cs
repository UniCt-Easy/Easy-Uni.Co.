    ...
    "";
        ...
        private void assegnaVarConfronto() {
            ...
            if (CfgFn.GetNoNullInt32(codicecreddeb) == 0) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stato selezionato il percipiente");
                return;
            }
            if (CfgFn.GetNoNullInt32(codiceprestazione) == 0) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stata selezionata la prestazione");
                return;
            }
            if ((datacontabile == DBNull.Value) || ((DateTime) datacontabile == QueryCreator.EmptyDate())) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stata immessa la data contabile della prestazione");
                return;
            }
            if ((compensolordo == DBNull.Value) || ((decimal) compensolordo <= 0)) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stato inserito il compenso lordo o è stato immesso un compenso negativo");
                return;
            }
        ...
        private void visualizzaMessaggio() {
            ...
            MetaFactory.factory.getSingleton<IMessageShower>().Show(this, messaggio, "Avvertimento", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }
        ...
        private void cancellaScritture() {
            ...
            if (!Post.DO_POST()) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Errore durante la cancellazione delle scritture in PD");
            }
        }
        ...
        void GeneraScrittureAP() {
            ...
            if (!(idser == DBNull.Value || CfgFn.GetNoNullInt32(idser) == 0)) {
                ...
                if (codeser == "08_PREMI") {
                    MetaFactory.factory.getSingleton<IMessageShower>().Show("Su Premi di studio e altri premi non è possibile " +
                                    "\ngenerare l'Anagrafe delle Prestazioni");
                    return;
                }
                if (codeser == "08_CONTRIB_C_ESERC") {
                    MetaFactory.factory.getSingleton<IMessageShower>().Show("Sui contributi in conto esercizio non è possibile " +
                                    "\ngenerare l'Anagrafe delle Prestazioni");
                    return;
                }
            }
        ...
        private void btnTrasfdipendenti_Click(object sender, EventArgs e) {
            if (Meta.IsEmpty) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Sei in modalità ricerca, effettua la ricerca prima di trasferire");
                return;
            }

            if (Meta.InsertMode) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Sei in modalità inserimento non puoi ancora trasferire il contratto");
                return;
            }
            ...
            if (Out == null) return;
            MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Trasferimento effettuato");
        }
    }
}
