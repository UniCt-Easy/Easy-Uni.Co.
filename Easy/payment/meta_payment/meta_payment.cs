    ...
    public class Meta_payment : Meta_easydata {
          ...
          private static bool  EnableNDoc_Treasurer(DataRow R, DataAccess Conn)
        {
            ...
            if ((idtreasurer == DBNull.Value) || (CfgFn.GetNoNullInt32(idtreasurer) == 0)) 
                return false;
            object flagautondoc_treasurer = Conn.DO_READ_VALUE("treasurer", QHS.CmpEq("idtreasurer", idtreasurer), "enable_ndoc_treasurer");
            if ((flagautondoc_treasurer != null) && (flagautondoc_treasurer != DBNull.Value) && (flagautondoc_treasurer.ToString().ToUpper() == "S"))
            {
                return true;
            }
		...
		public override DataRow Get_New_Row(DataRow ParentRow, DataTable T){
            ...
            if (T.Columns.Contains("npay_treasurer"))
            {
                ...
                RowChange.MarkAsCustomAutoincrement(T.Columns["npay_treasurer"], new RowChange.CustomCalcAutoID(CalcID));
            }
            
			DataRow R = base.Get_New_Row(ParentRow, T);
            if (EnableNDoc_Treasurer(R, Conn) && (!T.Columns.Contains("npay_treasurer")))
            {
                MessageBox.Show("Problemi nel calcolo del progressivo mandato per cassiere", "Attenzione");
            }
        ...
        public bool Verifica_config_SiopePlus() {
            ...
            object config = Conn.DO_READ_VALUE("opisiopeplus_config", QHS.CmpEq("code", "opi_siopeplus"), "config", null);
            if (config == null || config == DBNull.Value) return false;
            if (config.ToString().StartsWith("nomeutente|password|")) return false; // non è stato configurato il servizio di invio tramite Web Service
            return true;
        }
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield){
            ...
            if (R["idtreasurer"] == DBNull.Value) {
                errmess = "il cassiere è obbligatorio";
            ...
            {
                errmess = "Bisogna specificare se il mandato è in C/Competenza, in C/Residui oppure Misto";
                errfield = "flag";
                return false;
            }
			...
			DataTable Spesa = MyDS.Tables["expenseview"];
			if (Spesa==null) return true;
			decimal importo = GetCurrentImporto(MyDS);
			if (importo<=0){
				DialogResult RES= MessageBox.Show("L'importo del documento è nullo o negativo. Confermi l'operazione?", "Importo nullo o negativo", MessageBoxButtons.OKCancel);
            ...
            if ((ExpLastview != null) && Verifica_config_SiopePlus()) {
                ...
                // non è bloccate ai fini del salvataggio
