...
namespace registryottemperanzalegge68_99_anagraficadetail {
    public partial class Frm_registryottemperanzalegge68_99_anagraficadetail : MetaDataForm {

        public IOpenFileDialog opendlg;

        public Frm_registryottemperanzalegge68_99_anagraficadetail() {
            InitializeComponent();
        ...
        void AbilitaDisabilitaAllegati() {
            ...
            if (Meta.IsEmpty) {
                btnAllegaOttemperanzaLegge.Enabled = false;
                btnVisualizzaOttemperanzaLegge.Enabled = false;
                btnRimuoviOttemperanzaLegge.Enabled = false;

                return;
            }
            DataRow Curr = DS.registryottemperanzalegge68_99.Rows[0];

            if (Curr["ottemperanzacertification"] != DBNull.Value || Curr["idfilestorage"] != DBNull.Value) {
                btnAllegaOttemperanzaLegge.Enabled = false;
                btnVisualizzaOttemperanzaLegge.Enabled = true;
                btnRimuoviOttemperanzaLegge.Enabled = true;

                // File preso dall'attachment o dal MongoDb
                byte[] ByteArray = { };

                if (Curr["ottemperanzacertification"] != DBNull.Value)
                {
                    // Attachment
                    ByteArray = (byte[])Curr["ottemperanzacertification"];
                }
                else
                {
                    // MongoDb
                ...
                btnAllegaOttemperanzaLegge.Enabled = true;
                btnVisualizzaOttemperanzaLegge.Enabled = false;
                btnRimuoviOttemperanzaLegge.Enabled = false;
            }
        }
        ...
        void SalvaAllegato(string certification) {
            ...
            DataRow Curr = HelpForm.GetLastSelected(DS.registryottemperanzalegge68_99);
            if (Curr == null) return;
        ...
        private void VisualizzaAllegato(string certification) {
            ...
            string prefix = "SWMOREOTTEMPERANZA";
        ...
        private void btnAllegaDurc_Click(object sender, EventArgs e) {
            SalvaAllegato("ottemperanzacertification");
        }

        private void btnVisualizzaDurc_Click(object sender, EventArgs e) {
            VisualizzaAllegato("ottemperanzacertification");
        }

        private void btnRimuoviDurc_Click(object sender, EventArgs e) {
            if (DS.registryottemperanzalegge68_99.Rows[0]["idfilestorage"] != DBNull.Value)
            {
                DS.registryottemperanzalegge68_99.Rows[0]["idfilestorage"] = DBNull.Value;
            }
            else
            {
                DS.registryottemperanzalegge68_99.Rows[0]["ottemperanzacertification"] = DBNull.Value;
            }
            AbilitaDisabilitaAllegati();
        }
