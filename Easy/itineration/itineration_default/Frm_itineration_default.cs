...
namespace itineration_default { //missione//
    ...
    /// </summary>
    public class Frm_itineration_default : System.Windows.Forms.Form {
        #region Variabili
        ...
        private void InitializeComponent() {
			...
			this.label39.Text = "Localit‡ principale (facoltativo):";
    ...
    "  potranno  contabilizzare  solo  gli  anticipi  inerenti  alla  missione";
			...
			this.labQuotaEsente.Text = "Al rapporto di lavoro selezionato non Ë applicata la quota esente.";
			// 
			// checkBox1
			// 
    ...
    "LECT idreg FROM registrylegalstatus WHERE idposition IS NOT NULL)))";
            ...
            | System.Windows.Forms.AnchorStyles.Left) 
            | System.Windows.Forms.AnchorStyles.Right)));
			this.groupBox7.Controls.Add(this.btnDeleteSpesaSaldo);
			this.groupBox7.Controls.Add(this.btnEditSpesaSaldo);
			this.groupBox7.Controls.Add(this.dataGrid2);
			this.groupBox7.Controls.Add(this.btnInsertSpesaSaldo);
			this.groupBox7.Location = new System.Drawing.Point(480, 191);
			this.groupBox7.Name = "groupBox7";
			this.groupBox7.Size = new System.Drawing.Size(436, 280);
			this.groupBox7.TabIndex = 31;
			this.groupBox7.TabStop = false;
			this.groupBox7.Tag = "";
			this.groupBox7.Text = "Rendiconto Spese";
			// 
			// btnDeleteSpesaSaldo
			// 
			...
			this.textBox8.Text = "Somma delle Indennit‡ Lorde per ogni tappa estera";
			// 
			// textBox7
			// 
			...
			this.textBox7.Text = "Somma delle Indennit‡ Lorde per ogni tappa italiana";
			// 
			// textBox3
			// 
			...
			this.textBox2.Text = "Somma delle indennit‡ supplementari di ogni spesa";
			// 
			// textBox1
			// 
    ...
    "";
        ...
        public void MetaData_AfterLink() {
            ...
            if (tUniConfig == null || tUniConfig.Rows.Count == 0) {
                MessageBox.Show("Configurazione pluriennale non trovata", "Errore");
                controller.ErroreIrrecuperabile = true;
                return;
            }
        ...
        public void MetaData_BeforePost() {
            if (DS.itineration.Rows.Count == 0) return;
            EPM.beforePost();
            //non imposto pi˘ in automatico quel flag se ci sono spese a saldo, sotto richiesta di Emilia
            //DataRow Curr = DS.itineration.Rows[0];        
            ...
            if (DS.itineration.Rows.Count > 0 && EPM.UsaImpegniDiBudget) {
                ...
                if (curr.RowState == DataRowState.Unchanged && EPM.impegniAbilitati(curr)) {
                    int nEpExp = Conn.RUN_SELECT_COUNT("epexp",
                        QHS.CmpEq("idrelated", $"itinerationß{curr["iditineration"]}"), false);
                    if (nEpExp == 0) {
                        nEpExp = Conn.RUN_SELECT_COUNT("epexp",
                            QHS.Like("idrelated", $"itinerationß{curr["iditineration"]}ß%"), false);
                    }
                    if (nEpExp == 0) {
                        curr["completed"] = "N";
                        ...
                        MessageBox.Show(this,
                            "Non avendo generato gli impegni di budget, Ë stato rimosso il flag 'Considera eseguito quindi pagabile'",
                            "Avviso");
                    }
                }
            }
        ...
        private void CalcolaRitenute(bool ForceRecalc) {
            ...
            if ((LastOutCalcolaRitenute == null) ||
                ...
                if (supposedIncome == null || supposedIncome == DBNull.Value) {
                    MessageBox.Show(
                        "I dati relativi alla posizione retributiva dell'incaricato sono incomplete o mancanti. Il reddito presunto considerato sar‡ pari a zero.",
                        "Avviso");
                }
        ...
        void FiltraComboPrestazioneInBaseANiente(bool enableold) {
            ...
            if (enableold) {
                ...
                if (((DataTable) cmbPrestazione.DataSource).Select(filter).Length == 0) {
                    MessageBox.Show("Attenzione: la prestazione selezionata " +
                                    "non Ë previsto come tipo di prestazione assimilabile ad una missione");
                }
            }
        ...
        void DetectPosGiuridica() {
            ...
            object codicecreddeb = Curr["idreg"];
            if ((datainizio == DBNull.Value) || (((DateTime) datainizio) == QueryCreator.EmptyDate())) {
                MessageBox.Show("Non Ë stata immessa la data inizio prestazione");
                return;
            }
            if ((datafine == DBNull.Value) || (((DateTime) datafine) == QueryCreator.EmptyDate())) {
                MessageBox.Show("Non Ë stata immessa la data fine prestazione");
                return;
            }
            if ((codicecreddeb == DBNull.Value) || (((int) codicecreddeb) <= 0)) {
                MessageBox.Show("Non Ë stato selezionato l'incaricato");
                return;
            }
            string strdate = QueryCreator.quotedstrvalue((DateTime) datainizio, true);
            string strdatefine = QueryCreator.quotedstrvalue((DateTime) datafine, true);
        ...
        private void ImpostaPosGiuridica(bool changerole) {
            ...
            if (NposGiuridiche > 1) {
                ...
                while (true) {
                    ...
                    MessageBox.Show("E' necessario selezionare un Inquadramento dell'incaricato.");
                }
        ...
        /*
		La formula per il calcolo del coefficiente Ë la seguente:

		1 / (1 - (1 - TotAliAssDip - TotAliPreDip) * TotAliFisDip);

		dove: 

		TotAliAssDip Ë la somma delle aliquote delle ritenute assistenziali a carico del percipiente (*)

		TotAliPreDip Ë la somma delle aliquote delle ritenute previdenziali a carico del percipiente (*)

		TotAliFisDip Ë la somma delle aliquote delle ritenute fiscali a carico del percipiente (*)
		...
		*/
        ...
        decimal CoefficienteLordizzazione(DataTable ritenute) {
            ...
            string help2 =
                ...
                ") Ë la somma delle aliquote delle ritenute assistenziali a carico del percipiente\r\n" +
                "TotAliPreDip (" + TotAliPreDip.ToString("c") +
                ") Ë la somma delle aliquote delle ritenute previdenziali a carico del percipiente\r\n" +
                "TotAliFisDip (" + TotAliFisDip.ToString("c") +
                ") Ë la somma delle aliquote delle ritenute fiscali a carico del percipiente";
        ...
        void RicalcolaMissione() {
            ...
            ImpostaPosGiuridica(false);
            //ImpostaImpEsente(true); gi‡ chiamato da ImpostaPosGiuridica
            ...
            //Aggiorna  l'indennit‡ e la perc. di anticipo delle tappe.
        ...
        void AzzeraIndennitaTappa(DataRow Tappa) {
            MessageBox.Show("Non Ë stato possibile aggiornare l'indennit‡ della tappa N." +
                            Tappa["lapnumber"].ToString() +
                            "(" + Tappa["description"].ToString() + "), che Ë stata quindi lasciata invariata"
                );
            //Tappa["allowance"]=0;
        }
        ...
        private void btnAggiorna_Click(object sender, System.EventArgs e) {
            ...
            if (MessageBox.Show(this,
                ...
                "Le indennit‡ corrisposte delle tappe e gli anticipi delle spese.",
        ...
        private void btnSituazione_Click(object sender, System.EventArgs e) {
            ...
            try {
                ...
                MessageBox.Show("Errore chiamando show_itineration." +
                                "Dettaglio:" + E.Message);
            }
        ...
        void VisualizzaEtichetteAP() {
            ...
            if (Conn.RUN_SELECT_COUNT("serviceregistry", QHS.CmpEq("idrelated", idrelated), true) == 0) {
                ...
                // Se l'AP non Ë stata generata visualizza il button Genera, MA se Ë una missione inserita tramite web....
                if (CheckWeb()) {
                    DataRow Curr = DS.itineration.Rows[0];
                    int iditinerationstatus = CfgFn.GetNoNullInt32(Curr["iditinerationstatus"]);

                    // per le missioni inserite da web disabilitare la generazione di anagrafe prestazioni/impegni du budget ecc
                    // se lo stato non Ë "Approvata"
                    if (iditinerationstatus != 6) {
                        btnGeneraAP.Visible = false;
                        btnCollegaAP.Visible = false;
                        labAPnongenerato.Visible = false;
            ...
            if (DS.itinerationauthagency.Select(QHC.CmpNe("flagstatus", "S")).Length > 0) {
                ...
                foreach (DataRow R in DS.itinerationauthagency.Select()) {
                    ...
                    if (!asked) {
                        asked = true;
                        if (
                            MessageBox.Show(this, "Lo stato di autorizzazione sar‡ resettato.", "Avviso",
        ...
        private void btnInserisciAnticipo_Click(object sender, EventArgs e) {
            ...
            if (CfgFn.GetNoNullDecimal(Curr["totadvance", DataRowVersion.Original]) == 0) {
                ...
                if ((CfgFn.GetNoNullDecimal(advance) <= CfgFn.GetNoNullDecimal(totalgross))) {
                    ...
                    MessageBox.Show(this, "L''importo dell'anticipo non puÚ superare quello del lordo");
                }
            }
