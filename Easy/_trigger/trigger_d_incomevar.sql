...
if exists (select * from dbo.sysobjects where id = object_id(N'[trigger_d_incomevar]') and OBJECTPROPERTY(id, N'IsTrigger') = 1)
drop trigger [trigger_d_incomevar]
GO

CREATE   TRIGGER [trigger_d_incomevar] ON incomevar FOR DELETE
AS BEGIN
IF @@ROWCOUNT > 0
BEGIN
	...
	IF (SELECT COUNT(*) FROM incomeyear WHERE idinc = @idinc AND ayear = @yvar) = 0
	BEGIN	
		RETURN
	END
