...
if exists (select * from dbo.sysobjects where id = object_id(N'[trigger_d_epaccvar]') and OBJECTPROPERTY(id, N'IsTrigger') = 1)
drop trigger [trigger_d_epaccvar]
GO

CREATE TRIGGER [trigger_d_epaccvar] ON [epaccvar] FOR DELETE
AS 
BEGIN
	...
	IF (SELECT COUNT(*) FROM epaccyear WHERE idepacc = @idepacc AND ayear = @yvar) = 0
	BEGIN	
		RETURN
	END
