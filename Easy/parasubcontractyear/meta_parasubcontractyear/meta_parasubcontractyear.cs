...
//parasubcon§06000011
			...
			string q = "SELECT * FROM servicetax JOIN tax " +
			           ...
			           "WHERE " + QHS.AppAnd(QHS.CmpEq("tax.taxkind", 3),
				           QHS.CmpEq("servicetax.idser", Rparasubcontract["idser"]));
			...
			// commento a seguito di task 15545
			//if (inps15 && (!R["idotherinsurance"].Equals("002"))) {
			//	errmess =
			//		"Per i titolari di pensione diretta è necessario specificare 'Titolari di pensione diretta' come altra forma assicurativa";
			//	return false;
			//}

			//if (!inps15 && (R["idotherinsurance"].Equals("002"))) {
			//	errmess =
			//		"La prestazione scelta non è compatibile con l'altra forma assicurativa 'Titolari di pensione diretta'";
			//	errfield = "idser";
			//	return false;
			//}
			...
			if ((CfgFn.GetNoNullDecimal(R["regionaltax"]) > 0) ||
				...
				DateTime datamin = (dataInizio.Year == (int) R["ayear"])
					? dataInizio
					: new DateTime((int) R["ayear"], 1, 1);
				DateTime datamax = (dataFine.Year == (int) R["ayear"])
					? dataFine
					: new DateTime((int) R["ayear"], 12, 31);
				if ((CfgFn.GetNoNullInt32(R["startmonth"]) < datamin.Month) ||
				    (CfgFn.GetNoNullInt32(R["startmonth"]) > datamax.Month)) {
					errmess =
						"Il mese da cui si intende cominciare a pagare le rate delle addizionali non è congruo con l'esistenza del contratto";
					errfield = "startmonth";
					return false;
				}

				// Le rate devono essere pagate entro la fine del contratto e comunque entro l'anno solare
				if (CfgFn.GetNoNullInt32(R["startmonth"]) + CfgFn.GetNoNullInt32(R["ratequantity"]) - 1 >
				    datamax.Month) {
					errmess =
						"Le rate devono essere pagate entro la fine del contratto e comunque entro l'anno fiscale";
					errfield = "ratequantity";
					return false;
				}
			}
			...
			if (CfgFn.GetNoNullDecimal(R["citytax_account"]) > 0) {
				...
				if (CfgFn.GetNoNullInt32(R["startmonth_account"]) <= 0) {
					errmess =
						"Inserire il mese da cui iniziare a pagare le rate dell'acconto dell'addizionale comunale";
				...
				if ((CfgFn.GetNoNullInt32(R["startmonth_account"]) < datamin.Month) ||
					...
					errmess =
						"Il mese da cui si intende cominciare a pagare le rate dell'acconto dell'addizionale comunale non è congruo con l'esistenza del contratto";
					errfield = "startmonth_account";
					return false;
				}

				// Le rate devono essere pagate entro la fine del contratto e comunque entro l'anno solare
				if (CfgFn.GetNoNullInt32(R["startmonth_account"]) + CfgFn.GetNoNullInt32(R["ratequantity_account"]) -
				    1 > datamax.Month) {
					errmess =
						"Le rate devono essere pagate entro la fine del contratto e comunque entro l'anno fiscale";
					errfield = "ratequantity_account";
