	...
	public class Frm_pettycashoperation_wiz_reintegro : System.Windows.Forms.Form {
		...
		public void MetaData_AfterLink() {
			...
			Messaggi.Add("warn_salvataggio", "Avvertimento: se l'operazione viene confermata, verranno generati i " +
				"seguenti movimenti di contabilit‡ finanziaria. Se si vuole inserire l'operazione di reintegro " +
				"cliccare su Fine.");
		...
		private void SaveData() {		
            ...
            //qui Ë sicuro che calcola le class.indirette dei movimenti di reintegro perchÈ sono automatismi
...
//			if (MyPostData.DO_POST())ViewAutomatismi(DS);
        ...
        {
            // se c'Ë la riga con idexp e idunderwriting sommi l'importo
            // altrimenti la crei
		...
		bool CalcolaClassificazioni(DataAccess Conn,DataSet Auto){
			...
			foreach(DataRow CF in Auto.Tables["pettycashoperationsorted"].Rows){				
				...
				// in pettycashexpense noperation Ë sempre uguale a quello di reintegro (Maria)
		...
		void RicalcolaCampiCalcolati(){			
			foreach(DataRow RS in DS.expenseview.Rows){
                ...
                if (livsup != DBNull.Value) {
                    ...
                    if (Sup==null) {
                        MessageBox.Show($"Il movimento di spesa {nomefasesup} di id {livsup} non esiste o non Ë accessibile con il ruolo corrente", 
                                "Errore");
