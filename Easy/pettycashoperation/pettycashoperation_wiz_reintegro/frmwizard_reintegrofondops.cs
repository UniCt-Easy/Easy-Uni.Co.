	...
	public class Frm_pettycashoperation_wiz_reintegro : System.Windows.Forms.Form {
		...
		bool CallStoredProcedure() {
			...
			foreach(DataRow r in Out.Tables[0].Select(QHC.IsNotNull("parentidexp"))) {
                ...
                if (rrFound.Length == 0) {
                    ...
                    MetaFactory.factory.getSingleton<IMessageShower>().Show(err, "Errore");
		...
		private void SaveData() {		
			...
			if (DS.expense.Rows.Count==0){
				MetaFactory.factory.getSingleton<IMessageShower>().Show("Non sono state selezionate righe da reintegrare.");
				DS.AcceptChanges();
				return;
			}
		...
		void RicalcolaCampiCalcolati(){			
			foreach(DataRow RS in DS.expenseview.Rows){
                ...
                if (livsup != DBNull.Value) {
                    ...
                    if (Sup==null) {
                        MetaFactory.factory.getSingleton<IMessageShower>().Show($"Il movimento di spesa {nomefasesup} di id {livsup} non esiste o non è accessibile con il ruolo corrente", 
                                "Errore");
