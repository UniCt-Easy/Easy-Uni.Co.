	...
	public class Frm_pettycashoperation_default : System.Windows.Forms.Form {
        ...
        private void btnSpesa_Click(object sender, System.EventArgs e) {
            ...
            if (idpettycash.ToString() == "0") {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Selezionare prima il fondo economale.");
                return;
            }
            string filterpcash = QHC.CmpEq("idpettycash", idpettycash);
            DataRow[] PettyCash = DS.pettycashsetup.Select(filterpcash);
            if (PettyCash.Length == 0) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stata inserita la configuraz. del fondo economale per quest'anno");
                return;
            }
            ...
            if (oldIdExp != newIdExp) {
                if (MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Aggiorno i campi descrizione, documento e data documento dell'operazione " +
        ...
        void CercaFinanziamento_SolaCassa(DataRow rExpenseView) {
            ...
            if (F == null || F.Rows.Count == 0) {
                //MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Non ci sono finanziamenti per questa coppia progetto-voce di bilancio",
                //    "Informazione");
                return;
            }
        ...
        */
		...
		void ReCalcImporto_Occasionale(){
            ...
            if(importo < 0) {
                importo = 0;
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Di questa prestazione è stato già contabilizzato l'importo netto");
            }
		...
		void ReCalcImporto_Professionale(){
            ...
            if(importo < 0) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("E' stato già contabilizzato il netto di questo contratto");
                importo = 0;
            }
