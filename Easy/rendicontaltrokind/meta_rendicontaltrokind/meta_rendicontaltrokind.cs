...
namespace meta_rendicontaltrokind
{
    public class Meta_rendicontaltrokind : Meta_easydata 
	{
        public Meta_rendicontaltrokind(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "rendicontaltrokind") {
				Name = "Tipologia delle attività da rendicontare oltre le lezioni ";
		...
		public override DataRow Get_New_Row(DataRow ParentRow, DataTable T) {
			...
			RowChange.MarkAsAutoincrement(T.Columns["idrendicontaltrokind"], null, null, 0);
			RowChange.setMinimumTempValue(T.Columns["idrendicontaltrokind"], 9990000);
			//$Get_New_Row$
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield) {
			...
			switch (edit_type) {
				case "default": {
						...
						if (R["title"].ToString().Trim().Length > 128) {
							errmess = "Attenzione! Il campo 'Tipologia' può essere al massimo di 128 caratteri";
							errfield = "title";
							return false;
						}
						break;
		...
		public override void DescribeColumns(DataTable T, string ListingType) {
			...
			switch (ListingType) {
				case "default": {
						DescribeAColumn(T, "idrendicontaltrokind", "Codice", nPos++);
