	...
	public class Meta_taxsetup : Meta_easydata {
		...
		public override bool IsValid(DataRow R,out string errmess, out string errfield){            
			...
			if ((R["flagadminfinance"].ToString().ToLower()=="s")&&
				...
				errmess="In base alle informazioni inserite è necessario inserire una voce di bilancio contributi.";
				errfield=null;
				return false;
			}
			if ((R["idexpirationkind"]==DBNull.Value) || (R["idexpirationkind"].ToString()=="0")){
				errmess="E' necessario selezionare una periodicità valida per la ritenuta.";
				errfield=null;
				return false;
			}
            ...
            if (R["taxpaykind"] == DBNull.Value) {
                errmess = "Specificare la modalità nella quale effettuare il versamento delle ritenute";
                errfield = "taxpaykind";
                return false;
            }
