...
{
    public class Meta_corsostudiocaratteristica : Meta_easydata 
	{
        public Meta_corsostudiocaratteristica(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "corsostudiocaratteristica") {
				Name = "Caratteristiche del corso di studio";
		...
		public override DataRow Get_New_Row(DataRow ParentRow, DataTable T) {
			...
			RowChange.MarkAsAutoincrement(T.Columns["idcorsostudiocaratteristica"], null, null, 0);
			RowChange.setMinimumTempValue(T.Columns["idcorsostudiocaratteristica"], 9990000);
			//$Get_New_Row$
			...
			switch (edit_type) {
				case "default": {
						...
						if (R["idcorsostudio"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'Idcorsostudio' è obbligatorio";
							errfield = "idcorsostudio";
							return false;
						}
						if (R["idcorsostudiocaratteristica"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'Identificativo' è obbligatorio";
							errfield = "idcorsostudiocaratteristica";
							return false;
						}
		...
		public override void DescribeColumns(DataTable T, string ListingType) {
			...
			switch (ListingType) {
				case "default": {
						...
						DescribeAColumn(T, "!idcorsostudio_corsostudio_title", "Idcorsostudio", nPos++);
