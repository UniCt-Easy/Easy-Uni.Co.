...
namespace EasyPagamenti {
    ...
    public class GetVars {
        ...
        /// <returns></returns>
        public static DataAccess GetAppSystemDataAccess(out string error) {
	        ...
	        GetAppConfigVar(DS, "Server").TrimEnd(), GetAppConfigVar(DS, "Database").TrimEnd(),
	        GetAppConfigVar(DS, "User").TrimEnd(), GetAppConfigVar(DS, "Password").TrimEnd(),
            ...
            if (!Conn.Open()) {
                error = "Collegamento fallito a #" +
                        GetAppConfigVar(DS, "Server") + "#" + GetAppConfigVar(DS, "Database") + "#" +
                        GetAppConfigVar(DS, "User") + "#" + "Password"/*GetAppConfigVar(DS, "Password").TrimEnd()*/ + "#";
                return null;
            };            
        ...
        public static DataAccess GetSystemDataAccess(System.Web.UI.Page P, out string error) {
            ...
            DataAccess Conn = new DataAccess("myDsn",
                ...
                GetConfigVar(P, "User").TrimEnd(), GetAppConfigVar(DS, "Password").TrimEnd(),
                DateTime.Now.Year, DateTime.Now);
            ...
            if (!Conn.Open()) {
                error = "Collegamento fallito a #" +
                    ...
                    GetConfigVar(P, "User") + "#" + "Password"/*GetAppConfigVar(DS, "Password").TrimEnd()*/ + "#";
                return null;
            };
	    ...
	    public static DataSet GetAppConfigDataSet() {
	        FileStream FileS = null;
	        try {
	            DataSet DS = new DataSet();

	            string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"cfg");
	    ...
	    public static string GetAppConfigVar(DataSet DS, string varname) {
	        DataTable T = DS.Tables[0];
	        if (T.Rows.Count == 0) return null;
	        if (!T.Columns.Contains(varname)) return null;
	        DataRow R = T.Rows[0];
	        if (R[varname] == DBNull.Value) return null;
        ...
        /// <param name="err"></param>
        /// <returns></returns>
        public static DataAccess CreateAppUserConn(DataRow RowDipartimento,
                string user, string password, DateTime DataCont,
            out string err) {
            if (user != null && user != "") RowDipartimento["UserDB"] = CryptPassword(user);
            if (password != null && password != "") RowDipartimento["PassDB"] = CryptPassword(password);
            ...
            C.SetUsr("localreportdir",GetAppConfigVar(GetAppConfigDataSet(), "Report"));
            //Conn.sys["localreportdir"]= GetConfigVar(P,"Report");
            return C;
        }
