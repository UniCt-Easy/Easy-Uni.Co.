...
namespace pcc_default {
    public partial class frmDifferenze : MetaDataForm {
        ...
        public frmDifferenze(DataTable trasmesso, DataTable esito) {
            InitializeComponent();
            ExcelMenu = new ContextMenu();
            ExcelMenu.MenuItems.Add("Excel", Excel_Click);

            HelpForm.SetDataGrid(grdTrasmesso, trasmesso);
           
            grdTrasmesso.ContextMenu = ExcelMenu;

            HelpForm.SetDataGrid(grdEsito, esito);          
            grdEsito.ContextMenu = ExcelMenu;
