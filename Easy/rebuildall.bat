...
if not exist %MSBUILD% set MSBUILD="%ProgramW6432%\Microsoft Visual Studio\2022\Community\MSBuild\Current\Bin\MSBuild.exe"
...
for  %%i in (%WHAT%) do (
  echo Rebuilding: %%~ni [%%i]
  call %MSBUILD% %%i /m /nologo /verbosity:quiet /t:Rebuild /p:Configuration=Debug  /clp:ErrorsOnly
  if errorlevel 1 (set /a errors += 1)
  rem dir %~dp0\mainform\bin\Debug\*.dll > "rebuild_%%i.lst"
)
