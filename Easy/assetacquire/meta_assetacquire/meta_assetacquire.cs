...
//			}
			...
			{
				errmess = "E' necessario specificare una quantit‡ maggiore di zero";
				errfield = "number";
				return false;
			}
			if (CfgFn.GetNoNullDecimal(R["taxable"]) < 0) {
				errmess = "Non Ë possibile specificare un Imponibile negativo";
				errfield = "taxable";
				return false;
			}
			if (CfgFn.GetNoNullDecimal(R["discount"]) < 0) {
				errmess = "Non Ë possibile specificare una Percentuale di Sconto negativa";
				errfield = "discount";
				return false;
			}
			if (CfgFn.GetNoNullDecimal(R["taxrate"]) < 0) {
				errmess = "Non Ë possibile specificare un'Aliquota IVA negativa";
				errfield = "taxrate";
				return false;
			}
