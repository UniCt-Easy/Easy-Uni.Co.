...
namespace servicetrasmission_default {
    ...
    " prestazioni dell\'anno indicato";
			...
			this.label6.Text = "Comunicazione entro 15 giorni dal conferimento o dallíautorizzazione ";
			// 
			// label5
			// 
    ...
    "laboratori esterni ";
			...
			this.label18.Text = "Cancellazione dei dati gi‡ trasmessi relativi ai Dipendenti di questo Ente";
			// 
			// label19
			// 
			...
			this.label19.Text = "Cancellazione dei dati gi‡ trasmessi relativi ai Consulenti e Collaboratori ester" +
    "ni ";
			// 
			// tabAltro
			// 
    ...
    "o ësospesoí.";
		...
		private bool controlla_validita_codici(string kind) {
			...
			if (kind == "d") {
				...
				if (financialactivity.Rows.Count != 0) {
					mess1 = "I seguenti incarichi:" + mess1 + " hanno codice Attivit‡ Economica non valido. \n\r";
					error = true;
				}
			}
			...
			if (kind == "c") {
				...
				if (acquirekind.Rows.Count != 0) {
					mess3 = "I seguenti incarichi:" + mess3 + " hanno codice Modalit‡ Acquisizione non valido. \n\r";
					error = true;
				}
			}
			...
			MyQuery =
				...
				"  and serviceregistrykind.totransmit = 'S'" + filterAnno + filterNumero;
			if (kind == "d") {
			...
			if (kind == "d") {
				...
				if (PivaConferente.Rows.Count != 0) {
					mess7 = "I seguenti incarichi a dipendenti:\n\r" + mess7 +
					        " \n\r hanno Conferente = PERSONA GIURIDICA SENZA CF RILASCIATO IN ITALIA ma la p.iva non Ë estera.\n\r";
					error = true;
				}
			}
			...
			MyQuery =
				...
				" and A.idregistryclass = 22 and (serviceregistrykind.totransmit= 'S' or SR.idserviceregistrykind is null )" + filterAnno + filterNumero;
			if (kind == "c") {
				...
				if (PF_Societa.Rows.Count != 0) {
					mess8 = "I seguenti incarichi a consulenti:\n\r" + mess8 +
					        " \n\r hanno Tipologia consulente =Societ‡ ma sono registrate nell'Anagrafica come Persona Fisica, infatti hanno il codice Fiscale di 16 cifre.\n\r";
					error = true;
				}
			}
		...
		List<dipendente_typeNuovoPagamento> esportaPagamentoDipendente(DataRow R, string filter, string tag,
			...
			else { // se Ë un Dipendente non deve scrivere il semestre e sommare gli importi
		...
		private bool Annulla(string tipo) {
			...
			if (MessageBox.Show(this, "Sar‡ annullata la Trasmissione di TUTTI gli Incarichi e i Pagamenti dei " +
		...
		private bool ElaboraFileRitorno(XmlDocument document, List<string> messages = null) {
			...
			foreach (XmlElement esitoInserimentoIncarichi in Comunicazione.GetElementsByTagName("esitoInserimentoIncarichi")) {
				...
				if (esitoFile.Value.ToString().ToUpper() == "KO") {
					MessageShow(
						"Il file Trasmesso Ë stato elaborato con Esito Errato. Verificare gli errori segnalati nel file.",
						"Attenzione", messages);
					return false;
				}
				...
				foreach (XmlElement esitoNuoviIncarichi in esitoInserimentoIncarichi.GetElementsByTagName("esitoNuoviIncarichi")) {
					foreach (XmlElement dipendente in esitoNuoviIncarichi.GetElementsByTagName("dipendente")) {
						...
						if (esito.Value.ToString().ToUpper() == "OK") {
							// MessageBox.Show(this, "Il file Trasmesso Ë stato APPROVATO. Procedere col salvataggio... ");
			...
			if (modpagamenti) {
				...
				foreach (
					...
					if (CfgFn.GetNoNullInt32(rConsulente["yservreg"]) < 2011) {
						// Cambia il nome del campo attivit‡ economica: fino al 2010 Ë idfinancialactivity, dal 2011 diventa apfinancialactivitycode.
		...
		private bool GenerazioneFileUpdate(string tipo, int semestre, bool modpagamenti, int tipologia,
			...
			//// Per comunicare questi pagamenti, essendo di incarichi 2010, dovrÚ generare il file nel 2010 perchË
		...
		private bool GenerazioneFileInsert(string tipo, object semestre, int tipologia, bool perla = false) {
			...
			if (tipo.ToUpper() == "D") {
				...
				if (DS.serviceregistry.Rows.Count == 1000) {
					...
					if (n > 1000) {
						MessageBox.Show(this,
							"Ci sono pi˘ di 1000  incarichi da trasmettere, ma in base alle specifiche tecniche di trasmissione, " +
							"il file ne puÚ contenere massimo 1000. Si dovr‡, quindi, procedere con una seconda trasmissione.");
					}
				}
		...
		private void btnInviaModifichePagamentiConsulenti1Sem_Click(object sender, EventArgs e) {
			if (!controlla_validita_codici("c")) return;
		...
		variazione_incarico_dipendente updateDipendente( DataRow rDipendente) {
			...
			//w.datiincarico.dataRevoca = non c'Ë
		...
		inserimentoincaricodipendente inserisciDipendente(DataRow rDipendente) {
			...
			if (rDipendente["idapactivitykind"] != DBNull.Value) {
				...
				//}
				//else {
				//	rDipendente["perla_error"] = $"Tipo attivit‡ {rDipendente["idapactivitykind"]} non trovato.";
				//	return null;
				//}
			}
		...
		inserimento_incarico_consulente inserisciConsulente(DataRow rConsulente) {
			...
			if (rConsulente["ordinancelink"] != DBNull.Value) {
				...
				rConsulente["perla_error"] = "Link al decreto di conferimento dellíincarico assente";
				return null;
			}
