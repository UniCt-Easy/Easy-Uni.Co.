...
{
    public class Meta_mutuazione : Meta_easydata 
	{
        public Meta_mutuazione(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "mutuazione") {
				Name = "Mutuazione o fruizione";
		...
		public override DataRow Get_New_Row(DataRow ParentRow, DataTable T) {
			...
			RowChange.MarkAsAutoincrement(T.Columns["idmutuazione"], null, null, 0);
			RowChange.setMinimumTempValue(T.Columns["idmutuazione"], 9990000);
			//$Get_New_Row$
			...
			switch (edit_type) {
				case "default": {
						...
						if (R["idcanale"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'Canale mutuante' è obbligatorio";
							errfield = "idcanale";
							return false;
						}
						if (R["idcanale_from"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'Canale mutuato' è obbligatorio";
							errfield = "idcanale_from";
							return false;
						}
						...
						if (R["riferimento"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'Di riferimento per il canale' è obbligatorio";
		...
		public override void DescribeColumns(DataTable T, string ListingType) {
			...
			switch (ListingType) {
				case "default": {
						DescribeAColumn(T, "idmutuazione", "Identificativo", nPos++);
						DescribeAColumn(T, "!idcanale_from_canale_title", "Canale mutuato", nPos++);
						DescribeAColumn(T, "riferimento", "Di riferimento per il canale", nPos++);
						break;
					}
				//$DescribeAColumn$
			}
