	...
	public class FrmEntry_default : System.Windows.Forms.Form {
        ...
        void RicalcolaRatei() {
            ...
            foreach (DataRow RC in DS.entrydetail.Select()) {
                ...
                if (DS.account.Select(QHC.CmpEq("idacc", idaccParentDetail)).Length == 0) {
                    MetaFactory.factory.getSingleton<IMessageShower>().Show($"Conto di chiave {idaccParentDetail} non trovato nella tabella account.",
                        "Errore nei dati");
                    return;
                }
            ...
            foreach (DataRow RC in DS.entrydetail.Select()) {
                ...
                if (DS.account.Select(QHC.CmpEq("idacc", RC["idacc"])).Length == 0) {
                    MetaFactory.factory.getSingleton<IMessageShower>().Show($"Conto di chiave {RC["idacc"]} non trovato nella tabella account.",
                        "Errore nei dati");
                    return;
                }
