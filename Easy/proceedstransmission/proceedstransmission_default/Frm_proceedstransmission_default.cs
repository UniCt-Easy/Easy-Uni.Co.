...
namespace proceedstransmission_default { //trasmdocincasso//
	...
	/// </summary>
	public class Frm_proceedstransmission_default : MetaDataForm {
		...
		public void MetaData_AfterActivation() {
			...
			if (DS.config.Rows.Count > 0) {
				...
				MetaFactory.factory.getSingleton<IMessageShower>().Show(
					"Errore: Non è stata ancora effettuata la configurazione delle entrate per l'esercizio corrente.");
				Meta.CanSave = false;
		...
		private void CollegaScollegaAnnullamentiTotali() {
			...
			foreach (DataRow rVar in DS.incomevarview.Select()) {
				if (rVar.RowState == DataRowState.Detached) continue;
				if (CfgFn.GetNoNullInt32(rVar["autokind"]) != 11) continue;
			...
			foreach (DataRow rVar in DS.incomevarview.Select()) {
				...
				if ((originalVal == DBNull.Value) && (currVal != DBNull.Value)) {
					...
					foreach (DataRow VarAnn in T.Rows) {
						string filterRow = QHC.AppAnd(QHC.CmpEq("idinc", VarAnn["idinc"]),
		...
		private void CollegaAnnullamentiParziali()
		{
			...
			foreach (DataRow rVar in DS.incomevarview.Select())
			{
				...
				{
					string filter = "";
					filter = QHC.AppAnd(QHC.CmpEq("kpro", rVar["kpro"]), QHC.CmpEq("autokind", 10));
					foreach (DataRow VarAnn in DS.incomevarview.Select(filter))
					{
						VarAnn["kproceedstransmission"] = originalVal;
						VarAnn.AcceptChanges();
			...
			foreach (DataRow rVar in DS.incomevarview.Select())
			{
				...
				if ((originalVal == DBNull.Value) && (currVal != DBNull.Value))
				{
					...
					hVarTotali[rVar["kpro"]] = 1;
					filter = QHS.AppAnd(QHS.CmpEq("kpro", rVar["kpro"]), QHS.CmpEq("autokind", 10),QHS.IsNull("kproceedstransmission"));
					// select da db delle var collegate
					DataTable T = Meta.Conn.RUN_SELECT("incomevarview", "*", null, filter, null, false);
					if (T == null) continue;
