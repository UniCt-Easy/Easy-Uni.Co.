...
namespace assetunload_generautoamortization {
    public partial class Frm_assetunload_generautoamortization : MetaDataForm {
        private object CodiceTipoBuono = null;
        private object idinventoryagency = null;
        ...
        public Frm_assetunload_generautoamortization() {
            InitializeComponent();
        ...
        public void MetaData_AfterLink() {
            HelpForm.SetAllowMultiSelection(DS.assetamortizationunloadview, true);
            ...
            GetData.CacheTable(DS.inventoryagency, "(active='S')", "description", true);
        ...
        public void MetaData_AfterClear() {
            ...
            Text = "Generazione automatica scarico ammortamenti";
        }

        public void MetaData_BeforeFill() {
            GetData.DenyClear(DS.assetamortizationunloadview);
        }
        ...
        private void CollegaRigheADocumento(bool quiet) {
            ...
            DS.assetamortizationunloadview.Clear();
            ...
            MetaData.SetDefault(DS.assetunload, "ratificationdate", m_DataRatifica);
            Meta.EditNew();
            ...
            if (rdoAll.Checked || rdoBene.Checked || rdoParte.Checked) {
                DataAccess.RUN_SELECT_INTO_TABLE(Conn, DS.assetamortizationunloadview, sort, filter, null, false);
            }

            MetaData.FreshForm(this, false);
            int max = DS.assetamortizationunloadview.Rows.Count;
            for (int i = 0; i < max; i++) {
            ...
            if (max > 0) {
                ...
                //L'ho aggiunta perchè se non vi sono righe non viene abilitato il btn Successivo, ma solo Termina
        ...
        private string GetTempQuery(string tipo) {
            ...
            FromClause = " FROM ";
            WhereClause = " WHERE ";
            ParteFrom = "assetamortizationunloadview";
                ...
                if (G.IsSelected(i)) {
                    ...
                    string filterpiece = QHC.AppAnd(QHC.CmpEq("idasset", CurrRow["idasset"]),
                                     QHC.CmpGt("idpiece", 1), QHC.IsNull("idassetunload"), QHC.BitSet("flag", 0),
                                      QHC.CmpLt("amortizationquota", 0));
                    DataTable DaCollegare = Meta.Conn.RUN_SELECT("assetamortizationunloadview",
                    ...
                    foreach (DataRow Riga in DS.assetamortizationunloadview.Select(filterpiece)) {
                        Riga.BeginEdit();
                        //Meta.MarkTableAsNotEntityChild(DS.assetpieceview);
        ...
        private bool AccettaDocumento(/*string link*/) {
            bool success = true;
            int rowsBene = DS.assetamortizationunloadview.Rows.Count;
            if (rowsBene == 0) {
                success = false;
                return success;
            }
        ...
        private void VaiAvanti() {
            if ((TempTable != null) && (TempTable.Rows.Count > 0) && (DS.HasChanges())) {
                if (DS.assetamortizationunloadview.Rows.Count > 0) DS.assetamortizationunloadview.AcceptChanges();
                TempTable.Rows.RemoveAt(0);
                TempTable.AcceptChanges();
            }
        }
        ...
        private void comboBoxEnteInventario_KeyDown(object sender, KeyEventArgs e) {
        ...
        private void comboBoxEnteInventario_SelectedIndexChanged(object sender, EventArgs e) {
            if (comboBoxEnteInventario.SelectedIndex > 0) {
                idinventoryagency = comboBoxEnteInventario.SelectedValue;
                string filterEnte = qhc.CmpEq("idinventoryagency", idinventoryagency);
                DataTable ListInv = Meta.Conn.RUN_SELECT("inventory", "idinventory", null, filterEnte, null, false);
                DS.assetunloadkind.Clear();
                if (ListInv.Rows.Count > 0) {
                    string filterInv = qhc.FieldIn("idinventory", ListInv.Select());
                    DataAccess.RUN_SELECT_INTO_TABLE(Conn, DS.assetunloadkind, null, filterInv,  null, false);
                }
                cboTipo.Enabled = true;
                if (DS.assetunloadkind.Rows.Count > 0) {
                    HelpForm.SetComboBoxValue(cboTipo, DS.assetunloadkind.Rows[0]["idassetunloadkind"]);
