...
namespace meta_assetunload { //meta_buonoscaricoinventario//
	...
	public class Meta_assetunload : Meta_easydata {
		...
		protected override Form GetForm(string FormName) {
			...
			if (FormName == "generautoamortization") {
				Name = "Generazione automatica scarico ammortamenti";
				DefaultListType = "default";
				return MetaData.GetFormByDllName("assetunload_generautoamortization"); 
			}
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield) {
			...
			if ((assetload_flag & 1) == 1) {
				if ((R["idreg"]) == DBNull.Value) {
					if (!showClientMsg("Non è stato specificato il cessionario. Proseguo lo stesso?",
