...
namespace invoice_default { //documentoiva//
    ...
    "to Soccorso e accesso diretto)";
       ...
       private void PropagaModificheAiFratelli(DataRow rInvoicedetailModified) {
        ...
        private void AbilitaDisabilitaAfiprotfat() {
            ...
            var hasBeenSaved = new Lazy<bool>(() => !Meta.InsertMode && !Meta.IsEmpty);
   ...
   //         }
        ...
        private void btnAfiprotfat_Click(object sender, EventArgs e) {
            ...
            freshForm();
            var messageShower = MetaFactory.factory.getSingleton<IMessageShower>();

            using (IClient afiProtFatClient = new Client(conn, afiProtFatConfig)) {

                var idreg = currentRow["idreg"];

                var invoice = DS.invoice.get(conn, q.keyCmp(currentRow))?.First();
                var invoiceDetails = DS.invoicedetail.get(conn, q.keyCmp(currentRow));
                var invoiceAttachments = DS.invoiceattachment.get(conn, q.keyCmp(currentRow));
                var registry = DS.registry.get(conn, q.eq("idreg", idreg.ToString()))?.First();
                ...
                if (string.IsNullOrWhiteSpace(officeId)) {
                    ...
                    var nonZeroTotalsByIdmankind = invoiceDetails
                        ...
                        .Select(idmankindInvoicedetails => new {
                            ...
                            Total = idmankindInvoicedetails.Sum(invoiceDetail => InvoiceDetailTotalizer(invoiceDetail, invoice.exchangerate ?? 1)),
                        })
                    ...
                    if (nonZeroTotalsByIdmankind.Any()) {
                        ...
                        if (chosenRow == null) {
                            ...
                            messageShower.Show("E' necessario scegliere un riferimento amministrazione");
                            return;
                        }
                ...
                try {
                    ...
                    txtProtocolloEntrataRU.Text = afiProtFatClient.Protocollo(invoice, registry, address, invoiceAttachments, officeId);
                    MetaData.DoMainCommand(this, "mainsave");
                }
                catch (Exception ex) {

                    messageShower.Show($"Impossibile salvare il numero di protocollo: {ex.Message}");
                    return;
                }

                messageShower.Show($"Il documento ha ricevuto il numero '{txtProtocolloEntrataRU.Text}' di protocollo", "Successo");
            }
        }
    }
}
