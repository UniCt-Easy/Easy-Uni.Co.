...
using afiprotfat_unict;
using afiprotfat_unict.Client;
...
namespace invoice_default { //documentoiva//
    ...
    public class Frm_invoice_default : MetaDataForm {
        ...
        private Button btnAfiprotfat;
        private bool skipAfterFill = false;

        private Config afiProtFatConfig;
        public bool IsAfiProtFatConfigured => afiProtFatConfig != null && afiProtFatConfig != default;
        private static decimal InvoiceDetailTotalizer(invoicedetailRow detail, double exchangeRate) => (detail.taxable * detail.number * (Convert.ToDecimal(exchangeRate) * (1 - Convert.ToDecimal(detail.discount ?? 0)))) + detail.tax ?? 0;
        ...
        private void InitializeComponent() {
			...
			this.btnAfiprotfat = new System.Windows.Forms.Button();
			...
			// tabPrincipale
			// 
			this.tabPrincipale.Controls.Add(this.btnAfiprotfat);
			...
			// btnAfiprotfat
			// 
			this.btnAfiprotfat.Location = new System.Drawing.Point(748, 360);
			this.btnAfiprotfat.Name = "btnAfiprotfat";
			this.btnAfiprotfat.Size = new System.Drawing.Size(180, 21);
			this.btnAfiprotfat.TabIndex = 35;
			this.btnAfiprotfat.Text = "Protocolla";
			this.btnAfiprotfat.UseVisualStyleBackColor = true;
			this.btnAfiprotfat.Visible = false;
			this.btnAfiprotfat.Click += new System.EventHandler(this.btnAfiprotfat_Click);
			// 
			// chkEscludiInvio
			// 
			this.chkEscludiInvio.AutoSize = true;
			this.chkEscludiInvio.Location = new System.Drawing.Point(819, 332);
			...
			// 
			this.btnInserisciBollettaDoganale.Location = new System.Drawing.Point(748, 400);
    ...
    "nti associati.";
			...
			// txtProtocolDate
			// 
			this.txtProtocolDate.Location = new System.Drawing.Point(893, 280);
			...
			// lblProtocolDate
			// 
			this.lblProtocolDate.Location = new System.Drawing.Point(794, 283);
			...
			this.grpTesorierePerIncasso.Text = "Conto Corrente per Incasso";
			// 
			// cmbCodiceIstituto
			// 
			...
			// txtDataContabile
			// 
			this.txtDataContabile.Location = new System.Drawing.Point(893, 306);
			...
			// label11
			// 
			this.label11.Location = new System.Drawing.Point(791, 309);
    ...
    "recapito";
			...
			this.txtProtocolloEntrataRU.Leave += new System.EventHandler(this.txtProtocolloEntrataRU_Leave);
			// 
			// txtAnnotazioniRU
			// 
    ...
    "to Soccorso e accesso diretto)";
       ...
       private void PropagaModificheAiFratelli(DataRow rInvoicedetailModified) {
        ...
        private void SetModalitaPagamentoFE(object idreg) {
            ...
            if (flagbankitaliaproceeds == DBNull.Value) return;
        ...
        public void MetaData_AfterFill() {
            ...
            // ===============================================================================
            QueryCreator.setSkipInsertCopy(DS.invoiceattachment, true);
        }
        ...
        private void SetupAfiProtFat() {

            DataTable app_config = conn.RUN_SELECT("app_config", "*", "lt desc", QHS.CmpEq("code", "AFIPROTFAT"), "1", false);

            afiProtFatConfig = app_config.Rows.Count > 0 ? new Config(app_config.Rows[0]) : afiProtFatConfig;
        }

        private void AbilitaDisabilitaAfiprotfat() {
            if(!IsAfiProtFatConfigured) {
                btnAfiprotfat.Visible = false;
                return;
            }
            bool getForm = controller.GetFormData(true);
            bool isClosing = (Meta.formController.isClosing);
            bool destroyed = (Meta.destroyed);
            bool isEmpty = Meta.IsEmpty;

            bool notVisible = !getForm || isClosing || destroyed || isEmpty;
            ...
            // Usiamo la lazy evaluation sulle condizioni per rendere più leggibile la logica di valutazione della condizione finale di visibilità.
            // In questo modo possiamo scrivere le condizioni senza valutarle immediatamente.
            // check diretti su currentRow
            var isNotNational = new Lazy<bool>(() => currentRow?["flagintracom"].ToString().ToLowerInvariant() != "n"); // s, x, n
            var isNotSdi = new Lazy<bool>(() => currentRow?["idsdi_acquisto"] == DBNull.Value &&
            currentRow?["idsdi_acquistoestere"] == DBNull.Value &&
            currentRow?["idsdi_vendita"] == DBNull.Value);
            var hasNoProtocolNumber = new Lazy<bool>(() => string.IsNullOrWhiteSpace(currentRow?["arrivalprotocolnum"].ToString()));

            // check indiretti
            var isAcquisto = new Lazy<bool>(() => veroTipoFatturaAv(currentRow?["idinvkind"]) == "A"); //DS.invoicekind.Any() && (DS.invoicekind.First().flag >> 7 & 1) == 0;
            var hasValidAttachments = new Lazy<bool>(() => DS.invoiceattachment.AsEnumerable()
                .Where(row => row["idattachmentkind"] != DBNull.Value && (int)row["idattachmentkind"] == afiProtFatConfig.AttachmentkindId)
                .Any());

            btnAfiprotfat.Visible =
                isNotNational.Value &&
                isNotSdi.Value &&
                hasNoProtocolNumber.Value &&
                isAcquisto.Value &&
        ...
        private DataRow AddMandateDetailRow(DataRow RmanDet, decimal number) {
            ...
            //Se il metodo viene chiamato dall'importazione FE, copia anche le info della fattura rettificata, qualora si stia importando una nota di variazione.
            foreach (string colname in
    ...
    //                cmbDenominazione.SelectedIndex = -1;
        ...
        private void VisualizzaTabPageAutoFattura() {
            ...
            DataRow Curr = DS.invoice.Rows[0];
            // Controlliamo se esiste una fattura rettificata
            ...
            if (esisteFatturaMadre) {
                ...
                // Riempi il Grid dei dettagli fattura rettificata.
        ...
        private void AggiungidaFE(string modalita) {
            ...
            if (modalita == "contrattopassivo") {
                ...
                //Aggiornamento causali EP
                DataRow rSelected = AssociazioniRows[0];
                string filterMandate = QHS.AppAnd(QHS.CmpEq("idmankind", rSelected["idmankind"]),
                                                  QHS.CmpEq("yman", rSelected["yman"]),
                                                  QHS.CmpEq("nman", rSelected["nman"]));
                DataTable Mandate = Conn.RUN_SELECT("mandate", "*", null, filterMandate, null, false);
      ...
      private void btnInviaSdIFEvendita_Click(object sender, EventArgs e) {
            ...
            foreach (DataRow rFattura in tElectronicinvoice.Select()) {
                ...
                foreach (DataRow rDettFattura in tElectronicinvoicedetail.Select(QHC.AppAnd(filterDoc, QHC.CmpEq("recordkind", "DatiBeniServizi")))) {
                    ...
                    // Se è una Dichiarazione d'Intento o altri dati inseriti manualmente
                    // che richiedano la compilazione della sezione
                    if (rDettFattura["TipoDato"] != DBNull.Value &&
                          rDettFattura["TipoDato"].ToString().Trim() != "") {

                        // scrivo la sezione Altri Dati Gestionali se è stata compilata
                        writersdi.WriteStartElement("AltriDatiGestionali");
                        writersdi.WriteElementString("TipoDato", getLatin1(rDettFattura["TipoDato"]));
                        writersdi.WriteElementString("RiferimentoTesto", getLatin1(rDettFattura["RiferimentoTesto"]));
                        writersdi.WriteElementString("RiferimentoData", format(rDettFattura["RiferimentoData"]));
                        writersdi.WriteEndElement();// Chiude -  <AltriDatiGestionali>
                    }
                    writersdi.WriteEndElement();// Chiude -  <DettaglioLinee>
                }// chiusura foreach sui dettagli fattura
        ...
        private void btnCreaRigheStatodeldebito_Click(object sender, EventArgs e) {
            ...
            foreach (DataRow RDet in DS.invoicedetail.Select(fdet)) {
                ...
                double imponibiletotEUR = CfgFn.RoundValuta((imponibile * quantitaConfezioni * (1 - sconto) * tassocambio));
                double ivaEUR = CfgFn.GetNoNullDouble(RDet["tax"]);
                //Calcola gli importi
   ...
   //         }
        ...
        private void btnAfiprotfat_Click(object sender, EventArgs e) {
            if (Meta.destroyed) return;
            ...
            PostData.RemoveFalseUpdates(DS);
            if (DS.HasChanges()) {
                show(this, "Per protocollare la fattura occorre prima SALVARE");
                return;
            }
            using (IClient afiProtFatClient = new Client(conn, afiProtFatConfig)) {

                var idreg = currentRow["idreg"];
                var invoiceRow = DS.invoice.get(conn, q.keyCmp(currentRow))?.First();
                var invoiceDetailsRow = DS.invoicedetail.get(conn, q.keyCmp(currentRow));
                var invoiceAttachmentsRow = DS.invoiceattachment.get(conn, q.keyCmp(currentRow));
                var registryRow = DS.registry.get(conn, q.eq("idreg", idreg.ToString()))?.First();

                var registryAddressFilter = QHS.CmpEq("idreg", idreg);
                var registryAddressTable = conn.RUN_SELECT("registryaddress", "*", null, registryAddressFilter, null, false);
                var registryAddressRow = registryAddressTable.AsEnumerable()
                    .Where(row => (int)row["idaddresskind"] == afiProtFatConfig.AddressId && row["active"].ToString().ToLower() == "s")
                    .OrderByDescending(row => row["start"])
                    .FirstOrDefault();

                //var geo_city = conn.RUN_SELECT("geo_city", "*", null, QHS.CmpEq("idcity", registryAddressRow["idcity"]), null, false);
                //var geo_cityRow = geo_city.AsEnumerable()
                //    //.Where(city => registryAddressRow["idcity"] != DBNull.Value && registry.idcity == (int)city["idcity"])
                //    .FirstOrDefault();

                var geo_nation = conn.RUN_SELECT("geo_nation", "*", null, QHS.CmpEq("idnation", registryAddressRow?["idnation"]), null, false);
                var geo_nationRow = geo_nation.AsEnumerable()
                    //.Where(nation => registryAddressRow["idnation"] != DBNull.Value && registry.idnation == (int)nation["idnation"])
                    .FirstOrDefault();

                var addressParts = new[] {
                    registryAddressRow?["address"]?.ToString(),
                    //geo_cityRow?["title"]?.ToString(),
                    geo_nationRow?["title"]?.ToString()
                };

                var address = string.Join(", ", addressParts.Where(part => !string.IsNullOrWhiteSpace(part)));
                
                var invoicekinds = DS.invoicekind.get(conn, q.eq("idinvkind", currentRow["idinvkind"]));


                string officeId = string.Empty;
                string tmpOfficeId = invoicekinds.Any() ? invoicekinds.First().riferimento_amministrazione : string.Empty;

                if (!string.IsNullOrWhiteSpace(tmpOfficeId)) {

                    officeId = conn.DO_READ_VALUE("sdi_rifamm", $"idsdi_rifamm = '{tmpOfficeId}'", "codiceufficio").ToString();
                }

                if (string.IsNullOrWhiteSpace(officeId)) {

                    var nonZeroTotalsByIdmankind = invoiceDetailsRow
                        .Where(invoiceDetail => !string.IsNullOrWhiteSpace(invoiceDetail.idmankind))
                        .GroupBy(invoiceDetail => invoiceDetail.idmankind)
                        .Select(idmankindInvoicedetails => new {
                            IdMankind = idmankindInvoicedetails.Key,
                            Total = idmankindInvoicedetails.Sum(invoiceDetailRow => InvoiceDetailTotalizer(invoiceDetailRow, invoiceRow.exchangerate ?? 1)),
                        })
                        .Where(idMankindInvoicedetails => idMankindInvoicedetails.Total > 0);

                    if (nonZeroTotalsByIdmankind.Any()) {

                        string referenceIdMankind = nonZeroTotalsByIdmankind.OrderByDescending(nztbi => nztbi.Total).First().IdMankind;
                        tmpOfficeId = conn.DO_READ_VALUE("mandatekind", $"idmankind = '{referenceIdMankind}'", "riferimento_amministrazione").ToString();
                        officeId = conn.DO_READ_VALUE("sdi_rifamm", $"idsdi_rifamm = '{tmpOfficeId}'", "codiceufficio").ToString();
                    }
                    else {

                        MetaData meta_sdi_rifamm = MetaData.GetMetaData(this, "sdi_rifamm");
                        meta_sdi_rifamm.FilterLocked = true;
                        meta_sdi_rifamm.DS = DS.Clone();

                        DataTable sdi_rifamm = Conn.RUN_SELECT("sdi_rifamm", "*", null, null, null, false);

                        meta_sdi_rifamm.DS.Tables.Add(sdi_rifamm);

                        DataRow chosenRow = null;
                        security.DeleteAllUnselectable(sdi_rifamm);
                        chosenRow = meta_sdi_rifamm.SelectOne("default", null, null, null);

                        if (chosenRow == null) {
                            show(this, "E' necessario scegliere un riferimento amministrazione");
                            return;
                        }

                        officeId = chosenRow["codiceufficio"].ToString();
                    }
                }

                InvoiceParameters ip = new InvoiceParameters() {
                    docdate = invoiceRow.docdate ?? DateTime.Now,
                    idinvkind = invoiceRow.idinvkind,
                    ninv = invoiceRow.ninv,
                };

                RegistryParameters rp = new RegistryParameters() {
                    p_iva = format(registryRow.p_iva),
                    foreigncf = format(registryRow.foreigncf),
                    title = format(registryRow.title),
                };

                try {
                    //messageShower.Show($"{afiProtFatClient.Protocollo(invoice, registry, address, invoiceAttachments, officeId)}");
                    txtProtocolloEntrataRU.Text = afiProtFatClient.Protocollo(ip, rp, format(address), invoiceAttachmentsRow, officeId);
                    MetaData.DoMainCommand(this, "mainsave");
                }
                catch (Exception ex) {

                    show(this, $"Impossibile salvare il numero di protocollo: {ex.Message}");
                    return;
                }

                show(this, $"Il documento ha ricevuto il numero '{txtProtocolloEntrataRU.Text}' di protocollo", "Successo");
            }
        }

		private void txtProtocolloEntrataRU_Leave(object sender, EventArgs e) {
           AbilitaDisabilitaAfiprotfat();
        }
	}
}
