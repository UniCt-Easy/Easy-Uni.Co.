...
namespace invoice_default
{
	...
	public class frmAskDividi : System.Windows.Forms.Form
	{
		...
		public frmAskDividi(DataRow RigaSelezionata,MetaData Meta,MetaDataDispatcher Disp)
		{
			...
			DataRow Ordine = RigaSelezionata.GetParentRow("invoiceinvoicedetail");
			...
			object invoicekind=Conn.DO_READ_VALUE("invoicekind",
				"(idinvkind="+QueryCreator.quotedstrvalue(RigaSelezionata["idinvkind"],true)+")","description");
			txtTipo.Text = invoicekind.ToString();
			txtEsercizio.Text = RigaSelezionata["yinv"].ToString();
			txtNumero.Text = RigaSelezionata["ninv"].ToString();
        ...
        public frmAskDividi(DataRow [] listaSplit, MetaData Meta, MetaDataDispatcher Disp) {
            ...
            object invoicekind = Conn.DO_READ_VALUE("invoicekind",
                QHS.CmpEq("idinvkind", RigaSelezionata["idinvkind"]), "description");
            txtTipo.Text = invoicekind.ToString();
            txtEsercizio.Text = RigaSelezionata["yinv"].ToString();
            txtNumero.Text = RigaSelezionata["ninv"].ToString();
            txtRiga.Text = RigaSelezionata["rownum"].ToString();
		...
		private void InitializeComponent()
		{
            ...
            this.label4.Text = "Totale Q.t‡ ordinata";
            // 
            // label21
            // 
		...
		private void frmAskInfo_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
			...
			if(unabatassegnato<importoIndetraibile){
				MessageBox.Show("L'Iva Indetraibile del dettaglio originale Ë inferiore alla somma dei valori ripartiti");
				Info.Clear();
				e.Cancel = true;
				return;
			}
		...
		private void DividiCostoTotale (object sender) {
            ...
            decimal npackage = CfgFn.GetNoNullDecimal(Dettaglio["npackage"]);
			decimal discount = CfgFn.GetNoNullDecimal(Dettaglio["discount"]);
			decimal taxrate  = CfgFn.GetNoNullDecimal(Conn.DO_READ_VALUE("ivakind",
                QHS.CmpEq("idivakind", Dettaglio["idivakind"]), "rate"));
