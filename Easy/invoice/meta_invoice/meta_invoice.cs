	...
	public class Meta_invoice : Meta_easydata {
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield) {
            ...
            if (R["adate"] == DBNull.Value) {
                errmess = "La data registrazione Ë obbligatoria";
                errfield = "adate";
                return false;
            }
            ...
            if (dd.Year != CfgFn.GetNoNullInt32(R["yinv"])) {
                errmess = "La data registrazione non Ë coerente con l'anno fattura";
                errfield = "adate";
                return false;
            }
			...
			if (CfgFn.GetNoNullInt32(R["idreg"])<=0) {
				errmess="Il campo 'Cliente/Fornitore' Ë obbligatorio";
				errfield="idreg";
				return false;
			}
			...
			{
				errmess = "Per la fattura  di vendita Ë sempre necessario valorizzare il Codice Univoco, oppure l'Email o la PEC del Destinatario, anche in caso di soggetto privato";
			...
			if (docautomatico!="S" && R["doc"].ToString()=="") {
				errmess="Il campo 'Documento' Ë obbligatorio";
				errfield="doc";
				return false;
			}
            ...
            if (enable_fe == "N" && R["doc"].ToString().Length > 20) {
                errmess =
                    "Per le fatture non elettroniche Ë necessario limitare la lunghezza del campo documento a 20 caratteri.";
                errfield = "doc";
                return false;
            }

            if (R["docdate"] ==DBNull.Value) {
                errmess = "Il campo 'data documento' Ë obbligatorio";
                errfield = "docdate";
                return false;
                }
            ...
            if (R.RowState == DataRowState.Modified) {
                if (R["adate"].ToString() != R["adate", DataRowVersion.Original].ToString()) {
                    ...
                    if (Conn.RUN_SELECT_COUNT("entry", QHS.CmpEq("idrelated", idrel), false) > 0) {
                        errmess =
                            "La fattura ha gi‡ delle scritture collegate, occorre eliminarle prima di modificare " +
            ...
            if (isSSN(R["idinvkind"])) {
                if (R["ssntipospesa"] == DBNull.Value) {
                    errmess = "Per il tipo fattura selezionata Ë necessario indicare il tipo spesa ai fini del progetto tessera sanitaria";
            ...
            if (R.Table.Columns.Contains("flagbit")) {
                ...
                if (isBolettaDoganale && (R["toincludeinpaymentindicator"].ToString().ToUpper() == "S")) {
                    errmess = "Le Bollette doganali non vanno nell'indicatore di tempestivit‡";
            ...
            if ((CfgFn.GetNoNullInt32(R["idexpirationkind"]) == 1) && (R["adate"]==DBNull.Value)) {
                errmess = "Avendo scelto il tipo scadenza: Data contabile, Ë obbligatorio indicare la data registrazione";
                errfield = "adate";
                return false;
            }
            //  2	Data documento
            if ((CfgFn.GetNoNullInt32(R["idexpirationkind"]) == 2) && (R["docdate"] == DBNull.Value)) {
                errmess = "Avendo scelto il tipo scadenza: Data documento, Ë obbligatorio indicare la data documento";
                errfield = "docdate";
                return false;
            }
            //  3	Fine mese data documento
            if ((CfgFn.GetNoNullInt32(R["idexpirationkind"]) == 3) && (R["docdate"] == DBNull.Value)) {
                errmess = "Avendo scelto il tipo scadenza: Fine mese data documento, Ë obbligatorio indicare la data documento";
                errfield = "docdate";
                return false;
            }
            //  4	Fine mese data contabile
            if ((CfgFn.GetNoNullInt32(R["idexpirationkind"]) == 4) && (R["adate"] == DBNull.Value)) {
                errmess = "Avendo scelto il tipo scadenza: Fine mese data contabile, Ë obbligatorio indicare la data registrazione";
                errfield = "adate";
                return false;
            }
            //  5	Pagamento Immediato
            if ((CfgFn.GetNoNullInt32(R["idexpirationkind"]) == 5) && (R["adate"] == DBNull.Value)) {
                errmess = "Avendo scelto il tipo scadenza: Pagamento immediato, Ë obbligatorio indicare la data registrazione";
                errfield = "adate";
                return false;
            }
            //  6   Data ricezione
            if ((CfgFn.GetNoNullInt32(R["idexpirationkind"]) == 6) && (R["protocoldate"] == DBNull.Value)) {
                errmess = "Avendo scelto il tipo scadenza: Data ricezione, Ë obbligatorio indicare la data ricezione";
            ...
            if (R["flagintracom"].ToString().ToUpper() == "S" && IntraInfoRequired) {
                if (ExistsBeni){
                    if ((flag & 1) != 0){ 
                        ...
                        if (R["iso_destination"] == DBNull.Value){
                            errmess = "Il campo 'paese di destinazione' Ë obbligatorio";
                            errfield = "iso_destination";
                            return false;
                        }
                        if (R["idcountry_origin"] == DBNull.Value){
                            errmess = "Il campo 'provincia di origine' Ë obbligatorio";
                        ...
                        if (R["iso_origin"] == DBNull.Value){
                            errmess = "Il campo 'paese di origine' Ë obbligatorio";
                            errfield = "iso_origin";
                            return false;
                        }
                        if (R["iso_provenance"] == DBNull.Value){
                            errmess = "Il campo 'paese di provenienza' Ë obbligatorio";
                            errfield = "iso_provenance";
                            return false;
                        }
                        if (R["idcountry_destination"] == DBNull.Value){
                            errmess = "Il campo 'provincia destinazione' Ë obbligatorio";
                            errfield = "idcountry_destination";
                            return false;
                        }
                    }
                    if (R["idintrastatkind"] == DBNull.Value){
                        errmess = "Il campo 'natura della transazione' Ë obbligatorio";
                ...
                if (ExistsServizi){
                    if (R["docdate"] == DBNull.Value){
                        errmess = "Il campo 'data' Ë obbligatorio";
                        errfield = "docdate";
                        return false;
                    }

                    if (R["iso_payment"] == DBNull.Value){
                        errmess = "Il campo 'paese di pagamento' Ë obbligatorio";
                        errfield = "iso_payment";
                        return false;
                    }

                    if (R["idintrastatpaymethod"] == DBNull.Value){
                        errmess = "Il campo 'Modalit‡ di pagamento' Ë obbligatorio";
