...
namespace mainform //MenuMaker//
{
    ...
    public class menuMaker : IDisposable {
        ...
        public void CreateMenu(MainMenu Menu, string TableName, string Myfilter) {
            ...
            //Se la tabella Ë vuota esci
            if ((DT_Menu == null) || (DT_Menu.Rows.Count == 0)) return;


            int h3 = metaprofiler.StartTimer("Menu - sezione Terza");

            //costruisce una hash di liste ove ogni elemento Ë una lista di datarow ed HMenu[idmenu] punta alla lista 
        ...
        private void ControllaConfigurazioni(Menu mainMenu, DataAccess Conn) {
            ...
            if (!abilitaBilancio) {
                MessageBox.Show("La configurazione del BILANCIO non Ë stata definita per l'esercizio corrente. " +
            ...
            if (!abilitaMiss) {
                MessageBox.Show("La configurazione delle MISSIONI non Ë stata definita per l'esercizio corrente!",
                    "Attenzione",
                    MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                DisableMenu(mainMenu, "Compensi");
            }
            ...
            if ((SpeseEntrateEnabled == true) && (!abilitaEntrata)) {
                MessageBox.Show("La configurazione delle ENTRATE non Ë stata definita per l'esercizio corrente. " +
            ...
            if ((SpeseEntrateEnabled == true) && (!abilitaSpesa)) {
                MessageBox.Show("La configurazione delle SPESE non Ë stata definita per l'esercizio corrente. " +
                                "Non sar‡ possibile accedere ai menu Entrate/Spese", "Attenzione",
                    MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                SpeseEntrateEnabled = false;
            }
            ...
            if (!abilitaPatrimonio) {
                //MessageBox.Show("La configurazione del PATRIMONIO non Ë stata definita per l'esercizio corrente!", "Attenzione",
                //	MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                DisableMenu(mainMenu, "Cespiti");
            }
            else
            ...
            if (menucode != "" && metadata == "") {
                ...
                if (hashMenu.ContainsKey(idMenu)) {
                    ...
                    foreach (DataRow rChild in hashMenu[idMenu]) {
                        ...
                        Element childMenu = createMenuItem(hashMenu, toHide, rChild); //se Ë una voce contenitore, aggiunge le voci figlie
                ...
                if (MyItem.MenuItems.Count==0) return null; //non ha aggiunto nulla, Ë un contenitore vuoto che non va restituito
                return MyItem;
            }            
        ...
        // nel qual caso la funzione restituir‡ TRUE
        ...
        void updateStampeMenu(Element parent) {
            ...
            foreach (DataRow Module2 in AllModules) {
                if (lastmodule.Trim().ToLowerInvariant() == Module2["modulename"].ToString().Trim().ToLowerInvariant()) continue; //modulo gi‡ aggiunto
        ...
        public void ClearMenu(MainMenu M) {
            ...
            while (i < M.MenuItems.Count) {
                ...
                if (E == null) { //se non Ë convertibile in Element allora Ë di tipo design time
                    MenuItem MM = M.MenuItems[i]; //prende l'elemento considerato
                    if (MM.MenuItems.Count > 0 || MM.Text == "?") { //se ha figli o Ë la guida non lo cancella
                        i = i + 1;
                    }
                    else {
