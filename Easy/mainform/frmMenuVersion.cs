...
namespace mainform//CompEc//
{
	...
	/// </summary>
	public class frmMenuVersion : MetaDataForm
	{
		...
		private void btnOK_Click(object sender, System.EventArgs e) {
			if (txtNewVersion.Text.Trim()=="") {
				show("Inserire la nuova versione", "Attenzione",
					MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
			if (txtOldVersion.Text.CompareTo(txtNewVersion.Text)>0) {
				show("La nuova versione deve essere maggiore di quella corrente", "Attenzione",
					MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
			if (txtOldVersion.Text.Trim()==txtNewVersion.Text.Trim()) {
				if (show("Vuoi mantenere la stessa versione?", "Versione",
