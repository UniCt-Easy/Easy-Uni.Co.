...
namespace mainformweb {
	...
	public class frmMainWeb : MetaDataForm {
        ...
        bool checkExecutionPath() {
            ...
            if ((!Debugger.IsAttached) && (AppDomain.CurrentDomain.FriendlyName != "MetaDataDomain") && (isUnderTest==false)
                && ( !isBlazor)) {
                show("E' necessario eseguire il programma tramite loader.exe. Non è possibile aprire direttamente mainformweb.exe");
                return false;
            }

            string currDir = Environment.CurrentDirectory;
            string appDomain = AppDomain.CurrentDomain.BaseDirectory;
            if (!File.Exists(Path.Combine(appDomain, "mainformweb.exe"))) {
                show("Il programma non è installato bene o è eseguito da cartelle errate.");
                return false;
            }
            ...
            if (exePath.StartsWith(tempPath)) {
                show("Non è consentita l'esecuzione dell'applicazione da cartelle temporanee o compresse.","Errore");
                return false;
            }
        ...
        public frmMainWeb() {
            ...
            //try {
            //    Chat.WebView2.Installer.Install(
            //        Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "MicrosoftEdgeWebview2Setup.exe"),
            //        Convert.FromBase64String("NO/OZvgMzfVuxGl9MjkiynUceDCZueDRo47sBUd2GC8="),   // hash dell'attuale installer
            //        stream => new SHA256Managed().ComputeHash(stream)
            //    );
            //}
            //catch (Exception e) {
            //    ErrorLogger.Logger.logException(e.Message, e, conn?.Security, conn);
            //}
        ...
        private void InitializeComponent() {
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmMainWeb));
			...
			// 
			this.mnuLiveUpdateOnDemand.Index = 9;
			...
			// mnuCambiaPassword
			// 
			this.mnuCambiaPassword.Index = 10;
			...
			// 
			this.mnuCambiaPasswordDipart.Index = 11;
			...
			this.MetaDataToolBar.Size = new System.Drawing.Size(992, 120);
        ...
        void SetMenu(bool ShowAdminMenu,bool YOUR_PASSWORD) {
            ...
            if (MyDataAccess != null) {
                ...
                MainMenuMaker.CreateMenu(mainMenu1, "menuwebapp", null);
        ...
        private void controllaConfigurazioni(string filteresercizio) {
            ...
            if (!abilitaBilancio) {
                show("La configurazione del BILANCIO non è stata definita per l'esercizio corrente. " +
            ...
            if ((SpeseEntrateEnabled == true) && (!abilitaEntrata)) {
                show("La configurazione delle ENTRATE non è stata definita per l'esercizio corrente. " +
            ...
            if ((SpeseEntrateEnabled == true) && (!abilitaSpesa)) {
                show("La configurazione delle SPESE non è stata definita per l'esercizio corrente. " +
                        "Non sarà possibile accedere ai menu Entrate/Spese", "Attenzione",
                                       MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                SpeseEntrateEnabled = false;
            }
        ...
        void connect() {
            ...
            if (EsercizioTable == null || EsercizioTable.Rows.Count == 0) {
                ...
                if (MyDataAccess.RUN_SELECT_COUNT("accountingyear", null, false) == 0) {
                    ...
                    show("L'esercizio " + esercizio + " non è stato definito per questo Ente", "Attenzione",
                        MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            var sec = MyDataAccess.SelectCondition("menuwebapp",true);
            var usrMenu = MyDataAccess.GetUsr("menuwebapp")?.ToString();
        ...
        private void MenuExport_Click(object sender, System.EventArgs e) {
            ...
            dbanalyzer.ExportTableToXML(FileSaver.FileName, "menuwebapp", MyDataAccess, null);
        }
        ...
        bool Disconnect() {
            ...
            if ((OwnedForms.Length > 0) ||
                ...
                show("Per collegarsi ad un nuovo DB è necessario chiudere prima tutte le finestre.");
                return false;
            }
        ...
        //        show(this, "Per eseguire l'operazione richiesta è necessario prima\n" +
        ...
        private void QA_Click(object sender, System.EventArgs e) {
        ...
        private void menuItem16_Click_1(object sender, EventArgs e) {
            if ((this.OwnedForms.Length > 0) ||
                (MdiChildren.Length > 0)) {
                show("Per poter cambiare ruolo è necessario chiudere prima tutte le finestre.");
                return ;
            }
        ...
        private void mnuCheckDll_Click(object sender, EventArgs e) {
            ...
            read1.Close();
            show("La versione locale ricavata dal file " + versionfile + " è " + localVersion);
        ...
        //    if (!Chat.WebView2.Installer.IsWebView2Installed) {
        //        string logMessage = "WebView2 non installato";
        //        MetaFactory.factory.getSingleton<IMessageShower>().Show("Impossibile avviare la chat. Componenti non installati.", "Installazione", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        //        ErrorLogger.Logger.logException(logMessage, new Exception(logMessage), conn?.Security, conn);
        //        return;
        //    }
        ...
        //    string configErrorUserMessage = "Impossibile avviare la chat. Verificare la configurazione.";

        //    // Questa try serve per gestire l'eccezione che Maria mi ha chiesto di verificare anche se a LU e implementazione ben effettuati non servirebbe
        //    try {
        //        cfAgency = conn.GetSys("cfagency").ToString();
        //    }
        //    catch (Exception ex) {

        //        MetaFactory.factory.getSingleton<IMessageShower>().Show(configErrorUserMessage, "Configurazione", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        //        ErrorLogger.Logger.logException(ex.Message, ex, conn?.Security, conn);
        //        return;
        //    }

        //    if (string.IsNullOrWhiteSpace(cfAgency)) {

        //        string logMessage = "\"cfagency\" non valorizzato.";
        //        MetaFactory.factory.getSingleton<IMessageShower>().Show(configErrorUserMessage, "Configurazione", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        //        ErrorLogger.Logger.logException(logMessage, new Exception(logMessage), conn?.Security, conn);
        //        return;
        //    }
        ...
        //    try {
        //        user = conn.GetSys("user").ToString();
        //    }
        //    catch (Exception ex) {
        ...
        //        string logMessage = "\"user\" non valorizzato.";
        //        MetaFactory.factory.getSingleton<IMessageShower>().Show("Verificare la configurazione dell'Utente (user).", "Configurazione", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        //        ErrorLogger.Logger.logException(logMessage, new Exception(logMessage), conn?.Security, conn);
        //        return;
        //    }

        //    try {
        //        // Configurazione chat (serverUrl, adminId, adminToken, embeddingMode(channel/direct)), resourceName(channelName/botName)
        //        var chatConfig = conn.RUN_SELECT("app_config", "param", null, qhs.CmpEq("code", "CHAT"), null, false).AsEnumerable().FirstOrDefault()["param"].ToString().Split('|');
        //        WebChat chat = new WebChat(
        //            string.Join(".", cfAgency, user),
        //            new Uri(chatConfig[0]),
        //            chatConfig[1],
        //            chatConfig[2],
        //            WebChat.OptionEmbeddingMode((TEmbeddingMode)Enum.Parse(typeof(TEmbeddingMode), chatConfig[3], true), chatConfig[4])
        //        );
        //        MetaFactory.factory.getSingleton<IFormCreationListener>().create(chat, null);
        //        chat.MdiParent = this;
        //        chat.Show();
        //    }
        ...
        //        MetaFactory.factory.getSingleton<IMessageShower>().Show("Impossibile avviare la chat, errore nella comunicazione con il server. Riprovare più tardi.", "Comunicazione", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        //        ErrorLogger.Logger.logException(ex.Message, ex, conn?.Security, conn);
        //        return;
        //    }
        //    catch (Exception ex) {
        //        MetaFactory.factory.getSingleton<IMessageShower>().Show("Errore imprevisto.", "Imprevisto", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        //        ErrorLogger.Logger.logException(ex.Message, ex, conn?.Security, conn);
        //        return;
        //    }
        //}
