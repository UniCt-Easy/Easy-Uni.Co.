...
namespace mainformweb
{
    ...
    public class menuMaker :IDisposable
    {
        ...
        public menuMaker(Easy_DataAccess mainDA, Form F, EntityDispatcher D)
        {
            ...
            SelList.Add(new SelectBuilder("*").IntoTable(DS.menuwebapp).OrderBy("paridmenu,ordernumber"));
        ...
        bool voceFormConsentita(string metadata, string edittype)
        {
            if (tMenu == null)
            {
                tMenu = DA.CreateTableByName("menuwebapp", "*", false);
            }
            tMenu.Clear();
        ...
        public void ClearMenu(MainMenu M)
        {
            ...
            while (i < M.MenuItems.Count)
            {
                ...
                if (E == null)
                { //se non è convertibile in Element allora è di tipo design time
                    MenuItem MM = M.MenuItems[i]; //prende l'elemento considerato
                    if (MM.MenuItems.Count > 0 || MM.Text == "?")
                    { //se ha figli o è la guida non lo cancella
                        i = i + 1;
                    }
                    else
                    {
