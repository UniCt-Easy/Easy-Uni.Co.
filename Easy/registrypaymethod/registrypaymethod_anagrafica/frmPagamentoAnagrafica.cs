...
namespace registrypaymethod_anagrafica {//PagamentoAnagrafica//
    ...
    "il BBAN come da regolamento bancario";
			...
			this.tabAllegati.Size = new System.Drawing.Size(669, 525);
        ...
        void AbilitaDisabilitaAllegati() {
			...
			if (Curr["ccdedicato_doc"] != DBNull.Value || Curr["idfilestorage2"] != DBNull.Value) {
				...
				if (Curr["ccdedicato_doc"] != DBNull.Value) {
					// Attachment
					ByteArray = (byte[])Curr["ccdedicato_doc"];
				}
				else {
					// MongoDb
			...
			if (Curr["ccdedicato_cf"] != DBNull.Value || Curr["idfilestorage"] != DBNull.Value) {
                ...
                {
                    // Attachment
                    ByteArray = (byte[])Curr["ccdedicato_cf"];
                }
                else
                {
                    // MongoDb
        ...
        private void VisualizzaAllegato(string certification) {
            ...
            string idfilestorage = certification == "ccdedicato_doc" ? "idfilestorage2" : "idfilestorage";
            ...
            if (Curr[certification] != DBNull.Value){
                ...
                // MongoDb
                ByteArray = metaeasylibrary.HttpFileStorage.DownloadFile(conn, DS.registrypaymethod.TableName, Curr[idfilestorage].ToString()).GetAwaiter().GetResult();
				if (ByteArray == null) {
					show("Servizio Download degli Allegati non disponibile");
					return;
				}
