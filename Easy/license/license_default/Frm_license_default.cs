...
namespace license_default//licenzauso//
{
	...
	public class Frm_license_default : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.label6.Text = "Nome Universit‡:";
		...
		private void aggiornaCap(object idComune, object idprovincia) {
			...
			if (capPrincipale != null) {
				...
				if (cap == "") {
					...
					if ((cap != capPrincipale.ToString()) && (t.Rows.Count == 0)) {
						DialogResult dr = MessageBox.Show(this, "Il C.A.P. non Ë coerente con il comune scelto. Si desidera aggiornarlo?", "Avviso", MessageBoxButtons.YesNo);
			...
			if (!txtSiglaProv.Text.Equals(provDB)) {
				DialogResult dr = MessageBox.Show(this, "La sigla della provincia non Ë coerente con il comune scelto. Si desidera aggiornarla?", "Avviso", MessageBoxButtons.YesNo);
				if (dr==DialogResult.Yes) {
