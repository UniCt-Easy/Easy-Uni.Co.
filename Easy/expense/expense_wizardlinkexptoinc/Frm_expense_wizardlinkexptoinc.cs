...
namespace expense_wizardlinkexptoinc {//spesawizardelimina//
	...
	public class Frm_expense_wizardlinkexptoinc : System.Windows.Forms.Form {
		...
		private void InitializeComponent() {
			...
			this.txtNumIncome.SelectedTab = this.tabSelectExpense;
    ...
    "cora trasmessi";
			...
			// label28
			// 
			this.label28.Location = new System.Drawing.Point(11, 24);
		...
		bool GetExpense(){
            ...
            if (getGirofondo( )) {
                MessageBox.Show("L'associazione non è consentita con la modalità di pagamento girofondo");
                return false; 
            }
        ...
        bool getGirofondo () {
             if(DS.expenselastview.Rows.Count==0) return false;
             DataRow CurrExpense = DS.expenselastview.Rows[0];
             int paymethod_flag = CfgFn.GetNoNullInt32(CurrExpense["paymethod_flag"]); 
			//tipo girofondo
			//2^6+2^8 +2^9+ 2^10+2^11
			 //select power(2,6) + power(2,8) + power(2,9) + power(2,10) + power(2,11)    
             //F24EP
			 //ACCREDITOTESORERIAPROVINCIALESTATOPERTABA
			 //ACCREDITOTESORERIAPROVINCIALESTATOPERTABB
 
             if (!((paymethod_flag&3904)==0))
                return true;
             else
                return false;
        }
