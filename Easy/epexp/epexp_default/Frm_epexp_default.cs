	...
	public class Frm_epexp_default : MetaDataForm {
		...
		void checkCoerenzaCausale(DataRow accmotive) {
			...
			if (details.Rows.Count==0) {
				...
				MessageBox.Show("Il conto Ë stata azzerato poichË incorente con la causale selezionata");
			};
		}
		...
		void checkCoerenzaConto(DataRow account) {
			...
			if (details.Rows.Count==0) {
				...
				MessageBox.Show("La causale Ë stata reimpostata poichË incorente col conto selezionato");
			}
		}
		...
		private void abilitaDisabilitaSalvataggio() {
			...
			if (Curr["idrelated"].ToString() != "") {
				// Solo l'utente abilitato a edit_epexpresidui, Ë autorizzato a salvare, e se trattasi di residuo
