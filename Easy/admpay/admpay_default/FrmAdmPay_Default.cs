	...
	public class FrmAdmPay_Default : System.Windows.Forms.Form {
		...
		private void btnGenera_Click(object sender, System.EventArgs e) {
            ...
            if (dr != DialogResult.OK) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Non è stato scelto alcun compito", "Avvertimento");
                return;
            }
        ...
        public void GetEntryForDocument(EP_functions EP) {
            ...
            EP.D.Relations.Add("entrydetailentrydetailaccrual",
                ...
                false);
            if (TT3.Rows.Count > 0) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Ci sono ratei associati alle scritture che saranno scollegati. Sarà " +
		...
		private bool generaMovPrincipali(string IoE) {
            ...
            foreach (DataRow R in dsImport.Tables[tAdmPayMov].Rows) {
                ...
                if (rAppAss_LISTA.Length == 0) {
                    if (IoE=="I"){
                        MetaFactory.factory.getSingleton<IMessageShower>().Show("Il meta movimento di entrata n." + R["nincome"].ToString() + " non è associato ad un meta accertamento");
                        RemoveDummyCols();
                        return false;
                    }
                    if (IoE == "E") {
                        MetaFactory.factory.getSingleton<IMessageShower>().Show("Il meta movimento di spesa n." + R["nexpense"].ToString() + " non è associato ad un meta impegno");
                        RemoveDummyCols();
                        return false;
                    }
