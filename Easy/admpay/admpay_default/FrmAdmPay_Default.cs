...
namespace admpay_default {
	...
	public class FrmAdmPay_Default : System.Windows.Forms.Form {
        ...
        public void GetEntryForDocument(EP_functions EP) {
            ...
            //if (num==0) SottraiRatei=false; //Non c'Ë necessita di sotrarre risconti poichË non ve ne sono.
        }
        ...
        private DataRow SetEntry(EP_functions EP, MetaData MetaEntry, DataRow Curr, string kind) {
            ...
            string idRelated = "admpay" + "ß" + Curr["yadmpay"].ToString()
                + "ß" + Curr["nadmpay"].ToString() + "ß" + kind;
        ...
        */
		...
		private object individuaOggetto(DataRow rSource, string tName, string field, string IoE, string task) {
            ...
            if (WageImportSetup.Length == 0) {
                ...
                fillLog("La configurazione del task " + nomeTask + " non Ë specificata");
                return null;
            }
			...
			// Verr‡ presa la voce che avr‡ meno null associati.
            string tNameChild = "";
		...
		/// <summary>
		/// Metodo che restituisce il filtro che interrogher‡ la tabella di classificazione associata alla tabella tName
		/// </summary>
		/// <param name="tName">Nome della tabella principale</param>
		/// <param name="rSource">DataRow del foglio Excel</param>
        /// <param name="sortField">campi su cui Ë effettuato il confronto (usati in filtro result)</param>
		/// <returns></returns>
		private string costruisciFiltro(string tName, DataRow rSource, string task, out string sortField ) {
			...
			// Per ogni labels valorizzata il filtro sar‡ costruito sull'uguaglianza altrimenti sul NULL
			int MAXLABEL = 5;
			string fS = "";
			...
			// Il filtro prodotto sar‡ del tipo
        ...
        */
        ...
        private void fillLog(string oggetto, string info) {
            ...
            string msg = "Non Ë stato possibile ottenere " + descr_oggetto +
        ...
        private bool fillHt(ArrayList al, DataRow rSource, Hashtable htDest, string tName, string task) {
            ...
            foreach (string c in al) {
                if (rSource[c] != DBNull.Value) {
                    key += rSource[c].ToString().ToUpper() +"ß";
                }
                else {
                    key += "NULLß";
                }
        ...
        void EditEntry(string kind) {
            ...
            string idRelated = "admpay" + "ß" + R["yadmpay"].ToString()
                + "ß" + R["nadmpay"].ToString() + "ß" + kind;
