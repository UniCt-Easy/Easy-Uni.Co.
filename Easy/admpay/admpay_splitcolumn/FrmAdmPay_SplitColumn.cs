	...
	public class FrmAdmPay_SplitColumn : System.Windows.Forms.Form {
		...
		public static void apriFile(System.Data.DataTable DT,bool Header) { 
			...
			try {
				...
				MetaFactory.factory.getSingleton<IMessageShower>().Show( "Non è possibile eseguire l'esportazione in Excel. "+
        ...
        private bool transformColumnInRow(out System.Data.DataTable tNewLayout) {
            ...
            if (idsorkind == "") {
                MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Non è stata impostata la classificazione delle ritenute nella tabella di configurazione dell'importazione stipendi", "Errore");
                return false;
            }
        ...
        private bool interrogaFileExcel(string task) {
            ...
            if (!verificaValiditaFileExcel(task)) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Il file selezionato non è valido per il compito selezionato", "Errore");
                return false;
            }
