                ...
                if (idman != null && idman != DBNull.Value) {
                    ...
                    if (getman != DBNull.Value) {
                        if (getman.ToString() != idman.ToString())
                            MessageBox.Show("Il responsabile del Contratto è stato reimpostato come da U.P.B.");
                    }
        ...
        private void btnUnisciDettagli_Click(object sender, EventArgs e) {
            ...
            if (detailsGroup > 1) {
                ...
                foreach (DataRow DR in Selected) {
                    ...
                    if (ninvoiced > 0) {
                        MessageBox.Show("Alcuni dettagli dello stesso gruppo sono collegati a fattura. Non è possibile annullare la suddivisione.", "Avviso");
                        return;
                    }
                    if ((idinc_taxable.Length > 1) || (idinc_iva.Length > 1)) {
                        MessageBox.Show("Alcuni dettagli dello stesso gruppo sono contabilizzati. Non è possibile annullare la suddivisione.", "Avviso");
                        return;
                    }
