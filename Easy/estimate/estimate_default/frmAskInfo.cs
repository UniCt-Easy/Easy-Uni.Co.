...
namespace estimate_default
{
	...
	public class frmAskInfo : System.Windows.Forms.Form	{
		...
		decimal GetImponibileNear(decimal imponibiletest,bool silent){
			...
			while(cent < 0.3M){
				...
				if (totale1+totale2 == TotaleImponibile) {
					if (!silent) MetaFactory.factory.getSingleton<IMessageShower>().Show("L'imponibile è stato portato da "+
			...
			MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stato trovato un imponibile adeguato alle esigenze di divisione");
		...
		private void frmAskInfo_Closing(object sender, System.ComponentModel.CancelEventArgs e)	{
			...
			{
				MetaFactory.factory.getSingleton<IMessageShower>().Show("L'imponibile del dettaglio originale non è stato interamente suddiviso");
				Info.Clear();
				e.Cancel = true;
				return;
			}
			...
			{
				MetaFactory.factory.getSingleton<IMessageShower>().Show("L'Iva del dettaglio originale non è stata interamente suddivisa");
				Info.Clear();
				e.Cancel = true;
				return;
			}
			if (taxableassegnato<importo)
			{
				MetaFactory.factory.getSingleton<IMessageShower>().Show("L'imponibile del dettaglio originale è inferiore alla somma dei valori ripartiti");
				Info.Clear();
				e.Cancel = true;
				return;
			}
			if(taxassegnato>importoIva)
			{
				MetaFactory.factory.getSingleton<IMessageShower>().Show("L'Iva del dettaglio originale è inferiore alla somma dei valori ripartiti");
				Info.Clear();
				e.Cancel = true;
				return;
			}
			if(totrigaassegnato<(importoTotaleRiga)){
				MetaFactory.factory.getSingleton<IMessageShower>().Show("Il totale riga del dettaglio originale non è stato interamente suddiviso");
				Info.Clear();
				e.Cancel = true;
				return;
			}
			if(totrigaassegnato>(importoTotaleRiga)){
				MetaFactory.factory.getSingleton<IMessageShower>().Show("Il totale riga del dettaglio originale è inferiore alla somma dei valori ripartiti");
				Info.Clear();
				e.Cancel = true;
				return;
			}
