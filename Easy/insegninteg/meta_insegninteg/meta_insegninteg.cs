...
{
    public class Meta_insegninteg : Meta_easydata 
	{
        public Meta_insegninteg(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "insegninteg") {
		...
		public override DataRow Get_New_Row(DataRow ParentRow, DataTable T) {
			...
			RowChange.MarkAsAutoincrement(T.Columns["idinsegninteg"], null, null, 0);
			RowChange.setMinimumTempValue(T.Columns["idinsegninteg"], 9990000);
			//$Get_New_Row$
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield) {
			...
			switch (edit_type) {
				case "integrandi": {
						...
						if (R["denominazione"].ToString().Trim().Length > 256) {
							errmess = "Attenzione! Il campo 'Denominazione' può essere al massimo di 256 caratteri";
							errfield = "denominazione";
							return false;
						}
						if (R["idinsegn"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'Insegnamento padre' è obbligatorio";
							errfield = "idinsegn";
							return false;
						}
