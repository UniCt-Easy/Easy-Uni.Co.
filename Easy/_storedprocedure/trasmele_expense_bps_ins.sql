...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_bps_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_bps_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_bps_ins]
(
	@y int,
	@n int
)
...
SELECT
	...
	(SELECT MAX(cupcode)
		 ...
		 WHERE (idexp_taxable IN (SELECT EL1.idparent 
								   ...
								   WHERE EL1.idchild = S.idexp and EL1.nlevel=@fasecontrattopassivo) 
			OR idexp_iva IN (SELECT EL2.idparent 
							   FROM expenselink EL2
							  WHERE EL2.idchild = S.idexp and EL2.nlevel=@fasecontrattopassivo))
	   ...
	   (SELECT MAX( mandate.cigcode )
		 ...
		 WHERE (mandatedetail.idexp_taxable IN (SELECT idparent 
									FROM expenselink 
								   WHERE idchild = S.idexp and nlevel=@fasecontrattopassivo) 
...
AND (p.idpaydisposition IS  NULL AND
		--ISNULL(p.idpaymethodTRS, '01')<> '13' AND -- i mandati stipendi ad anagrafiche cumulative non devono essere associati a ritenute
...
update #payment set saldo = curramount - isnull((select sum(curramount)  from #tax where #tax.idexp = #payment.idexp),0)
