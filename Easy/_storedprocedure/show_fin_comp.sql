...
CREATE    PROCEDURE [show_fin_comp]
(
	@date datetime,
	@idfin int,
	@finpart char(1)
...
BEGIN
	...
	JOIN income M
		ON M.idinc = MY.idinc
	JOIN incometotal MT 			--> Ho aggiunto incometotal x poter risalire al valore di Competenza o residuo
		ON MT.idinc = MY.idinc
		AND MT.ayear = MY.ayear
	JOIN finlink FLK
		ON FLK.idchild = MY.idfin
			...
			BEGIN
				IF (@finpart = 'E' AND (@nlevel is null ) AND @flagincomesurplus = 'N')-- Significa che stiamo sul titolo
				  Begin
					...
					INSERT INTO #situation	VALUES(@lbl_avail_ass,ISNULL(@prev_curr_M, 0) - ISNULL(@tot_mov_C, 0) - ISNULL(@tot_var_C, 0)- ISNULL(@creditsurplus,0), 'S')
				  End	
				Else
				  Begin
					SELECT @lbl_avail_ass = @lbl_avail_ass + ')'
					INSERT INTO #situation	VALUES(@lbl_avail_ass,ISNULL(@prev_curr_M, 0) - ISNULL(@tot_mov_C, 0) - ISNULL(@tot_var_C, 0), 'S')
				  End	
			END
...
BEGIN
	...
	BEGIN	
		...
		SELECT @tot_mov_R = ISNULL(amount, 0) 
			FROM #mov_R 
			WHERE nphase = @nphase
		INSERT INTO #situation	VALUES('1) Movimenti residui (' + @desc_phase + ')', @tot_mov_R, '')

		SELECT @tot_var_R = ISNULL(amount, 0) 
			FROM #var_R 
			WHERE nphase = @nphase
