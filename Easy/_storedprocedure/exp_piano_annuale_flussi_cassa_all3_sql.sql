...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_piano_annuale_flussi_cassa_all3_sql]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_piano_annuale_flussi_cassa_all3_sql]
GO
...
----------------------------------------------------------------------------
CREATE    PROCEDURE [exp_piano_annuale_flussi_cassa_all3_sql]
(
...
--- exec exp_piano_annuale_flussi_cassa_all3_sql 2025, {ts '2025-09-04 00:00:00'} , 'S', 21,22
--- exec exp_piano_annuale_flussi_cassa_all3_sql  2026, {ts '2026-04-04 00:00:00'} , 'N', 21,22 -- CONS CAGLIARI
AS BEGIN
...
(
			SELECT	'ENTRATE' as kind,
					coalesce(M5.label,M2.Label,M1.Label,'Altri incassi (non mappati)') as label,
					coalesce(M5.printingorder,M2.printingorder,M1.printingorder, '-') as printingorder,
				...
				JOIN sortinglevel sl
					ON sortInc.idsorkind = sl.idsorkind and sortInc.nlevel = sl.nlevel 
				...
				GROUP BY  sortInc.sortcode, HPV.competencydate,
					coalesce(M5.label,M2.Label,M1.Label,'Altri incassi (non mappati)'),
					coalesce(M5.printingorder,M2.printingorder,M1.printingorder, '-')
				--ORDER BY  sorFin.sortcode, month(HPV.competencydate)
				--ORDER BY  sorFin.sortcode, month(HPV.competencydate)
				) 
...
(
SELECT 'USCITE' as kind,
		coalesce(M5.label,M2.Label,M1.Label,'Altri pagamenti (non mappati)') as label,
		coalesce(M5.printingorder,M2.printingorder,M1.printingorder, '-')as printingorder,
	...
	JOIN sortinglevel sl
		ON sortExp.idsorkind = sl.idsorkind and sortExp.nlevel = sl.nlevel 
...
SELECT '#PERCENTUALI_RICALCOLATE', * FROM #PERCENTUALI_RICALCOLATE
 ...
 --   END  AS 'Conf. Bilancio'
	...
	END
		SELECT 'SOSPESI_ATTIVI' as bill,@date_rif as data_riferimento_apertura ,tocover,*  from billview
		where billview.ybill = @ayear  
		AND billview.billkind = 'C'
		AND billview.adate <= @date_rif 
		AND @trimestre >= 0 AND ISNULL(tocover,0) > 0

		SELECT 'SOSPESI_PASSIVI' as bill,@date_rif as data_riferimento_apertura ,tocover,*  from billview
		where billview.ybill = @ayear  
...
--go
--exec exp_piano_annuale_flussi_cassa_all4_sql 2025,  {ts '2025-04-03 00:00:00'}, 'N', 29,30	  --unisalento
--go
--exec exp_piano_annuale_flussi_cassa_all4_sql 2025,  {ts '2025-04-03 00:00:00'},NULL, 31,32    --unicampania
--go
--exec exp_piano_annuale_flussi_cassa_all4_sql 2024,  {ts '2024-04-01 00:00:00'},NULL, 26,27   --- unirc