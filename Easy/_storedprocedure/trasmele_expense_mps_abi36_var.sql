...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_mps_abi36_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_mps_abi36_var]
GO
...
CREATE        PROCEDURE [trasmele_expense_mps_abi36_var]
(
	@y int,
	@n int
)
...
-- CONTROLLO N. 17. anagrafica priva di codice fiscale in un pagamento per cassa
IF EXISTS
(SELECT * FROM paymentcommunicated
        join expenselast
        	ON paymentcommunicated.idexp = expenselast.idexp
        join expense
              on expenselast.idexp = expense.idexp 
		JOIN expensevar V
	ON expenselast.idexp = V.idexp
	AND V.autokind IN (10,11) 
