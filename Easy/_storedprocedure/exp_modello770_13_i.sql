if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_13_i]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_13_i]
GO
--exec exp_modello770_13_i
CREATE PROCEDURE [exp_modello770_13_i]
AS BEGIN

-------------------------------------------------------------------------------------------------------
----------------------------------------- RECORD DI TIPO "I": Dati relativi ---------------------------
----------------------------------al prospetto SY del mod. 770/2013 semplificato ----------------------
-------------------------------------------------------------------------------------------------------
...
SELECT 
		#expense2012.cf_reg,
		#expense2012.cf_reg_distrained,
		isnull(sum(#expense2012.taxablenet),0),
		isnull(sum(#expense2012.employtax),0), 
		isnull(sum(#expense2012.admintax),0),
		isnull(sum(#expense2012.grossamount),0),--Importo Lordo del Pagamento
		#expense2012.codeser 
FROM #expense2012
GROUP BY #expense2012.cf_reg,
		#expense2012.cf_reg_distrained,#expense2012.codeser
