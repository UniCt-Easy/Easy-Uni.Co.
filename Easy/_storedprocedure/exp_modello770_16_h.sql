...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_16_i_stranieri]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_16_i_stranieri]
GO
 
--exec exp_modello770_16_h
CREATE PROCEDURE [exp_modello770_16_i_stranieri]
AS BEGIN
-- OK
...
-- Sezione dichiarativa	
	...
	DECLARE registry_crs CURSOR FOR 
		SELECT DISTINCT isnull(cf,p_iva), #expense2015.idreg, surname, coalesce(registry.cf,registry.p_iva,registry.foreigncf) 
		FROM #expense2015
		join registry on registry.idreg = #expense2015.idreg
