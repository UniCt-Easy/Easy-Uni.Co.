...
if exists (select * from dbo.sysobjects where id = object_id(N'[compute_root_sortingkind_expense]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [compute_root_sortingkind_expense]
GO
--setuser'amm'



CREATE         PROCEDURE [compute_root_sortingkind_expense] 
	@ayear int,
	@idexp int,
	@idreg int,
	@idupb varchar(36),
...
INSERT INTO #tmp 
	          (idautosort,idfin,
		idupb,idsorkindreg,idsorreg,idman,
...
BEGIN
		SELECT * FROM sortingkind where nphaseexpense = @nphase AND codesorkind <> @codesorkind_siopespese
...
BEGIN
	...
	begin
		...
		and codesorkind <> @codesorkind_siopespese
			and (start is null or start<=@ayear) and (stop is null or stop>=@ayear)
