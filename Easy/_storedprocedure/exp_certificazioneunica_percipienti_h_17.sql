if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_percipienti_h_17]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_percipienti_h_17]
GO
 
--exec [exp_certificazioneunica_percipienti_h_17]  NULL 
CREATE PROCEDURE [exp_certificazioneunica_percipienti_h_17]
(
	@cf varchar(20)
 )
 -- estraggo l'elenco dei percipienti, autonomi, che hanno eseguito prestazioni mappate con il Record H
 -- dal 2017 aggiungo anche i percipienti (creditori pignoratizi) che hanno beneficiato di somme in virtù di pignoramenti 
...
AS BEGIN
	...
	CREATE TABLE #percipienti_record_H_2016
	(
		idreg int,
	...
	INSERT INTO #percipienti_record_H_2016
		(
			idreg
		)
	SELECT DISTINCT
...
GROUP BY T.idreg, T.idreg_distrained, T.idexp,
	...
	SELECT * FROM	#percipienti_record_H_2016
