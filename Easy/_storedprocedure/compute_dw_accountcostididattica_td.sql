...
/*--------------------------------------------------------------------------------------------------------------------------
			...
			Questa stored procedure legge dal DB utente e scrive nella tabella  "accountcostididattica_td" del db  >> DataWareHouse <<				 

---------------------------------------------------------------------------------------------------------------------------*/

-- setuser'amministrazione'

if exists (select * from dbo.sysobjects where id = object_id(N'compute_dw_accountcostididattica_td') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure compute_dw_accountcostididattica_td
GO
...
SET ANSI_NULLS ON 
GO
--- exec compute_dw_accountcostididattica_td
-- select * from DataWareHouse_ENTE.dbo.accountcostididattica_td
Create PROCEDURE [compute_dw_accountcostididattica_td]
AS
BEGIN
	DELETE DataWareHouse_ENTE.dbo.accountcostididattica_td

	DECLARE @dtelab date

	SET @dtelab = GETDATE()

	INSERT INTO DataWareHouse_ENTE.dbo.accountcostididattica_td( 
			ayear, idacc, 
	...
	SELECT 
		...
		and substring(A.codeplaccount,1,6)= 'b) VII'
		and accountkind.description = 'costi'


	update	DataWareHouse_ENTE.dbo.accountcostididattica_td set actual_codicecontoeconomico = codicecontoeconomico
						where actual_codicecontoeconomico is null

--	;WITH somma_costi AS (
--    SELECT
--        idacc,
--        SUM(amount) AS totale_costi
--    FROM DataWareHouse_ENTE.dbo.entrydetailfact
--    WHERE rowkind = 'costi'
--    GROUP BY idacc
--)
--UPDATE DataWareHouse_ENTE.dbo.accountcostididattica_td 
--	SET amount = - s.totale_costi ----------------------> CAMBIO SEGNO
--	FROM somma_costi s
--	WHERE DataWareHouse_ENTE.dbo.accountcostididattica_td.idacc = s.idacc;	
