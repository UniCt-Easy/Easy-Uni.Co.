...
if exists (select * from dbo.sysobjects where id = object_id(N'[split_invoicedetail_expensevar]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [split_invoicedetail_expensevar]
GO
...
CREATE           PROCEDURE [split_invoicedetail_expensevar]
(
	@ayear int 
)
AS
...
SELECT COUNT(DISTINCT expensevar.idexp),invoice.idinvkind,invoice.yinv,invoice.ninv
FROM invoice 
JOIN invoicedetail
...
WHERE invoicedetail.yinv = @ayear and expensevar.movkind = 1
	...
	AND invoicedetail.yinv = @ayear 
GROUP BY invoice.idinvkind,invoice.yinv,invoice.ninv
HAVING COUNT(DISTINCT expensevar.idexp) >1	
