...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_22]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_22]
GO
 
CREATE PROCEDURE [exp_certificazioneunica_g_22]
...
-- exec exp_certificazioneunica_g_22  478,1,'S' 
AS BEGIN 
	...
	declare @31dic2021 datetime
	set @31dic2021 = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_22  @idreg, @progrCom, 'G',NULL, @print
	...
	set @1gen2021_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-01'})

	declare @13gen2021_XXX datetime
	set @13gen2021_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-13'})

	declare @31dic2021_XXX datetime
	set @31dic2021_XXX = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})

	declare @1gen2022_XXX datetime
	set @1gen2022_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-01'})

	declare @12gen2021_XXX datetime
	set @12gen2021_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-12'})
	...
	declare @30giu2021 datetime
	set @30giu2021 = dateadd(yy, @annoredditi-2000, {d '2000-06-30'})
	
	declare @01lug2021 datetime
	set @01lug2021 = dateadd(yy, @annoredditi-2000, {d '2000-07-01'})
	...
	SET @ritenuta_addreg_irpef =
		...
		DECLARE @ritenuta_addcom_irpef_acconto_2021 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_acconto_2021 =
		ISNULL(
		...
		SET @ritenuta_addcom_irpef_acconto_2021 =
		@ritenuta_addcom_irpef_acconto_2021 +
		ISNULL(
		...
		-- Non si filtra sulla certificazione in quanto le prestazioni che non rientrano nel CUD
		-- non pagano le addizionali regionali e quindi il loro contributo è nullo
		DECLARE @ritenuta_addcom_irpef_saldo_2021 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_saldo_2021 =
		ISNULL(
			(SELECT SUM(employtax)
			FROM #cedolini
			JOIN expenselink
	...
	SET @ritenuta_addcom_irpef_saldo_2021 =
	@ritenuta_addcom_irpef_saldo_2021 +
	ISNULL(
	...
	SELECT @ritenuta_addcom_irpef_saldo_2021  = @ritenuta_addcom_irpef_saldo_2021 - @ritenuta_addcom_irpef_acconto_2021   
	...
	if @ritenuta_addcom_irpef_saldo_2021 < 0
	begin
		set @ritenuta_addcom_irpef_acconto_2021 = @ritenuta_addcom_irpef_acconto_2021 + @ritenuta_addcom_irpef_saldo_2021
		set @ritenuta_addcom_irpef_saldo_2021 = null
	end
	else
	begin
		if @ritenuta_addcom_irpef_acconto_2021 < 0
		begin
			set @ritenuta_addcom_irpef_saldo_2021 = @ritenuta_addcom_irpef_saldo_2021 + @ritenuta_addcom_irpef_acconto_2021
			set @ritenuta_addcom_irpef_acconto_2021 = null
		end
	end
...
------------------------------------------------------------------------------------------------------ FINE CAF ---------------------------------------------------------------------------------------------------
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen2021_XXX),
		ISNULL(F.stopapplication, @31dic2021_XXX),
		ISNULL(F.starthandicap, @1gen2022_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		begin
			...
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen2021_XXX))
				...
				begin
					update #PrimoFiglio set
						...
						minoreDiTreAnni = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen2021_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese ELSE 0 end,
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						case WHEN datediff(m, dateadd(m, @mese-1, @1gen2021_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
			...
			begin
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen2021_XXX), 
	...
	BEGIN
		...
		IF (
			...
			OR
			((Year(@start) < @annoredditi) AND (@workingdays = 365) AND (@stop < @31dic2021)  ) 

			OR (Year(@stop) < @annoredditi   ) 
			OR
				(	-- IN FORZA AL 31 12, con data ultimo cedolino dell'anno inferiore 
					(@stopcontract > @31dic2021)  AND (@stop < @31dic2021) 
				) 
			)	
			)
			AND (@workingdays >0)		
		 ...
		 ELSE 
			...
			if (	(@workingdays = 365) AND (@stopcontract > @31dic2021) /*IN FORZA AL 31 12*/ )  SET @DB001011 = NULL --- IN TAL CASO NON COMPILIAMO PERIODI PARTICOLARI
		...
		(
			...
			(	(year(@start)<@annoredditi)
				...
				OR
				(
				   (@stopcontract <= @31dic2021)  AND (@stop < @stopcontract) 
				)
			) 
		)
		OR
			...
			(
				/*(@workingdays_2_semestre) >0 AND*/ (@stop < @01lug2021)
			)
		)	
	...
	SET @DB001027= isnull(@ritenuta_addcom_irpef_saldo_2021,0)
	
	IF ((year(@stopcontract) = @annoredditi) AND (@stopcontract <= @31dic2021))
	BEGIN
		SET @DB001024= isnull(@ritenuta_addreg_irpef,0)
		SET @DB001028= isnull(@ritenuta_addcom_irpef_saldo_2021,0)
	END
	...
	BEGIN 
		...
		if @startpayroll < @1gen2021_XXX set @oldstart = @1gen2021_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic2021_XXX set @oldstop =  @31dic2021_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
		
			if @startpayroll < @1gen2021_XXX set @startpayroll = @1gen2021_XXX
			if @stoppayroll > @31dic2021_XXX set @stoppayroll = @31dic2021_XXX
