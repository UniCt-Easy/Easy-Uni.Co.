...
-- exp_interscambio_compensi_csa {d '2025-12-31'}, '2025', {d '2025-01-01'}, {d '2025-12-31'}, '001201', '2'
-- exp_interscambio_csa_record08   {d '2023-12-31'}, '2023', '70049', 'U00007', {d '2023-01-01'}, {d '2023-12-31'}, '001201', '1'
 
AS
-- exec exp_interscambio_compensi_csa {ts '2017-02-01 01:02:03'} ,2017,{ts '2017-02-01 01:02:03'} ,{ts '2017-02-28 01:02:03'}, '001383', 1
BEGIN

DECLARE @cfagency varchar(30)
SELECT  @cfagency = cf from license
IF ISNULL(@cfagency,'') = '02772010878'
BEGIN
		DECLARE @dip_nomesp varchar(100)
		SET @dip_nomesp =  'exp_interscambio_compensi_csa_ct'
...
CREATE TABLE #Missione( iditineration int,idreg int,extmatricula varchar(40),idexp int,movkind int,curramount decimal(19,2),
	...
	itinerationvisible char(1), idregistrylegalstatus int, flag_geo char(1),voceimponibile varchar(4),vocequotaesente varchar(4)
)
...
INSERT INTO #Missione (iditineration,idreg,extmatricula,idexp,movkind,curramount,voce8000refund_i,voce8000refund_e,capitolo_i,
		capitolo_e,ymov,nmov,transmissiondate,codeser,service,module,itinerationvisible,idregistrylegalstatus, flag_geo,voceimponibile,vocequotaesente)
SELECT I.iditineration,I.idreg,R.extmatricula,EL.idexp,E.movkind,EL.curramount + isnull(PagamentoAnticipoMissione.amount,0),S.voce8000refund_i,S.voce8000refund_e,V8_i.capitolo,
		V8_e.capitolo,EL.ymov,EL.nmov,PT.transmissiondate,S.codeser,S.description,S.module, S.itinerationvisible, I.idregistrylegalstatus, 'I', '8159','8158'
FROM itineration I 
...
SELECT
	...
	'S',
	'I', -- flag_geo,
	round( convert(decimal(19,2), 
	isnull(S.owncarkm,0)) * isnull(S.owncarkmcost,0) + convert(decimal(19,2), 
	isnull(S.admincarkm,0)) *  isnull(S.admincarkmcost,0) + convert(decimal(19,2), 
	isnull(S.footkm,0)) * isnull(S.footkmcost,0),2),
...
where (select count(*) from itinerationrefund where  itinerationrefund.iditineration = S.iditineration)=0
		and 	round( convert(decimal(19,2), isnull(S.owncarkm,0)) * isnull(S.owncarkmcost,0) 
			+ convert(decimal(19,2), isnull(S.admincarkm,0)) *  isnull(S.admincarkmcost,0) 
			+ convert(decimal(19,2), isnull(S.footkm,0)) * isnull(S.footkmcost,0),2)
			<> 0


-- Assumiamo per diaria il particolare tipo rimborso forfettario O7_FORFETTARIO
CREATE TABLE #DiariaMissione(iditineration int, flagadvancebalance char(1), flag_geo char(1), amount decimal(19,2),exemptionquota decimal(19,2),taxableprev decimal(19,2) )
INSERT INTO  #DiariaMissione(iditineration, flagadvancebalance, flag_geo, amount )
SELECT S.iditineration, S.flagadvancebalance, S.flag_geo, SUM(S.amount) 
FROM itinerationrefund	S
	JOIN #Missione M ON S.iditineration = M.iditineration
WHERE S.flagadvancebalance = 'S' -- Attenzione dobbiamo considerare solo le spese a saldo
	AND S.iditinerationrefundkind = @idtipo_rimborso_forfettario
GROUP BY S.iditineration, S.flagadvancebalance,S.flag_geo
...
--INSERT INTO  #DiariaMissione(iditineration, flagadvancebalance, flag_geo, amount )
--SELECT S.iditineration, 'S', 'E', MAX(total)
--FROM itinerationlap	S
--JOIN itineration I ON I.iditineration = S.iditineration
--JOIN #Missione M ON I.iditineration = M.iditineration
--WHERE S.idforeigncountry IS NOT NULL -- Attenzione dobbiamo considerare solo le indennità di missione all'estero
--GROUP BY S.iditineration 
...
--SELECT '#DiariaMissione',* FROM #DiariaMissione
--SELECT '#SpeseMissione',* FROM #SpeseMissione

--- CONVENZIONE PER DISTINGUERE LE MISSIONI ITALIA DALLE MISSIONI ESTERE
--- Le missioni con rimborso forfettario estero sono estere 
UPDATE #Missione SET flag_geo = 'E' , voceimponibile = '8055',vocequotaesente = '8054'
	FROM #DiariaMissione 
	WHERE #Missione.iditineration = #DiariaMissione.iditineration
			AND #DiariaMissione.flag_geo = 'E'
			AND #Missione.flag_geo = 'I' 

UPDATE #Missione SET flag_geo = 'E', voceimponibile = '8055',vocequotaesente = '8054' 
		WHERE exists (SELECT * FROM #SpeseMissione WHERE #Missione.iditineration = #SpeseMissione.iditineration AND #SpeseMissione.flag_geo = 'E')
			AND #Missione.flag_geo = 'I' 

--- diaria estera sulla tappa estera 
UPDATE #Missione SET flag_geo = 'E', voceimponibile = '8055',vocequotaesente = '8054' 
		WHERE exists (SELECT * FROM itinerationlap WHERE #Missione.iditineration = itinerationlap.iditineration AND itinerationlap.idforeigncountry is not null)
			AND #Missione.flag_geo = 'I' 
...
FROM expense E
	...
	and ISNULL(V8.flag_geo, 'I')  = ISNULL(#DiariaMissione.flag_geo, 'I')
...
GROUP BY S.flagcsausability ,E.idreg, R.extmatricula, EL.idexp,PT.transmissiondate,
	...
	ISNULL(V.capitoloquotaesente,V8.capitolo),V.flagcsausability,#DiariaMissione.exemptionquota,
	#DiariaMissione.flag_geo,V8.flag_geo, EL.curramount, I.totalgross, PagamentoAnticipo.amount
...
INSERT INTO #pagamenti(idexp,kind,voce8000refund_i,capitolo,importo,idreg,extmatricula,ymov,
			...
			SELECT
				...
				Round((#Missione.curramount*#Italia.amount)/ 
								ISNULL((SELECT SUM(#SpeseMissione.amount) FROM #SpeseMissione WHERE #SpeseMissione.iditineration = #Missione.iditineration),0),2),
			#Missione.idreg,
			#Missione.extmatricula,
			#Missione.ymov,
			#Missione.nmov,
			#Missione.transmissiondate,
			#Missione.codeser,#Missione.service,#Missione.module,#Missione.itinerationvisible,#Missione.idregistrylegalstatus
FROM #Missione
	JOIN #SpeseMissione #Italia	ON #Missione.iditineration = #Italia.iditineration 
	AND #Italia.flag_geo = 'I'
...
SELECT
		...
		Round((#Missione.curramount*#Estero.amount)/ 
				ISNULL((SELECT SUM(#SpeseMissione.amount) FROM #SpeseMissione	WHERE #SpeseMissione.iditineration = #Missione.iditineration),0),2),
		#Missione.idreg,
		#Missione.extmatricula,
		#Missione.ymov,#Missione.nmov,
		#Missione.transmissiondate,
		#Missione.codeser,
		#Missione.service,#Missione.module,#Missione.itinerationvisible,#Missione.idregistrylegalstatus
FROM #Missione
		JOIN #SpeseMissione #Estero ON #Missione.iditineration = #Estero.iditineration 
		AND #Estero.flag_geo <> 'I'
...
SELECT
		EL.idexp, 'I', V.vocequotaesente, ISNULL(V.capitoloquotaesente,V8.capitolo),
		(SELECT importo FROM #pagamenti P1 	where P1.idexp = EL.idexp 	AND P1.voce8000 = '8047') - MAX(ETO.taxablegross), 
		E.idreg,R.extmatricula,
		E.ymov, E.nmov, PT.transmissiondate, S.codeser, S.description, S.module, S.itinerationvisible
FROM expense E
	JOIN expenselast EL ON E.idexp = EL.idexp
	JOIN expensetaxofficial ETO ON ETO.idexp = EL.idexp
...
---- GESTISCO INSERIMENTO DI IMPONIBILI, QUOTE ESENTI, RITENUTE C/ENTE E C/DIPENDENTE SOLO PER LE MISSIONI
---- in questo caso ho bisogno di distinguere le due diverse mappature per Missioni Italia e Missioni Estero
------------------------------------------------ inizio missioni
INSERT INTO #pagamenti(
idexp,kind,voce8000,capitolo,importo,idreg,extmatricula,ymov,nmov, transmissiondate,codeser,service,module,itinerationvisible
)
...
FROM expense E 
	...
	JOIN expensetaxofficial ETO ON ETO.idexp = EL.idexp
	JOIN tax T ON ETO.taxcode = T.taxcode
	JOIN #Missione M ON M.idexp = EL.idexp
	JOIN voce8000lookup V ON V.taxcode = T.taxcode
	JOIN voce8000 V8 ON V.voceimponibile = V8.voce AND ISNULL(V8.flag_geo, 'I') = M.flag_geo  
	join payment P on P.kpay = EL.kpay
	join paymenttransmission PT on P.kpaymenttransmission = PT.kpaymenttransmission
	JOIN service S ON S.idser = EL.idser
	JOIN registry R ON R.idreg = E.idreg
...
--	======================================================================================================================================================================================
 -- ======================================================= Legge la QUOTA ESENTE da itinerationamountdetail =============================================================================
--	======================================================================================================================================================================================
SELECT
		EL.idexp, 'I', V.vocequotaesente, ISNULL(V.capitoloquotaesente,V8.capitolo),
		idett.quotaesente,
...
FROM expense E
		...
		JOIN expensetaxofficial ETO ON ETO.idexp = EL.idexp
		JOIN tax T ON ETO.taxcode = T.taxcode
		LEFT OUTER  JOIN #Missione M ON M.idexp = EL.idexp
		left outer join itinerationamountdetail idett on  idett.iditineration = M.iditineration
		LEFT OUTER  JOIN voce8000lookup V ON V.taxcode = T.taxcode
		LEFT OUTER  JOIN voce8000 V8 ON V.vocequotaesente= V8.voce AND ISNULL(V8.flag_geo, 'I') = M.flag_geo
		JOIN payment P ON P.kpay = EL.kpay
		JOIN paymenttransmission PT	 on P.kpaymenttransmission = PT.kpaymenttransmission
		JOIN service S ON S.idser = EL.idser
		JOIN registry R ON R.idreg = E.idreg
WHERE P.ypay = @ayear
		...
		AND R.extmatricula is not null
		AND (( V.flagcsausability & @mask) <>0 )
		AND  V.vocequotaesente in ('8054','8158')
		AND  (LTRIM(RTRIM(R.extmatricula)) = LTRIM(RTRIM(@extmatricula)) OR @extmatricula IS NULL)
GROUP BY S.flagcsausability ,E.idreg, R.extmatricula, EL.idexp,PT.transmissiondate,
		E.ymov,E.nmov,	S.codeser,S.description, S.module, S.itinerationvisible ,	V.vocequotaesente,  
		ISNULL(V.capitoloquotaesente,V8.capitolo),V.flagcsausability,idett.quotaesente
...
FROM expense E
	...
	JOIN expensetaxofficial ETO ON ETO.idexp = EL.idexp
	JOIN #Missione M ON M.idexp = EL.idexp
	JOIN voce8000lookup V ON V.taxcode = ETO.taxcode AND V.voceimponibile = M.voceimponibile
	JOIN voce8000 V8 ON V.voce = V8.voce    
	JOIN voce8000 V8Imp ON V.voceimponibile = V8Imp.voce AND ISNULL(V8Imp.flag_geo, 'I') = M.flag_geo  
	join payment P on P.kpay = EL.kpay
	join paymenttransmission PT on P.kpaymenttransmission = PT.kpaymenttransmission
	JOIN service S ON S.idser = EL.idser
	JOIN registry R ON R.idreg = E.idreg
...
--AND ISNULL(S.itinerationvisible,'N')= 'N'
		AND  (LTRIM(RTRIM(R.extmatricula)) = LTRIM(RTRIM(@extmatricula)) OR @extmatricula IS NULL)
GROUP BY S.flagcsausability , EL.idexp, V.voce,	E.idreg, R.extmatricula,PT.transmissiondate,V8.flag_geo,V.voceimponibile,
		E.ymov,E.nmov,S.codeser,S.description, S.module, S.itinerationvisible , ISNULL(V.capitolovoce,V8.capitolo),V.flagcsausability
...
-- per le Missioni cancello la codifica di imponibile e quota esente eventualmente incoerenti con la Missione valorizzata Italia/Estera in base a una convenzione.
-- La convenzione usata per considerare le Missioni come Estere è la presenza di un rimborso forfettario estero (Diaria) oppure di spese estere.
-- Infatti per gestire la trasmissione delle Spese di Missioni a Ufficio Stipendi si è dovuta raddoppiare la configurazione in VoceCSALookup mentre prima per le Missioni
-- era una sola, e in conseguenza di tale mappatura raddoppiata possiamo avere dati incoerenti nella tabella temporanea #Pagamenti

--8054	I	Mem. missioni estero (esenti) quota esente estera
--8055	I	Mem. missioni estero (impon.) quota imponibile estera

--8158	I	Mem. missioni Italia (esenti) quota esente Italia
--8159	I	Mem. missioni Italia (impon.) quota imponibile Italia

DELETE FROM #Pagamenti WHERE module = 'MISSIONE' AND kind IN ('I') /*imponibile o ritenuta*/ AND voce8000 in ('8055', '8054' ) --- imponibile e quota esente missioni Estere
AND EXISTS (SELECT * FROM #Missione WHERE #Pagamenti.idexp = #Missione.idexp AND  #Missione.flag_geo = 'I') -- ma la Missione è svolta in Italia

DELETE FROM #Pagamenti WHERE module = 'MISSIONE' AND kind IN ('I') /*imponibile o ritenuta*/AND voce8000 in ('8158', '8159' )  --- imponibile e quota esente missioni Italia
AND EXISTS (SELECT * FROM #Missione WHERE #Pagamenti.idexp = #Missione.idexp AND  #Missione.flag_geo = 'E') -- ma la Missione è estera

-- In casi di spese con flag_geo incoerenti, quindi per le missioni che presentano sia spese sostenute all'estero che in Italia, 
-- adotto la stessa convenzione nel valorizzare '8160' '8161', ma nella pratica sono considerate missioni estere. Secondo la convenzione
-- che necessariamente si deve adottare, è sufficiente una spesa sostenuta all'estero per considerare una missione estera.
UPDATE  #Pagamenti SET voce8000refund_i = NULL, voce8000refund_e = '8161' WHERE module = 'MISSIONE'  /*totale spese*/ AND voce8000refund_i = '8160'  
AND EXISTS (SELECT * FROM #Missione WHERE #Pagamenti.idexp = #Missione.idexp AND  #Missione.flag_geo = 'E') -- ma la Missione è svolta all'estero

UPDATE  #Pagamenti SET voce8000refund_i = '8160' , voce8000refund_e = null WHERE module = 'MISSIONE'  /*totale spese*/ AND voce8000refund_e = '8161' 
AND EXISTS (SELECT * FROM #Missione WHERE #Pagamenti.idexp = #Missione.idexp AND  #Missione.flag_geo = 'I') -- ma la Missione è svolta in Italia
