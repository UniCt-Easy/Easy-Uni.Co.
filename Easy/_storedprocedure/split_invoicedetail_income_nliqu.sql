...
CREATE TABLE #income (idinc int)-- contiene i pagamenti della fattura
...
JOIN invoicedeferred
	on incomeinvoice.idinvkind = invoicedeferred.idinvkind
	AND incomeinvoice.yinv = invoicedeferred.yinv 
	AND incomeinvoice.ninv = invoicedeferred.ninv
JOIN ivapay
...
BEGIN
	...
	SELECT @nidinc_pay = COUNT(*) FROM #income
	...
	BEGIN
		...
		SET @curramount = (SELECT ISNULL(amount,0) FROM incomeyear WHERE idinc = @idinc)
