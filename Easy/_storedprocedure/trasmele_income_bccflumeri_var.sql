...
(
	yproceedstransmission, nproceedstransmission, kpro, ydoc, ndoc, idinc, curramount,curramount_expense,
...
LEFT OUTER JOIN payment pm
	ON el.kpay = pm.kpay
WHERE ((il.flag&32) <> 0) 
AND d.kpro IN (SELECT kpro FROM #proceedsvar) --> Solo le reversali variate  o annullate totalmente
...
SELECT
	...
	#proceeds.idpro, sorting.sortcode,sorting.description,  SUM(#proceeds.curramount), SUM(incomesorted.amount), SUM(#proceeds.curramount_expense),
