...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_h_23]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_h_23]
GO
 ...
 /*
 declare @newProg int
 exec exp_certificazioneunica_h_23 235862,1, 'N', @newProg out
 select @newProg
 */
 --select @newProg
CREATE PROCEDURE [exp_certificazioneunica_h_23]
...
where (ISNULL(p.flagexcludefromcertificate,'N') = 'S')) 
	...
	update #expense2022 set motive770 = motive770service.idmot,
		...
		where motive770service.idser = #expense2022.idser
		...
		--SELECT 
		--	#expense2022.employtaxamount,
		--	#expense2022.idexp,
		--	#expense2022.idser,
		--	#expense2022.motive770,
		--	#expense2022.socialseccode,
		--	#expense2022.cfagency,
		--	#expense2022.titleagency 
		--FROM #expense2022
		--join expenselink  			on expenselink.idchild = #expense2022.idexp			and nlevel = @expensephase
...
---AUX0025 = Imponibile Lordo SUM (feegross) - Imponibile Netto (SUM(taxablenet) da expensetax where (taxkind=1 ossia fiscali)
	...
	--24 - nel caso di compensi, non assoggettati a ritenuta d’acconto, corrisposti ai soggetti in regime forfetario di cui all’articolo 1, della L. 190/2014.
		...
		UPDATE  #expense2022 set notsubjecttaxcode = @notsubjecttaxcode  
		WHERE #expense2022.idexp= @idexp
	END
...
-- laddove esso non sia valorizzato per poter paginare nello stesso modulo
	...
	UPDATE #expense2022 SET notsubjecttaxcode = ( select top 1 A.notsubjecttaxcode 
	FROM #recHNonArrot A WHERE 
	A.idexp =  #expense2022.idexp AND isnull(A.notsubjecttaxcode,'') <> 0)
	WHERE isnull(#expense2022.notsubjecttaxcode,'') = 0
		...
		begin
			...
			ELSE
				BEGIN  --- creo una nuova certificazione
					...
					exec exp_certificazioneunica_d_23  @idreg, @newprogrCom, 'H',NULL, @print
				END
...
insert into #recordH (progr,modulo, quadro, riga, colonna, stringa,decimale, intero, data)
			exec exp_certificazioneunica_h_pign_23  @idreg, @newprogrCom, @modulo, @fattoprestazioni,  @inseritopignoramenti out, @newmodulo out
