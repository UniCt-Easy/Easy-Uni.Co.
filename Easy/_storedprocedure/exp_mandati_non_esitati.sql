...
if OBJECTPROPERTY(object_id('exp_mandati_non_esitati'), 'IsProcedure') = 1
	drop procedure exp_mandati_non_esitati
go
--setuser 'amministrazione'
--exec exp_mandati_non_esitati 2021, {ts '2021-10-14 00:00:00.000'}, 1852, null, 6736, null, null
CREATE  PROCEDURE  [exp_mandati_non_esitati]
(
	@year smallint,
	@date datetime,
	@idsor01 int=null,
...
from (
	select * from (
		SELECT expense.idexp,
				(SELECT SUM(curramount)	from expensetotal I join expense S on I.idexp=S.idexp JOIN expenselast EL ON EL.idexp = S.idexp	WHERE EL.idexp=expenselast.idexp AND I.ayear = payment.ypay)
				-
				ISNULL( (SELECT SUM(amount) from banktransaction P where P.idexp=expenselast.idexp),0) as nonesitato
		FROM expense (nolock)
		...
		where payment.ypay = @year
		and (@gen_01 <= paymenttransmission.transmissiondate and paymenttransmission.transmissiondate <= @date)
		and datediff(day, paymenttransmission.transmissiondate, @date) >= @ndays
	) tab
	where nonesitato > 0
) non_esitati
JOIN expense on expense.idexp = non_esitati.idexp
