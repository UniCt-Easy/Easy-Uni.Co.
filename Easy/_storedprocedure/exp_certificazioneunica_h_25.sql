...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_h_25]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_h_25]
GO
 ...
 */
 --select @newProg
CREATE PROCEDURE [exp_certificazioneunica_h_25]
...
delete from #expense2024 where idexp in( 
SELECT #expense2024.idexp FROM #expense2024
join expenselink el on el.idchild = #expense2024.idexp 
...
where (ISNULL(p.flagexcludefromcertificate,'N') = 'S')) and @maxexpensephase = 1
	...
	update #expense2024 set motive770 = motive770service.idmot,
		...
		where motive770service.idser = #expense2024.idser
		...
		--SELECT 
		--	#expense2024.employtaxamount,
		--	#expense2024.idexp,
		--	#expense2024.idser,
		--	#expense2024.motive770,
		--	#expense2024.socialseccode,
		--	#expense2024.cfagency,
		--	#expense2024.titleagency 
		--FROM #expense2024
		--join expenselink  			on expenselink.idchild = #expense2024.idexp			and nlevel = @expensephase
...
---AUX0025 = Imponibile Lordo SUM (feegross) - Imponibile Netto (SUM(taxablenet) da expensetax where (taxkind=1 ossia fiscali)
	...
	--select 'imponibili netti fiscali', SUM(taxablenet) --somma imponibili netti ove la rit fiscale non è zero esclusi stranieri conv.
	--						FROM expensetaxofficial
	--			              		join tax
	--							ON tax.taxcode = expensetaxofficial.taxcode
	--						where expensetaxofficial.idexp = @idexp
	...
	-- NEL 2024 DISTINGUIAMO I SEGUENTI CASI CHE L'ANNO SCORSO NON DISTINGUEVAMO (task 15738)
	...
	-- (CertUnica12 regime forfettario). In quest'ultimo caso andrà messo il codice 24.
	...
	--23 - nel caso di Assegni di servizio civile universale di cui all’art. 16 del D.lgs. n. 40 del 2017 per i quali la norma ha
	...
	--24 - nel caso di compensi, non assoggettati a ritenuta d’acconto, corrisposti ai soggetti in regime forfetario di cui all’articolo 1, della L. 190/2014.
		...
		UPDATE  #expense2024 set notsubjecttaxcode = @notsubjecttaxcode  
		WHERE #expense2024.idexp= @idexp
	END
...
-- laddove esso non sia valorizzato per poter paginare nello stesso modulo
	...
	UPDATE #expense2024 SET notsubjecttaxcode = ( select top 1 A.notsubjecttaxcode 
	FROM #recHNonArrot A WHERE 
	A.idexp =  #expense2024.idexp AND isnull(A.notsubjecttaxcode,'') <> 0)
	WHERE isnull(#expense2024.notsubjecttaxcode,'') = 0
		...
		begin
			...
			ELSE
				BEGIN  --- creo una nuova certificazione
					...
					exec exp_certificazioneunica_d_25  @idreg, @newprogrCom, 'H',NULL, @print
				END
...
insert into #recordH (progr,modulo, quadro, riga, colonna, stringa,decimale, intero, data)
			exec exp_certificazioneunica_h_pign_25  @idreg, @newprogrCom, @modulo, @fattoprestazioni,  @inseritopignoramenti out, @newmodulo out
