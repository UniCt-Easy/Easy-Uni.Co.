...
if exists (select * from dbo.sysobjects where id = object_id(N'exp_certificazioneunica_h_pign_16') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure exp_certificazioneunica_h_pign_16
GO
...
--exec exp_certificazioneunica_h_pign_16 212,1, 'S', @inserito out
--select @newProg
CREATE PROCEDURE exp_certificazioneunica_h_pign_16
...
-- RECORD DI TIPO "H": Casi particolari operazioni straordinarie
	...
	--2016	AU	101	Codice Fiscale debitore   principale
	--2016	AU	102	Somme erogate
	--2016	AU	103	Ritenute operate
	--2016	AU	104	Ritenute non operate
	--2016	AU	105 Codice Fiscale debitore   principale
	--2016	AU	106	Somme erogate
	--2016	AU	107	Ritenute operate
	--2016	AU	108	Ritenute non operate
...
FROM
	...
	WHERE expense.ymov = @annoredditi  
			AND E.stop IS NULL
			AND (@idreg=expense.idreg or @idreg is null)
	GROUP BY expense.idreg,E.idexp,expenselast.idser,W.idreg_distrained,
		E.description ,expense.description,       expenselast.kpay
                ) AS T
...
INTO @cf_reg, @cf_reg_distrained, @taxablenet, @employtax,@admintax,@grossamount,@servicecode770
	...
	WHILE @@FETCH_STATUS = 0
		...
		IF @servicecode770 = '11_PIGNORA_ESE' 
			BEGIN
				INSERT INTO #recordPignoramenti(progr,modulo, quadro, riga, colonna,decimale ) VALUES (@progrCom,@progrModulo, 'AU', 1,  convert(varchar(3),102 + 4*(@contaPagamenti%2)), @grossamount)
				INSERT INTO #recordPignoramenti(progr,modulo, quadro, riga, colonna,intero ) VALUES (@progrCom,@progrModulo, 'AU', 1,  convert(varchar(3),104 + 4*(@contaPagamenti%2)), 1)
			END
		ELSE
			BEGIN
