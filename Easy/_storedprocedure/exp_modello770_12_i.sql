...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_12_i]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_12_i]
GO
--exec exp_modello770_12_i
CREATE PROCEDURE [exp_modello770_12_i]
AS BEGIN

-------------------------------------------------------------------------------------------------------
----------------------------------------- RECORD DI TIPO "I": Dati relativi ---------------------------
----------------------------------al prospetto SY del mod. 770/2012 semplificato ----------------------
-------------------------------------------------------------------------------------------------------
...
-- RECORD DI TIPO "I": Dati relativi al prospetto SY del mod. 770/2012 semplificato
...
SELECT 
		#expense2011.cf_reg,
		#expense2011.cf_reg_distrained,
		isnull(sum(#expense2011.taxablenet),0),
		isnull(sum(#expense2011.employtax),0), 
		isnull(sum(#expense2011.admintax),0),
		isnull(sum(#expense2011.grossamount),0),--Importo Lordo del Pagamento
		#expense2011.codeser 
FROM #expense2011
GROUP BY #expense2011.cf_reg,
		#expense2011.cf_reg_distrained,#expense2011.codeser
	...
	WHILE @@FETCH_STATUS = 0
		...
		*/
		...
		IF @codeser = '11_PIGNORA_ESE' 
			BEGIN
				INSERT INTO #recordI(progr,quadro, riga, colonna,intero ) VALUES (@progrModulo, 'SY' + '00' + convert(varchar(1),@contaPagamenti%5 + 2),1, '003',ROUND(@grossamount,0,1))
		...
		ELSE
			BEGIN
				INSERT INTO #recordI(progr,quadro, riga, colonna,intero ) VALUES (@progrModulo, 'SY' + '00' + convert(varchar(1),@contaPagamenti%5 + 2),1, '003',ROUND(@taxablenet,0,1))
				INSERT INTO #recordI(progr,quadro, riga, colonna,intero ) VALUES (@progrModulo, 'SY' + '00' + convert(varchar(1),@contaPagamenti%5 + 2),1,'004',ROUND(@employtax,0,1))
			END
