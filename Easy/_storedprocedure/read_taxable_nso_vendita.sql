...
if exists (select * from dbo.sysobjects where id = object_id(N'[read_taxable_nso_vendita]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [read_taxable_nso_vendita]
GO
SET ANSI_NULLS ON
GO
...
CREATE procedure read_taxable_nso_vendita (
...
BEGIN
	...
	select @x = 
		...
		(replace(replace(replace(replace(replace(replace(@alltext,
	...
	set @res= convert(decimal(19,6), @amount)
