...
Begin
	...
	BEGIN
			...
			BEGIN
				...
				INSERT INTO #logerror
				SELECT 'S', @iddbdepartment, 'L''aliquota corretta per ' + tax + ', scaglione ' + convert(varchar(4),nbracket) + ', deve essere ' + convert(varchar(20),employrate * 100) + '%'  ,''
				FROM #config_corrente_inps
				END

-- Se il compenso prevedeva la ritenuta 'INPS contributo DIS_COLL', allora l'aliquota della ritenuta 'INPS soggetti non mutuati'va considerata maggiorata dello 0,51 %
			IF (SELECT COUNT(*)      FROM #listacollaboratori
	        WHERE taxref like '%INPS%' AND
			...
			BEGIN
				...
				INSERT INTO #logerror
				SELECT 'S', @iddbdepartment, 'L''aliquota corretta per ' + tax + ', scaglione ' + convert(varchar(4),nbracket) + ', deve essere ' + convert(varchar(20),employrate +0.0051 * 100) + '%'  ,''
				FROM #config_corrente_inps
				END
    END
