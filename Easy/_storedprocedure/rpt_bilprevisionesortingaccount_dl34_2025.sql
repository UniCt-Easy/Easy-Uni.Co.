...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_bilprevisionesortingaccount_dl34_2025]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_bilprevisionesortingaccount_dl34_2025]
GO
...
-- exec rpt_bilprevisionesortingaccount_dl34_2025 2025, 32, 4, '%', 'N', 'S', NULL, NULL, NULL, NULL, NULL, 'S' 

CREATE  PROCEDURE [rpt_bilprevisionesortingaccount_dl34_2025]
	@ayear			int,

	@idsorkind_dl34_2025 int, 
	@levelusable tinyint,
...
BEGIN
/* IMPORTANTE:
 voci di bilancio classificate con DI_394_08.06.17 Uscite
...
SET @MostraTutteVoci = 'N' -- Forziamo questo parametro a N perchè non si desidera vedere le voci a importo 0
...
where f5.ayear = @ayear
		...
		AND sorFin.idsorkind = @idsorkind_dl34_2025
		...
		AND (sorFin.nlevel = @levelusable
			OR (sorFin.nlevel < @levelusable
				and (select count(*) from sorting S where S.idsorkind = @idsorkind_dl34_2025 and S.paridsor = sorFin.idsor)=0
				AND (sl.flag&2)<>0
			   )
			)
...
SELECT @lencod_lev6 = flag / 256 FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 6
DECLARE @startpos6 int 
SELECT @startpos6 = @startpos5 + @lencod_lev5

DECLARE @lencod_lev7 int 
SELECT @lencod_lev7 = flag / 256 FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 7
...
SELECT @lev_desc6 = description FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 6
DECLARE @lev_desc7 varchar(50)
SELECT @lev_desc7 = description FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 7
...
BEGIN
		...
		LEFT OUTER JOIN sorting f6
			ON flk6.idparent = f6.idsor  and f6.idsorkind = @idsorkind_dl34_2025	
		LEFT OUTER JOIN sortinglink flk7
			ON flk6.idchild = f.idsor AND flk6.nlevel = 7
		LEFT OUTER JOIN sorting f7
			ON flk7.idparent = f7.idsor  and f7.idsorkind = @idsorkind_dl34_2025	
		where  ( (@MostraTutteVoci = 'S'  or @suppressifblank='N') OR  (isnull(#data.tosuppress, 'S')='N')) and
