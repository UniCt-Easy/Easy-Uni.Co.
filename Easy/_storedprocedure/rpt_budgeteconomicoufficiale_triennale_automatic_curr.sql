...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_budgeteconomicoufficiale_triennale_automatic_curr]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_budgeteconomicoufficiale_triennale_automatic_curr]
GO
...
--	exec rpt_budgeteconomicoufficiale_triennale_automatic_curr 2019, '%','S', {ts '2019-10-09 00:00:00'}
-- A.	Budget Economico triennale che mostra la previsione corrente 
-- Previsione Corrente (x )  = exec sp alla data + Var. approvate di tipo 1,3,4

CREATE      PROCEDURE [rpt_budgeteconomicoufficiale_triennale_automatic_curr](
...
update #spbudget set amount2 = isnull(amount2,0) + isnull(initialprevision2,0)
update #spbudget set amount3 = isnull(amount3,0) + isnull(initialprevision3,0)
