...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_12]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_12]
GO
...
CREATE    PROCEDURE [exp_modello770_12] (
	@recordGH varchar(2)='GH'
	)
AS BEGIN

	declare @statementyear smallint
	set @statementyear = 2012
	...
	INSERT INTO #modello770 EXEC exp_modello770_12_a

	DECLARE @progr_raggiunto int

	if @recordGH = 'GH'
	begin
		INSERT INTO #modello770 EXEC exp_modello770_12_g '770'
	end

--	INSERT INTO #modello770 EXEC exp_modello770_12_h
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Il seguente codice è la sp del reord H, ossia "exp_modello770_12_h"
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	declare @SS003001 dec(19,2)
	declare @SS003002 dec(19,2)

	declare @annodichiarazione int
	set @annodichiarazione = 2012

	declare @annoredditi int
	set @annoredditi = 2011
...
-- Sezione dichiarativa	
	...
	update #expense2011 set motive770 = motive770service.idmot
		from motive770service 
		where motive770service.idser = #expense2011.idser
		AND motive770service.ayear = @annodichiarazione-1

	declare @cognome varchar(60)
	DECLARE registry_crs CURSOR FOR 
		SELECT DISTINCT isnull(cf,p_iva), #expense2011.idreg, surname FROM #expense2011
		join registry on registry.idreg = #expense2011.idreg
		order by surname
	open registry_crs
	...
	--AU001009 Provincia (sigla)
		...
		if exists (select * from #expense2011 where idreg=@idreg and motive770='N')
		begin
	...
	--AU001017 Codice stato estero
		...
		DECLARE cursoreexpense CURSOR FOR 
			SELECT 
				#expense2011.employtax,
				#expense2011.idexp,
				#expense2011.idser,
				#expense2011.motive770
			FROM #expense2011
			join expenselink 
				on expenselink.idchild = #expense2011.idexp
				and nlevel = @expensephase
			left outer join expenseprofservice
				on expenselink.idparent = expenseprofservice.idexp
			WHERE #expense2011.idreg=@idreg 
		...
		BEGIN
			...
			SELECT @au25_26ritIRPEF = ISNULL(SUM(expensetaxofficial.employtax),0)
			...
			SELECT  @au34ritprevidenzialedip = isnull(SUM(expensetaxofficial.employtax),0),
				@au33ritprevidenzialeamm = isnull(SUM(expensetaxofficial.admintax),0)
				FROM expensetaxofficial
				JOIN tax
					ON tax.taxcode = expensetaxofficial.taxcode
				WHERE expensetaxofficial.idexp = @idexp
...
--	AUX024 = AUX0021 - AUX0022 - AUX0023
	...
	--AUXXX034 Contributi previdenziali a carico del percipiente
			...
			set @au35speseRimborsate=0
			IF (@ycon is not null)  -- si tratta di un contratto occasionale
				select  @au35speseRimborsate =  ISNULL( (SELECT P_refund_lastyear  from #annualpayedrefundH),0)

			set  @au35speseRimborsate = @au35speseRimborsate * @quota_contratto
...
INSERT INTO #modello770 EXEC exp_modello770_12_i 
DROP TABLE #annualpayedrefundH
DROP TABLE #recordh
DROP TABLE #recHNonArrot
DROP TABLE #expense2011
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Fine esecuzione della sp exp_modello770_11_h
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	INSERT INTO #modello770 EXEC exp_modello770_12_b @numdichquadrog, @numdichquadroh
