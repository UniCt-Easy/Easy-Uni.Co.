...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_partitario_anagrafica_spese_web]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_partitario_anagrafica_spese_web]
GO
...
CREATE     PROCEDURE [rpt_partitario_anagrafica_spese_web]
	@ayear int,
	@codelevel tinyint,
	@codefin varchar(50),
	@start datetime,
...
WHERE ((fin.flag&1)=1) -- Spesa
	...
	AND (@idreg IS NULL OR expense.idreg = @idreg)
...
WHERE  (@idfin IS NULL OR EY.idfin = @idfin)	
	AND (@idreg IS NULL OR OP.idreg = @idreg)
	AND OP.adate between @start and @stop
