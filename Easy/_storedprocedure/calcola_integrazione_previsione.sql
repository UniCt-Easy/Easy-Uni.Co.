	...
	IF ((@idacc is not null)AND(@reval_to_date <>0)OR((@reval_to_date_2 <>0)OR(@reval_to_date_3  <>0))) 
		...
		--SELECT @maxidautomaticbudget = max(idautomaticbudget) from #automaticbudget
			...
			SELECT
				...
				@idasset, @idpiece,@adate, @idupb, @idacc,@ayear
		END
	FETCH NEXT FROM amt_crs INTO @idinventoryamortization, @idasset, @idpiece,@idupb, @description, @amortizationquota,@idaccmotiveload,@idacc
END
DEALLOCATE amt_crs
...
CREATE TABLE #budgetsituation
 ...
 BEGIN
	...
	BEGIN
			...
			print 'riserve'
			print @riserve_ex_cofi
			print @totcosti_upb
			print @totricavi_upb
			print @totcosti_upb - @totricavi_upb
			IF ((@totcosti_upb - @totricavi_upb) > 0) 
			BEGIN
				IF (@riserve_ex_cofi) >= (@totcosti_upb - @totricavi_upb)
					SELECT @amount= ISNULL(@totcosti_upb - @totricavi_upb,0)
				ELSE
					SELECT @amount= ISNULL(@riserve_ex_cofi,0)
					print @amount
				-- creazione di una previsione di ricavo su un conto specIFico di ricavo 
					...
					SELECT
						/*@maxidautomaticbudget + 1,*/@idaccmotiveload,'RESERVE',@amount,0,0,
