...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_19]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_19]
GO
 
CREATE PROCEDURE [exp_certificazioneunica_g_19]
...
-- exec exp_certificazioneunica_g_19  25438,1,'N' 
AS BEGIN 
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_19  @idreg, @progrCom, 'G',NULL, @print
	...
	declare @1gen18_XXX datetime
	set @1gen18_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-01'})

	declare @13gen18_XXX datetime
	set @13gen18_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-13'})

	declare @31dic18_XXX datetime
	set @31dic18_XXX = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})

	declare @1gen19_XXX datetime
	set @1gen19_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-01'})

	declare @12gen18_XXX datetime
	set @12gen18_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-12'})
...
--setuser 'amm'
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen18_XXX),
		ISNULL(F.stopapplication, @31dic18_XXX),
		ISNULL(F.starthandicap, @1gen19_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		begin
			...
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen18_XXX))
				...
				begin
					update #PrimoFiglio set
						...
						minoreDiTreAnni = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen18_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese ELSE 0 end,
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						case WHEN datediff(m, dateadd(m, @mese-1, @1gen18_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
			...
			begin
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen18_XXX), 
	...
	BEGIN
		...
		IF (
			...
			OR
			((Year(@start) < @annoredditi) AND (@workingdays = 365) AND (@stop < @31dic17)  ) 

			OR (Year(@stop) < @annoredditi   ) 
			OR
				(	-- IN FORZA AL 31 12, con data ultimo cedolino dell'anno inferiore 
					(@stopcontract > @31dic17)  AND (@stop < @31dic17) 
				) 
			)	
			)
			AND (@workingdays >0)		
		...
		(	(year(@start)<@annoredditi)
			...
			OR
			(
			   (@stopcontract <= @31dic17)  AND (@stop < @stopcontract) 
			)
		) 
	...
	IF ((year(@stopcontract) = @annoredditi) AND (@stopcontract <= @31dic17))
	BEGIN
	...
	BEGIN 
		...
		if @startpayroll < @1gen18_XXX set @oldstart = @1gen18_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic18_XXX set @oldstop =  @31dic18_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
		
			if @startpayroll < @1gen18_XXX set @startpayroll = @1gen18_XXX
			if @stoppayroll > @31dic18_XXX set @stoppayroll = @31dic18_XXX
