...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_bilconsuntivo_gestionale_all6]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_bilconsuntivo_gestionale_all6]
GO
...
-- setuser 'amm'
-- exec rpt_bilconsuntivo_gestionale_all6 2018, {ts '2018-07-31 00:00:00'}, 'E', 5, '%', 'N', 'S', 'S', 'S', NULL, NULL, NULL, NULL, NULL
-- exec rpt_bilconsuntivo 2018,{ts '2018-07-30 00:00:00'},'E',5,'%','N','N','S','N', null, null, null, null, null


CREATE       PROCEDURE [rpt_bilconsuntivo_gestionale_all6]
(
	@ayear int,
	@date datetime,
	@finpart char(1)='S',
...
select ISNULL(FLK.idparent,finyear.idfin),
	isnull(@fixedidupb,finyear.idupb),
		ISNULL(SUM(finyear.prevision),0),ISNULL(SUM(finyear.secondaryprev),0)
from finyear 	
...
LEFT OUTER JOIN finlink FLK		ON FLK.idchild = finyear.idfin AND FLK.nlevel = @levelusable_original
...
LEFT OUTER JOIN finlink FLK		ON FLK.idchild = FVD.idfin AND FLK.nlevel = @levelusable_original
WHERE FV.yvar = @ayear
	AND FV.adate <= @date
...
BEGIN
	...
	LEFT OUTER JOIN finlink FLK		ON FLK.idchild = IY.idfin AND FLK.nlevel = @levelusable_original
	WHERE I.adate <= @date
		AND IY.ayear = @ayear
...
BEGIN
   ...
   if exists(select * from fin F 
	...
	UPDATE #data	SET mov_maxphase_C = ISNULL(initialprevision, 0) + ISNULL(variation_A, 0) + ISNULL(variation_D, 0)
...
-- Se la stampa è per capitolo, e solo l'articolo è flaggato come avanzo. Il capitolo deve essere inteso come avanzo
update #data set flagadvance = 'S'
where exists (SELECT * FROM fin 
					JOIN  finlast
							ON finlast.idfin = fin.idfin
					JOIN finlink 
							ON finlink.idparent = #data.idfin  
							AND finlink.idchild = finlast.idfin  
					WHERE (fin.flag&16 <>0)  )

-- i report del consuntivo non gestiscono l'omissione della voce di bilancio non utilizzata di livello 5, per cui gestiamo nella sp questa eventualità
if ( (@suppressifblank='S') and @levelusable_original>=5)
Begin
	delete D from #data D
	join fin on D.idfin = fin.idfin
	where 	
	ISNULL(D.initialprevision,0) = 0 and 	ISNULL(D.variation_A,0)  = 0 and ISNULL(D.variation_D, 0) = 0 and
	ISNULL(D.secondaryprevision,0)  = 0 and 	ISNULL(D.secondaryvariation,0)  = 0 and 	ISNULL(D.currentarrears,0)  = 0 and 	ISNULL(D.mov_finphase_C,0)  = 0 and 
	ISNULL(D.var_finphase_C,0)  = 0 and 	ISNULL(D.mov_maxphase_C,0)  = 0 and 	ISNULL(D.var_maxphase_C,0)  = 0 and 	
	ISNULL(D.mov_finphase_R,0)  = 0 and 	ISNULL(D.var_finphase_R_P,0)  = 0 and ISNULL(D.var_finphase_R_N,0) = 0 and	ISNULL(D.mov_maxphase_R,0)  = 0 and 	ISNULL(D.var_maxphase_R,0) = 0 
	and fin.nlevel>=5
End
...
SELECT 
	...
	ISNULL(SUM(variation_A),0) AS variation_A,
	ISNULL(SUM(variation_D),0) AS variation_D,
	...
	ISNULL(SUM(var_finphase_R_P),0) AS var_finphase_R_P,
	ISNULL(SUM(var_finphase_R_N),0) AS var_finphase_R_N,
...
LEFT OUTER JOIN finlink flk1		ON flk1.idchild = f.idfin AND flk1.nlevel = 1
LEFT OUTER JOIN fin f1				ON flk1.idparent = f1.idfin 
LEFT OUTER JOIN finlink flk2		ON flk2.idchild = f.idfin AND flk2.nlevel = 2
LEFT OUTER JOIN fin f2				ON flk2.idparent = f2.idfin 
LEFT OUTER JOIN finlink flk3		ON flk3.idchild = f.idfin AND flk3.nlevel = 3
LEFT OUTER JOIN fin f3				ON flk3.idparent = f3.idfin 
LEFT OUTER JOIN finlink flk4		ON flk4.idchild = f.idfin AND flk4.nlevel = 4
LEFT OUTER JOIN fin f4				ON flk4.idparent = f4.idfin 
LEFT OUTER JOIN finlink flk5		ON flk5.idchild = f.idfin AND flk5.nlevel = 5
LEFT OUTER JOIN fin f5				ON flk5.idparent = f5.idfin 
where  ( (@MostraTutteVoci = 'S'  or @suppressifblank='N') OR  (#data.idfin is not null or isnull(#data.tosuppress, 'S')='N')) 
		and
		f.ayear=@ayear 
...
GROUP BY upb.printingorder,upb.idupb,upb.title,upb.codeupb,
	...
	f1.title,f2.title,f3.title,f4.title,f5.title,f.printingorder,
	showincomesurplus,f.nlevel,isnull(#data.tosuppress, @suppressifblank),
	f.flag,f1.flag,f2.flag,f3.flag,f4.flag,f5.flag,flagadvance
