...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE rpt_newpartitario_spesa_tutte_fasi
	@ayear int,
	@kind char(1),
	@idupb	varchar(36),
	@nlevel tinyint,
...
AS
BEGIN
--exec rpt_newpartitario_spesa_tutte_fasi 2011, 'R', '%', 3, {ts '2011-01-01 00:00:00'}, {ts '2011-12-31 00:00:00'}, 'N', 'N', 'S', 'N', NULL 
...
BEGIN
	...
	IF (@kind = 'R') 
			...
			WHERE  ((expensetotal.flag&1) = @kind_bit)
				...
				AND isnull(@fixedidupb,expenseyear.idupb) like @idupb 
...
BEGIN
	...
	BEGIN
		...
		JOIN fin
			ON fin.idfin = FL1.idparent	
		WHERE  isnull(@fixedidupb, expenseyear.idupb) like @idupb 
			AND expenseyear.ayear = @ayear
			AND (expensetotal.flag&1) = @kind_bit
