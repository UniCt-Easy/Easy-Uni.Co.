...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_bpcassinate_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_bpcassinate_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_bpcassinate_ins]
(
	@y int,
	@n int
)
AS BEGIN
--
--[trasmele_expense_bpcassinate_ins] 2014,2
--------------------------------------------------------------
...
WHERE  paydisposition.kpay in (select  kpay from #payment
...
GROUP BY #payment.ypaymenttransmission, #payment.npaymenttransmission, #payment.ydoc,
	...
	#payment.idpay, SUBSTRING(sorting.sortcode,@npos,DATALENGTH(sorting.sortcode)),sorting.description,
	#payment.idexp, 
	#payment.exp_curramount, #payment.curramount,#payment.iddetail,
