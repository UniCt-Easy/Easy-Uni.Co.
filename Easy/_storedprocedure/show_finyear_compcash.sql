...
if exists (select * from dbo.sysobjects where id = object_id(N'[show_finyear_compcash]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [show_finyear_compcash]
GO
...
CREATE                PROCEDURE [show_finyear_compcash]
	@date datetime,
	@idupb varchar(36),
	@idfin int ,
	@finpart char(1)
...
DECLARE @assessmentphase tinyint  -- Fase paragonabile all'accertamento o all'impegno
...
BEGIN
	...
	DECLARE	@creditpart decimal(19,2) -- Totale assegnazione dei crediti
	DECLARE	@creditvariation decimal(19,2) -- Totale variazione assegnazione dei crediti
	DECLARE @creditsurplus_distribution decimal(19,2) -- Totale ripartizione dell'avanzo di amministrazione
	DECLARE	@proceedspart decimal(19,2) -- Totale assegnazioni di cassa
	DECLARE	@proceedsvariation decimal(19,2) -- Totale variazioni assegnazioni di cassa
	DECLARE @floatfund_distribution decimal(19,2) -- Totale ripartizione dell'avanzo di cassa	
	IF (@assured <> 'S')
	BEGIN
		SELECT @creditpart = SUM(creditpart.amount)
		FROM creditpart
		JOIN finlink 
		...
		SELECT @creditsurplus_distribution = SUM(D.amount)
		FROM finvardetail D
		JOIN finvar V
			ON V.yvar = D.yvar
			AND V.nvar = D.nvar
		...
		SELECT @creditvariation = SUM(D.amount)
		FROM finvardetail D
		JOIN finvar V
			ON V.yvar = D.yvar
			AND V.nvar = D.nvar
		...
		SELECT @proceedspart = SUM(proceedspart.amount)
		FROM proceedspart
		JOIN finlink 
		...
		SELECT @floatfund_distribution = SUM(D.amount)
		FROM finvardetail D
		JOIN finvar V
			ON V.yvar = D.yvar
			AND V.nvar = D.nvar
		...
		SELECT @proceedsvariation = SUM(D.amount)
		FROM finvardetail D
		JOIN finvar V
			ON V.yvar = D.yvar
			AND V.nvar = D.nvar
	...
	BEGIN
		SET @creditpart = 0
		SET @creditsurplus_distribution = 0
		SET @creditvariation = 0
		SET @proceedspart = 0
		SET @floatfund_distribution = 0
		SET @proceedsvariation = 0
	END
...
IF (@finpart = 'S') 
	AND (EXISTS (SELECT * FROM creditpart 
	...
	AND (@assured <> 'S')
BEGIN
	INSERT INTO #situation VALUES ('Avanzo di Amministrazione',@creditsurplus_distribution,'')
	INSERT INTO #situation	VALUES ('Assegnazione crediti',	@creditpart, '')
	INSERT INTO #situation	VALUES ('Variazioni e Storni di Crediti', @creditvariation, '')
	INSERT INTO #situation	VALUES ('Previsione da accreditare', 
		ISNULL(@current_prevision, 0) -
		ISNULL(@creditsurplus_distribution,0) -
		ISNULL(@creditpart, 0) - 
		ISNULL(@creditvariation, 0),'S')
END
...
BEGIN
	...
	---------------------------------------------------------------------------------------------------------------------------------------------
		...
		BEGIN
			...
			INSERT INTO #situation	VALUES('2) Variazioni competenza ('+ @phasedescr	+ ')', @var_C, '')
			...
			ELSE IF (@phasecod = @maxphase) -- Informazioni inerenti gli incassi
				BEGIN
					...
					ISNULL((SELECT SUM(amount) FROM #var_finphase_C WHERE nphase = @residualphase),0)
					INSERT INTO #situation VALUES('Accertato di competenza da incassare',
					@movE_finphase_C -
					ISNULL(@mov_C,0) - ISNULL(@var_C,0),'S')
				END
		END
		FETCH NEXT FROM	fase_cursor INTO @phasecod, @phasedescr
	END
...
------------------------------------------------------------------------------------------------------------------------------------------------
	...
	SET @totalpro_C = 
		...
		ISNULL((SELECT SUM(amount) FROM #var_finphase_C WHERE nphase = @maxphase),0)
...
IF (@finpart = 'S')
	AND (EXISTS	
		...
		WHERE yvar = @ayear
			...
			AND @assured <> 'S'
	BEGIN
		INSERT INTO #situation VALUES ('Avanzo di cassa',@floatfund_distribution,'')
		INSERT INTO #situation	VALUES ('Assegnazione incassi',	@proceedspart, '')
		INSERT INTO #situation	VALUES ('Variazioni e Storni di Incassi', @proceedsvariation, '')
		INSERT INTO #situation	VALUES ('Previsione da incassare', 
			ISNULL(@current_secprevision, 0) -
			ISNULL(@floatfund_distribution, 0) -
			ISNULL(@proceedspart, 0) -
			ISNULL(@proceedsvariation, 0), 'S')
	END
------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	BEGIN
		...
		IF (@phasecod = @maxphase)
			BEGIN
				...
				ISNULL((SELECT SUM(amount) FROM #var_finphase_C WHERE nphase = @assessmentphase),0) +
	...
	-- Calcolo della fase successiva per sottrarla a quella attuale
		...
		BEGIN
			...
			SET @nextphase_C = ISNULL(@nextphase_C,0) + 
				ISNULL((SELECT ISNULL(amount,0) FROM #var_finphase_C WHERE nphase = (@phasecod + 1)),0)
		...
		INSERT INTO #situation 
		VALUES('2. Totale Variazioni competenza (' + @phasedescr +')',ISNULL(@var_C,0),'')
		INSERT INTO #situation 
...
BEGIN
	...
	-- Calcolo della fase successiva per sottrarla a quella attuale
		...
		INSERT INTO #situation VALUES('1. Totale Movimenti competenza (' + @phasedescr +')',
				ISNULL(@mov_C,0),'')
		INSERT INTO #situation VALUES('2. Totale Variazioni competenza (' + @phasedescr +')',
				ISNULL(@var_C,0),'')
