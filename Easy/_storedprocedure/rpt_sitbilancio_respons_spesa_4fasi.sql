...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_sitbilancio_respons_spesa_4fasi]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_sitbilancio_respons_spesa_4fasi]
GO
...
CREATE  PROCEDURE [rpt_sitbilancio_respons_spesa_4fasi]
	@ayear			int,
	@nlevel			tinyint,
	@hierarchy		char(1),
	@idman			int,
...
AS
BEGIN
/*	exec rpt_sitbilancio_respons_spesa_4fasi 2012, 3, 'N', null, '%', 'N', 'N', 'N', null,{ts '1900-12-31 00:00:00'},{ts '2012-12-31 00:00:00'}, null,null,null,null,null	*/
DECLARE @idfin int
...
CREATE TABLE #fin_situation(
	...
	var_second_phase_comp		decimal(19,2),
	second_phase_resid		decimal(19,2),
	var_second_phase_resid		decimal(19,2),
	desc_second_phase 		varchar(50),					

	third_phase_comp		decimal(19,2),
	var_third_phase_comp		decimal(19,2),
	third_phase_resid		decimal(19,2),
	var_third_phase_resid		decimal(19,2),
	desc_third_phase		varchar(50),		
...
DECLARE @nfinphase tinyint -- accantonamento precedente alla fase di impegno
...
SELECT  @nsecondphase = expenseregphase FROM uniconfig
...
BEGIN
	...
	WHERE 
		...
		isnull(second_phase_comp,0.0) =0 AND 
		isnull(var_second_phase_comp,0.0) =0 AND 
		isnull(second_phase_resid,0.0) =0 AND 
		isnull(var_second_phase_resid,0.0) =0 AND 

		isnull(third_phase_comp,0.0) =0 AND 
		isnull(var_third_phase_comp,0.0) =0 AND 
		isnull(third_phase_resid,0.0) =0 AND 
		isnull(var_third_phase_resid,0.0) =0 AND 
...
BEGIN
	...
	SELECT 
		...
		@finphase 				 	 'desc_fin_phase',	

		sum(isnull(second_phase_comp,0.0)) 		 'second_phase_comp',
		sum(isnull(var_second_phase_comp,0.0)) 	 	 'var_second_phase_comp',
		sum(isnull(second_phase_resid,0.0)) 		 'second_phase_resid'   ,
		sum(isnull(var_second_phase_resid,0.0)) 	 'var_second_phase_resid',
		@secondphase 				 	 'desc_second_phase',		

		sum(isnull(third_phase_comp,0.0)) 		 'third_phase_comp',
		sum(isnull(var_third_phase_comp,0.0)) 	 	 'var_third_phase_comp',
		sum(isnull(third_phase_resid,0.0)) 		 'third_phase_resid'   ,
		sum(isnull(var_third_phase_resid,0.0)) 	 	 'var_third_phase_resid',
		@thirdphase 				 	 'desc_third_phase',		
