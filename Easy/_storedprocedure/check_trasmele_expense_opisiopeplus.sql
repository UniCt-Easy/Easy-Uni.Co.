...
-- EXEC check_trasmele_expense_opisiopeplus 2020, 990
DECLARE @k int
...
--	Se ci sono più classificazioni sul movimento e : più dati distinti in fattura o fattura senza siope => diventa obbligatorio il Siope in fatura
  ...
  -- il controllo agisce solo se ci sono fatture di tipo commerciale
	...
	insert into #errors(error)
	select 'ARCONET-Importo codice economico siope non quadra con la somma di Importo COFOG siope. Numero mandato : '+ convert(varchar(10),numero_mandato) + ', beneficiario '+anagrafica_beneficiario+', Pagamento N.'+ convert(varchar(10), E.nmov)+'.' 
	from 	#checktrace T1
	join expense E on T1.idexp = E.idexp
	...
	insert into #errors(error)
	select 'ARCONET-Importo beneficiario non quadra con la somma di importo codice economico siope. Numero mandato : '+ convert(varchar(10),numero_mandato) + ', beneficiario '+anagrafica_beneficiario+', Pagamento N.'+ convert(varchar(10), E.nmov)+'.' 
	from 	#checktrace T1
	join expense E on T1.idexp = E.idexp
