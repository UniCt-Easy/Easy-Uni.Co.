...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_bpcassinate_abi36_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_bpcassinate_abi36_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_bpcassinate_abi36_ins]
(
	@y int,
	@n int
)
AS BEGIN
--
--[trasmele_expense_bpcassinate_abi36_ins] 2015,2
--------------------------------------------------------------
...
SELECT t.ypaymenttransmission, t.npaymenttransmission, d.ypay, d.npay, s.idexp,  
	...
	CASE
		WHEN (pd.iban IS NOT NULL) THEN '02' -- bonifico  -- paymethodcode = 1
		WHEN ((pd.paymethodcode = 2) AND (pd.iban IS NULL)) THEN  '01' -- cassa
