...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_bppugliese_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_bppugliese_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_bppugliese_ins]
(
	@y int,
	@n int
)
AS BEGIN
--
--[trasmele_expense_bppugliese_ins] 2015,1
--------------------------------------------------------------
...
SELECT
	...
	--CASE
	--	WHEN (((el.paymethod_flag & 64) = 0) AND ((el.paymethod_flag & 256) = 0) AND 
	--			((el.paymethod_flag & 512) = 0)  AND ((el.paymethod_flag & 1024) = 0) AND ((el.paymethod_flag & 2048) = 0))   THEN NULL
	...
	CASE
		...
		ELSE NULL
	END, -- informazione obbligatoria solo per i girofondi in BI secondo lo standard ABI ma per questa banca resta obbligatoria sempre
