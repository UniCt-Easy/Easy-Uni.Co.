...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_unicredit_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_unicredit_var]
GO
...
CREATE        PROCEDURE [trasmele_expense_unicredit_var]
(
	@y int,
	@n int
)
...
-- CONTROLLO N. 6. Codice IBAN  o CC Estero devono essere valorizzati nel caso di modalità di pagamento 69 (Bonifico SEPA estero)
INSERT INTO #error (message)
(SELECT 'Al movimento n.' + CONVERT(varchar(6),paymentcommunicated.nmov) 
...
(
	...
	ABI varchar(5),
	CAB varchar(5),
	cc varchar(30), -- Viene impostato a 25 in quanto gli utenti possono adoperare un C/C che eccede tale lunghezza. Corretto a 30 per uniformarlo a expenselast, ove vale 30
	cin char(1),
	iban varchar(50),
...
-- Partita Iva
	CASE
		...
		WHEN ctc.flaghuman = 'N' AND isnull(c.p_iva,c.cf) IS NOT NULL
			AND DATALENGTH(isnull(c.p_iva,c.cf)) <= @len_pi
...
WHERE I.nphase = @maxincomephase
	AND ((I.autokind = 6) /* <--Recupero*/	OR (I.autokind = 14) /*<--automatismo generico*/OR (I.autokind = 4 AND I.idreg = P.idreg)/*<--Ritenuta*/
		  OR (I.autokind in (20,21,30,31) AND I.idreg = P.idreg AND @csa_flagtransmissionlinking = 'S') /*<--csa*/)
	AND IT.ayear = @y
...
(
	...
	idexp, idinc, ypro, npro, yproceedstransmission, curramount, curramount_expense, 
	idreg, autokind,
...
GROUP BY
	...
	paymenthodistructions, refexternaldoc, foreign_cc, biccode,flag_sepa, flag_foreign_transfer,
	stamp_charge,committeecode,committeeamount, extracode,ISNULL(info_fatture,''), cu,bank_charges_exempt
...
-- RECORD RT - Va compilato solo in caso di modifiche SIOPE per ritenute a carico del dipendente
INSERT INTO #trace (y, n, ndoc, nrow, out_str)
SELECT
