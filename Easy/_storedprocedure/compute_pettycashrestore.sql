...
WHERE yoperation = @ayear
	...
	AND idpettycash = @idpettycash
	AND ((pettycashoperation.flag & 8) <> 0)
...
SELECT
	...
	--Se il mov. collegato ha un UPB, e questo UPB ha un resp., prende questo Resp.
	-- diversamente prende il Resp. del movimento di spesa, oppure il resp. della piccola spesa.
	case when EY.idupb is not null and  isnull((select idman from upb where idupb = EY.idupb),0)<>0  then (select idman from upb where idupb = EY.idupb)
		else ISNULL(E.idman, PCO.idman)
	end,
	PCO.nbill,
	PCO.amount
