 ...
 */
 --select @newProg
CREATE PROCEDURE [exp_certificazioneunica_h_24]
...
where (ISNULL(p.flagexcludefromcertificate,'N') = 'S')) 
	...
	update #expense2023 set motive770 = motive770service.idmot,
		...
		where motive770service.idser = #expense2023.idser
		...
		--SELECT 
		--	#expense2023.employtaxamount,
		--	#expense2023.idexp,
		--	#expense2023.idser,
		--	#expense2023.motive770,
		--	#expense2023.socialseccode,
		--	#expense2023.cfagency,
		--	#expense2023.titleagency 
		--FROM #expense2023
		--join expenselink  			on expenselink.idchild = #expense2023.idexp			and nlevel = @expensephase
...
---AUX0025 = Imponibile Lordo SUM (feegross) - Imponibile Netto (SUM(taxablenet) da expensetax where (taxkind=1 ossia fiscali)
	...
	-- NEL 2023 DISTINGUIAMO I SEGUENTI CASI CHE L'ANNO SCORSO NON DISTINGUEVAMO (task 15738)
	...
	--24 - nel caso di compensi, non assoggettati a ritenuta d’acconto, corrisposti ai soggetti in regime forfetario di cui all’articolo 1, della L. 190/2014.
		...
		UPDATE  #expense2023 set notsubjecttaxcode = @notsubjecttaxcode  
		WHERE #expense2023.idexp= @idexp
	END
...
-- laddove esso non sia valorizzato per poter paginare nello stesso modulo
	...
	UPDATE #expense2023 SET notsubjecttaxcode = ( select top 1 A.notsubjecttaxcode 
	FROM #recHNonArrot A WHERE 
	A.idexp =  #expense2023.idexp AND isnull(A.notsubjecttaxcode,'') <> 0)
	WHERE isnull(#expense2023.notsubjecttaxcode,'') = 0
		...
		begin
			...
			ELSE
				BEGIN  --- creo una nuova certificazione
					...
					exec exp_certificazioneunica_d_24  @idreg, @newprogrCom, 'H',NULL, @print
				END
...
insert into #recordH (progr,modulo, quadro, riga, colonna, stringa,decimale, intero, data)
			exec exp_certificazioneunica_h_pign_24  @idreg, @newprogrCom, @modulo, @fattoprestazioni,  @inseritopignoramenti out, @newmodulo out
