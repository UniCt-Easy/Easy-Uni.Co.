...
CREATE  PROCEDURE [trasmele_income_unicredit_var]
(
	@y int,
	@n int
)
...
SELECT
	...
	ISNULL(e.description,'') ,
	--ISNULL(e.fulfilled,'N'),
	'N', -- gli incassi vituali non sono a regolarizzazione
	e.idpayment,
...
SELECT
	...
	#proceeds.ydoc, #proceeds.ndoc, #proceeds.ndocformatted,
	...
	#proceeds.cupcodefin, #proceeds.cupcodeupb, 
	#proceeds.cupcodeincome,#proceeds.flagpendingincome
FROM #proceeds
JOIN incomesorted
...
SELECT
	...
	CASE
		WHEN ASCII(SUBSTRING(pi_ver,1,1)) NOT BETWEEN 48 AND 57
		THEN @nota_pivaestera + pi_ver + SPACE(150 - LEN(@nota_pivaestera) - LEN(pi_ver))
		ELSE SPACE(150)
	END + 
	-- Codice Generico
	SPACE(20) +
...
pi_ver, cf_ver, free_stamp, proceedsdescr, transmissiondate, fulfilled, cu, CUP
