...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_25_i]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_25_i]
GO
...
--exec exp_modello770_25_i
CREATE PROCEDURE [exp_modello770_25_i]
AS BEGIN

-------------------------------------------------------------------------------------------------------
----------------------------------------- RECORD DI TIPO "I": Dati relativi ---------------------------
----------------------------------al prospetto SY del mod. 770/2018 -----------------------------------
-------------------------------------------------------------------------------------------------------
...
-- RECORD DI TIPO "I": Dati relativi al prospetto SY del mod. 770/2018 
...
INTO @cf_reg, @cf_reg_distrained, @taxablenet, @employtax,@admintax,@grossamount,@servicecode770
	WHILE @@FETCH_STATUS = 0
			...
			BEGIN
				-- INIZIO PARTE POSIZIONALE
					...
					--8 Identificativo del produttore del software (codice fiscale)
						INSERT INTO #recordI (progr, quadro, riga, colonna, stringa) VALUES(@progrModulo, 'HRI', 1, '07', @cfsoftwarehouse)
		...
		*/
		...
		ELSE
			BEGIN
				INSERT INTO #recordI(progr,quadro, riga, colonna,decimale ) VALUES (@progrModulo, 'SY' + '0'+RIGHT(REPLICATE('0', 2) + convert(varchar(2),@contaPagamenti%5 + 7),2),1, '003',@taxablenet)
			END
...
-- Sezione dichiarativa	
	...
	BEGIN
		...
		SELECT TOP 1 
			...
			WHERE registryaddress.idreg = @idreg
				AND registryaddress.start <= @31dic16
				AND (registryaddress.stop IS NULL OR registryaddress.stop >= @31dic16)
			ORDER BY
			CASE idaddresskind
				WHEN @stand THEN 1
				WHEN @domfi THEN 2
	...
	--SY016	008	Codice stato estero	N3
		...
		DECLARE cursoreexpense CURSOR FOR 
			...
			--left outer join expenseprofservice
			--	on expenselink.idparent = expenseprofservice.idexp
			left outer join expenseinvoice 			on #expense2016_stranieri.idexp = expenseinvoice.idexp
			WHERE #expense2016_stranieri.idreg=@idreg 
					and isnull(movkind,0) <> 2	--Serve ad escludere le contab. professionali con causale 2 - Contabilizzazione iva documento
	...
	BEGIN
		...
		print  @idexp
			...
			if (isnull(@impon_contratto,0)>0) begin 
