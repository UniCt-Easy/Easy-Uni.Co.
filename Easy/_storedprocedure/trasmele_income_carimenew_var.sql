...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_income_carimenew_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_income_carimenew_var]
GO
...
CREATE  PROCEDURE [trasmele_income_carimenew_var]
(
	@y int,
	@n int
)
...
SET @ABI_codecarime = '03111'   -- 03111 ABI UBI BANCA 
...
cap_ver =
	...
	WHERE #proceeds.idreg = #address.idreg),
location_ver =
	(SELECT substring(ISNULL(location,''),1,30)
	FROM #address
	WHERE #proceeds.idreg = #address.idreg),
province_ver =
...
SET @codice_ABI_BT = @ABI_codecarime
---------------------------------------------
----- fine testata flusso -------------------
---------------------------------------------
