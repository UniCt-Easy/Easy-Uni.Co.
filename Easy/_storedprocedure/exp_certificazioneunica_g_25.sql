...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_25]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_25]
GO
 
CREATE PROCEDURE [exp_certificazioneunica_g_25]
...
-- exec exp_certificazioneunica_g_25  81856,1,'S'  --55750

AS BEGIN 
	declare @annodichiarazione int
	set @annodichiarazione = 2025

	declare @annoredditi int
	set @annoredditi = 2024

	declare @31dic2024 datetime
	set @31dic2024 = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_25  @idreg, @progrCom, 'G',NULL, @print
	...
	set @1gen2024_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-01'})

	declare @13gen2024_XXX datetime
	set @13gen2024_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-13'})

	declare @31dic2024_XXX datetime
	set @31dic2024_XXX = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})

	declare @1gen2025_XXX datetime
	set @1gen2025_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-01'})

	declare @12gen2025_XXX datetime
	set @12gen2025_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-12'})
	...
	declare @30giu2024 datetime
	set @30giu2024 = dateadd(yy, @annoredditi-2000, {d '2000-06-30'})
	
	declare @01lug2024 datetime
	set @01lug2024 = dateadd(yy, @annoredditi-2000, {d '2000-07-01'})
	...
	SET @ritenuta_addreg_irpef =
		...
		DECLARE @ritenuta_addcom_irpef_acconto_2024 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_acconto_2024 =
		ISNULL(
		...
		SET @ritenuta_addcom_irpef_acconto_2024 =
		@ritenuta_addcom_irpef_acconto_2024 +
		ISNULL(
		...
		-- Non si filtra sulla certificazione in quanto le prestazioni che non rientrano nel CUD
		-- non pagano le addizionali regionali e quindi il loro contributo è nullo
		DECLARE @ritenuta_addcom_irpef_saldo_2024 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_saldo_2024 =
		ISNULL(
			(SELECT SUM(employtax)
			FROM #cedolini
			JOIN expenselink
	...
	SET @ritenuta_addcom_irpef_saldo_2024 =
	@ritenuta_addcom_irpef_saldo_2024 +
	ISNULL(
	...
	SELECT @ritenuta_addcom_irpef_saldo_2024  = @ritenuta_addcom_irpef_saldo_2024 - @ritenuta_addcom_irpef_acconto_2024  
	...
	if @ritenuta_addcom_irpef_saldo_2024 < 0
	begin
		set @ritenuta_addcom_irpef_acconto_2024 = @ritenuta_addcom_irpef_acconto_2024 + @ritenuta_addcom_irpef_saldo_2024
		set @ritenuta_addcom_irpef_saldo_2024 = null
	end
	else
	begin
		if @ritenuta_addcom_irpef_acconto_2024 < 0
		begin
			set @ritenuta_addcom_irpef_saldo_2024 = @ritenuta_addcom_irpef_saldo_2024 + @ritenuta_addcom_irpef_acconto_2024
			set @ritenuta_addcom_irpef_acconto_2024 = null
		end
	end
...
------------------------------------------------------------------------------------------------------ FINE CAF ---------------------------------------------------------------------------------------------------
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen2024_XXX),
		ISNULL(F.stopapplication, @31dic2024_XXX),
		ISNULL(F.starthandicap, @1gen2025_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		begin
			...
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen2024_XXX))
			...
			begin
				...
				begin
					update #PrimoFiglio set
						...
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), dateadd(yy, 3, @birthdate)) >= 0 and @mese <= @meseFebbraio THEN @valore_mese ELSE 0 end,
						maggioreDi21Anni = isnull(maggioreDi21Anni, 0) +
								case WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2024_XXX)) >= 0 /*and @mese >= @meseInizio21anni*/ THEN @valore_mese ELSE 0 end,
						--GenFebACarico = isnull(GenFebACarico, 0) +
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--				and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2024_XXX)) < 0
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), dateadd(yy, 3, @birthdate)) >= 0 and @mese <= @meseFebbraio THEN @valore_mese end,
						case WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2024_XXX)) >= 0 /*and @mese >= @meseInizio21anni*/ THEN @valore_mese end,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--		and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2024_XXX)) < 0
			...
			begin
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						--minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), 
						--			dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese 
						--			ELSE 0 
						--		end,
						maggioreDi21Anni = isnull(maggioreDi21Anni,0) +
								case WHEN datediff(m, dateadd(yy, 21, @birthdate), 
									dateadd(m, @mese-1, @1gen2024_XXX)) >= 0 THEN @valore_mese 
									ELSE 0 
								end,
						--GenFebACarico = isnull(GenFebACarico, 0) +
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), dateadd(yy, 3, @birthdate)) < 0 
					...
					insert into #AltroFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
						case WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2024_XXX)) >= 0 THEN @valore_mese end,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2024_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--		and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2024_XXX)) < 0
						--		and @mese <= @meseFebbraio THEN @valore_mese end,
						case @detrazionePerConiugeACarico WHEN '' THEN isnull(@appliancepercentage,1) end,
	...
	BEGIN
		...
		IF (
			...
			OR
			((Year(@start) < @annoredditi) AND (@workingdays = 365) AND (@stop < @31dic2024)  ) 

			OR (Year(@stop) < @annoredditi   ) 
			OR
				(	-- IN FORZA AL 31 12, con data ultimo cedolino dell'anno inferiore 
					(@stopcontract > @31dic2024)  AND (@stop < @31dic2024) 
				) 
			)	
			)
			AND (@workingdays >0)		
		 ...
		 ELSE 
			...
			if (	(@workingdays = 365) AND (@stopcontract > @31dic2024) /*IN FORZA AL 31 12*/ )  SET @DB001011 = NULL --- IN TAL CASO NON COMPILIAMO PERIODI PARTICOLARI
		...
		(
			...
			(	(year(@start)<@annoredditi)
				...
				OR
				(
				   (@stopcontract <= @31dic2024)  AND (@stop < @stopcontract) 
				)
			) 
		)
		OR
			...
			(
				/*(@workingdays_2_semestre) >0 AND*/ (@stop < @01lug2024)
			)
		)	
		...
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, intero) VALUES(@progrCom,1,  'DB001', 1, '741', '5') 
	...
	SET @DB001027= isnull(@ritenuta_addcom_irpef_saldo_2024,0)
	
	IF ((year(@stopcontract) = @annoredditi) AND (@stopcontract <= @31dic2024))
	BEGIN
		SET @DB001024= isnull(@ritenuta_addreg_irpef,0)
		SET @DB001028= isnull(@ritenuta_addcom_irpef_saldo_2024,0)
	END
	...
	BEGIN 
		...
		if @startpayroll < @1gen2024_XXX set @oldstart = @1gen2024_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic2024_XXX set @oldstop =  @31dic2024_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
		
			if @startpayroll < @1gen2024_XXX set @startpayroll = @1gen2024_XXX
			if @stoppayroll > @31dic2024_XXX set @stoppayroll = @31dic2024_XXX
