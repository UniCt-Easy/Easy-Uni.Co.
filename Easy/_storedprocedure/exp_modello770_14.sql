...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_14]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_14]
GO
...
CREATE    PROCEDURE [exp_modello770_14] (
	@recordGH varchar(2)='GH'
	)
AS BEGIN

	declare @statementyear smallint
	set @statementyear = 2013
	...
	EXEC exp_modello770_14_a

	DECLARE @progr_raggiunto int

	--if @recordGH = 'GH'
	--begin
	--	INSERT INTO #modello770 EXEC exp_modello770_14_g '770'
	--end

--	INSERT INTO #modello770 EXEC exp_modello770_14_h
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Il seguente codice è la sp del reord H, ossia "exp_modello770_14_h"
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	declare @SS003001 dec(19,2)
	declare @SS003002 dec(19,2)

	declare @annodichiarazione int
	set @annodichiarazione = 2014

	declare @annoredditi int
	set @annoredditi = 2013

	declare @31dic13 datetime
	set @31dic13 = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})
...
-- Sezione dichiarativa	
	...
	DECLARE @au15foreigncf varchar(25)
	--
	DECLARE @maxexpensephase char(1)
	SELECT  @maxexpensephase = MAX(nphase) FROM expensephase

	CREATE TABLE #expense2013
	(
		idexp int,
		idreg int,
	...
	update #expense2013 set motive770 = motive770service.idmot
		from motive770service 
		where motive770service.idser = #expense2013.idser
		AND motive770service.ayear = @annodichiarazione-1

	declare @cognome varchar(60)
	DECLARE registry_crs CURSOR FOR 
		SELECT DISTINCT isnull(cf,p_iva), #expense2013.idreg, surname FROM #expense2013
		join registry on registry.idreg = #expense2013.idreg
		order by surname
	open registry_crs
	...
	BEGIN
		...
		SELECT
			...
			@au15foreigncf = registry.foreigncf,
		...
		BEGIN
			...
			--sezione di impostazione vecchio comune poiché quello del 770 non è aggiornato, DA RIMUOVERE nel 2011!!
			SELECT
	...
	--9 Identificativo del produttore del software (codice fiscale)
		...
		DECLARE @au18codiceNazione varchar(20)
		...
		SELECT TOP 1 
			...
			@au18codiceNazione = isnull(geo_nation_agency.value, case registryaddress.flagforeign when 'S' then '-1' end),
			--CASE
			--	sezione di impostazione vecchio stato poiché quello del 770 non è aggiornato, DA RIMUOVERE nel 2011!!
			...
			WHERE registryaddress.idreg = @idreg
				AND registryaddress.start <= @31dic13
				AND (registryaddress.stop IS NULL OR registryaddress.stop >= @31dic13)
			ORDER BY
			CASE idaddresskind
				WHEN @stand THEN 1
				WHEN @domfi THEN 2
	...
	--AU001007 Provincia di nascita (sigla)
		...
		DECLARE @au16locationestera varchar(160)
		DECLARE @au17addressestero varchar(160)
		DECLARE @tmpidcity int



		set @au16locationestera = null
		set @au17addressestero = null
		IF (@au18codiceNazione is not null)
		BEGIN
			set @au16locationestera = @au8location
			set @au17addressestero = @au12address
	...
	--AU001009 Provincia (sigla)
		...
		if exists (select * from #expense2013 where idreg=@idreg and motive770='N')
		begin
	...
	--AU001012 Via e numero civico
		INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, stringa) VALUES(@progrCom, 'AU', 1, '012', @au12address)
		IF (@au18codiceNazione is not null)
		BEGIN
		-- I campi da AU001013 ad AU001016 vengono valorizzati qualora si tratti di persone residenti all'estero
	--AU001015 Codice di identificazione fiscale estero
			INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, stringa) VALUES(@progrCom, 'AU', 1, '015', @au15foreigncf)
	--AU001016 Località di residenza estera
			INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, stringa) VALUES(@progrCom, 'AU', 1, '016', @au16locationestera)
	--AU001017 Via e numero civico
			INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, stringa) VALUES(@progrCom, 'AU', 1, '017', @au17addressestero)
	...
	--AU001018 Codice stato estero
			INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, intero) VALUES(@progrCom, 'AU', 1, '018', CONVERT(int,@au18codiceNazione))
		END

		DECLARE @employtaxamount decimal(19,2)
		DECLARE @au23SommeNonSoggetteARitenutaPerRegimeConvenzionale decimal(19,2)
		DECLARE @au25_AltreSommeNonSoggetteARitenuta decimal(19,2)
		DECLARE @au22_ammontarelordocorrisposto decimal(19,2)
		DECLARE @au26imponibile decimal(19,2)
		DECLARE @idexp int
		DECLARE @idser int
		DECLARE @taxcodefiscale int
		DECLARE @au27_28ritIRPEF decimal(19,2)
		DECLARE @au38ritprevidenzialeamm decimal(19,2)
		DECLARE @au39ritprevidenzialedip decimal(19,2)
		DECLARE @au19causale char(1)
		DECLARE cursoreexpense CURSOR FOR 
			SELECT 
				#expense2013.employtax,
				#expense2013.idexp,
				#expense2013.idser,
				#expense2013.motive770
			FROM #expense2013
			join expenselink 
				on expenselink.idchild = #expense2013.idexp
				and nlevel = @expensephase
			left outer join expenseprofservice
				on expenselink.idparent = expenseprofservice.idexp
			WHERE #expense2013.idreg=@idreg 
				and isnull(movkind,0) <> 2

		OPEN cursoreexpense
		FETCH NEXT FROM cursoreexpense
			INTO @employtaxamount, @idexp, @idser, @au19causale
		...
		BEGIN
			...
			SELECT @au27_28ritIRPEF = ISNULL(SUM(expensetaxofficial.employtax),0)
			...
			SELECT  @au39ritprevidenzialedip = isnull(SUM(expensetaxofficial.employtax),0),
				@au38ritprevidenzialeamm = isnull(SUM(expensetaxofficial.admintax),0)
				FROM expensetaxofficial
				JOIN tax
					ON tax.taxcode = expensetaxofficial.taxcode
				WHERE expensetaxofficial.idexp = @idexp
	...
	--AUXXX019 Causale
			INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, stringa) VALUES(@progrCom, 'AU', @contaPrestazioni, '019', @au19causale)

	-- Il campo AU001018 non viene valorizzato in quanto il campo AU001017 vale sempre A o M e mai G o H e, nel contempo il campo 18 non viene valorizzato
...
-- AUX0021 Ammontare lordo corrisposto = Imponibile Lordo SUM (feegross) di quel percipiente.
	set @au22_ammontarelordocorrisposto=0
	...
	-- calcola @au22_ammontarelordocorrisposto
	...
	BEGIN
		...
		EXEC compute_casualcontract @annoredditi,@ycon,@ncon
			...
			SELECT @au22_ammontarelordocorrisposto = 
			...
			BEGIN
				SET @au22_ammontarelordocorrisposto = @imponibilereale
				
			END
			
			set @quota_contratto =  @au22_ammontarelordocorrisposto/@imponibilereale 
	...
	set @au22_ammontarelordocorrisposto= isnull(@au22_ammontarelordocorrisposto,0)
		
	INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimfisc) VALUES(@progrCom, 'AU', @contaPrestazioni, '022', @au22_ammontarelordocorrisposto)

-- AUXXX023 Somme non soggette a ritenuta per regime convenzionale
-- leggere il feegross delle prestazioni che hanno la ritenuta IRPEF STRANIERI con convenzione. Tali prestazioni hanno ritenuta a zero
	SET @au23SommeNonSoggetteARitenutaPerRegimeConvenzionale=0
	SELECT @au23SommeNonSoggetteARitenutaPerRegimeConvenzionale = ISNULL(SUM(taxablenet),0)
		FROM expensetaxofficial 
		JOIN tax 
			ON tax.taxcode = expensetaxofficial.taxcode
		WHERE expensetaxofficial.idexp = @idexp 
	...
	IF (@au23SommeNonSoggetteARitenutaPerRegimeConvenzionale <> 0)
	BEGIN			
		INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimfisc) VALUES(@progrCom, 'AU', @contaPrestazioni, '023', @au23SommeNonSoggetteARitenutaPerRegimeConvenzionale)
	END
	
---AUX0024 = Imponibile Lordo SUM (feegross) - Imponibile Netto (SUM(taxablenet) da expensetax where (taxkind=1 ossia fiscali)
	set @au25_AltreSommeNonSoggetteARitenuta=0
	SET @au25_AltreSommeNonSoggetteARitenuta = 
					@au22_ammontarelordocorrisposto
					...
					- @au23SommeNonSoggetteARitenutaPerRegimeConvenzionale
					
	-- mettiamo 3 se diverso da zero 
	if (@au25_AltreSommeNonSoggetteARitenuta <> 0) 
	BEGIN
		--- AUXXX024 Codice NP Vale sempre 3 
		INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, intero) VALUES(@progrCom, 'AU', @contaPrestazioni, '024', 3)
		--- AUX0025
		INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimfisc) VALUES(@progrCom, 'AU', @contaPrestazioni, '025', @au25_AltreSommeNonSoggetteARitenuta)
	END
		

--	AUXXX023 Imponibile
--	AUX026 = AUX0022 - AUX0023 - AUX0025
	SET @au26imponibile = @au22_ammontarelordocorrisposto - @au23SommeNonSoggetteARitenutaPerRegimeConvenzionale-@au25_AltreSommeNonSoggetteARitenuta

	INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimfisc) VALUES(@progrCom, 'AU', @contaPrestazioni, '026', @au26imponibile)
			...
			BEGIN
				SET @colonna = '028' --AUXXX028 Ritenute a titolo di imposta
				set @SS003002 = isnull(@SS003002, 0) + isnull(@au27_28ritIRPEF, 0)
			END
			ELSE
			BEGIN
				SET @colonna = '027' --AUXXX027 Ritenute a titolo di acconto
				set @SS003001 = isnull(@SS003001, 0) + isnull(@au27_28ritIRPEF, 0)
			END
	--AUXXX027 Ritenute a titolo di acconto
	--AUXXX028 Ritenute a titolo di imposta
			INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimfisc) VALUES(@progrCom, 'AU', @contaPrestazioni, @colonna, @au27_28ritIRPEF)
	--AUXXX038 Contributi previdenziali a carico del soggetto erogante
			IF (@au38ritprevidenzialeamm <> 0)
			BEGIN
				INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimprev)  VALUES(@progrCom, 'AU', @contaPrestazioni, '038', @au38ritprevidenzialeamm)
			END
	--AUXXX039 Contributi previdenziali a carico del percipiente
			IF (@au39ritprevidenzialedip<> 0)
			BEGIN
				INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimprev)  VALUES(@progrCom, 'AU', @contaPrestazioni, '039', @au39ritprevidenzialedip)
			END
		
			declare @au40speseRimborsate dec(19,2)
			set @au40speseRimborsate=0
			IF (@ycon is not null)  -- si tratta di un contratto occasionale
				select  @au40speseRimborsate =  ISNULL( (SELECT P_refund_lastyear  from #annualpayedrefundH),0)

			set  @au40speseRimborsate = @au40speseRimborsate * @quota_contratto
	--AUXXX040 Spese rimborsate
			IF (@au40speseRimborsate <> 0)
			BEGIN	
				INSERT INTO #recHNonArrot (progr, quadro, riga, colonna, decimprev)  VALUES(@progrCom, 'AU', @contaPrestazioni, '040', @au40speseRimborsate)
			END
...
-- J.T.R. Commentate per il 2011, nel 2011 si valutra se riempire il prospetto SS
...
--	INSERT INTO #recordH (progr, quadro, riga, colonna, intero) VALUES(1, 'SS', 3, '002', isnull(@SS003002, 0))
	...
	begin
		...
		set @cursore = cursor for 
			...
			from #recHNonArrot 
			where progr = @progr and quadro='AU' and colonna = '019'
		...
		begin
			insert into #recordH (progr, quadro, riga, colonna, stringa)
				values (@progr, 'AU', @riga, '019', @causale)
...
-- SELECT * FROM #recordh  -- Era la select finale della SP del Record H

--	INSERT INTO #modello770 EXEC exp_modello770_11_h -- E' stata sostituita con:
INSERT INTO #modello770 SELECT * FROM #recordh 
...
EXEC exp_modello770_14_i 
 
DROP TABLE #annualpayedrefundH
DROP TABLE #recordh
DROP TABLE #recHNonArrot
DROP TABLE #expense2013
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Fine esecuzione della sp exp_modello770_14_h
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	DECLARE @numdichquadroi int
	SELECT @numdichquadroi = MAX(intero) FROM #modello770 WHERE quadro='HRI' AND colonna='03'
	set @numdichquadroi = isnull(@numdichquadroi, 0)
	...
	(
	 ...
	 EXEC exp_modello770_14_b @numdichquadrog, @numdichquadroh, @numdichquadroi
...
--	INSERT INTO #modello770 (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRJ', 1, '8', '')
