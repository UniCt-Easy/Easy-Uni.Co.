...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_sitpatrim_gen_dettimmobil]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_sitpatrim_gen_dettimmobil]
GO
...
CREATE PROCEDURE [rpt_sitpatrim_gen_dettimmobil]
	@year int,
...
AS BEGIN
-- setuser 'amm'
-- exec [rpt_sitpatrim_gen_dettimmobil] 2017, null, {D '2017-03-27'}, 'N',  null
...
(
	...
	-- Rivalutazioni Positive e Negative ufficiali  (di BENI E DI ACCESSORI) sino all'anno scorso purchï¿½ RISULTINO SCARICATI IN QUEST'ANNO
...
BEGIN
	SELECT 
		...
		CLASS.codeinv as 'Codice della classificazione inventariale',
		CLASS.nlevel as 'Livello della classificazione',
		--#patrimonio.idmot,
		MOTIVE.description as 'Causale',
		CLASS.description as 'Descrizione della classificazione inventariale',
		initial_amount as 'Costo storico di acquisto dei cespiti al 31/12/(XX-1)',
		var_aum ,
		var_dim ,
...
select distinct 
	...
	SELECT 
		...
		null as 'Causale',
		CLASS.description as 'Descrizione della classificazione inventariale',
		ISNULL(SUM(initial_amount),0) as 'Costo storico di acquisto dei cespiti al 31/12/(XX-1) (a)',
		ISNULL(SUM(var_aum),0) as 'Variazioni in aumento (b)',
		ISNULL(SUM(var_dim),0) as 'Variazioni in diminuzione (c)',
		ISNULL(SUM(final_amount),0) as 'Costo storico di acquisto dei cespiti in carico al 31/12/XX (d)',

		ISNULL(SUM(ammortization_pre_in_carico),0) as ['Ammortamenti al 31/12/(XX-1) di beni non scaricati nell'anno XX (e1)'],
		ISNULL(SUM(ammortization_pre_scaricati),0) as ['Ammortamenti al 31/12/(XX-1) di beni Scaricati nell'anno XX (e2)'],

		ISNULL(SUM(initial_amount),0) - 
		ISNULL(SUM(ammortization_pre_in_carico),0) - 
		ISNULL(SUM(ammortization_pre_scaricati),0) as 'Consistenza Iniziale f = a - e1 - e2',
				  
		ISNULL(SUM(ammortization_post),0) as ['Ammortamenti dell'anno XX (g)'],
