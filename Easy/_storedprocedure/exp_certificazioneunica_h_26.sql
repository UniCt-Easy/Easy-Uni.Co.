...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_h_26]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_h_26]
GO
 ...
 */
 --select @newProg
CREATE PROCEDURE [exp_certificazioneunica_h_26]
...
--cio con flagexcludefromcertificate = S
delete from #expense2025 where idexp in( 
SELECT #expense2025.idexp FROM #expense2025
join expenselink el on el.idchild = #expense2025.idexp 
...
where (ISNULL(p.flagexcludefromcertificate,'N') = 'S')) and @maxexpensephase = 1
	...
	update #expense2025 set motive770 = motive770service.idmot,
		...
		where motive770service.idser = #expense2025.idser
		...
		--SELECT 
		--	#expense2025.employtaxamount,
		--	#expense2025.idexp,
		--	#expense2025.idser,
		--	#expense2025.motive770,
		--	#expense2025.socialseccode,
		--	#expense2025.cfagency,
		--	#expense2025.titleagency 
		--FROM #expense2025
		--join expenselink  			on expenselink.idchild = #expense2025.idexp			and nlevel = @expensephase
...
---AUX0025 = Imponibile Lordo SUM (feegross) - Imponibile Netto (SUM(taxablenet) da expensetax where (taxkind=1 ossia fiscali)
	...
	--select 'au04', @au04_ammontarelordocorrisposto
	--select 'imponibili netti fiscali', SUM(taxablenet) --somma imponibili netti ove la rit fiscale non  zero esclusi stranieri conv.
	--						FROM expensetaxofficial
	--			              		join tax
	--							ON tax.taxcode = expensetaxofficial.taxcode
	--						where expensetaxofficial.idexp = @idexp
	...
	-- NEL 2025 DISTINGUIAMO I SEGUENTI CASI CHE L'ANNO SCORSO NON DISTINGUEVAMO (task 15738)
	-- A) Per i professionisti che hanno effettuato solo prestazioni senza ritenuta, mettiamo codice 21
	-- ad eccezione del caso in cui sulla parcella sia stata utilizzata un'aliquota iva 
	-- classificata con classivakind =  014
	-- (CertUnica12 regime forfettario). In quest'ultimo caso andr messo il codice 24.
	...
	if (@au07_AltreSommeNonSoggetteARitenuta <> 0 OR isnull(@speseRimborsateProf_nonimp, 0) <> 0 ) 
	BEGIN
	---VALE PER OCCASIONALI E PROFESSIONALI
	...
	--23 - nel caso di Assegni di servizio civile universale di cui allart. 16 del D.lgs. n. 40 del 2017 per i quali la norma ha
	--previso la loro esenzione;
	--24 - nel caso di compensi, non assoggettati a ritenuta dacconto, corrisposti ai soggetti in regime forfetario di cui allarticolo 1, della L. 190/2014.
		SET @notsubjecttaxcode = 22   
		-----------------------------------------------------------------
		--------------------  PROFESSIONALI -----------------------------
		-----------------------------------------------------------------
			--abbiamo dovuto utilizzare, per i contratti professionali,
			...
			-- allo scopo di distinguere, per i professionisti, il tipo di regime 
			-- in cui rientrano le somme non soggette a ritenuta (dei minimi / forfettario)
			-- pure se sono presenti spese non imponibili non devono essere esposte nella casella 07
		IF  ISNULL(@sortcodeivakind,'') = '013' 
		...
		if ISNULL(@sortcodeivakind,'') in ('013', '014')
		begin
		-- con regime forfettario o dei minimi se ci sono spese non imponibili sono state inserite per errore nella parcella, quindi si decide di azzerarle, 
		-- decurtiamo le altre somme non soggette a ritenuta dello stesso importo
			set @speseRimborsateProf_nonimp = 0
		end
		
		if isnull(@speseRimborsateProf_nonimp, 0) <> 0 and ISNULL(@sortcodeivakind,'') not in ('013', '014')
		begin
			set @notsubjecttaxcode = 22
				---- ASSUMIAMO CHE le spese non imponibili non sono incluse nell'ammontare lordo
				set @au07_AltreSommeNonSoggetteARitenuta = @au07_AltreSommeNonSoggetteARitenuta + @speseRimborsateProf_nonimp
				---In questo caso le spese non imponibili vanno sommate all'ammontare lordo corrisposto
				INSERT INTO  #recHNonArrot(progr,  modulo, quadro, riga, colonna, decimfisc,causale,socialseccode,idexp) 
				VALUES(@progrCom,@contaPrestazioni,'AU',1 , '004', @speseRimborsateProf_nonimp,@au01_causale,@au33_socialseccode,@idexp)
		end

		SET @au06_CodiceAltreSommeNonSoggetteARitenuta = @notsubjecttaxcode

		--- AU001006 Codice NP Vale   21 oppure 24 
		...
		UPDATE  #expense2025 set notsubjecttaxcode = @notsubjecttaxcode  
		WHERE #expense2025.idexp= @idexp
	END
	
	--select 'au07_AltreSommeNonSoggetteARitenuta', @au07_AltreSommeNonSoggetteARitenuta
	--select * from #recHNonArrot
	SET @au08_imponibile = isnull(@au04_ammontarelordocorrisposto,0) + isnull(@speseRimborsateProf_nonimp,0)- isnull(@au05_SommeNonSoggetteARitenutaPerRegimeConvenzionale,0)-isnull(@au07_AltreSommeNonSoggetteARitenuta,0)
	--SELECT '@au08_imponibile', @au08_imponibile
	...
	--CAMPO 29 (Codice fiscale Ente Previdenziale) CAMPO NUOVO indicare il codice fiscale dellEnte previdenziale. Pertanto deve essere inserito un valore costante pari a "80078750587 quando la certificazione prevede l'applicazione delle ritenute previdenziali.
	...
	--CAMPO 31 Ente Previdenziale CAMPO NUOVO (lunica casistica che verr gestita,  la lettera A (ALTRO) quando la certificazione prevede l'applicazione delle ritenute previdenziali.
	--CAMPO 32 (Codice Azienda)- NON VALORIZZARLO
	--CAMPO 33 (Categoria) - NON VALORIZZARLO

	--CAMPO 34 (Contributi previdenziali a carico del soggetto erogante)   il vecchio CAMPO 20 del precedente modello fiscale (CU 2017)
	--CAMPO 35 (Contributi previdenziali a carico del percipiente)  il vecchio CAMPO 21 del precedente modello fiscale (CU 2017)
	...
	BEGIN
		----- la causale M2, indica le prestazioni ENPAPI che  un istituto previdenziale diverso da INPS
	...
	--								) 
		...
		--- allineo tutti i dati inseriti per il pagamento in oggetto  sull'eventuale codice @notsubjecttaxcode per poter paginare in pi moduli
...
-- laddove esso non sia valorizzato per poter paginare nello stesso modulo
	...
	UPDATE #expense2025 SET notsubjecttaxcode = ( select top 1 A.notsubjecttaxcode 
	FROM #recHNonArrot A WHERE 
	A.idexp =  #expense2025.idexp AND isnull(A.notsubjecttaxcode,'') <> 0)
	WHERE isnull(#expense2025.notsubjecttaxcode,'') = 0
		...
		begin
			...
			ELSE
				BEGIN  --- creo una nuova certificazione
					...
					insert into #recordH (progr,modulo, quadro, riga, colonna, stringa,decimale, data,intero)
					exec exp_certificazioneunica_d_26  @idreg, @newprogrCom, 'H',NULL, @print
				END
			...
			-- Se ci sono pi prestazioni che differiscono per il valore del Campo "Codice", ossia AU 006
			-- In questi casi, deve mostrare in "Ammontare lordo corrisposto" = AU 004, la somma degli ammontari 
			-- e restuire due (o pi ) moduli nei quali valorizzare i diversi "Codice" 6
			insert into #recordH (progr, modulo, quadro, riga, colonna, decimale)
				select @newprogrCom,@modulo, #recHNonArrot.quadro, 1, #recHNonArrot.colonna, sum(#recHNonArrot.decimfisc) 
				from #recHNonArrot 
				where isnull(#recHNonArrot.causale,'') = isnull(@causale,'')
...
insert into #recordH (progr,modulo, quadro, riga, colonna, stringa,decimale, intero, data)
			exec exp_certificazioneunica_h_pign_26  @idreg, @newprogrCom, @modulo, @fattoprestazioni,  @inseritopignoramenti out, @newmodulo out
