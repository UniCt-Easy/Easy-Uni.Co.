...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_d_22]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_d_22]
GO

-- exec exp_certificazioneunica_d_22  19335 , 1, 'G', {d '2020-12-31'},'S'
CREATE PROCEDURE [exp_certificazioneunica_d_22]
...
DECLARE @dateindi_01_gen_22 datetime

SET @dateindi_01_gen_21 = DATEADD(yy, @annoredditi - 2000, {d '2000-01-01'})
SET @dateindi_01_gen_22 = DATEADD(yy, @annodichiarazione - 2000, {d '2000-01-01'})
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= @dateindi_01_gen_21))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
		AND (cdi.start <= @dateindi_01_gen_21)
        AND ((cdi.stop IS NULL AND  cdi.active <>'N') OR (cdi.stop >= @dateindi_01_gen_21))
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= ISNULL(@datarif,@dateindi_01_gen_22)))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
        AND (cdi.start <= ISNULL(@datarif,@dateindi_01_gen_22))
        AND ((cdi.stop IS NULL AND cdi.active <>'N') OR (cdi.stop >= ISNULL(@datarif,@dateindi_01_gen_22)))
        AND cdi.idreg = registryaddress.idreg)
        AND idreg = @idreg
