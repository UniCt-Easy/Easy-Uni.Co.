...
if exists (select * from dbo.sysobjects where id = object_id(N'[trg_ins_epaccamount]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trg_ins_epaccamount]
GO



CREATE    PROCEDURE [trg_ins_epaccamount]
(
	@ayear int,	
	@idmov int,
	@paridmov int,
...
FROM epacctotal
JOIN epacc				ON epacctotal.idepacc = epacc.idepacc
WHERE epacc.paridepacc = @idmov
			AND epacctotal.ayear = @ayear
...
SET 	@available5 = isnull(@amount5,0) - isnull(@sumofson5,0)
	...
	INSERT INTO epacctotal (idepacc,ayear,curramount,curramount2,curramount3,curramount4,curramount5,
