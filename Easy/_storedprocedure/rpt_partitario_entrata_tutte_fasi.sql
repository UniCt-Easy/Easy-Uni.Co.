...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE rpt_partitario_entrata_tutte_fasi
	@ayear int,
	@kind char(1),
	@idupb	varchar(36),
	@nlevel tinyint,
...
-- Movimenti di entrata / fase bilancio
	INSERT INTO #income(
		...
		left outer JOIN finlink FL1			ON FL1.idchild = incomeyear.idfin	
												AND FL1.nlevel = @nlevel
		JOIN fin							ON fin.idfin = isnull(FL1.idparent, incomeyear.idfin)
...
-- Variazione movimenti di entrata / fase bilancio
	...
	JOIN incomeyear				ON incomeyear.idinc = incomevar.idinc
	JOIN incometotal			ON incometotal.idinc = incomeyear.idinc
										AND incometotal.ayear = incomeyear.ayear
	JOIN upb					ON incomeyear.idupb = upb.idupb	
	left outer JOIN finlink		ON finlink.idchild = incomeyear.idfin	
										AND finlink.nlevel = @level_input
	left outer JOIN finlink FL1	ON FL1.idchild = incomeyear.idfin	
										AND FL1.nlevel = @nlevel
	JOIN fin					ON fin.idfin = isnull(FL1.idparent, incomeyear.idfin)
...
left outer JOIN finlink			ON finlink.idchild = HPV.idfin	
									AND finlink.nlevel = @level_input
left outer JOIN finlink FL1		ON FL1.idchild = HPV.idfin	
									AND FL1.nlevel = @nlevel
JOIN fin
...
BEGIN
	...
	SELECT 
		...
		incomevar.amount
		FROM incomevar
		JOIN historyproceedsview HPV		ON HPV.idinc = incomevar.idinc
		JOIN upb							ON HPV.idupb = upb.idupb	
		JOIN income							ON income.idinc = incomevar.idinc
		left outer JOIN finlink				ON finlink.idchild = HPV.idfin	
												AND finlink.nlevel = @level_input
		left outer JOIN finlink FL1			ON FL1.idchild = HPV.idfin	
												AND FL1.nlevel = @nlevel
		JOIN fin							ON fin.idfin = isnull(FL1.idparent,HPV.idfin)
		WHERE incomevar.yvar = @ayear
			AND incomevar.adate BETWEEN @start AND @stop
			AND isnull(incomevar.autokind,'') <>'22'
			AND HPV.idupb like @idupb 
...
FROM income
JOIN incomelink EL1				ON EL1.idchild = income.idinc  AND EL1.nlevel = 1
LEFT OUTER JOIN incomelink EL2	ON EL2.idchild = income.idinc  AND EL2.nlevel = 2
LEFT OUTER JOIN incomelink EL3	ON EL3.idchild = income.idinc  AND EL3.nlevel = 3
LEFT OUTER JOIN incomelink EL4	ON EL4.idchild = income.idinc  AND EL4.nlevel = 4
LEFT OUTER JOIN income E1		ON E1.idinc = EL1.idparent
LEFT OUTER JOIN income E2		ON E2.idinc = EL2.idparent
LEFT OUTER JOIN income E4		ON E4.idinc = EL4.idparent
LEFT OUTER JOIN income E3		ON E3.idinc = EL3.idparent
