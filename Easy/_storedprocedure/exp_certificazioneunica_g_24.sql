...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_24]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_24]
GO
 
CREATE PROCEDURE [exp_certificazioneunica_g_24]
...
-- exec exp_certificazioneunica_g_24  47673,1,'S'  --55750

AS BEGIN 
	declare @annodichiarazione int
	set @annodichiarazione = 2024

	declare @annoredditi int
	set @annoredditi = 2023

	declare @31dic2023 datetime
	set @31dic2023 = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_24  @idreg, @progrCom, 'G',NULL, @print
	...
	declare @1gen2023_XXX datetime
	set @1gen2023_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-01'})

	declare @13gen2023_XXX datetime
	set @13gen2023_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-13'})

	declare @31dic2023_XXX datetime
	set @31dic2023_XXX = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})

	declare @1gen2024_XXX datetime
	set @1gen2024_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-01'})

	declare @12gen2024_XXX datetime
	set @12gen2024_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-12'})
	...
	declare @30giu2023 datetime
	set @30giu2023 = dateadd(yy, @annoredditi-2000, {d '2000-06-30'})
	
	declare @01lug2023 datetime
	set @01lug2023 = dateadd(yy, @annoredditi-2000, {d '2000-07-01'})
	...
	JOIN 	payrolltax cr on cr.idpayroll= P.idpayroll
	JOIN	tax ON cr.taxcode = tax.taxcode  AND taxkind = 1	AND geoappliance IS NULL
	WHERE #contratti.padre IS NULL AND tax.taxref = '14_BONUS_FISCALE' -- Non dovrebbe essere presente, in quando la sua applicazione è cessata.
	...
	SET @ritenuta_addreg_irpef =
		...
		DECLARE @ritenuta_addcom_irpef_acconto_2023 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_acconto_2023 =
		ISNULL(
		...
		SET @ritenuta_addcom_irpef_acconto_2023 =
		@ritenuta_addcom_irpef_acconto_2023 +
		ISNULL(
		...
		-- Non si filtra sulla certificazione in quanto le prestazioni che non rientrano nel CUD
		-- non pagano le addizionali regionali e quindi il loro contributo è nullo
		DECLARE @ritenuta_addcom_irpef_saldo_2023 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_saldo_2023 =
		ISNULL(
			(SELECT SUM(employtax)
			FROM #cedolini
			JOIN expenselink
	...
	SET @ritenuta_addcom_irpef_saldo_2023 =
	@ritenuta_addcom_irpef_saldo_2023 +
	ISNULL(
	...
	SELECT @ritenuta_addcom_irpef_saldo_2023  = @ritenuta_addcom_irpef_saldo_2023 - @ritenuta_addcom_irpef_acconto_2023  
	...
	if @ritenuta_addcom_irpef_saldo_2023 < 0
	begin
		set @ritenuta_addcom_irpef_acconto_2023 = @ritenuta_addcom_irpef_acconto_2023 + @ritenuta_addcom_irpef_saldo_2023
		set @ritenuta_addcom_irpef_saldo_2023 = null
	end
	else
	begin
		if @ritenuta_addcom_irpef_acconto_2023 < 0
		begin
			set @ritenuta_addcom_irpef_saldo_2023 = @ritenuta_addcom_irpef_saldo_2023 + @ritenuta_addcom_irpef_acconto_2023
			set @ritenuta_addcom_irpef_acconto_2023 = null
		end
	end
...
------------------------------------------------------------------------------------------------------ FINE CAF ---------------------------------------------------------------------------------------------------
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen2023_XXX),
		ISNULL(F.stopapplication, @31dic2023_XXX),
		ISNULL(F.starthandicap, @1gen2024_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		begin
			...
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen2023_XXX))
			...
			begin
				...
				begin
					update #PrimoFiglio set
						...
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), dateadd(yy, 3, @birthdate)) >= 0 and @mese <= @meseFebbraio THEN @valore_mese ELSE 0 end,
						maggioreDi21Anni = isnull(maggioreDi21Anni, 0) +
								case WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2023_XXX)) >= 0 /*and @mese >= @meseInizio21anni*/ THEN @valore_mese ELSE 0 end,
						--GenFebACarico = isnull(GenFebACarico, 0) +
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--				and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2023_XXX)) < 0
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), dateadd(yy, 3, @birthdate)) >= 0 and @mese <= @meseFebbraio THEN @valore_mese end,
						case WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2023_XXX)) >= 0 /*and @mese >= @meseInizio21anni*/ THEN @valore_mese end,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--		and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2023_XXX)) < 0
			...
			begin
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						--minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), 
						--			dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese 
						--			ELSE 0 
						--		end,
						maggioreDi21Anni = isnull(maggioreDi21Anni,0) +
								case WHEN datediff(m, dateadd(yy, 21, @birthdate), 
									dateadd(m, @mese-1, @1gen2023_XXX)) >= 0 THEN @valore_mese 
									ELSE 0 
								end,
						--GenFebACarico = isnull(GenFebACarico, 0) +
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), dateadd(yy, 3, @birthdate)) < 0 
					...
					insert into #AltroFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
						case WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2023_XXX)) >= 0 THEN @valore_mese end,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2023_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--		and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2023_XXX)) < 0
						--		and @mese <= @meseFebbraio THEN @valore_mese end,
						case @detrazionePerConiugeACarico WHEN '' THEN isnull(@appliancepercentage,1) end,
	...
	--DECLARE	@DB001013 INT			-- Giorni I semestre-- Dalla CU 2022 non serve più
	--DECLARE	@DB001014 INT			-- Giorni II semestre-- Dalla CU 2022 non serve più
	...
	BEGIN
		...
		SET @DB001009= @stop  -- Data fine Rapporto di Lavoro Non deve essere valorizzato in caso di continuazione del rapporto di lavoro oltre il 31/12/2021 ricavabile dalla data fine del contratto inserito. N.B.: il campo 9 è alternativo al campo 10.
		...
		IF (
		    (
			-- Rimuovo perchè con questa condizione valorizza periodi particolari a 4 e manda in errore la CU. 
			...
			OR
			((Year(@start) < @annoredditi) AND (@workingdays = 365) AND (@stop < @31dic2023)  ) 

			OR (Year(@stop) < @annoredditi   ) 
			OR
				(	-- IN FORZA AL 31 12, con data ultimo cedolino dell'anno inferiore 
					(@stopcontract > @31dic2023)  AND (@stop < @31dic2023) 
				) 
			)	
			)
			AND (@workingdays >0)		
		 ...
		 ELSE 
			...
			--di Lavoro non è possibile compilare il campo Periodi Particolari [col.11]
			
			if (	(@workingdays = 365) AND (@stopcontract > @31dic2023) /*IN FORZA AL 31 12*/ )  SET @DB001011 = NULL --- IN TAL CASO NON COMPILIAMO PERIODI PARTICOLARI

			-- Solo per questo CF di Unina2_easy, per il quale la sp imposta @DB001011 = 4 e la CU va in errore, impostiamo il valore null. Vedi task n. 18245
			if (@au1cf='CPPLSN79D55D708T')
			Begin
				set @DB001011 = NULL
			End
		...
		(
			...
			(	(year(@start)<@annoredditi)
				...
				OR
				(
				   (@stopcontract <= @31dic2023)  AND (@stop < @stopcontract) 
				)
			) 
		)
		OR
			--- la cessazione del rapporto di lavoro prima del 1 luglio 2021 non è compatibile con la 
			--- compilazione giorni lavorati del secondo semestre
			(
				/*(@workingdays_2_semestre) >0 AND*/ (@stop < @01lug2023)
			)
		)	
	...
	SET @DB001027= isnull(@ritenuta_addcom_irpef_saldo_2023,0)
	
	IF ((year(@stopcontract) = @annoredditi) AND (@stopcontract <= @31dic2023))
	BEGIN
		SET @DB001024= isnull(@ritenuta_addreg_irpef,0)
		SET @DB001028= isnull(@ritenuta_addcom_irpef_saldo_2023,0)
	END
	...
	BEGIN
		...
		END
		 ...
		 --Per la compilazione del punto 400 è necessario utilizzare uno dei seguenti codici:
		 --			1. se il sostituto d’imposta ha riconosciuto al dipendente il trattamento integrativo e lo ha erogato tutto o in parte;
		 --			2. se il sostituto d’imposta non ha riconosciuto al dipendente il trattamento integrativo ovvero lo ha riconosciuto, ma non lo ha erogato
	...
	DECLARE @DB001545 DECIMAL(19,2)	    -- Addizionale comunale Acconto 2021
	DECLARE @DB001546 DECIMAL(19,2)	    -- Addizionale comunale Saldo 2021
	...
	BEGIN
		...
		DECLARE @DC001049	INT				-- Mesi per i quali è stata presentata la denuncia UniEmens - Tutti CB
	...
	BEGIN
		...
		DECLARE @DC001037	INT				-- Mesi per i quali è stata presentata la denuncia UniEmens - Tutti CB
		DECLARE @DC001038	VARCHAR(12)		-- Mesi per i quasi è stata presentata la denuncia UniEmens - Tutti con esclusione di
		...
		SET @DC001037 = isnull(@emensTuttiIMesidipendentipubblici,0)  -- Mesi per i quali è stata presentata la denuncia UniEmens - Tutti CB
		SET @DC001038 = @mesiConEmensdipendentipubblici			  -- Mesi per i quasi è stata presentata la denuncia UniEmens - non è più quelli esclusi, ma i mesi inclusi
	...
	BEGIN 
		...
		if @startpayroll < @1gen2023_XXX set @oldstart = @1gen2023_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic2023_XXX set @oldstop =  @31dic2023_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
		
			if @startpayroll < @1gen2023_XXX set @startpayroll = @1gen2023_XXX
			if @stoppayroll > @31dic2023_XXX set @stoppayroll = @31dic2023_XXX
