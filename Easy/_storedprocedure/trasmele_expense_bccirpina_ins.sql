...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_bccirpina_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_bccirpina_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_bccirpina_ins]
(
	@y int,
	@n int
)
...
--------------- BANCA CREDITO COOPERATIVO IPRINA -------------
--------------------------------------------------------------
...
DECLARE @ABI_codebanca varchar(5)
SET @ABI_codebanca = '08661'  -- 08661  ABI BANCA DI CREDITO COOPERATIVO IRPINA
...
SELECT
	...
	CASE
		...
		ELSE ''
	END, -- informazione destinazione (LIBERA/VINCOLATA) obbligatoria se l'Ente è in regime TU, altrimenti NON deve essere specificato
	CASE
...
AND (p.idpaymethodTRS <> '13' AND -- i mandati stipendi ad anagrafiche cumulative non devono essere associati a ritenute
...
SET @codice_ABI_BT = @ABI_codebanca
---------------------------------------------
----- fine testata flusso -------------------
---------------------------------------------
-- Tabella di output
...
SELECT 
				   ...
				   CASE   
					   ...
					   ELSE null
  				   END,
  				   null , -- TRATTAMENTO SPESE APPLICHIAMO LA CONVENZIONE DI TESORERIA
  				   null,
  				   -- PIAZZATURA --
  				   CASE 
