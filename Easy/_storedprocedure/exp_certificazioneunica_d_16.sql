...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_d_16]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_d_16]
GO
 
-- exec exp_certificazioneunica_d_16  3, 1, 'H', {d '2016-12-31'},'S'
CREATE PROCEDURE [exp_certificazioneunica_d_16]
...
-- Sezione dichiarativa	
	...
	--2016	DA001	001	Codice fiscale	CF
	--2016	DA001	002	Cognome o denominazione	AN
	--2016	DA001	003	Nome	AN
	--2016	DA001	004	Comune di Residenza	AN
	--2016	DA001	005	Provincia di Residenza (sigla)	PR
	--2016	DA001	006	CAP	AN
	--2016	DA001	007	Via e numero civico	AN
	--2016	DA001	008	Telefono/Fax	AN
	--2016	DA001	009	Indirizzo di posta elettronica del sostituto	AN
	--2016	DA001	010	Codice Attività	AN
	--2016	DA001	011	Codice Sede	AN (solo per chi ha più sedi)
	...
	DECLARE @DA002023 VARCHAR(200)
	DECLARE @DA002024 VARCHAR(2)
	DECLARE @DA002025 VARCHAR(50)
...
SET @dateindi_01_gen_16 = DATEADD(yy, @annodichiarazione - 2000, {d '2000-01-01'})
...
DECLARE @IND005 VARCHAR(100) -- Riga 5 EMAIL
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= ISNULL(@datarif,@dateindi_01_gen_16)))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
        AND (cdi.start <= ISNULL(@datarif,@dateindi_01_gen_16))
        AND ((cdi.stop IS NULL AND cdi.active <>'N') OR (cdi.stop >= ISNULL(@datarif,@dateindi_01_gen_16)))
        AND cdi.idreg = registryaddress.idreg)
        AND idreg = @idreg
...
SELECT @DA002023 = location FROM #address WHERE nation = 'ITALIA'
SELECT @DA002024 = province FROM #address WHERE nation = 'ITALIA'
SELECT @DA002025 = codice_catastale FROM #address WHERE nation = 'ITALIA'
...
--select * from #address
	--2016	DA002	001	Codice fiscale	CF
	--2016	DA002	002	Cognome	AN
	--2016	DA002	003	Nome	AN
	--2016	DA002	004	Sesso	AN
	--2016	DA002	005	Data di nascita	DT
	--2016	DA002	006	Comune (o Stato estero) di nascita	AN
	--2016	DA002	007	Provincia di nascita (sigla)	PN
	--2016	DA002	008	Categorie particolari	AN -- non compiliamo
	--2016	DA002	009	Eventi eccezionali	N1 -- non compiliamo 
	--2016	DA002	010	Casi di esclusione dalla  precompilata	N1 -- non compiliamo
	--2016	DA002	020	Comune	AN  (Domicilio fiscale al 1/1/2015)
	--2016	DA002	021	Provincia (sigla)	PR  (Domicilio fiscale al 1/1/2015)
	--2016	DA002	022	Codice comune	AN (Domicilio fiscale al 1/1/2015) 
	--2016	DA002	023	Comune	AN   (Domicilio fiscale al 1/1/2016)
	--2016	DA002	024	Provincia (sigla)	PR  (Domicilio fiscale al 1/1/2016)
	--2016	DA002	025	Codice comune	PR  (Domicilio fiscale al 1/1/2016)
	--2016	DA002	030	Dati relativi al rappresentante	CF  -- minori, incapaci ecc.. (non compiliamo)
	--2016	DA002	040	Codice di identificazione fiscale estero	AN
	--2016	DA002	041	Località di residenza estera	AN
	--2016	DA002	042	Via e numero civico	AN
	--2016	DA002	043	Non Residenti Shumacker CB
	--2016	DA002	044	Codice stato estero	N3
...
IF (@tipoRecord <> 'H')
	BEGIN
		...
		-- (Domicilio fiscale al 1/1/2016)
		INSERT INTO #recordd (progr,modulo, quadro, riga, colonna, stringa) VALUES(@progrCert,@progrModulo, 'DA002', 1, '023', @DA002023)
