...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_17]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_17]
GO
--setuser 'amm'
--setuser 'amministrazione' 
--exec exp_certificazioneunica_g_17 14155,1,'N'
CREATE PROCEDURE [exp_certificazioneunica_g_17]
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_17  @idreg, @progrCom, 'G',NULL, @print
	...
	declare @12gen17_XXX datetime
	set @12gen17_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-12'})
	...
	SET @ritenuta_addreg_irpef =
		...
		DECLARE @ritenuta_addcom_irpef_acconto_2016 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_acconto_2016 =
		ISNULL(
		...
		SET @ritenuta_addcom_irpef_acconto_2016 =
		@ritenuta_addcom_irpef_acconto_2016 +
		ISNULL(
		...
		-- Non si filtra sulla certificazione in quanto le prestazioni che non rientrano nel CUD
		-- non pagano le addizionali regionali e quindi il loro contributo è nullo
		DECLARE @ritenuta_addcom_irpef_saldo_2016 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_saldo_2016 =
		ISNULL(
			(SELECT SUM(employtax)
			FROM #cedolini
			JOIN expenselink
	...
	SET @ritenuta_addcom_irpef_saldo_2016 =
	@ritenuta_addcom_irpef_saldo_2016 +
	ISNULL(
	...
	SELECT @ritenuta_addcom_irpef_saldo_2016  = @ritenuta_addcom_irpef_saldo_2016 - @ritenuta_addcom_irpef_acconto_2016   
	...
	if @ritenuta_addcom_irpef_saldo_2016 < 0
	begin
		set @ritenuta_addcom_irpef_acconto_2016 = @ritenuta_addcom_irpef_acconto_2016 + @ritenuta_addcom_irpef_saldo_2016
		set @ritenuta_addcom_irpef_saldo_2016 = null
	end
	else
	begin
		if @ritenuta_addcom_irpef_acconto_2016 < 0
		begin
			set @ritenuta_addcom_irpef_saldo_2016 = @ritenuta_addcom_irpef_saldo_2016 + @ritenuta_addcom_irpef_acconto_2016
			set @ritenuta_addcom_irpef_acconto_2016 = null
		end
	end
...
--setuser 'amm'
	...
	SELECT DISTINCT #contratti.idser, ISNULL(service.servicecode770,service.codeser),service.description,
	...
	SET @periodiretributivisoggettodenuncia = REPLACE(REPLACE(@mesiSenzaEmensdipendentipubblici,'0','X'), '1', 'Z')
	
	SET @periodiretributivisoggettodenuncia = REPLACE(REPLACE(@mesiSenzaEmensdipendentipubblici,'X','1'), 'Z', '0')
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen16_XXX),
		ISNULL(F.stopapplication, @31dic16_XXX),
		ISNULL(F.starthandicap, @1gen17_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		begin
			...
			if @figlimaggiori = 0 and isnull(@appliancepercentage,1) = 1 and @idaffinity = 'F' 
					and @flagdependent='S'
					and not exists (SELECT * from parasubcontractfamily where idcon = @idcontract and idaffinity='C') 
			begin
				set @detrazionePerConiugeACarico = 'C'
			end

			declare @diffhand int
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen16_XXX))
				...
				begin
					update #PrimoFiglio set
						...
						minoreDiTreAnni = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen16_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese ELSE 0 end,
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						case WHEN datediff(m, dateadd(m, @mese-1, @1gen16_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
			...
			begin
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen16_XXX), 
	...
	BEGIN
		...
		SET @DB001009= @stop  -- Data fine Rapporto di Lavoro Non deve essere valorizzato in caso di continuazione del rapporto di lavoro oltre il 31/12/2016 ricavabile dalla data fine del contratto inserito. N.B.: il campo 9 è alternativo al campo 10.
		
		-- "In forza al 31/12"
		IF (@stopcontract > @31dic16)
			BEGIN
			SET @DB001010	= 1  
			END
		ELSE 
		...
		IF (
			...
			OR
			((Year(@start) < @annoredditi) AND (@workingdays = 365) AND (@stop < @31dic16)  ) 

			OR (Year(@stop) < @annoredditi) 
			OR
				(	-- IN FORZA AL 31 12, con data ultimo cedolino dell'anno inferiore 
					(@stopcontract > @31dic16)  AND (@stop < @31dic16) 
				) 
			)					
			SET @DB001011 = 4  
		 ELSE 
			BEGIN
				IF (@workingdays < 365) AND (  (@workingdays <> (datediff(d,@start, @stop) + 1)) )					
		...
		(	(year(@start)<@annoredditi)
			...
			OR
			(
			   (@stopcontract <= @31dic16)  AND (@stop < @stopcontract) 
			)
		) 
	...
	DECLARE	@DB001027 DECIMAL(19,2)	-- Addizionale comunale all'Irpef - Saldo 2016
	-- Si intendono i rapporti cessati entro il 31 12 dell'anno redditi
	DECLARE @DB001024 DECIMAL(19,2)	--  DB001024 Addizionale regionale 2016 rapporti cessati
	DECLARE @DB001028 DECIMAL(19,2)	--  DB001028 Addizionale comunale all'Irpef - Rapporti cessati 2016
	
	SET @DB001021= isnull(@ritenuta_irpef,0)
	SET @DB001022= isnull(@ritenuta_addreg_irpef,0)
	SET @DB001027= isnull(@ritenuta_addcom_irpef_saldo_2016,0)
	
	IF ((year(@stopcontract) = @annoredditi) AND (@stopcontract <= @31dic16))
	BEGIN
		SET @DB001024= isnull(@ritenuta_addreg_irpef,0)
		SET @DB001028= isnull(@ritenuta_addcom_irpef_saldo_2016,0)
	END
	...
	DECLARE @DB001468 int	-- Redditi Esenti - Codice  -- prevedere una tabella di configurazione delle prestazioni?
	DECLARE @DB001469 decimal(19,2)	-- Redditi Esenti - Ammontare
	...
	BEGIN 
		...
		if( isnull(@reddito_esente,0) - isnull(@rimborso,0) >=0)
			Begin
				set @reddito_esente = isnull(@reddito_esente,0) - isnull(@rimborso,0)
			End

		SET @reddito_esente  = isnull(@reddito_esente,0)
 
		SET @DB001468 = @exemptioncode					 
		SET @DB001469 = @reddito_esente					 
		if(@DB001469 <>0)
		Begin
			INSERT INTO #recordG (progr, modulo,quadro, riga, colonna, intero)   VALUES(@progrCom,1,'DB001', @counter, '468', @DB001468)
			INSERT INTO #recordG (progr, modulo,quadro, riga, colonna, decimale) VALUES(@progrCom,1,'DB001', @counter, '469', @DB001469)
		End	
	...
	DECLARE @DB001545 DECIMAL(19,2)	    -- Addizionale comunale Acconto 2016
	DECLARE @DB001546 DECIMAL(19,2)	    -- Addizionale comunale Saldo 2016
	...
	--il campo n.41 è il campo n.9 del modello anno precedente
	--il campo n.42 è il campo n.10 del modello anno precedente
	--il campo n.43 è il campo n.11 del modello anno precedente
	--il campo n.44 è il campo n.12 del modello anno precedente
	--il campo n.45 è il campo n.13 del modello anno precedente
	--il campo n.46 è il campo n.14 del modello anno precedente 
	--il campo n.47 NUOVO  "tipo rapporto" rif. codici pag 61 istruzioni - deve assumere le indicazioni del tipo rapporto nei contratti parasubordinati
	--il campo n.48 NUOVO  "codice fiscale PA/Azienda" Nel punto 48 Codice fiscale Amministrazione/azienda, inserire il codice fiscale dell’Amministrazione o dell’azienda indicata nella denuncia emens diversa dal sostituto che presenta la certificazione (SOLO SE DIVERSA)
	
	--DC001	041	2017-01-11 14:43:42.773	assistenza	Compensi corrisposti sul parasubordinato
	--DC001	042	2017-01-31 00:00:00.000	assistenza	Contributi Dovuti
	--DC001	043	2017-01-31 00:00:00.000	assistenza	Contributi a carico del lavoratore
	--DC001	044	2017-01-31 00:00:00.000	assistenza	Contributi versati
	--DC001	045	2017-01-31 00:00:00.000	assistenza	Mesi per i quasi è stata presentata la denuncia UniEmens - Tutti
	--DC001	046	2017-01-31 00:00:00.000	assistenza	Mesi per i quasi è stata presentata la denuncia UniEmens - Tutti con esclusione di
	--DC001	047	2017-01-31 00:00:00.000	assistenza	Tipo Rapporto da contratto parasubordinati e mi fermo qua
	--DC001	048	2017-01-31 00:00:00.000	assistenza	Codice fiscale PPAA/Azienda
	--DC001	049	2017-01-31 00:00:00.000	assistenza	Codice Fiscale Ente Previdenziale 
	--DC001	050	2017-01-31 00:00:00.000	assistenza	Codice Fiscale Denominazione Ente Previdenziale
	...
	BEGIN
		...
		DECLARE @DC001041	DECIMAL (19,2)	-- Compensi corrisposti sul parasubordinatoVP
		DECLARE @DC001042	DECIMAL (19,2)	-- Contributi Dovuti VP
		DECLARE @DC001043	DECIMAL (19,2)	-- Contributi a carico del lavoratore VP
		DECLARE @DC001044	DECIMAL (19,2)	-- Contributi versati VP
		DECLARE @DC001045	INT				-- Mesi per i quali è stata presentata la denuncia UniEmens - Tutti CB
		DECLARE @DC001046	VARCHAR(12)		-- Mesi per i quali è stata presentata la denuncia UniEmens
		DECLARE @DC001047	VARCHAR(2)		-- Tipo rapporto Emens
		
		--SET @DC001001 = @agencynumber 
		SET @DC001041= isnull(@compensoprev,0)
		SET @DC001042= isnull(@ritprevdovuta,0)
		SET @DC001043= isnull(@ritprevtrattenuta,0)
		SET @DC001044= isnull(@ritprevpagata,0)
		SET @DC001045= isnull(@emensTuttiIMesi,0)
		SET @DC001046= @mesiSenzaEmens
		SET @DC001047= @idemenscontractkind
		...
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale)	VALUES(@progrCom, 1, 'DC001', 1, '041', @DC001041)
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale)	VALUES(@progrCom, 1, 'DC001', 1, '042', @DC001042)
		...
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, intero)		VALUES(@progrCom, 1, 'DC001', 1, '045', @DC001045)
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, stringa)	VALUES(@progrCom, 1, 'DC001', 1, '046', @DC001046)
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, stringa)	VALUES(@progrCom, 1, 'DC001', 1, '047', @DC001047)
	END
	------------------------------------------------------------------------------------------------------------------------------
	...
	BEGIN
		...
		--DC001	017	2017-01-11 14:43:42.773	assistenza	Anno di riferimento
		--DC001	018	2017-01-11 14:43:42.773	assistenza	Imponibile pensionistico
		--DC001	019	2017-01-11 14:43:42.773	assistenza	Contributi pensionistici dovuti
		--DC001	020	2017-01-11 14:43:42.773	assistenza	Contributi pensionistici a carico lavoratore trattenuti
		--DC001	021	2017-01-11 14:43:42.773	assistenza	Imponibili TFS
		--DC001	022	2017-01-11 14:43:42.773	assistenza	Contributi TFS
		--DC001	023	2017-01-11 14:43:42.773	assistenza	Contributi TFS a carico lavoratore trattenuti
		--DC001	024	2017-01-11 14:43:42.773	assistenza	Imponibile TFR
		--DC001	025	2017-01-11 14:43:42.773	assistenza	Contributi TFR dovuti 
		--DC001	026	2017-01-11 14:43:42.773	assistenza	Imponibile Gestione Credito
		--DC001	027	2017-01-11 14:43:42.773	assistenza	Contributo Gestione Credito dovuti
		--DC001	028	2017-01-11 14:43:42.773	assistenza	Contributo Gestione Credito trattenuti a carico del lavoratore
		...
		DECLARE @DC001038	VARCHAR(12)		-- Periodi retributivi soggetto denuncia CB12, opposto di @mesiSenzaEmensdipendentipubblici
		...
		SET @DC001036 = @mesiSenzaEmensdipendentipubblici			  -- Mesi per i quasi è stata presentata la denuncia UniEmens - Tutti con esclusione di
	...
	BEGIN 
		...
		if @startpayroll < @1gen16_XXX set @oldstart = @1gen16_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic16_XXX set @oldstop =  @31dic16_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
			if @startpayroll < @1gen16_XXX set @startpayroll = @1gen16_XXX
			if @stoppayroll > @31dic16_XXX set @stoppayroll = @31dic16_XXX
			...
			begin
				...
				--@DC001035 		-- Qualifica AN Vale B, C, D, E, F,G, H, L, M, N, P,Q, Z CREARE TABELLA DI CONFIGURAZIONE
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna,  intero) VALUES(@progrCom, 1,  'DC001', 1, '071',  @DC001071)
				-- @DC001036 Posizione assicurativa territoriale N11
				...
				End
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, stringa) VALUES(@progrCom, 1, 'DC001', 1, '072', @DC001072)
				-- @DC001037 Data inizio D4
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, data) VALUES(@progrCom, 1, 'DC001', 1, '073', @DC001073)
				-- @DC001038 Data fine D4
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, data) VALUES(@progrCom, 1, 'DC001', 1, '074', @DC001074)
				-- @DC001039  Codice comune AN  
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, stringa) VALUES(@progrCom, 1, 'DC001', 1, '075', @DC001075)
				--@DC001040  -- Personale viaggiante CB   NON LO METTIAMO 
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna,  intero) VALUES(@progrCom, 1,  'DC001', 1, '076',  @DC001076)
...
------------------------------------------------------------------------------------------------------------------
	...
	IF ((@DB001022 <> 0)  --@ritenuta_addreg_irpef
		 OR 
		(@DB001027 <> 0)) --@ritenuta_addcom_irpef_saldo_2016
	BEGIN
		SET @NN =
		CASE 
	...
	AND
	(@DB001367 =  0) --@ritenuta_addcom_irpef_saldo_2016 non applicata
	BEGIN
		SET @NN =
		'BB - Saldo dell''addizionale comunale all''IRPEF non operata in quanto' + 
		' in possesso dei requisiti reddituali per usufruire interamente della fascia' +
	...
	BEGIN 
		...
		SET @reddito_esente_servicecode770  = isnull(@reddito_esente_servicecode770,0)-isnull(@rimborso,0)
		...
		IF	(SELECT COUNT(*)
			...
			AND (
				...
				BEGIN
						...
						SET @NN = @text_for_annotation + ' ' +(SELECT description FROM #ser WHERE servicecode770 = @servicecode770)  +  ': € ' + CONVERT(varchar(16),
...
*/
	...
	BEGIN 
		...
		INSERT INTO #recordg (progr, quadro,modulo, riga, colonna, stringa)
		VALUES(@progrCom, 'DN001', 1,1, RIGHT('000' +  convert(varchar(3), @counter_an),3),@codean  )	
		FETCH NEXT FROM rowcursor_ann
		INTO @codean		 
	END
