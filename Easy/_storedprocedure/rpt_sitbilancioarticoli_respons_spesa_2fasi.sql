...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_sitbilancioarticoli_respons_spesa_2fasi]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_sitbilancioarticoli_respons_spesa_2fasi]
GO
...
CREATE   PROCEDURE [rpt_sitbilancioarticoli_respons_spesa_2fasi]
	@ayear			int,
	@nlevel			tinyint,
	@hierarchy		char(1),
	@idman			int,
...
AS
BEGIN
--exec rpt_sitbilancioarticoli_respons_spesa_2fasi 2006, {ts '2006-08-27 00:00:00'}, null, '3', 'N', 15, 'N'
...
UPDATE #fin_situation
	...
	var_main_prevision  =   
		ISNULL((SELECT SUM(finvardetail.amount) 
			FROM finvardetail
			JOIN finvar
				ON @ayear  = finvar.yvar 
				AND finvardetail.nvar = finvar.nvar
			JOIN finlast
				ON  finlast.idfin  = finvardetail.idfin --fin.idfin 
			JOIN finlink
				ON  finlink.idchild = finvardetail.idfin --fin.idfin   
			WHERE finlink.idparent = #fin_situation.idfin
		...
		var_max_phase_resid= 
			ISNULL((SELECT SUM(expensevar.amount)
				...
				JOIN historypaymentview HPV
					ON HPV.idexp = expensevar.idexp
				JOIN finlast
					ON  finlast.idfin  = HPV.idfin --fin.idfin 
				JOIN finlink
					ON  finlink.idchild = HPV.idfin --fin.idfin   
					AND finlink.idparent = #fin_situation.idfin
...
BEGIN
	...
	SET var_assign_credit = 
		isnull((select SUM(d.amount)
			...
			where 
				finlink.idparent = #fin_situation.idfin
	 ...
	 assign_credit = 	
		isnull((select SUM(creditpart.amount)
			...
			JOIN finlast
				ON  finlast.idfin  = creditpart.idfin --incomeyear.idfin 
			JOIN finlink
				ON  finlink.idchild = creditpart.idfin --incomeyear.idfin   
			where finlink.idparent = #fin_situation.idfin
				AND finlast.idman = #fin_situation.idman
	...
	UPDATE #fin_situation
		...
		assign_cash = 
			isnull(( select SUM(proceedspart.amount)
				...
				JOIN finlast
					ON  finlast.idfin  = proceedspart.idfin --incomeyear.idfin 
				JOIN finlink
					ON  finlink.idchild = proceedspart.idfin -- incomeyear.idfin  
				where  finlink.idparent = #fin_situation.idfin
					AND finlast.idman = #fin_situation.idman
					AND adate between @start and @stop),0.0)
			+
...
SELECT 
	...
	fin.title,fin.printingorder,fin.nlevel,flag_assign_credit,flag_assign_cash,#fin_situation.idman,
	manager.title	
	ORDER BY #fin_situation.idman,fin.printingorder


-- 
--exec rpt_sitbilancioarticoli_respons_spesa_2fasi 2006, {ts '2006-12-31 00:00:00'}, null, '2', 'S', null, 'S'
END
