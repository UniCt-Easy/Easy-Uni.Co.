...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_bps_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_bps_var]
GO
...
CREATE        PROCEDURE [trasmele_expense_bps_var]
(
	@y int,
	@n int
)
...
--- 10 VARIAZIONI DI ANNULLAMENTO PARZIALE - NON CONSENTITO DA BPS
...
WHERE I.nphase = @maxincomephase
	AND ((I.autokind = 6) /* <--Recupero*/	OR (I.autokind = 14) /*<--automatismo generico*/OR (I.autokind = 4 AND I.idreg = P.idreg)/*<--Ritenuta*/ OR 
	(I.autokind in (20,21,30,31) AND I.idreg = P.idreg AND @csa_flagtransmissionlinking = 'S' ))
	AND IT.ayear = @y
...
--- al momento commento questa parte in quanto BPS non ha ancora richiesto 
-- il collegamento tra pagamento e incasso di pari importo relativo
--all'applicazione di controbuti su partita di giro
/*
...
INSERT INTO #error (message)
SELECT 'Il Pagamento n° ' + CONVERT(varchar(6),el.nmov) + '/' + CONVERT(varchar(4),el.ymov) + 
' descrizione: ' + el.description +
' contenuto nel mandato n° ' + CONVERT(varchar(6),t.ndoc) + '/' + CONVERT(varchar(4),t.ydoc)
+ ' è collegato a un numero di ritenute superiore a ' + CONVERT(varchar(4),@max_count_tax)
+ ' Si consiglia di controllare il pagamento e le reversali collegate.'
FROM #tax t
JOIN expenselastview el on el.idexp = t.idexp
GROUP BY t.ydoc, t.ndoc,el.ymov, el.nmov, el.description 
HAVING COUNT(idpro) > @max_count_tax
ORDER BY  t.ydoc, t.ndoc, el.ymov, el.nmov 
 
INSERT INTO #error (message)
SELECT  
'L''incasso n°' + CONVERT(varchar(6),t.nmov_income) + '/' + CONVERT(varchar(4),t.ymov_income) + 
'  - reversale n° '  + CONVERT(varchar(6),t.npro) + '/' + CONVERT(varchar(4),t.ypro) +
' risulta  collegato al pagamento n° ' + CONVERT(varchar(6),el.nmov) + '/' + CONVERT(varchar(4),el.ymov) + 
' contenuto nel mandato n° ' + CONVERT(varchar(6),t.ndoc) + '/' + CONVERT(varchar(4),t.ydoc)
+ ' con numero di ritenute superiore a ' + CONVERT(varchar(4),@max_count_tax)
+ ' Si consiglia di controllare il pagamento e le reversali collegate.'
FROM #tax t
JOIN expenselastview el on el.idexp = t.idexp
WHERE EXISTS (SELECT t1.ydoc, t1.ndoc,t1.idexp FROM #tax t1 WHERE t.idexp = t1.idexp
GROUP BY t1.ydoc, t1.ndoc,t1.idexp
HAVING COUNT(t1.idpro) > @max_count_tax)
ORDER BY  t.ydoc, t.ndoc, el.ymov, el.nmov,t.ypro, t.npro,t.ymov_income,
t.nmov_income
...
GROUP BY #payment.ypaymenttransmission, #payment.npaymenttransmission, #payment.ydoc,
	...
	#payment.idpay, SUBSTRING(sorting.sortcode,@npos,DATALENGTH(sorting.sortcode)),sorting.description,
	#payment.idexp, 
	#payment.cupcodeexpense, #payment.cupcodedetail,
...
(
	...
	----------------- spese ---------------------
	---------------------------------------------
	soggetto_destinatario_delle_spese varchar(20),
	natura_pagamento varchar(20),	
	---------------------------------------------
	----------------- beneficiario --------------
	---------------------------------------------
	...
	iban varchar(50),
	bic varchar(20),
	identificativo_end_to_end varchar(20),
	-----------------------------------------------
...
SELECT 
				   ...
				   null
FROM #payment
GROUP BY #payment.ndoc,payment_adate,opkind
...
SELECT 
				   ...
				   CASE 
  						...
  						WHEN  #payment.opkind = 'ANNULLO'    THEN SUM(#payment.originalamount)
  						ELSE NULL
  				   END,
				   CASE   
					   ...
					   WHEN (idpaymethodTRS = '09')  THEN 'F24EP' --(7)	 GIROFONDO + VALORIZZAZ COel.idcabDICE CONTABILITA SPECIALE FISSO CON VALORE 1777
				   ...
				   cf_ben, 
				   CASE 
						WHEN (ISNULL(idpaymethodTRS, '01') <> '03')  THEN  #deputy.title_deputy
						ELSE NULL
				   END,
				   CASE 
						WHEN (ISNULL(idpaymethodTRS, '01') <> '03')   THEN #deputy.address_deputy
						ELSE NULL
				   END,
				   CASE 
				   ...
				   END,
				   CASE 
						WHEN (ISNULL(idpaymethodTRS, '01') <> '03') THEN  #deputy.location_deputy
						ELSE NULL
				   END,
				   CASE 
				   ...
				   -----------------------------------------------------------
				   CASE 
						WHEN (idpaymethodTRS = '03')  THEN  #deputy.title_deputy
						ELSE NULL
				   END,
				   CASE 
						WHEN (idpaymethodTRS = '03')   THEN #deputy.address_deputy
						ELSE NULL
				   END,
				   CASE 
				   ...
				   END,
				   CASE 
						WHEN (idpaymethodTRS = '03') THEN   #deputy.location_deputy
						ELSE NULL
				   END,
				   CASE 
				   ...
				   END,
  				   CASE 
						WHEN ((deny_bank_details <> 'S')  AND (ISNULL(idpaymethodTRS, '01') <> '03') AND (fulfilled = 'N'))   THEN  bank
						ELSE NULL
				   END,
  				   -- SEPA CREDIT TRANSFER --
  				   CASE 
				   ...
				   END,
  				   -- CODICE INPS -- 
  				   null,
  				   paymentdescr, -- CAUSALE PAGAMENTO
  				   null,
...
LEFT JOIN #deputy
	...
	GROUP BY
		...
		refexternaldoc,expenselast_paymentdescr, #payment.txt
...
SELECT 
				   ...
				   null,
				   null,
				   SUM(#payment.originalamount)
FROM #payment
WHERE #payment.opkind =  'ANNULLO'
GROUP BY  ndoc, idpay, #payment.opkind 
ORDER BY  #payment.ndoc, #payment.idpay	
