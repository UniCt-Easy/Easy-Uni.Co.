...
if exists (select * from dbo.sysobjects where id = object_id(N'[compute_rilevazioni_anac_mono]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [compute_rilevazioni_anac_mono]
GO
...
--exec compute_rilevazioni_anac_mono 2025, null
-- select * from mandateview
CREATE PROCEDURE [compute_rilevazioni_anac_mono](
	@ayear int,
...
create table #impegni(
	...
	nman					int,	
	cigcode					varchar(10), 
	importoimpegnato		decimal(19,2), 
	ImponibileOrdine		decimal(19,2), 
	PagatoDelSingoloImpegno decimal(19,2),
	totaleLiquidato			decimal(19,2)
	)


/* Movimenti collegati ai contratti passivi */
INSERT INTO #impegni (
	ImponibileOrdine
	,idexp
	,idmankind
...
WHERE MD.idexp_taxable IS NOT NULL
	AND MD.stop IS NULL
GROUP BY MD.idexp_taxable
	,MD.idmankind
	,MD.yman
	,MD.nman
	,MD.cigcode
		
--select 'CONTRATTI PASSIVI',* from #impegni 
...
GROUP BY ID.idexp_taxable
	...
	,MD.cigcode
END
	--select 'FATTURE',* from #impegni ORDER BY idmankind, yman, nman, idexp
...
select 
		...
		--sum(I.totaleLiquidato),
		--sum(I.importoimpegnato),
		--sum(I.PagatoDelSingoloImpegno)
	from mandateview M
