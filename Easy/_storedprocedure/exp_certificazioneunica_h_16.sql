...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_h_16]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_h_16]
GO
 
 --setuser 'amministrazione'
 --declare @newProg int
 --exec exp_certificazioneunica_h_16 21,1, 'N', @newProg out
 --select @newProg
CREATE PROCEDURE [exp_certificazioneunica_h_16]
...
-- Sezione dichiarativa	
	...
	update #expense2015 set motive770 = motive770service.idmot
		from motive770service 
		where motive770service.idser = #expense2015.idser
		...
		SELECT 
			#expense2015.employtaxamount,
			#expense2015.idexp,
			#expense2015.idser,
			#expense2015.motive770
		FROM #expense2015
		join expenselink  			on expenselink.idchild = #expense2015.idexp			and nlevel = @expensephase
...
--	AUX026 = AU001004 - AU0010005 - AU001007
	...
	--CAMPO 34 (Contributi previdenziali a carico del soggetto erogante)  è il vecchio CAMPO 20 del precedente modello fiscale (CU 2015)
	--CAMPO 35 (Contributi previdenziali a carico del percipiente) è il vecchio CAMPO 21 del precedente modello fiscale (CU 2015)
 ...
 --SELECT * FROM #recHNonArrot
	...
	declare @cursore cursor
		...
		begin
			...
			exec exp_certificazioneunica_d_16  @idreg, @newprogrCom, 'H',NULL, @print
...
insert into #recordH (progr,modulo, quadro, riga, colonna, stringa,decimale, intero, data)
			exec exp_certificazioneunica_h_pign_16  @idreg, @ultimoinserito,@fattoqualcosa,  @inserito out
...
---  
 ...
 --declare @newProg int
 --exec exp_certificazioneunica_h_16 1126,3, 'N', @newProg out
