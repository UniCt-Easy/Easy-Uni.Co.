...
if exists (select * from dbo.sysobjects where id = object_id(N'[do_azzera_patrimonio]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [do_azzera_patrimonio]
GO
...
CREATE      PROCEDURE [do_azzera_patrimonio]
(
	 @vardate datetime,
...
AS BEGIN
declare @ayear int
set @ayear=year(@vardate)

declare @idinventoryagency int
select @idinventoryagency= idinventoryagency from inventoryagency where codeinventoryagency=@codeinventoryagency
...
BEGIN TRANSACTION
	...
	'Variazione di azzeramento consistenza', 0, @vardate , 
	'azzeratutto', GETDATE(), 'azzeratutto', GETDATE())
