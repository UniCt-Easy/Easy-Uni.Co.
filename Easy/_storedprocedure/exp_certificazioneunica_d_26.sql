...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_d_26]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_d_26]
GO
--setuser 'amministrazione'
-- exec exp_certificazioneunica_d_26  19335 , 1, 'G', {d '2020-12-31'},'S'
CREATE PROCEDURE [exp_certificazioneunica_d_26]
...
-- Sezione dichiarativa	
	...
	--2019	DA001	001	Codice fiscale	CF
	--2019	DA001	002	Cognome o denominazione	AN
	--2019	DA001	003	Nome	AN
	--2019	DA001	004	Comune di Residenza	AN
	--2019	DA001	005	Provincia di Residenza (sigla)	PR
	--2019	DA001	006	CAP	AN
	--2019	DA001	007	Via e numero civico	AN
	--2019	DA001	008	Telefono/Fax	AN
	--2019	DA001	009	Indirizzo di posta elettronica del sostituto	AN
	--2019	DA001	010	Codice Attività	AN
	--2019	DA001	011	Codice Sede	AN (solo per chi ha più sedi)
	...
	DECLARE @DA002043 VARCHAR(100)
	DECLARE @DA002045 VARCHAR(4)
...
DECLARE @dateindi_01_gen_25 datetime
DECLARE @dateindi_01_gen_26 datetime

SET @dateindi_01_gen_25 = DATEADD(yy, @annoredditi - 2000, {d '2000-01-01'})
SET @dateindi_01_gen_26 = DATEADD(yy, @annodichiarazione - 2000, {d '2000-01-01'})
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= @dateindi_01_gen_25))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
		AND (cdi.start <= @dateindi_01_gen_25)
        AND ((cdi.stop IS NULL AND  cdi.active <>'N') OR (cdi.stop >= @dateindi_01_gen_25))
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= ISNULL(@datarif,@dateindi_01_gen_26)))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
        AND (cdi.start <= ISNULL(@datarif,@dateindi_01_gen_26))
        AND ((cdi.stop IS NULL AND cdi.active <>'N') OR (cdi.stop >= ISNULL(@datarif,@dateindi_01_gen_26)))
        AND cdi.idreg = registryaddress.idreg)
        AND idreg = @idreg
...
SELECT @DA002043 = address FROM #address WHERE nation <> 'ITALIA'
SELECT @DA002045 = codice_iso_nazione FROM #address WHERE nation <> 'ITALIA'

--select * from #address
	--2019	DA002	001	Codice fiscale	CF
	--2019	DA002	002	Cognome	AN
	--2019	DA002	003	Nome	AN
	--2019	DA002	004	Sesso	AN
	--2019	DA002	005	Data di nascita	DT
	--2019	DA002	006	Comune (o Stato estero) di nascita	AN
	--2019	DA002	007	Provincia di nascita (sigla)	PN
	--2019	DA002	008	Categorie particolari	AN -- non compiliamo
	--2019	DA002	009	Eventi eccezionali	N1 -- non compiliamo 
	--2019	DA002	010	Casi di esclusione dalla  precompilata	N1 -- non compiliamo
	--2019	DA002	020	Comune	AN  (Domicilio fiscale al 1/1/2017)
	--2019	DA002	021	Provincia (sigla)	PR  (Domicilio fiscale al 1/1/2017)
	--2019	DA002	022	Codice comune	AN (Domicilio fiscale al 1/1/2017) 
	--2019	DA002	023	Comune	AN   (Domicilio fiscale al 1/1/2020)
	--2019	DA002	024	Provincia (sigla)	PR  (Domicilio fiscale al 1/1/2020)
	--2019	DA002	025	Codice comune	PR  (Domicilio fiscale al 1/1/2020)
	--2019	DA002	030	Dati relativi al rappresentante	CF  -- minori, incapaci ecc.. (non compiliamo)
	--2019	DA002	040	Codice di identificazione fiscale estero	AN
	--2019	DA002	041	Località di residenza estera	AN
	--2019	DA002	042	Via e numero civico	AN
	--2019	DA002	043	Non Residenti Shumacker CB
	--2019	DA002	044	Codice stato estero	N3
...
End
	INSERT INTO #recordd (progr, modulo,quadro, riga, colonna, stringa) VALUES(@progrCert, @progrModulo, 'IND', 1, '005', @IND005) -- lo mostra sempre
IF (@tipoRecord <> 'H')
	BEGIN
	...
	END
	--Riservato ai percipienti esteri COMPILIAMO LA SEZIONE SOLO SE IL PERCIPIENTE HA UN CODICE FISCALE ESTERO + RESIDENZA ESTERA
	if ((isnull(@DA002040,'') <> '')  AND (ISNULL(@DA002041,'') <> ''))
	BEGIN
		...
		INSERT INTO #recordd (progr,modulo, quadro, riga, colonna, stringa) VALUES(@progrCert, @progrModulo,'DA002', 1, '043', @DA002043)
		INSERT INTO #recordd (progr,modulo, quadro, riga, colonna, intero)  VALUES(@progrCert, @progrModulo,'DA002', 1, '045', @DA002045)
	END
	---------------------------------------------------- 
    ...
    SELECT progr,modulo, quadro, riga, colonna, stringa,decimale, data, intero FROM #recordd 
