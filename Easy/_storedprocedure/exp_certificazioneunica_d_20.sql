...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_d_20]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_d_20]
GO

-- exec exp_certificazioneunica_d_20  19335 , 1, 'G', {d '2019-12-31'},'S'
CREATE PROCEDURE [exp_certificazioneunica_d_20]
...
DECLARE @dateindi_01_gen_19 datetime
DECLARE @dateindi_01_gen_20 datetime

SET @dateindi_01_gen_19 = DATEADD(yy, @annoredditi - 2000, {d '2000-01-01'})
SET @dateindi_01_gen_20 = DATEADD(yy, @annodichiarazione - 2000, {d '2000-01-01'})
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= @dateindi_01_gen_19))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
		AND (cdi.start <= @dateindi_01_gen_19)
        AND ((cdi.stop IS NULL AND  cdi.active <>'N') OR (cdi.stop >= @dateindi_01_gen_19))
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= ISNULL(@datarif,@dateindi_01_gen_20)))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
        AND (cdi.start <= ISNULL(@datarif,@dateindi_01_gen_20))
        AND ((cdi.stop IS NULL AND cdi.active <>'N') OR (cdi.stop >= ISNULL(@datarif,@dateindi_01_gen_20)))
        AND cdi.idreg = registryaddress.idreg)
        AND idreg = @idreg
...
--select * from #address
	...
	--2019	DA002	023	Comune	AN   (Domicilio fiscale al 1/1/2019)
	--2019	DA002	024	Provincia (sigla)	PR  (Domicilio fiscale al 1/1/2019)
	--2019	DA002	025	Codice comune	PR  (Domicilio fiscale al 1/1/2019)
