...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_intesasanpaolo_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_intesasanpaolo_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_intesasanpaolo_ins]
(
	@y int,
	@n int
)
...
--
--setuser 'amm'
--trasmele_expense_intesasanpaolo_ins 2016,1
--------------------------------------------------------------
...
WHERE paymentcommunicated.kpaymenttransmission = @kpaymenttransmission
	-- modalità di pagamento girofondo, valutare il girofondo F24EP
			   ...
			   ((expenselast.paymethod_flag & 2048) <> 0) 
		   	  ...
		   	  AND paymethod.methodbankcode  <> '09' --- escluso il GIROFONDO F24EP
...
AND (p.idpaydisposition IS  NULL AND -- i mandati stipendi ad anagrafiche cumulative non devono essere associati a ritenute
	...
	--SELECT @csa_flagtransmissionlinking
	--SELECT * FROM #payment
	--SELECT * FROM #TAX
...
-- CONTROLLO N. 14. Movimento di Spesa a Importo zero
INSERT INTO #error (message)
