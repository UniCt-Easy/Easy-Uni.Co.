...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_riepilogoiva]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_riepilogoiva]
GO
...
-- exec rpt_riepilogoiva  1, 2017, 2 , 4, 'N', 'N', 'ACQCOMM','T'
-- exec rpt_riepilogoregistroacquisti  2,2015,8,8,'S'
CREATE PROCEDURE [rpt_riepilogoiva]
...
(
	...
	(SELECT
		...
		case
			-- se la fattura è di vendita e soggetta a split payment, non ha senso parlare di iva differita. Quindi viene intesa come immediata. Vedi task 6507  
...
--2)
	...
	WHERE 
		...
		AND ISNULL(IDET.rounding,'N') <>'S'  --salta i dettagli di arrotondamento, task 7360
		and (isnull(IDET.flagbit,0) & 4) = 0
		-- Sta prendendo quelle a iva differita, quindi se sono di acquito, ok, ma se sono di vendita soggette a split NON le deve prendere perchè vengono intese a iva immediata e rientrano nella prima SELECT, ora sta prendendo
...
JOIN #ivaregisterkind IRK
			ON IRK.idivaregisterkind = IR.idivaregisterkind		
		WHERE (@idsor01 IS NULL OR I.idsor01 = @idsor01)
		AND (@idsor02 IS NULL OR I.idsor02 = @idsor02)
		AND (@idsor03 IS NULL OR I.idsor03 = @idsor03)
		AND (@idsor04 IS NULL OR I.idsor04 = @idsor04)
		AND (@idsor05 IS NULL OR I.idsor05 = @idsor05)			
...
-- Tabella finale, dove sono presenti i vari tipi IVA con gli importi calcolati
	...
	SELECT idivaregisterkind,idivakind,
		SUM(taxabletotal),SUM(ivagross),SUM(ivaunabatable),SUM(ivaabatable) 
	FROM #invoicedet where flagdeferred = 'N' 
	group by idivaregisterkind,idivakind

	

	INSERT INTO #ivakind
	( idivaregisterkind, idivakind,		
		taxabletotal_dif,		ivatotal_dif,		ivaunabatable_dif,		ivaabatable_dif		
	)

	SELECT idivaregisterkind, idivakind,SUM(taxabletotal),SUM(ivagross),SUM(ivaunabatable),SUM(ivaabatable) 
	FROM #invoicedet where flagdeferred = 'S' 
	group by idivaregisterkind, idivakind

	INSERT INTO #ivakind
	(	idivaregisterkind, 		idivakind,
		taxabletotal_totdif,		ivatotal_totdif,		ivaunabatable_totdif,		ivaabatable_totdif		
	)
	SELECT idivaregisterkind, idivakind,SUM(taxabletotal),SUM(ivagross),SUM(ivaunabatable),SUM(ivaabatable) 
...
SELECT
	...
	group by  IRK.description, ivakind.codeivakind,ivakind.description,ivakind.rate,ivakind.unabatabilitypercentage
END
