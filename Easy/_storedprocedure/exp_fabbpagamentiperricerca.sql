...
-- 1030102001 
-- exp_fabbpagamentiperricerca '2020', '3', '%', null, null, null, null, null,'S'
...
if (@showdetail = 'S' )
Begin
	exec exp_fabbpagamentiperricerca_dett @ayear, @monthstop, @idupb, @idsor01, @idsor02, @idsor03, @idsor04, @idsor05
...
group by 		ES.idsor,	ES.values1,	ES.values2
...
INSERT INTO #datiout(	 idsor,	importoCUMULATO_01_4,		importoMENSILE_01_4)
select 		idsor, sum(importoCUMULATO),	sum(importoMENSILE)	
from #pagamenti
where cofog = '01.4'
group by  		idsor

INSERT INTO #datiout(	 idsor,	importoCUMULATO_04_8,		importoMENSILE_04_8)
select 		idsor, sum(importoCUMULATO),	sum(importoMENSILE)
from #pagamenti
where cofog = '04.8'
group by  		idsor

INSERT INTO #datiout(	 idsor,	importoCUMULATO_07_5,		importoMENSILE_07_5)
select 		idsor, sum(importoCUMULATO),	sum(importoMENSILE)	
from #pagamenti
where cofog = '07.5'
group by  		idsor

INSERT INTO #datiout(	 idsor,	importoCUMULATO_UE1,		importoMENSILE_UE1)
select 		idsor, sum(importoCUMULATO),	sum(importoMENSILE)
from #pagamenti
where cod_ue = '01'
group by  		idsor

INSERT INTO #datiout(	 idsor,	importoCUMULATO_UE2,		importoMENSILE_UE2)
select 	idsor, sum(importoCUMULATO),	sum(importoMENSILE)
from #pagamenti
where cod_ue = '02'
group by  		idsor

INSERT INTO #datiout(	 idsor,	importoCUMULATO_UE3,		importoMENSILE_UE3)
select 	idsor, sum(importoCUMULATO),	sum(importoMENSILE)
from #pagamenti
where cod_ue = '03'
group by  		idsor

INSERT INTO #datiout(	 idsor,	importoCUMULATO_UE4,		importoMENSILE_UE4)
select 	idsor, sum(importoCUMULATO),	sum(importoMENSILE)
from #pagamenti
where cod_ue = '04'
group by  		idsor
...
INSERT INTO #datiout(idsor, codiceprogrammasiope01_MENSILE, codiceprogrammasiope01_CUMULATO )
select idsor, sum(importoMENSILE_01_4), sum(importoCUMULATO_01_4)
from #datiout
group by idsor

INSERT INTO #datiout(idsor, codiceprogrammasiope02_MENSILE, codiceprogrammasiope02_CUMULATO)
select idsor,  sum(isnull(importoMENSILE_04_8,0) + isnull(importoMENSILE_07_5,0)),  sum(isnull(importoCUMULATO_04_8,0) + isnull(importoCUMULATO_07_5,0))
from #datiout
group by idsor
...
INSERT INTO #datiout(idsor, codicemissionesiope01_MENSILE, codicemissionesiope01_CUMULATO)
select idsor, sum(isnull(codiceprogrammasiope01_MENSILE,0) + isnull(codiceprogrammasiope02_MENSILE,0)) , 
			sum(isnull(codiceprogrammasiope01_CUMULATO,0) + isnull(codiceprogrammasiope02_CUMULATO,0))
from #datiout
group by idsor
