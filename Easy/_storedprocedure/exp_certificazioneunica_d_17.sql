...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_d_17]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_d_17]
GO
 
-- exec exp_certificazioneunica_d_17  28427, 1, 'H', {d '2017-12-31'},'S'
CREATE PROCEDURE [exp_certificazioneunica_d_17]
...
-- Sezione dichiarativa	
	...
	--2017	DA001	001	Codice fiscale	CF
	--2017	DA001	002	Cognome o denominazione	AN
	--2017	DA001	003	Nome	AN
	--2017	DA001	004	Comune di Residenza	AN
	--2017	DA001	005	Provincia di Residenza (sigla)	PR
	--2017	DA001	006	CAP	AN
	--2017	DA001	007	Via e numero civico	AN
	--2017	DA001	008	Telefono/Fax	AN
	--2017	DA001	009	Indirizzo di posta elettronica del sostituto	AN
	--2017	DA001	010	Codice Attività	AN
	--2017	DA001	011	Codice Sede	AN (solo per chi ha più sedi)
...
DECLARE @dateindi_01_gen_17 datetime

SET @dateindi_01_gen_16 = DATEADD(yy, @annoredditi - 2000, {d '2000-01-01'})
SET @dateindi_01_gen_17 = DATEADD(yy, @annodichiarazione - 2000, {d '2000-01-01'})
...
LEFT OUTER JOIN geo_nation_agency N -- eventuale nazione di residenza, se estero
    ...
    AND N.stop IS NULL
WHERE
((registryaddress.stop IS NULL AND registryaddress.active <>'N' ) OR (registryaddress.stop >= ISNULL(@datarif,@dateindi_01_gen_17)))
    AND registryaddress.start =
    (SELECT MAX(cdi.start)
    FROM registryaddress cdi
    WHERE cdi.idaddresskind = registryaddress.idaddresskind
        AND (cdi.start <= ISNULL(@datarif,@dateindi_01_gen_17))
        AND ((cdi.stop IS NULL AND cdi.active <>'N') OR (cdi.stop >= ISNULL(@datarif,@dateindi_01_gen_17)))
        AND cdi.idreg = registryaddress.idreg)
        AND idreg = @idreg
...
--select * from #address
	--2017	DA002	001	Codice fiscale	CF
	--2017	DA002	002	Cognome	AN
	--2017	DA002	003	Nome	AN
	--2017	DA002	004	Sesso	AN
	--2017	DA002	005	Data di nascita	DT
	--2017	DA002	006	Comune (o Stato estero) di nascita	AN
	--2017	DA002	007	Provincia di nascita (sigla)	PN
	--2017	DA002	008	Categorie particolari	AN -- non compiliamo
	--2017	DA002	009	Eventi eccezionali	N1 -- non compiliamo 
	--2017	DA002	010	Casi di esclusione dalla  precompilata	N1 -- non compiliamo
	--2017	DA002	020	Comune	AN  (Domicilio fiscale al 1/1/2016)
	--2017	DA002	021	Provincia (sigla)	PR  (Domicilio fiscale al 1/1/2016)
	--2017	DA002	022	Codice comune	AN (Domicilio fiscale al 1/1/2016) 
	--2017	DA002	023	Comune	AN   (Domicilio fiscale al 1/1/2017)
	--2017	DA002	024	Provincia (sigla)	PR  (Domicilio fiscale al 1/1/2017)
	--2017	DA002	025	Codice comune	PR  (Domicilio fiscale al 1/1/2017)
	--2017	DA002	030	Dati relativi al rappresentante	CF  -- minori, incapaci ecc.. (non compiliamo)
	--2017	DA002	040	Codice di identificazione fiscale estero	AN
	--2017	DA002	041	Località di residenza estera	AN
	--2017	DA002	042	Via e numero civico	AN
	--2017	DA002	043	Non Residenti Shumacker CB
	--2017	DA002	044	Codice stato estero	N3
