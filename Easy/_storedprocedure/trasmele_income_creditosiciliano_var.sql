...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_income_creditosiciliano_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_income_creditosiciliano_var]
GO
...
CREATE  PROCEDURE [trasmele_income_creditosiciliano_var]
(
	@y int,
	@n int
)
...
-- Partita Iva estera
	...
	End,
	CASE
		WHEN ((il.flag&32) <> 0) THEN null -- --- incasso con spese accessorie
