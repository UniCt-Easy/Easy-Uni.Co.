...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_16]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_16]
GO
--setuser 'amm' 
--exec exp_certificazioneunica_g_16 1464,1,'N'
CREATE PROCEDURE [exp_certificazioneunica_g_16]
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_16  @idreg, @progrCom, 'G',NULL, @print
	...
	SET @ritenuta_addreg_irpef =
		...
		DECLARE @ritenuta_addcom_irpef_acconto_2015 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_acconto_2015 =
		ISNULL(
		...
		SET @ritenuta_addcom_irpef_acconto_2015 =
		@ritenuta_addcom_irpef_acconto_2015 +
		ISNULL(
		...
		-- Non si filtra sulla certificazione in quanto le prestazioni che non rientrano nel CUD
		-- non pagano le addizionali regionali e quindi il loro contributo è nullo
		DECLARE @ritenuta_addcom_irpef_saldo_2015 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_saldo_2015 =
		ISNULL(
			(SELECT SUM(employtax)
			FROM #cedolini
			JOIN expenselink
	...
	SET @ritenuta_addcom_irpef_saldo_2015 =
	@ritenuta_addcom_irpef_saldo_2015 +
	ISNULL(
	...
	SELECT @ritenuta_addcom_irpef_saldo_2015  = @ritenuta_addcom_irpef_saldo_2015 - @ritenuta_addcom_irpef_acconto_2015   
	...
	if @ritenuta_addcom_irpef_saldo_2015 < 0
	begin
		set @ritenuta_addcom_irpef_acconto_2015 = @ritenuta_addcom_irpef_acconto_2015 + @ritenuta_addcom_irpef_saldo_2015
		set @ritenuta_addcom_irpef_saldo_2015 = null
	end
	else
	begin
		if @ritenuta_addcom_irpef_acconto_2015 < 0
		begin
			set @ritenuta_addcom_irpef_saldo_2015 = @ritenuta_addcom_irpef_saldo_2015 + @ritenuta_addcom_irpef_acconto_2015
			set @ritenuta_addcom_irpef_acconto_2015 = null
		end
	end
...
--setuser 'amm'
	...
	SELECT
		...
		ISNULL(F.starthandicap, @1gen16_XXX),
		ISNULL(F.cf, ''),
	...
	BEGIN
		...
		SET @DB001009= @stop  -- Data fine Rapporto di Lavoro Non deve essere valorizzato in caso di continuazione del rapporto di lavoro oltre il 31/12/2015 ricavabile dalla data fine del contratto inserito. N.B.: il campo 9 è alternativo al campo 10.
		
		-- "In forza al 31/12"
		IF (@stopcontract > @31dic15)
			BEGIN
			SET @DB001010	= 1  
			END
		ELSE 
		...
		--IF @workingdays < (datediff(d,@start, @stop) + 1)
		 -- Non ha senso considerare data min cedolino e data max cedolino, potendo esistere cedolini di anni precedenti. Vedi task 8122
		...
		(	(year(@start)<@annoredditi)
			OR
			(	-- IN FORZA AL 31 12
				(@stopcontract > @31dic15)  AND (@stop < @31dic15)	
			) 
			OR
			(
			   (@stopcontract <= @31dic15)  AND (@stop < @stopcontract) 
			)
		) 
	...
	DECLARE	@DB001027 DECIMAL(19,2)	-- Addizionale comunale all'Irpef - Saldo 2015
	
	SET @DB001021= isnull(@ritenuta_irpef,0)
	SET @DB001022= isnull(@ritenuta_addreg_irpef,0)
	SET @DB001027= isnull(@ritenuta_addcom_irpef_saldo_2015,0)
	...
	BEGIN 
		...
		print @exemptioncode
			...
			select @rimborso = sum(ROUND(estimatedetail.taxable * estimatedetail.number * 
				...
				where  (isnull(estimate.idreg, estimatedetail.idreg) = @idreg )
	...
	DECLARE @DB001545 DECIMAL(19,2)	    -- Addizionale comunale Acconto 2015
	DECLARE @DB001546 DECIMAL(19,2)	    -- Addizionale comunale Saldo 2015
	...
	BEGIN
		...
		--exec exp_certificazioneunica_g_16 21,1
...
------------------------------------------------------------------------------------------------------------------
	...
	IF ((@DB001022 <> 0)  --@ritenuta_addreg_irpef
		 OR 
		(@DB001027 <> 0)) --@ritenuta_addcom_irpef_saldo_2015
	BEGIN
		SET @NN =
		CASE 
	...
	AND
	(@DB001367 =  0) --@ritenuta_addcom_irpef_saldo_2015 non applicata
	BEGIN
		SET @NN =
		'BB - Saldo dell''addizionale comunale all''IRPEF non operata in quanto' + 
		' in possesso dei requisiti reddituali per usufruire interamente della fascia' +
	...
	BEGIN 
		...
		SET @reddito_esente_servicecode770  = isnull(@reddito_esente_servicecode770,0)
		...
		IF	(SELECT COUNT(*)
			...
			AND (
					...
					AND isnull(@DC001023,0) = 0	  --> @taxablegross_dipendentipubblici	 
				...
				BEGIN
						...
						SET @NN = @text_for_annotation + ' ' +(SELECT description FROM #ser WHERE servicecode770 = @servicecode770)  +  ': € ' + CONVERT(varchar(16),@reddito_esente_servicecode770)  
