...
create PROCEDURE [exp_modello770_09_h]
AS BEGIN
...
-- Sezione dichiarativa	
	...
	update #expense2008 set motive770 = motive770service.idmot
		from motive770service 
		where motive770service.idser = #expense2008.idser
		AND motive770service.ayear = @annodichiarazione-1

	declare @cognome varchar(60)

	DECLARE registry_crs CURSOR FOR 
		SELECT DISTINCT isnull(cf,p_iva), #expense2008.idreg, surname FROM #expense2008
		join registry on registry.idreg = #expense2008.idreg
		order by surname
	OPEN registry_crs
	...
	--9 Identificativo del produttore del software (codice fiscale)
		...
		SELECT TOP 1 
			...
			WHERE registryaddress.idreg = @idreg
				AND registryaddress.start <= @31dic08
				AND (registryaddress.stop IS NULL OR registryaddress.stop >= @31dic08)
			ORDER BY
			CASE idaddresskind
				WHEN @stand THEN 1
				WHEN @domfi THEN 2
	...
	--AU001009 Provincia (sigla)
		...
		if exists (select * from #expense2008 where idreg=@idreg and motive770='N')
		begin
	...
	--AU001017 Codice stato estero
		...
		DECLARE cursoreexpense CURSOR FOR 
			SELECT 
				#expense2008.employtaxamount,
				#expense2008.idexp,
				#expense2008.idser,
				#expense2008.motive770
			FROM #expense2008
			join expenselink 
				on expenselink.idchild = #expense2008.idexp
				and nlevel = @expensephase
			left outer join expenseprofservice
				on expenselink.idparent = expenseprofservice.idexp
			WHERE #expense2008.idreg=@idreg 
...
-- AUX0021 Ammontare lordo corrisposto = Imponibile Lordo SUM (feegross) di quel percipiente.
	...
	BEGIN
		...
		exec compute_casualcontract @annoredditi,@ycon,@ncon
			 ...
			 SET @imponibilereale = 
				ISNULL( (SELECT payed_lastyear  from #annualpayedrefund),0)
...
--	AUX024 = AUX0021 - AUX0022 - AUX0023
	...
	--AUXXX034 Contributi previdenziali a carico del percipiente
			...
			IF (@ycon is not null)  -- si tratta di un contratto occasionale
				select  @au35speseRimborsate =  ISNULL( (SELECT P_refund_lastyear  from #annualpayedrefund),0)
...
--	set @SS003001 = isnull(@SS003001, 0) + isnull(@au25_26ritIRPEF, 0)
-- J.T.R. Commentate per il 2009, nel 2009 si valutra se riempire il prospetto SS
