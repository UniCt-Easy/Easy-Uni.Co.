...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_varprevisione_crediti_cassa]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_varprevisione_crediti_cassa]
GO
...
--	exec rpt_varprevisione_crediti_cassa 2014, 1,20,'PREVISIONE','S','N','N',null,null,null,null,null,null,'S'

CREATE   PROCEDURE rpt_varprevisione_crediti_cassa
	@yvar          	int, 
	@nvarbegin   	int,
	@nvarend     	int,
	...
	BEGIN
		...
		WHERE
			finvar.yvar = @yvar
			AND ( finvar.variationkind <> 5 and @onlyinitial = 'N' or finvar.variationkind = 5 and @onlyinitial = 'S')
		...
		GROUP BY SK.description,S01.description,finvar.yvar, finvar.nvar,finvar.description, finvar.enactment,finvar.variationkind,finvar.nenactment,finvar.enactmentdate,finvar.adate,
			finvar.flagprevision,	finvar.flagcredit,	finvar.flagproceeds,finvar.flagsecondaryprev, config.fin_kind     	     
		ORDER BY finvar.nvar
		
	END
	IF ( @reportkind = 'CREDITI' and @onlyinitial = 'N') -- serve solo a non produrre troppe righe
	BEGIN
		...
		WHERE
			...
			AND (@idsor04 IS NULL OR upb.idsor04 = @idsor04)
			AND (@idsor05 IS NULL OR upb.idsor05 = @idsor05)		 
		GROUP BY SK.description,S01.description,finvar.yvar,finvar.nvar,finvar.description,finvar.enactment,
			finvar.variationkind,finvar.flagcredit,finvar.flagproceeds,finvar.nenactment,
			finvar.enactmentdate,finvar.adate			    	     
		ORDER BY finvar.nvar
		END
	IF (@reportkind = 'CASSA' and @onlyinitial = 'N') -- serve solo a non produrre troppe righe
	BEGIN
