...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_21]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_21]
GO
 
CREATE PROCEDURE [exp_certificazioneunica_g_21]
...
-- exec exp_certificazioneunica_g_21  478,1,'S' 
AS BEGIN 
	...
	declare @31dic2020 datetime
	set @31dic2020 = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_21  @idreg, @progrCom, 'G',NULL, @print
	...
	declare @1gen2020_XXX datetime
	set @1gen2020_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-01'})

	declare @13gen2020_XXX datetime
	set @13gen2020_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-13'})

	declare @31dic2020_XXX datetime
	set @31dic2020_XXX = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})

	declare @1gen2021_XXX datetime
	set @1gen2021_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-01'})

	declare @12gen2020_XXX datetime
	set @12gen2020_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-12'})
	...
	declare @30giu2020 datetime
	set @30giu2020 = dateadd(yy, @annoredditi-2000, {d '2000-06-30'})
	
	declare @01lug2020 datetime
	set @01lug2020 = dateadd(yy, @annoredditi-2000, {d '2000-07-01'})
	...
	SET @ritenuta_addreg_irpef =
		...
		DECLARE @ritenuta_addcom_irpef_acconto_2020 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_acconto_2020 =
		ISNULL(
		...
		SET @ritenuta_addcom_irpef_acconto_2020 =
		@ritenuta_addcom_irpef_acconto_2020 +
		ISNULL(
		...
		-- Non si filtra sulla certificazione in quanto le prestazioni che non rientrano nel CUD
		-- non pagano le addizionali regionali e quindi il loro contributo è nullo
		DECLARE @ritenuta_addcom_irpef_saldo_2020 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_saldo_2020 =
		ISNULL(
			(SELECT SUM(employtax)
			FROM #cedolini
			JOIN expenselink
	...
	SET @ritenuta_addcom_irpef_saldo_2020 =
	@ritenuta_addcom_irpef_saldo_2020 +
	ISNULL(
	...
	SELECT @ritenuta_addcom_irpef_saldo_2020  = @ritenuta_addcom_irpef_saldo_2020 - @ritenuta_addcom_irpef_acconto_2020   
	...
	if @ritenuta_addcom_irpef_saldo_2020 < 0
	begin
		set @ritenuta_addcom_irpef_acconto_2020 = @ritenuta_addcom_irpef_acconto_2020 + @ritenuta_addcom_irpef_saldo_2020
		set @ritenuta_addcom_irpef_saldo_2020 = null
	end
	else
	begin
		if @ritenuta_addcom_irpef_acconto_2020 < 0
		begin
			set @ritenuta_addcom_irpef_saldo_2020 = @ritenuta_addcom_irpef_saldo_2020 + @ritenuta_addcom_irpef_acconto_2020
			set @ritenuta_addcom_irpef_acconto_2020 = null
		end
	end
...
------------------------------------------------------------------------------------------------------ FINE CAF ---------------------------------------------------------------------------------------------------
	...
	ELSE --< 365
	BEGIN
		SELECT  @workingdays_1_semestre = count(*) from #workdays where worked='S' AND  giorno <= @30giu2020
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen2020_XXX),
		ISNULL(F.stopapplication, @31dic2020_XXX),
		ISNULL(F.starthandicap, @1gen2021_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		begin
			...
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen2020_XXX))
				...
				begin
					update #PrimoFiglio set
						...
						minoreDiTreAnni = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen2020_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese ELSE 0 end,
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						case WHEN datediff(m, dateadd(m, @mese-1, @1gen2020_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
			...
			begin
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen2020_XXX), 
	...
	DECLARE	@DB001013 INT			-- Giorni I semestre
	DECLARE	@DB001014 INT			-- Giorni II semestre
	...
	BEGIN
		...
		SET @DB001009= @stop  -- Data fine Rapporto di Lavoro Non deve essere valorizzato in caso di continuazione del rapporto di lavoro oltre il 31/12/2020 ricavabile dalla data fine del contratto inserito. N.B.: il campo 9 è alternativo al campo 10.
		...
		IF (
			...
			OR
			((Year(@start) < @annoredditi) AND (@workingdays = 365) AND (@stop < @31dic2020)  ) 

			OR (Year(@stop) < @annoredditi   ) 
			OR
				(	-- IN FORZA AL 31 12, con data ultimo cedolino dell'anno inferiore 
					(@stopcontract > @31dic2020)  AND (@stop < @31dic2020) 
				) 
			)	
			)
			AND (@workingdays >0)		
		 ...
		 ELSE 
			...
			if (	(@workingdays = 365) AND (@stopcontract > @31dic2020) /*IN FORZA AL 31 12*/ )  SET @DB001011 = NULL --- IN TAL CASO NON COMPILIAMO PERIODI PARTICOLARI


		-- DICHIARAZIONE DA CONFERMARE: indica anomalie che riguardano il conteggio dei giorni lavorati  @workingdays (@DB001006)
		--- ed eventualmente i giorni lavorati dei due semestri a partire dal 2021
		...
		(
			...
			(	(year(@start)<@annoredditi)
				...
				OR
				(
				   (@stopcontract <= @31dic2020)  AND (@stop < @stopcontract) 
				)
			) 
		)
		OR
			--- la cessazione del rapporto di lavoro prima del 1 luglio 2020 non è compatibile con la 
			--- compilazione giorni lavorati del secondo semestre
			(
				(@workingdays_2_semestre) >0 AND (@stop < @01lug2020)
			)
		)	
	...
	DECLARE	@DB001027 DECIMAL(19,2)	-- Addizionale comunale all'Irpef - Saldo 2020
	-- Si intendono i rapporti cessati entro il 31 12 dell'anno redditi
	DECLARE @DB001024 DECIMAL(19,2)	--  DB001024 Addizionale regionale 2020 rapporti cessati
	DECLARE @DB001028 DECIMAL(19,2)	--  DB001028 Addizionale comunale all'Irpef - Rapporti cessati 2020
	
	SET @DB001021= isnull(@ritenuta_irpef,0)
	SET @DB001022= isnull(@ritenuta_addreg_irpef,0)
	SET @DB001027= isnull(@ritenuta_addcom_irpef_saldo_2020,0)
	
	IF ((year(@stopcontract) = @annoredditi) AND (@stopcontract <= @31dic2020))
	BEGIN
		SET @DB001024= isnull(@ritenuta_addreg_irpef,0)
		SET @DB001028= isnull(@ritenuta_addcom_irpef_saldo_2020,0)
	END
	...
	DECLARE @DB001368 DECIMAL (19,2)	-- Ulteriore Detrazione non gestito
	...
	DECLARE @DB001400 INT				-- TRATTAMENTO INTEGRATIVO - Codice Bonus Vale 1 o 2
	DECLARE @DB001401 DECIMAL (19,2)	-- TRATTAMENTO INTEGRATIVO - Trattamento erogato
	DECLARE @DB001402 DECIMAL (19,2)	-- TRATTAMENTO INTEGRATIVO - Trattamento non erogato
	...
	BEGIN
		...
		BEGIN
			SET @DB001400= 2 -- non riconosciuto
			SET @DB001401= 0
			SET @DB001402= 0
		END
		ELSE
		BEGIN
			SET @DB001400= 1 --  riconosciuto
			SET @DB001401= @bonus_fiscale_integrativo
			SET @DB001402= 0
		END
		...
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, intero)		VALUES(@progrCom, 1, 'DB001', 1, '400', @DB001400)
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale)	VALUES(@progrCom, 1, 'DB001', 1, '401', @DB001401)
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale)	VALUES(@progrCom, 1, 'DB001', 1, '402', @DB001402)
	END
	--select * from #recordG  
	--------------------------------------------------------------------------------------------
	----------------------------------- ONERI DEDUCIBILI --------------------------------------- 
	...
	DECLARE @DB001545 DECIMAL(19,2)	    -- Addizionale comunale Acconto 2020
	DECLARE @DB001546 DECIMAL(19,2)	    -- Addizionale comunale Saldo 2020
	...
	BEGIN 
		...
		if @startpayroll < @1gen2020_XXX set @oldstart = @1gen2020_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic2020_XXX set @oldstop =  @31dic2020_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
		
			if @startpayroll < @1gen2020_XXX set @startpayroll = @1gen2020_XXX
			if @stoppayroll > @31dic2020_XXX set @stoppayroll = @31dic2020_XXX
...
------------------------------------------------------------------------------------------------------------------
	...
	IF ((@DB001022 <> 0)  --@ritenuta_addreg_irpef
		 OR 
		(@DB001027 <> 0)) --@ritenuta_addcom_irpef_saldo_2020
	BEGIN
		SET @NN =
		CASE 
	...
	AND
	(@DB001367 =  0) --@ritenuta_addcom_irpef_saldo_2020 non applicata
	BEGIN
		SET @NN =
		'BB - Saldo dell''addizionale comunale all''IRPEF non operata in quanto' + 
		' in possesso dei requisiti reddituali per usufruire interamente della fascia' +
