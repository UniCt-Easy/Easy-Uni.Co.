...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_percipienti_g_18]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_percipienti_g_18]
GO
 
--exec [exp_certificazioneunica_percipienti_g_18]  NULL
CREATE PROCEDURE [exp_certificazioneunica_percipienti_g_18]
(
	@cf varchar(20)
 )
...
AS BEGIN
	...
	CREATE TABLE #percipienti_record_G_2017
	(
		idreg int,
	...
	INSERT INTO #percipienti_record_G_2017
		(
			idreg
		)
	...
	SELECT * FROM	#percipienti_record_G_2017
