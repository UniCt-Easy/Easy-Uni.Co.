...
------- NUOVA VERSIONE ----------------
---------------------------------------
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_consuntivobudgeteconomicoufficiale_automatic]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_consuntivobudgeteconomicoufficiale_automatic]
GO
...
CREATE      PROCEDURE [rpt_consuntivobudgeteconomicoufficiale_automatic](
 ...
 declare @cursore cursor
	...
	begin
		...
		SET @_entry = ISNULL(( SELECT SUM(entrydetail.amount)
		   ...
		   update #dati set _var =  isnull(@_prevcorr,0)  - isnull(@_initprev,0) where label = @label
		   update #dati set _prevcorr = isnull(@_prevcorr,0) where label = @label
		   ...
		   update #dati set _saldo =  isnull(@_prevcorr,0)  - isnull(@_preacc,0)  - isnull(@_preimp,0) where label = @label
...
UPDATE #dati SET _var =  (select sum(_var * segno) FROM #dati child
where child.parent_label = #dati.label)		where  #dati.label = 'RISULTATO ECONOMICO'

UPDATE #dati SET _prevcorr =  (select sum(_prevcorr * segno) FROM #dati child
where child.parent_label = #dati.label)		where  #dati.label = 'RISULTATO ECONOMICO'

UPDATE #dati SET _saldo =  (select sum(_saldo * segno) FROM #dati child
where child.parent_label = #dati.label)		where  #dati.label = 'RISULTATO ECONOMICO'

UPDATE #dati SET _entry = (select sum(_entry * segno) FROM #dati child
where child.parent_label = #dati.label)		where  #dati.label = 'RISULTATO ECONOMICO'

UPDATE #dati SET _nonrealizzato = (select sum(_nonrealizzato * segno) FROM #dati child
where child.parent_label = #dati.label)		where  #dati.label = 'RISULTATO ECONOMICO'
