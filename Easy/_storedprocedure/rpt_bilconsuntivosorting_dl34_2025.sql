...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_bilconsuntivosorting_dl34_2025]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_bilconsuntivosorting_dl34_2025]
GO
...
-- exec rpt_bilconsuntivosorting_dl34_2025 2025, {ts '2025-07-16 00:00:00'}, 32, '%', 'N', 'S', NULL, NULL, NULL, NULL, NULL

CREATE       PROCEDURE [rpt_bilconsuntivosorting_dl34_2025]
(
	@ayear int,
	...
	@idsorkind_dl34_2025 int, --> Classificazione Dl 34/2025 Spese
	@idupb varchar(36)='%',
	@showupb char(1)='N',
...
FROM sortinglevel
WHERE idsorkind = @idsorkind_dl34_2025
...
--WHERE idsorkind = @idsorkind_dl34_2025 AND (flag&2)<>0
...
DECLARE @maxphase tinyint
	...
	SELECT @maxphase = MAX(nphase) FROM expensephase
		...
		where f5.ayear = @ayear
				...
				AND sl.idsorkind = @idsorkind_dl34_2025
				AND SortSiope.idsorkind = @idsorkind_dl34_2025
				...
				AND (SortSiope.nlevel = @levelusable
					OR (SortSiope.nlevel < @levelusable
						and (select count(*) from sorting S where S.idsorkind = @idsorkind_dl34_2025 and S.paridsor = SortSiope.idsor)=0
						AND (sl.flag&2)<>0
					   )
					)
		...
		JOIN expenselast ILAST
			on ILAST.idexp = HPV.idexp
		JOIN finsorting FS		ON FS.idfin = F.idfin	
		JOIN sorting SortSiope		ON SortSiope.idsor = FS.idsor				
		JOIN sortinglevel sl	ON SortSiope.nlevel = sl.nlevel 
		LEFT OUTER JOIN sortinglink SLK on SLK.idchild = SortSiope.idsor and SLK.nlevel = @levelusable_original
...
BEGIN
   ...
   if exists(select * from fin F 
					...
					AND (SortSiope.nlevel = @levelusable
						OR (SortSiope.nlevel < @levelusable
							and (select count(*) from sorting S 
								 where S.idsorkind = @idsorkind_dl34_2025 and S.paridsor = SortSiope.idsor)=0
							AND (sl.flag&2)<>0)
						)
					and D.idsor is null
				)
	BEGIN	
		insert into #data (idsor,idupb,pagamenti,flagadvance)
	...
	UPDATE #data	SET pagamenti = ISNULL(initialprevision, 0) + ISNULL(variation, 0)
		where  flagadvance = 'S'
...
where isnull(pagamenti  ,0)<>0 
	 

DECLARE @lencod_lev1 int
SELECT @lencod_lev1 = flag / 256 FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 1
DECLARE @startpos1 int
SELECT @startpos1 = 1
DECLARE @lencod_lev2 int
SELECT @lencod_lev2 = flag / 256 FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 2
DECLARE @startpos2 int
SELECT @startpos2 = @startpos1 + @lencod_lev1
DECLARE @lencod_lev3 int
SELECT @lencod_lev3 = flag / 256 FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 3
DECLARE @startpos3 int
SELECT @startpos3 = @startpos2 + @lencod_lev2
DECLARE @lencod_lev4 int
SELECT @lencod_lev4 = flag / 256 FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 4
DECLARE @startpos4 int
SELECT @startpos4 = @startpos3 + @lencod_lev3
DECLARE @lencod_lev5 int 
SELECT @lencod_lev5 = flag / 256 FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 5
DECLARE @startpos5 int 
SELECT @startpos5 = @startpos4 + @lencod_lev4
 
DECLARE @lev_desc1 varchar(50)
SELECT @lev_desc1 = description FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 1
DECLARE @lev_desc2 varchar(50)
SELECT @lev_desc2 = description FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 2
DECLARE @lev_desc3 varchar(50)
SELECT @lev_desc3 = description FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 3
DECLARE @lev_desc4 varchar(50)
SELECT @lev_desc4 = description FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 4
DECLARE @lev_desc5 varchar(50)
SELECT @lev_desc5 = description FROM sortinglevel WHERE idsorkind = @idsorkind_dl34_2025 AND nlevel = 5
...
select @sortingkind = description from sortingkind where idsorkind = @idsorkind_dl34_2025
...
select @CountRoot = count(distinct s.sortcode) from sorting s
	...
	where s.idsorkind = @idsorkind_dl34_2025 
...
FROM sorting f
	JOIN sortinglevel fl
				ON f.nlevel = fl.nlevel AND fl.idsorkind = @idsorkind_dl34_2025
cross join  upb 
...
GROUP BY upb.printingorder,upb.idupb,upb.title,upb.codeupb,
	...
	showexpensesurplus,f.nlevel,isnull(#data.tosuppress, @suppressifblank)
...
SELECT 
	...
	f.nlevel
FROM  sorting f
		JOIN sortinglevel fl			ON f.nlevel = fl.nlevel AND  fl.idsorkind = @idsorkind_dl34_2025
	left outer join #data				on #data.idsor=f.idsor 
	LEFT OUTER JOIN sortinglink flk1	ON flk1.idchild = f.idsor AND flk1.nlevel = 1
	LEFT OUTER JOIN sorting f1			ON flk1.idparent = f1.idsor and f1.idsorkind = @idsorkind_dl34_2025
	LEFT OUTER JOIN sortinglink flk2	ON flk2.idchild = f.idsor AND flk2.nlevel = 2
	LEFT OUTER JOIN sorting f2			ON flk2.idparent = f2.idsor and f2.idsorkind = @idsorkind_dl34_2025
	LEFT OUTER JOIN sortinglink flk3	ON flk3.idchild = f.idsor AND flk3.nlevel = 3
	LEFT OUTER JOIN sorting f3			ON flk3.idparent = f3.idsor  and f3.idsorkind = @idsorkind_dl34_2025
	LEFT OUTER JOIN sortinglink flk4	ON flk4.idchild = f.idsor AND flk4.nlevel = 4
	LEFT OUTER JOIN sorting f4			ON flk4.idparent = f4.idsor  and f4.idsorkind = @idsorkind_dl34_2025
	LEFT OUTER JOIN sortinglink flk5	ON flk5.idchild = f.idsor AND flk5.nlevel = 5
	LEFT OUTER JOIN sorting f5			ON flk5.idparent = f5.idsor  and f5.idsorkind = @idsorkind_dl34_2025
