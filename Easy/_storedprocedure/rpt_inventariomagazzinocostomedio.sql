if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_inventariomagazzinocostomedio]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_inventariomagazzinocostomedio]
GO
...
CREATE       PROCEDURE rpt_inventariomagazzinocostomedio(
	@idstore int,
	@adate datetime
)

AS BEGIN
--	 exec rpt_inventariomagazzinocostomedio null, { ts '2011-06-13 00:00:00'}
...
(select  SUM(S.number * ROUND((S.amount/S.number),5)) / sum(S.number)
  from stock S 
  where S.idlist=L.idlist and  M.idstore = S.idstore)
FROM list L
JOIN stock S
...
SELECT 
	...
	Giacenza ,
	costomedio
FROM #INVENTARIO where Giacenza>0
