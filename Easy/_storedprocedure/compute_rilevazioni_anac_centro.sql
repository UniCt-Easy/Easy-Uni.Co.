...
if exists (select * from dbo.sysobjects where id = object_id(N'[compute_rilevazioni_anac_centro]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [compute_rilevazioni_anac_centro]
GO
...
--exec compute_rilevazioni_anac_centro 2020, null
-- select * from mandateview
CREATE PROCEDURE [compute_rilevazioni_anac_centro](
	@ayear int,
...
where MD.idexp_taxable is not null AND (MD.stop is null ) -- non annullato 
...
select 
		...
		S.sortcode as 'Codice Centro',
		S.description as 'Descrizione Centro'
	from mandateview M
	...
	left outer join sorting S on M.idsor01 = S.idsor
