...
-- exp_partitario_debiti_crediti_test '2024', {d '2024-01-01'}, {d '2024-11-06'}, '1', null, 'E', 'PD0902010101', '%', 'N', 'N', 37878, 'T', 'N', 'N', null, null, null, null, null
...
(
	...
	@onlyepopen char(1)='N',--considerare solo le scritture di apertura.
	@idsor01 int = null,
	@idsor02 int = null,
	@idsor03 int = null,
	@idsor04 int = null,
...
SET @lenfilteracc = DATALENGTH(RTRIM(ISNULL(@filteraccount,''))) --print @lenfilteracc
	...
	CREATE TABLE #bilanciostatopatrimoniale(
		yoriginal int,
		adate datetime,
		entry_idrelated  varchar(100), 
		entry_adate datetime,
...
-- Calcola la lunghezza del filtro in base ad nlevel		
	...
	INSERT INTO #bilanciostatopatrimoniale(
		adate,				--> questa viene valorizzata SOLO se manca idrelated
		entry_idrelated, 
		entry_adate,		--> questa viene valorizzata per TUTTI e usata nel caso in cui dalla decodifica dell'idrelated non Ë stata valorizzata una data apertura debito/credito
		idpatrimony,
		idupb,
		dare,avere,
		idacc,
		...
		WHERE entry.yentry = @ayear				
			...
			and (@onlyepopen ='S' and entry.identrykind = 7 or @onlyepopen='N')
...
--)
--select '#bilanciostatopatrimoniale',* from #bilanciostatopatrimoniale
...
select * from [fn_decode_idrelated_tab] (@lista_id) 
;
--select * from #bilanciostatopatrimoniale where idrelated = 'payrollß31944'
--select * from #Tdrel where idrelated = 'payrollß31944'
--select * from #bilanciostatopatrimoniale;
-- WITH RankedDates AS (
--    SELECT 
--        idrelated,
--        entry_adate,
--        ROW_NUMBER() OVER (PARTITION BY  idrelated ORDER BY 
--					 CASE WHEN entry_idrelated like 'paytrans%' or entry_idrelated like 'protrans%'  THEN null  ELSE entry_adate  END  DESC
--		) AS rank 
--        FROM #bilanciostatopatrimoniale WHERE idrelated is not null
--) 
-- SELECT * FROM RankedDates order by idrelated
IF (@showupb ='N')
BEGIN
-- RankedDates.ROW_NUMBER(): Genera un numero di riga ordinando ogni gruppo (idrelated) 
-- in base a una data calcolata pari a 1) entry_adate, se Ë scrittura di documento e 2) la data nulla,  se Ë  scrittura di trasmissione)
-- rank = 1: Filtra solo la riga con il valore migliore (la data della scrittura del documento).
 WITH RankedDates AS (
    SELECT 
        idrelated,
        entry_adate,
	...
	SELECT  
		...
		CASE WHEN (	#bilanciostatopatrimoniale.idrelated IS NOT NULL) THEN	#Tdrel.rifdoccollegato ELSE ' ' END  as 'Contratto collegato',
		...
		CASE WHEN (	#bilanciostatopatrimoniale.idrelated IS NOT NULL) 
					THEN ISNULL(#Tdrel.daterif,RankedDates.entry_adate) 
					ELSE #bilanciostatopatrimoniale.adate  END as 'Data apertura Debito e Credito',
		CASE WHEN (	#bilanciostatopatrimoniale.idrelated IS NOT NULL) 
					THEN  year(isnull(#Tdrel.daterif,RankedDates.entry_adate)) 
					ELSE year(#bilanciostatopatrimoniale.adate)  END as 'Anno apertura Credito/ Debito',
		...
		LEFT OUTER join RankedDates on #bilanciostatopatrimoniale.idrelated = RankedDates.idrelated AND isnull(RankedDates.rank,1) = 1
		...
		GROUP BY registry.title, registry.idreg, account.codeacc,account.title, patrimony.codepatrimony, patrimony.patpart,patrimony.title,
				#bilanciostatopatrimoniale.idrelated, #Tdrel.kind, #Tdrel.rifdoc,#Tdrel.rifdoccollegato,#Tdrel.docdate,#bilanciostatopatrimoniale.adate,
				isnull(#Tdrel.daterif,RankedDates.entry_adate)
 ...
 WITH RankedDates AS (
	...
	SELECT  
		...
		CASE WHEN (	#bilanciostatopatrimoniale.idrelated IS NOT NULL) 
					THEN ISNULL(#Tdrel.daterif,RankedDates.entry_adate) 
					ELSE #bilanciostatopatrimoniale.adate  END as 'Data apertura Debito e Credito',
		CASE WHEN (	#bilanciostatopatrimoniale.idrelated IS NOT NULL) THEN  year(isnull(#Tdrel.daterif,RankedDates.entry_adate)) 
			ELSE year(#bilanciostatopatrimoniale.adate)  END as 'Anno apertura Credito/ Debito',
		...
		left outer join #Tdrel on #bilanciostatopatrimoniale.idrelated  = #Tdrel.idrelated  
		left outer join RankedDates on #bilanciostatopatrimoniale.idrelated = RankedDates.idrelated
		where patrimony.ayear = @ayear and (@patpart = 'E' or patrimony.patpart = @patpart)  AND isnull(RankedDates.rank,1) = 1
		GROUP BY registry.title, registry.idreg, account.codeacc, account.title, patrimony.codepatrimony,patrimony.patpart, patrimony.title, 
		...
		#Tdrel.kind, #Tdrel.rifdoc, #Tdrel.rifdoccollegato,#Tdrel.docdate, #Tdrel.daterif,	#bilanciostatopatrimoniale.adate, 
		isnull(#Tdrel.daterif,RankedDates.entry_adate)
		HAVING (sum(#bilanciostatopatrimoniale.dare)<>0 or sum(#bilanciostatopatrimoniale.avere) <>0) AND
		(@suppressifblank = 'N'  OR (sum(isnull(#bilanciostatopatrimoniale.dare,0) + isnull(#bilanciostatopatrimoniale.avere,0))  <> 0) )
