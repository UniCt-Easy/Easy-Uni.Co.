...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_situazionebudget_new]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_situazionebudget_new]
GO
...
-- exec exp_situazionebudget_new  2024, {ts '2024-12-09 00:00:00'}, 'S', '%', 'S', '%', NULL, NULL, NULL, NULL, NULL

CREATE PROCEDURE [exp_situazionebudget_new]
	@ayear	    	int,
	@stop    	datetime,
	@showupb char(1)='N',
	@idupb varchar(36)='%',
...
create table  #costipagati (		
			...
			EXEC  exp_budgetcostipagati @ayear, @idupb, @codeacc, 'R'
...
select 
		O.ayear	as Esercizio	,
		O.codeacc as CodConto				,
		O.codeupb		as CodUpb		,
		O.accountkind			,
		sum(O.init_amount)	as PrevIniziale		,
		sum(O.var_amount)	as VarPrevIniziale		,
		sum(O.fase1_amount)	as Preimpegni	,
		sum(O.fase2_amount) as Impegni		,
		sum(O.available_amount)	as disponibile,
		sum(O.payed) as costipagati
from #output O
group by O.ayear,
		O.codeacc,
		O.codeupb,
		O.accountkind	
ORDER BY  O.accountkind,   O.ayear

END

go

-- exec exp_situazionebudget_new  2024, {ts '2024-12-09 00:00:00'}, 'S', '00010001000200010004', 'S', 'CB07a0016.0001', NULL, NULL, NULL, NULL, NULL
-- exec exp_situazionebudget_new  2024, {ts '2024-12-09 00:00:00'}, 'S', '%', 'S', '%', NULL, NULL, NULL, NULL, NULL
