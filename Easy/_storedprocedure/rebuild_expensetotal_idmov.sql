...
-- Questa rebuild va eseguita se si è certi che expenseyear e expensevar contengano valori corretti,
-- perchè valorizza expensetotal sulla base dei loro valori.
...
if exists (select * from dbo.sysobjects where id = object_id(N'[rebuild_expensetotal_idmov]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rebuild_expensetotal_idmov]
GO

CREATE   PROCEDURE  rebuild_expensetotal_idmov
(
	@ayear int =  NULL,
	@idmov int
)
AS BEGIN
	...
	DELETE from expensetotal where ( (@ayear is null)  OR ayear=@ayear ) and idexp = @idmov 
...
--bit 1: firstyear
		...
		WHERE  ((@ayear is null)  OR ayear=@ayear  ) and expense.idexp = @idmov  ;

		with sumvar(idexp,yvar,amount) as 
		(select idexp,yvar,sum(amount) from expensevar 
				where ( (@ayear is null) OR expensevar.yvar=@ayear) and expensevar.idexp = @idmov 
				group by idexp,yvar)
		...
		where ((@ayear is null) OR expensetotal.ayear=@ayear) and expensetotal.idexp = @idmov 
	...
	;
		...
		(select E.parentidexp, ET.ayear, SUM(ET.curramount) from 
				...
				WHERE ( (@ayear is null) OR ET.ayear=@ayear) and e.parentidexp = @idmov 
		...
		where ((@ayear is null) OR ET.ayear=@ayear)and et.idexp = @idmov 
;
