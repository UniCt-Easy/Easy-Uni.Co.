 ...
 if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_bpcassinate_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_bpcassinate_var]
GO
...
CREATE        PROCEDURE [trasmele_expense_bpcassinate_var]
(
	@y int,
	@n int
)
...
INSERT INTO #payment (ypaymenttransmission, npaymenttransmission, kpay,ydoc, ndoc, idexp, 
	ymov, nmov, nphase, phase,
...
and #payment.idpaydisposition is null
				
 UPDATE #payment SET originalamount = isnull(exp_curramount,0) 
									- ISNULL((select sum(expensevar.amount)  -- Le var. sono negative, quindi col meno d'avanti, cambio il segno.
									FROM expensevar
									join paymenttransmission 
										ON expensevar.kpaymenttransmission = paymenttransmission.kpaymenttransmission
...
WHERE #payment.opkind = 'ANNULLO'
and #payment.idpaydisposition is not null
...
WHERE I.nphase = @maxincomephase
	AND ((I.autokind = 6) /* <--Recupero*/	OR (I.autokind = 14) /*<--automatismo generico*/OR (I.autokind = 4 AND I.idreg = P.idreg)/*<--Ritenuta*/  OR (I.autokind in (20,21,30,31) AND I.idreg = P.idreg ))
	AND IT.ayear = @y
