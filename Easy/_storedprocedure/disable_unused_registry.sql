...
if exists (select * from dbo.sysobjects where id = object_id(N'[disable_unused_registry]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [disable_unused_registry]
GO

CREATE PROCEDURE disable_unused_registry (
	@esercizio int
)
AS 
BEGIN
...
UPDATE #Object SET ayear = 'ayear' WHERE tablename = 'unifiedtax'
UPDATE #Object SET ayear = 'ayear' WHERE tablename = 'unifiedtaxcorrige'

DELETE FROM #Object where tablename in ('payment_bank','proceeds_bank')--> le escludiamo perchè non hanno l'anno e sono inglobate in income/expense
...
BEGIN
-- Lo scritp interroga incomeview/expenseview per leggere ayear.
...
/*select registry.lt, registry.cu, #reg.usable , registry.active,* from #reg 
join registry
	on #reg.idreg = registry.idreg
where  #reg.usable = 'N'
and registry.lt < {ts '2011-03-14 00:00:00'}
order by registry.lt
3460
*/


UPDATE registry SET active = 'N', lu='myscript'
				FROM #reg
