...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_buono_ordine_sun]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_buono_ordine_sun]
GO
...
CREATE      PROCEDURE [rpt_buono_ordine_sun]
	@ayear int,
	@printkind char(1),
...
-- exec rpt_buono_ordine_sun 2021, 'I', 'AMM_BO_IST_COSV_SF', 1, 1, NULL, 'N', 'N', {d '2021-11-08'}, 'N', NULL, NULL, NULL, NULL, NULL
IF (@ayear=0)
BEGIN
	SET @ayear='1900'
END
...
left join epexpyear on epexpyear.idepexp = mandatedetail.idepexp and epexpyear.ayear = @ayear
left join upb upb2 on upb2.idupb = epexpyear.idupb
left join account on account.idacc = epexpyear.idacc
WHERE mandate.yman = @ayear
...
select 	yman, 
	...
	#mandatedetail_det.codeupb,
	#mandatedetail_det.titleupb,	
	#mandatedetail_det.codeacc,
	#mandatedetail_det.titleacc
...
-- Il campo totaleriga deve essere calcolato come la somma dell'imponibile scontato + l'iva 
	...
	,#mandatedetail.ivadescription, --task 10649
	 ...
	 #mandatedetail.codeupb,
	 #mandatedetail.titleupb,
	 #mandatedetail.codeacc,
	 #mandatedetail.titleacc
FROM #mandate
JOIN mandate
