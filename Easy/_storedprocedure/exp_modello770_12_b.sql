...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_12_b]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_12_b]
GO
...
-- exec exp_modello770_12_b 1,1
create PROCEDURE [exp_modello770_12_b]
(
	@numdichquadrog int,
	@numdichquadroh int
)
...
-- 121	Codice stato estero
	INSERT INTO #recordb (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRB', 1, '121', @manager_r_code_e)
-- 122	Stato federato, provincia, contea
	INSERT INTO #recordb (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRB', 1, '122', @manager_r_nation_e)
-- 123	Località di residenza
	INSERT INTO #recordb (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRB', 1, '123', @manager_r_location_e)
-- 124	Indirizzo estero
	INSERT INTO #recordb (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRB', 1, '124', @manager_r_address_e)
-- 125	telefono - cellulare del rappresentante
	INSERT INTO #recordb (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRB', 1, '125', @manager_phonenumber)
-- 126	Data apertura fallimento
-- 127	Codice fiscale societa o ente dichiarante
	INSERT INTO #recordb (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRB', 1, '127', @2cf)

-- 152 - Campo da impostare con il valore 1 se nella compilazione delle colonne da 2 a 7 del rigo SX4 hanno concorso importi inferiori all'unità di euro

-- NB: Il seguente campo viene valorizzato perchè va in errore la procedura di validazione. L'errore è il seguente
-- Dati del Frontespizio:
-- Campo da impostare con valore 1 se nella compilazione delle colonne da 2 a 7 del rigo SX4 hanno concorso importi inferiori all'unita' di euro - Valore non ammesso
-- Il campo Vale 0 o 1, è stato posto a 1 perchè noi attualmente non compiliamo il quadro SX, quindi gli importi sono iinferiori all'unita' di euro.
	INSERT INTO #recordb (progr, quadro, riga, colonna, stringa) VALUES(1, 'HRB', 1, '152', 1) 
