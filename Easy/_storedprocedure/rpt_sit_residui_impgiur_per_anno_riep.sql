...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_sit_residui_impgiur_per_anno_riep]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_sit_residui_impgiur_per_anno_riep]
GO


CREATE PROCEDURE [rpt_sit_residui_impgiur_per_anno_riep]
	@ayear 			int,
	@date			datetime,
	@finpart 		char(1),
	@levelusable 		tinyint,
...
-- exec rpt_sit_residui_impgiur_per_anno_riep 2009,{ts '2009-06-07 00:00:00'},'S',3,'0001','S','N'
-- exec rpt_sit_residui_impgiur_per_anno_riep 2008,{ts '2008-12-31 00:00:00'},'S',3,'%','S','S'
...
SET cash_residual =	
	...
	group by  #cash_residual.idupb,#cash_residual.idfin,#cash_residual.ayear1 --- F1
	), 0.0)
	+
	ISNULL((SELECT sum(#cash_residual.total) FROM #cash_residual
	WHERE #cash_residual.idfin = #report_residual.idfin
...
--select * from #report_residual
--select @idupboriginal
...
SELECT 
	...
	upb,
	upbprintingorder,
	isnull(sum(initial_residual),0) initial_residual,
	isnull(sum(var_residual),0) var_residual,
	isnull(sum(cash_residual),0) cash_residual,
	isnull(sum(var_cash_residual),0) var_cash_residual
FROM #report_residual
