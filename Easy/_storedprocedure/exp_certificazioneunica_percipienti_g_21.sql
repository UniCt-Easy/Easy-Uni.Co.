...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_percipienti_g_21]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_percipienti_g_21]
GO
 
--exec [exp_certificazioneunica_percipienti_g_21]  NULL
CREATE PROCEDURE [exp_certificazioneunica_percipienti_g_21]
(
	@cf varchar(20)
 )
...
AS BEGIN
	...
	CREATE TABLE #percipienti_record_G_2020
	(
		idreg int,
	...
	INSERT INTO #percipienti_record_G_2020
		(
			idreg
		)
	...
	SELECT * FROM	#percipienti_record_G_2020
