...
if exists (select * from dbo.sysobjects where id = object_id(N'[sp_email_missioni_del_giorno_sdoppiate]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [sp_email_missioni_del_giorno_sdoppiate]
GO
...
CREATE   PROCEDURE [sp_email_missioni_del_giorno_sdoppiate]
(
...
, @missioni_impegnate VARCHAR(1) = NULL -- N (non impegnate), I (impegnate), T (tutte)
)
...
declare @tipo_elenco NVARCHAR(MAX)

if (@missioni_impegnate = 'T' OR @missioni_impegnate is null)
begin set @tipo_elenco = 'Resoconto missioni da elaborare (impegnate e non impegnate)' end
else if (@missioni_impegnate = 'N')
begin set @tipo_elenco = 'Resoconto missioni da impegnare' end
else if (@missioni_impegnate = 'I')
begin set @tipo_elenco = 'Resoconto missioni impegnate da elaborare' end

set @message_subject = @tipo_elenco + '. Data: ' + convert(varchar,format(convert(date,getdate()),'dd-MM-yyyy') ) + ' - Utente: ' + @idcustomuser + ' - Ruolo: ' + @title
...
CASE isnull(SI.idsor,'')
	WHEN '' THEN 'Missione da impegnare' 
	ELSE 'Missione impegnata'
 END as [Class. contabile]
...
LEFT JOIN itinerationsorting  SI		with (nolock)	on SI.iditineration= itineration.iditineration
where 
...
AND (
			...
			--AND -- Controllo eseguito in basso in base al residuo da erogare solo per le missioni impegnate
...
AND 
( isnull(SI.idsor,'0') like
  CASE 
  WHEN @missioni_impegnate = 'N' THEN '0' 
  WHEN @missioni_impegnate = 'I' THEN '21633'
  ELSE '%'
  END
)

AND -- Nel caso la missione sia impegnata allora mi serve anche il residuo da pagare altrimenti no
( 
	(	IR.totalgross-
			(case when linkedsaldo>0 
				then IR.linkedsaldo+IR.linkedanpag 
  ...
  CASE 
  WHEN @missioni_impegnate = 'N' THEN -1
  WHEN @missioni_impegnate = 'I' THEN 0
  ELSE 0
  END
)
...
--setuser
--[amministrazione].[sp_email_missioni_del_giorno_sdoppiate] '','','sonia.lombardo@unict.it ',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'T'
--[amministrazione].[sp_email_missioni_del_giorno_sdoppiate] '','','sonia.lombardo@unict.it ',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'N'
--[amministrazione].[sp_email_missioni_del_giorno_sdoppiate] '','','sonia.lombardo@unict.it ',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'I'
