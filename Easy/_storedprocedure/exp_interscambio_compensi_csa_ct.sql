...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_interscambio_compensi_csa_ct]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_interscambio_compensi_csa_ct]
GO
...
CREATE PROCEDURE [exp_interscambio_compensi_csa_ct]
(
...
--- inseriamo le missioni assumendole tutte Italia per il momento  --'I', '8159','8158'
...
capitolo_e,ymov,nmov,transmissiondate,codeser,service,module,itinerationvisible,idregistrylegalstatus, flag_geo,voceimponibile,vocequotaesente,ritenute)
...
CASE
	WHEN exists (select * from  expensetaxofficial RT WHERE RT.idexp = EL.idexp) THEN 'S'
	ELSE 'N'
END 
FROM itineration I 
...
--SELECT '#Missione',* FROM #Missione order by iditineration
...
flagtaxableexpense char(1)
)

;with Missione(iditineration, flag_geo, voce8000refund_i, voce8000refund_e )
as (
SELECT DISTINCT iditineration, flag_geo, voce8000refund_i, voce8000refund_e 
FROM #Missione
)
--- inserisco le sole spese missione a saldo, imponibili, diverse dal tipo @idtipo_rimborso_forfettario
INSERT INTO #SpeseMissione(iditineration, flagadvancebalance, flag_geo, amount, voce8000refund_i,voce8000refund_e, flagtaxableexpense)
SELECT
S.iditineration,
...
CASE  WHEN (( S.flagtaxableexpense & 1) = 0)  then 'N'  ELSE 'S' END  --  Spesa imponibile
...
AND (( S.flagtaxableexpense & 1) <> 0)   --inserisco solo le spese tracciabili
AND S.iditinerationrefundkind <> @idtipo_rimborso_forfettario
GROUP BY  S.iditineration, S.flagadvancebalance, S.flag_geo, M.voce8000refund_i, M.voce8000refund_e,( S.flagtaxableexpense & 1)
HAVING SUM(S.amount) <> 0

-- Non considero i rimborsi chilometrici  perchè non sono tassabili
...
FROM #DiariaMissione WHERE #Missione.iditineration = #DiariaMissione.iditineration
...
--AND ISNULL(S.itinerationvisible,'N')= 'N'
	AND  (LTRIM(RTRIM(R.extmatricula)) = LTRIM(RTRIM(@extmatricula)) OR @extmatricula IS NULL)
	AND M.ritenute = 'S'
GROUP BY S.flagcsausability ,E.idreg, R.extmatricula, EL.idexp,PT.transmissiondate,	E.ymov,E.nmov,	S.codeser,M.ritenute ,
		S.description, S.module, S.itinerationvisible ,	V.voceimponibile,ISNULL(V.capitoloimponibile,V8.capitolo),V.flagcsausability
...
GROUP BY S.flagcsausability ,E.idreg, R.extmatricula, EL.idexp,PT.transmissiondate,
E.ymov,E.nmov,	S.codeser,S.description, S.module, S.itinerationvisible ,	V.vocequotaesente,  
ISNULL(V.capitoloquotaesente,V8.capitolo),V.flagcsausability,M.ritenute
...
--AND ISNULL(S.itinerationvisible,'N')= 'N'
		AND  (LTRIM(RTRIM(R.extmatricula)) = LTRIM(RTRIM(@extmatricula)) OR @extmatricula IS NULL)
		AND M.ritenute = 'S'
GROUP BY S.flagcsausability , EL.idexp, V.voce,	E.idreg, R.extmatricula,PT.transmissiondate,V8.flag_geo,V.voceimponibile,M.ritenute,
		E.ymov,E.nmov,S.codeser,S.description, S.module, S.itinerationvisible , ISNULL(V.capitolovoce,V8.capitolo),V.flagcsausability
...
WHERE P.ypay = @ayear
	...
	AND M.ritenute = 'S'
GROUP BY S.flagcsausability ,EL.idexp, V.voce,ISNULL(V.capitolovoce,V8.capitolo),	E.idreg, R.extmatricula, PT.transmissiondate,
E.ymov,E.nmov,S.codeser,S.description, S.module, S.itinerationvisible , V.flagcsausability, M.ritenute 
