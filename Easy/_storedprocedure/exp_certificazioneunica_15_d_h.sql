...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_15_d_h]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_15_d_h]
GO
 
--exec exp_certificazioneunica_15_d_h  
CREATE PROCEDURE [exp_certificazioneunica_15_d_h]
...
AS BEGIN
	...
	CREATE TABLE #prestazioni_trasmesse_2014
	(
		idexp int,
		idreg int,
		idser int,
	...
	INSERT INTO #prestazioni_trasmesse_2014
		(
			idexp, 
			idreg, 
			idser
	...
	update #prestazioni_trasmesse_2014 set motive770 = motive770service.idmot
		from motive770service 
		where motive770service.idser = #prestazioni_trasmesse_2014.idser
...
service.codeser  as 'Codice Prestazione',
service.description  as 'Prestazione',
motive770.idmot as 'Codice Causale',
motive770.description as ' Descr. Causale'
 FROM #prestazioni_trasmesse_2014
JOIN expenseview  
	ON #prestazioni_trasmesse_2014.idexp = expenseview.idexp
JOIN registry 
	ON #prestazioni_trasmesse_2014.idreg = registry.idreg
JOIN service 
	ON #prestazioni_trasmesse_2014.idser = service.idser
...
SELECT DISTINCT  isnull(cf,p_iva), #prestazioni_trasmesse_2014.idreg, title FROM #prestazioni_trasmesse_2014
		join registry on registry.idreg = #prestazioni_trasmesse_2014.idreg
		order by title
OPEN registry_crs
...
FETCH NEXT FROM registry_crs into @au1cf, @idreg, @title
	...
	SELECT @au1cf,@idreg,@title
		--INSERT INTO #recD_H
		--EXEC [exp_certificazioneunica_d_15]  @idreg, @progrCom
		
		INSERT INTO #recD_H
		EXEC [exp_certificazioneunica_h_15]  @idreg, @progrCom
	...
	SELECT * FROM #recD_H
DROP TABLE #prestazioni_trasmesse_2014
END
