if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_budgetinvestimentiufficiale]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_budgetinvestimentiufficiale]
GO
...
-- exec rpt_budgetinvestimentiufficiale 2018, 18,'%','S',null,null,null,null,null, 'S','N'
CREATE      PROCEDURE [rpt_budgetinvestimentiufficiale](
...
if (@budgetpuro='S') 
	Begin
		exec rpt_budgetinvestimentiufficiale_puro @ayear, @idupb,@showchildupb,	@idsor01,@idsor02,@idsor03,	@idsor04,@idsor05,@show_investimentiprogettiricerca
		RETURN
	End
...
insert into  #budget_derivato (code, sortcode, description,printingorder, amount_c,amount_i, amount_p, amount )
