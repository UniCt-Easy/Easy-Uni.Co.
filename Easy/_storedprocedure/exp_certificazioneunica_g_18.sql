...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_18]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_18]
GO
 
CREATE PROCEDURE [exp_certificazioneunica_g_18]
...
-- exec exp_certificazioneunica_g_18  25438,1,'N' 
AS BEGIN 
...
-- Sezione dichiarativa	
	...
	exec exp_certificazioneunica_d_18  @idreg, @progrCom, 'G',NULL, @print
	...
	set @1gen17_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-01'})

	declare @13gen17_XXX datetime
	set @13gen17_XXX = dateadd(yy, @annoredditi-2000, {d '2000-01-13'})

	declare @31dic17_XXX datetime
	set @31dic17_XXX = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})

	declare @1gen18_XXX datetime
	set @1gen18_XXX = dateadd(yy, @annodichiarazione-2000, {d '2000-01-01'})
...
--setuser 'amm'
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen17_XXX),
		ISNULL(F.stopapplication, @31dic17_XXX),
		ISNULL(F.starthandicap, @1gen18_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		begin
			...
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen17_XXX))
				...
				begin
					update #PrimoFiglio set
						...
						minoreDiTreAnni = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen17_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese ELSE 0 end,
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						case WHEN datediff(m, dateadd(m, @mese-1, @1gen17_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
			...
			begin
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
								case WHEN datediff(m, dateadd(m, @mese-1, @1gen17_XXX), 
	...
	BEGIN 
		...
		if @startpayroll < @1gen17_XXX set @oldstart = @1gen17_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic17_XXX set @oldstop =  @31dic17_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
		
			if @startpayroll < @1gen17_XXX set @startpayroll = @1gen17_XXX
			if @stoppayroll > @31dic17_XXX set @stoppayroll = @31dic17_XXX
