...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_budgeteconomicoufficialeDI34_2025_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_budgeteconomicoufficialeDI34_2025_var]
GO
...
SET ANSI_NULLS ON 
GO
-- exec rpt_budgeteconomicoufficialeDI34_2025_var 2025, {d '2025-03-12'}, 22,'%','S',null,null,null,null,null
CREATE PROCEDURE [rpt_budgeteconomicoufficialeDI34_2025_var] (
...
declare @A_III_ProventiPerGestioneDiretta_var decimal(19,2)
SELECT	@A_III_ProventiPerGestioneDiretta_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@A_III_ProventiPerGestioneDiretta = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @A_IV1_UtilizzoRiservePatrimonioNetto_var decimal(19,2)
SELECT	@A_IV1_UtilizzoRiservePatrimonioNetto_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@A_IV1_UtilizzoRiservePatrimonioNetto = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @A_IV2_AltriProventi_var decimal(19,2)
SELECT	@A_IV2_AltriProventi_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@A_IV2_AltriProventi = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @A_IV_UtilizzoRiservePatrimonioNetto_var decimal(19,2)
set @A_IV_UtilizzoRiservePatrimonioNetto_var = isnull(@A_IV1_UtilizzoRiservePatrimonioNetto_var, 0) + isnull(@A_IV2_AltriProventi_var, 0)

-- Variazione Rimanenze
declare @A_V_VariazioniRimanenze decimal(19,2)
declare @A_V_VariazioniRimanenze_var decimal(19,2)
SELECT	@A_V_VariazioniRimanenze_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@A_V_VariazioniRimanenze = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @A_VI_IncrementoImmobilizzazioni_var decimal(19,2)
SELECT	@A_VI_IncrementoImmobilizzazioni_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@A_VI_IncrementoImmobilizzazioni = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VII1a_CostiDocentiRicercatori_var decimal(19,2)
SELECT	@B_VII1a_CostiDocentiRicercatori_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VII1a_CostiDocentiRicercatori = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VII1b_CollaborazioniScientifiche_var decimal(19,2)
SELECT	@B_VII1b_CollaborazioniScientifiche_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VII1b_CollaborazioniScientifiche = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VII1c_DocentiAContratto_var decimal(19,2)
SELECT	@B_VII1c_DocentiAContratto_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VII1c_DocentiAContratto = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VII1d_EspertiLinguistici_var decimal(19,2)
SELECT	@B_VII1d_EspertiLinguistici_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VII1d_EspertiLinguistici = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VII1e_AltroPersonale_var decimal(19,2)
SELECT	@B_VII1e_AltroPersonale_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VII1e_AltroPersonale = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VII2_CostiPersonaleDirigente_var decimal(19,2)
SELECT	@B_VII2_CostiPersonaleDirigente_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VII2_CostiPersonaleDirigente = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
set @B_VII_CostiPersonale = isnull(@B_VII1a_CostiDocentiRicercatori, 0) + isnull(@B_VII1b_CollaborazioniScientifiche, 0) + isnull(@B_VII1c_DocentiAContratto, 0) + isnull(@B_VII1d_EspertiLinguistici, 0) +
								isnull(@B_VII1e_AltroPersonale, 0) + isnull(@B_VII2_CostiPersonaleDirigente, 0)
declare @B_VII_CostiPersonale_var decimal(19,2)
set @B_VII_CostiPersonale_var = isnull(@B_VII1a_CostiDocentiRicercatori_var, 0) + isnull(@B_VII1b_CollaborazioniScientifiche_var, 0) + isnull(@B_VII1c_DocentiAContratto_var, 0) + isnull(@B_VII1d_EspertiLinguistici_var, 0) +
								  isnull(@B_VII1e_AltroPersonale_var, 0) + isnull(@B_VII2_CostiPersonaleDirigente_var, 0)
...
declare @B_VIII1_CostiSostegnoStudenti_var decimal(19,2)
SELECT	@B_VIII1_CostiSostegnoStudenti_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII1_CostiSostegnoStudenti = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII2_CostiDirittoStudio_var decimal(19,2)
SELECT	@B_VIII2_CostiDirittoStudio_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII2_CostiDirittoStudio = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII3_CostiRicercaAttivitaEditoriale_var decimal(19,2)
SELECT	@B_VIII3_CostiRicercaAttivitaEditoriale_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII3_CostiRicercaAttivitaEditoriale = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII4_TrasferimentiPartner_var decimal(19,2)
SELECT	@B_VIII4_TrasferimentiPartner_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII4_TrasferimentiPartner = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII5_AcquistoMaterialeConsumo_var decimal(19,2)
SELECT	@B_VIII5_AcquistoMaterialeConsumo_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII5_AcquistoMaterialeConsumo = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII6_VariazioneRimanenze_var decimal(19,2)
SELECT	@B_VIII6_VariazioneRimanenze_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII6_VariazioneRimanenze = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII7_AcquistoLibri_var decimal(19,2)
SELECT	@B_VIII7_AcquistoLibri_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII7_AcquistoLibri = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII8_AcquistoServizi_var decimal(19,2)
SELECT	@B_VIII8_AcquistoServizi_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII8_AcquistoServizi = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII9_AcquistoAltriMateriali_var decimal(19,2)
SELECT	@B_VIII9_AcquistoAltriMateriali_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII9_AcquistoAltriMateriali = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII10_VariazioneRimanenze_var decimal(19,2)
SELECT	@B_VIII10_VariazioneRimanenze_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII10_VariazioneRimanenze = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII11_CostiGodimento_var decimal(19,2)
SELECT	@B_VIII11_CostiGodimento_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII11_CostiGodimento = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_VIII12_AltriCosti_var decimal(19,2)
SELECT	@B_VIII12_AltriCosti_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_VIII12_AltriCosti = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
set @VIII_CostiGestione = isnull(@B_VIII1_CostiSostegnoStudenti, 0) + isnull(@B_VIII2_CostiDirittoStudio, 0) + isnull(@B_VIII3_CostiRicercaAttivitaEditoriale, 0) + isnull(@B_VIII4_TrasferimentiPartner, 0) +
						isnull(@B_VIII5_AcquistoMaterialeConsumo, 0) + isnull(@B_VIII6_VariazioneRimanenze, 0) + isnull(@B_VIII7_AcquistoLibri, 0) + isnull(@B_VIII8_AcquistoServizi, 0) +
						isnull(@B_VIII9_AcquistoAltriMateriali, 0) + isnull(@B_VIII10_VariazioneRimanenze, 0) + isnull(@B_VIII11_CostiGodimento, 0) + isnull(@B_VIII12_AltriCosti, 0)
declare @VIII_CostiGestione_var decimal(19,2)
set @VIII_CostiGestione_var = isnull(@B_VIII1_CostiSostegnoStudenti_var, 0) + isnull(@B_VIII2_CostiDirittoStudio_var, 0) + isnull(@B_VIII3_CostiRicercaAttivitaEditoriale_var, 0) + isnull(@B_VIII4_TrasferimentiPartner_var, 0) +
							isnull(@B_VIII5_AcquistoMaterialeConsumo_var, 0) + isnull(@B_VIII6_VariazioneRimanenze_var, 0) + isnull(@B_VIII7_AcquistoLibri_var, 0) + isnull(@B_VIII8_AcquistoServizi_var, 0) +
							isnull(@B_VIII9_AcquistoAltriMateriali_var, 0) + isnull(@B_VIII10_VariazioneRimanenze_var, 0) + isnull(@B_VIII11_CostiGodimento_var, 0) + isnull(@B_VIII12_AltriCosti_var, 0)
/*	
...
declare @B_IX1_AmmortamentiImmobImmateriali_var decimal(19,2)
SELECT	@B_IX1_AmmortamentiImmobImmateriali_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_IX1_AmmortamentiImmobImmateriali = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_IX2_AmmortamentiImmobMateriali_var decimal(19,2)
SELECT	@B_IX2_AmmortamentiImmobMateriali_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_IX2_AmmortamentiImmobMateriali = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_IX3_SvalutazioniImmobilizzazioni_var decimal(19,2)
SELECT	@B_IX3_SvalutazioniImmobilizzazioni_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_IX3_SvalutazioniImmobilizzazioni = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_IX4_SvalutazioniCrediti_var decimal(19,2)
SELECT	@B_IX4_SvalutazioniCrediti_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_IX4_SvalutazioniCrediti = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @IX_AmmortamentiSvalutazioni_var decimal(19,2)
set @IX_AmmortamentiSvalutazioni_var = isnull(@B_IX1_AmmortamentiImmobImmateriali_var, 0) + isnull(@B_IX2_AmmortamentiImmobMateriali_var, 0) + isnull(@B_IX3_SvalutazioniImmobilizzazioni_var, 0) + isnull(@B_IX4_SvalutazioniCrediti_var, 0)

declare @B_X_AccantonamentiRischiOneri decimal(19,2)
declare @B_X_AccantonamentiRischiOneri_var decimal(19,2)
SELECT	@B_X_AccantonamentiRischiOneri_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_X_AccantonamentiRischiOneri = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
declare @B_XI_OneriDversiGestione_var decimal(19,2)
SELECT	@B_XI_OneriDversiGestione_var = isnull(sum(bvd.amount), 0)
	FROM budgetvar bv
	join budgetvardetail bvd	on bvd.ybudgetvar = bv.ybudgetvar and  bvd.nbudgetvar = bv.nbudgetvar
	join sorting s				on s.idsor = bvd.idsor
	JOIN upb u					on u.idupb = bvd.idupb
...
SELECT	@B_XI_OneriDversiGestione = isnull(sum(budgetprevision.prevision), 0)
	FROM budgetprevision 
	join sorting S		on budgetprevision.idsor = S.idsor
	JOIN upb U			ON budgetprevision.idupb = U.idupb 
	WHERE (budgetprevision.ayear = @ayear or @ayear is null)
...
set @risultatoEconomicoPresunto = @A_I_ProventiPropri + @A_II_Contributi /*+ @A_III_ProventiPerAttivitaAssistenziale*/ + @A_III_ProventiPerGestioneDiretta +
								@A_IV_UtilizzoRiservePatrimonioNetto + @A_V_VariazioniRimanenze + @A_VI_IncrementoImmobilizzazioni 
								-(@B_VII_CostiPersonale + @VIII_CostiGestione + @IX_AmmortamentiSvalutazioni + @B_X_AccantonamentiRischiOneri +
								@B_XI_OneriDversiGestione) + @C_ProventiOneri + @D_Rettifiche + @E_ProventiOneriStraordinari
								- @F_Imposte

declare @risultatoEconomicoPresunto_var decimal(19,2)
set @risultatoEconomicoPresunto_var = @A_I_ProventiPropri_var + @A_II_Contributi_var /*+ @A_III_ProventiPerAttivitaAssistenziale_var*/ + @A_III_ProventiPerGestioneDiretta_var +
									@A_IV_UtilizzoRiservePatrimonioNetto_var + @A_V_VariazioniRimanenze_var + @A_VI_IncrementoImmobilizzazioni_var 
									- (@B_VII_CostiPersonale_var + @VIII_CostiGestione_var + @IX_AmmortamentiSvalutazioni_var + @B_X_AccantonamentiRischiOneri_var + 
									@B_XI_OneriDversiGestione_var) + @C_ProventiOneri_var + @D_Rettifiche_var + @E_ProventiOneriStraordinari_var
									- @F_Imposte_var
...
isnull(@A_III_ProventiPerGestioneDiretta_var,0)  		as 'A_III_ProventiPerGestioneDiretta_var', 
isnull(@A_IV1_UtilizzoRiservePatrimonioNetto,0)  		as 'A_IV1_UtilizzoRiservePatrimonioNetto',  
isnull(@A_IV1_UtilizzoRiservePatrimonioNetto_var,0)  	as 'A_IV1_UtilizzoRiservePatrimonioNetto_var', 
isnull(@A_IV2_AltriProventi,0)  							as 'A_IV2_AltriProventi',  
isnull(@A_IV2_AltriProventi_var,0)  						as 'A_IV2_AltriProventi_var',  
isnull(@A_IV_UtilizzoRiservePatrimonioNetto,0)			as 'A_IV_UtilizzoRiservePatrimonioNetto',
isnull(@A_IV_UtilizzoRiservePatrimonioNetto_var,0)		as 'A_IV_UtilizzoRiservePatrimonioNetto_var',
isnull(@A_V_VariazioniRimanenze,0)						as 'A_V_VariazioniRimanenze',
isnull(@A_V_VariazioniRimanenze_var,0)					as 'A_V_VariazioniRimanenze_var',
isnull(@A_VI_IncrementoImmobilizzazioni,0)				as 'A_VI_IncrementoImmobilizzazioni',
isnull(@A_VI_IncrementoImmobilizzazioni_var,0)			as 'A_VI_IncrementoImmobilizzazioni_var',
isnull(@B_VII1a_CostiDocentiRicercatori,0)  			as 'B_VII1a_CostiDocentiRicercatori',  
isnull(@B_VII1a_CostiDocentiRicercatori_var,0)  		as 'B_VII1a_CostiDocentiRicercatori_var',  
isnull(@B_VII1b_CollaborazioniScientifiche,0)  		as 'B_VII1b_CollaborazioniScientifiche',  
isnull(@B_VII1b_CollaborazioniScientifiche_var,0)		as 'B_VII1b_CollaborazioniScientifiche_var',  
isnull(@B_VII1c_DocentiAContratto,0)					as 'B_VII1c_DocentiAContratto',  
isnull(@B_VII1c_DocentiAContratto_var,0)  				as 'B_VII1c_DocentiAContratto_var',  
isnull(@B_VII1d_EspertiLinguistici,0)  				as 'B_VII1d_EspertiLinguistici',  
isnull(@B_VII1d_EspertiLinguistici_var,0)  			as 'B_VII1d_EspertiLinguistici_var',  
isnull(@B_VII1e_AltroPersonale,0)  					as 'B_VII1e_AltroPersonale',  
isnull(@B_VII1e_AltroPersonale_var,0)  				as 'B_VII1e_AltroPersonale_var',
isnull(@B_VII2_CostiPersonaleDirigente,0)   			as 'B_VII2_CostiPersonaleDirigente',   
isnull(@B_VII2_CostiPersonaleDirigente_var,0)   		as 'B_VII2_CostiPersonaleDirigente_var',
isnull(@B_VII_CostiPersonale,0)						as 'B_VII_CostiPersonale',
isnull(@B_VII_CostiPersonale_var,0)					as 'B_VII_CostiPersonale_var',

isnull(@B_VIII1_CostiSostegnoStudenti,0)  				as 'B_VIII1_CostiSostegnoStudenti',  
isnull(@B_VIII1_CostiSostegnoStudenti_var,0)  			as 'B_VIII1_CostiSostegnoStudenti_var',  
isnull(@B_VIII2_CostiDirittoStudio,0)  					as 'B_VIII2_CostiDirittoStudio',  
isnull(@B_VIII2_CostiDirittoStudio_var,0)  				as 'B_VIII2_CostiDirittoStudio_var',  
isnull(@B_VIII3_CostiRicercaAttivitaEditoriale,0)			as 'B_VIII3_CostiRicercaAttivitaEditoriale',  
isnull(@B_VIII3_CostiRicercaAttivitaEditoriale_var,0)		as 'B_VIII3_CostiRicercaAttivitaEditoriale_var',   
isnull(@B_VIII4_TrasferimentiPartner,0)					as 'B_VIII4_TrasferimentiPartner',  
isnull(@B_VIII4_TrasferimentiPartner_var,0)  				as 'B_VIII4_TrasferimentiPartner_var',   
isnull(@B_VIII5_AcquistoMaterialeConsumo,0)				as 'B_VIII5_AcquistoMaterialeConsumo',  
isnull(@B_VIII5_AcquistoMaterialeConsumo_var,0)  			as 'B_VIII5_AcquistoMaterialeConsumo_var',  
isnull(@B_VIII6_VariazioneRimanenze,0)					as 'B_VIII6_VariazioneRimanenze',  
isnull(@B_VIII6_VariazioneRimanenze_var,0)  				as 'B_VIII6_VariazioneRimanenze_var',  
isnull(@B_VIII7_AcquistoLibri,0)  						as 'B_VIII7_AcquistoLibri',  
isnull(@B_VIII7_AcquistoLibri_var,0)  					as 'B_VIII7_AcquistoLibri_var',  
isnull(@B_VIII8_AcquistoServizi,0)  						as 'B_VIII8_AcquistoServizi',  
isnull(@B_VIII8_AcquistoServizi_var,0)  					as 'B_VIII8_AcquistoServizi_var',  
isnull(@B_VIII9_AcquistoAltriMateriali,0)   				as 'B_VIII9_AcquistoAltriMateriali',   
isnull(@B_VIII9_AcquistoAltriMateriali_var,0)   			as 'B_VIII9_AcquistoAltriMateriali_var',    
isnull(@B_VIII10_VariazioneRimanenze,0)  					as 'B_VIII10_VariazioneRimanenze',  
isnull(@B_VIII10_VariazioneRimanenze_var,0)  				as 'B_VIII10_VariazioneRimanenze_var',  
isnull(@B_VIII11_CostiGodimento,0)						as 'B_VIII11_CostiGodimento',  
isnull(@B_VIII11_CostiGodimento_var,0)  					as 'B_VIII11_CostiGodimento_var',  
isnull(@B_VIII12_AltriCosti,0)							as 'B_VIII12_AltriCosti',  
isnull(@B_VIII12_AltriCosti_var,0)  						as 'B_VIII12_AltriCosti_var',  
isnull(@VIII_CostiGestione,0)								as 'VIII_CostiGestione',
isnull(@VIII_CostiGestione_var,0)							as 'VIII_CostiGestione_var',
isnull(@B_IX1_AmmortamentiImmobImmateriali,0)  			as 'B_IX1_AmmortamentiImmobImmateriali',  
isnull(@B_IX1_AmmortamentiImmobImmateriali_var,0)  		as 'B_IX1_AmmortamentiImmobImmateriali_var',  
isnull(@B_IX2_AmmortamentiImmobMateriali,0)  			as 'B_IX2_AmmortamentiImmobMateriali',  
isnull(@B_IX2_AmmortamentiImmobMateriali_var,0) 			as 'B_IX2_AmmortamentiImmobMateriali_var',   
isnull(@B_IX3_SvalutazioniImmobilizzazioni,0)  			as 'B_IX3_SvalutazioniImmobilizzazioni',  
isnull(@B_IX3_SvalutazioniImmobilizzazioni_var,0)  		as 'B_IX3_SvalutazioniImmobilizzazioni_var',  
isnull(@B_IX4_SvalutazioniCrediti,0)   					as 'B_IX4_SvalutazioniCrediti',   
isnull(@B_IX4_SvalutazioniCrediti_var,0)  				as 'B_IX4_SvalutazioniCrediti_var',   
isnull(@IX_AmmortamentiSvalutazioni,0)					as 'IX_AmmortamentiSvalutazioni',
isnull(@IX_AmmortamentiSvalutazioni_var,0)				as 'IX_AmmortamentiSvalutazioni_var',
isnull(@B_X_AccantonamentiRischiOneri,0)  				as 'B_X_AccantonamentiRischiOneri',  
isnull(@B_X_AccantonamentiRischiOneri_var,0)  			as 'B_X_AccantonamentiRischiOneri_var',  
isnull(@B_XI_OneriDversiGestione,0)  					as 'B_XI_OneriDversiGestione',  
isnull(@B_XI_OneriDversiGestione_var,0)  				as 'B_XI_OneriDversiGestione_var',  
...
--exec rpt_budgeteconomicoufficialeDI34_2025_var 2021, {d '2021-11-11'}, 22,'%','S',null,null,null,null,null
