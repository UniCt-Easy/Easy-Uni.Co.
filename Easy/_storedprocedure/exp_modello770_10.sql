...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_modello770_10]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_10]
GO
...
CREATE    PROCEDURE [exp_modello770_10] (
	@recordGH varchar(2)='GH'
	)
AS BEGIN
	declare @statementyear smallint
	set @statementyear = 2010
	...
	INSERT INTO #modello770 EXEC exp_modello770_10_a

	DECLARE @progr_raggiunto int

	if @recordGH = 'GH'
	begin
		INSERT INTO #modello770 EXEC exp_modello770_10_g '770'
	end

--	INSERT INTO #modello770 EXEC exp_modello770_10_h
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Il seguente codice è la sp del reord H, ossia "exp_modello770_10_h"
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	declare @31dic09_XXX datetime
	set @31dic09_XXX = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})
...
-- Sezione dichiarativa	
	...
	update #expense2009 set motive770 = motive770service.idmot
		from motive770service 
		where motive770service.idser = #expense2009.idser
		AND motive770service.ayear = @annodichiarazione-1

	declare @cognome varchar(60)

	DECLARE registry_crs CURSOR FOR 
		SELECT DISTINCT isnull(cf,p_iva), #expense2009.idreg, surname FROM #expense2009
		join registry on registry.idreg = #expense2009.idreg
		order by surname
	OPEN registry_crs
	...
	--9 Identificativo del produttore del software (codice fiscale)
		...
		SELECT TOP 1 
			...
			WHERE registryaddress.idreg = @idreg
				AND registryaddress.start <= @31dic09_XXX
				AND (registryaddress.stop IS NULL OR registryaddress.stop >= @31dic09_XXX)
			ORDER BY
			CASE idaddresskind
				WHEN @stand THEN 1
				WHEN @domfi THEN 2
	...
	--AU001009 Provincia (sigla)
		...
		if exists (select * from #expense2009 where idreg=@idreg and motive770='N')
		begin
	...
	--AU001017 Codice stato estero
		...
		DECLARE cursoreexpense CURSOR FOR 
			SELECT 
				#expense2009.employtaxamount,
				#expense2009.idexp,
				#expense2009.idser,
				#expense2009.motive770
			FROM #expense2009
			join expenselink 
				on expenselink.idchild = #expense2009.idexp
				and nlevel = @expensephase
			left outer join expenseprofservice
				on expenselink.idparent = expenseprofservice.idexp
			WHERE #expense2009.idreg=@idreg 
...
--	INSERT INTO #modello770 EXEC exp_modello770_10_h -- E' stata sostituita con:
INSERT INTO #modello770 SELECT * FROM #recordh 

DROP TABLE #annualpayedrefundH
DROP TABLE #recordh
DROP TABLE #recHNonArrot
DROP TABLE #expense2009
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Fine esecuzione della sp exp_modello770_10_h
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	INSERT INTO #modello770 EXEC exp_modello770_10_b @numdichquadrog, @numdichquadroh
