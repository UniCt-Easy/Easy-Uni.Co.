...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_creditosiciliano_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_creditosiciliano_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_creditosiciliano_ins]
(
	@y int,
	@n int
)
AS BEGIN
--SETUSER'AMMINISTRAZIONE'
--[trasmele_expense_creditosiciliano_ins] 2015,3
--------------------------------------------------------------
...
WHERE paymentcommunicated.ypaymenttransmission = @y
	AND paymentcommunicated.npaymenttransmission = @n
	AND paymethod.methodbankcode NOT IN ('01','02', '03','04','05','06','07','08','09','10','11','12','13','14')
)
...
-- CONTROLLO N. 16. Movimento di Spesa senza trattamento spese
INSERT INTO #error (message)
(SELECT 'Il movimento n.' + CONVERT(varchar(6),nmov) 
+ '/' + CONVERT(varchar(4),ymov) + ' non ha una modalità di trattamento spese'
FROM paymentcommunicated
WHERE ypaymenttransmission = @y
	AND npaymenttransmission = @n
	AND idchargehandling is null)
...
(
	...
	ABI varchar(5),
	CAB varchar(5),
	cc varchar(12), -- Viene impostato a 25 in quanto gli utenti possono adoperare un C/C che eccede tale lunghezza
	cin_iban varchar(2),
	cin char(1),
...
(
	...
	idchargehandling ,chargehandling,
...
SELECT
	...
	END,
	CASE
		WHEN DATALENGTH(ISNULL(el.cc,'')) <= @len_cc
		THEN   ISNULL(el.cc,'')
		ELSE SUBSTRING(el.cc,1,@len_cc)
	END,
	...
	null,null,
	chargehandling.idchargehandling  ,
...
-- SE HO SPECIFICATO UNA TIPOLOGIA DI TRATTAMENTO SPESE NON ESENTE, VALE SOLO PER IL PRIMO PAGAMENTO DI QUEL PERCIPIENTE
UPDATE #payment SET bank_charges_exempt  = 'N'
WHERE  ISNULL(#payment.idpay,0) =  (SELECT MIN(idpay) FROM #payment B
WHERE  B.idreg = #payment.idreg  
AND  B.idpaymethodTRS = #payment.idpaymethodTRS) -- a parità di modalità di pagamento 
AND    #payment.idexp = (SELECT top 1 idexp FROM #payment C
WHERE  C.idreg = #payment.idreg 
AND    C.idpaymethodTRS = #payment.idpaymethodTRS 
ORDER BY C.ndoc,C.idpay)
AND    #payment.idchargehandling is not null
AND    (SELECT COUNT(*) FROM chargehandling   
		 WHERE chargehandling.idchargehandling  = #payment.idchargehandling 
		 AND (chargehandling.flag&2^0) = 0) > 0  --- tipologia trattamento spese non esente
 

--- SE HO SPECIFICATO UNA TIPOLOGIA DI TRATTAMENTO SPESE NON ESENTE, NON VALE PER I SUCCESSIVI PAGAMENTI DI QUEL PERCIPIENTE
--- LA SOSTITUISCO CON UNA TIPOLOGIA ESENTE
UPDATE #payment SET bank_charges_exempt  = 'S' WHERE bank_charges_exempt IS NULL  
AND  ISNULL(#payment.idpay,0) >  (SELECT MIN(idpay) FROM #payment B
WHERE  B.idreg = #payment.idreg  
AND  B.idpaymethodTRS = #payment.idpaymethodTRS) -- a parità di modalità di pagamento 
AND  #payment.idchargehandling is not null
AND    (SELECT COUNT(*) FROM chargehandling   
...
-- I PAGAMENTI ESENTI, O LADDOVE L'UTENTE NON ABBIA SPECIFICATO NESSUNA TIPOLOGIA DI TRATTAMENTO SPESE, RESTANO INVARIATI
-- VALORIZZO INVECE IL TIPO ESENZIONE PREDEFINITO, PER I  PAGAMENTI  SUCCESSIVI AL PRIMO NON ESENTE
UPDATE #payment SET 	chargehandling = @handlingbankcode_default,
	exemption_charge_payment_kind = @payment_kind_default,
	exemption_charge_motive = @motive_default
WHERE bank_charges_exempt  = 'S' 
...
--WHERE  paydisposition.kpay in (select distinct kpay from #payment
...
(
	...
	numero_conto_corrente_beneficiario varchar(30),
...
SELECT 
				   ...
				   CASE   
					   ...
					   WHEN (idpaymethodTRS = '15') THEN 'BONIFICO ESTERO EURO'
