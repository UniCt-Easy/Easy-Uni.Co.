...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_income_bps_var]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_income_bps_var]
GO
...
CREATE  PROCEDURE [trasmele_income_bps_var]
(
	@y int,
	@n int
)
...
SET @ABI_codebps = '05696'   -- 05696 ABI BANCA POPOLARE DI SONDRIO
...
FROM #proceeds
JOIN incomebill ON #proceeds.idinc = incomebill.idinc 
WHERE #proceeds.fulfilled = 'S'
 and (#proceeds.flagpendingincome = 'N' OR (#proceeds.flagpendingincome = 'S' AND #proceeds.idpro <> 2))
...
GROUP BY #proceeds.ndoc, #proceeds.idpro,#proceeds.opkind , tipo_riscossione, ccp_collection,tipo_entrata,
...
nbill, isnull(#proceeds.txt,''),npay,idpay,flagcompensation
...
SELECT 
				   ...
				   idpro, 
				   '9999',
				   SUM(#proceeds.originalamount)
FROM #proceeds
WHERE opkind = 'ANNULLO'
GROUP BY  ndoc, idpro, #proceeds.opkind  
ORDER BY  ndoc, idpro, #proceeds.opkind  
