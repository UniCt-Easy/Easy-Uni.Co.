...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_income_creditosiciliano_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_income_creditosiciliano_ins]
GO
...
CREATE  PROCEDURE [trasmele_income_creditosiciliano_ins]
(
	@y int,
	@n int
)
...
SELECT
	p.idexp, e.idinc, di.ypro, di.npro, ie.curramount, se.curramount,
