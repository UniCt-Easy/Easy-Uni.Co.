...
-- dove fase 1 =@nfinphase è anche la fase dei Residui, fase 2= @nregphase, fase 3 = maxphase
DECLARE @nfinphase tinyint
DECLARE @nregphase tinyint
SELECT @nfinphase = expensefinphase,
 	@nregphase = expenseregphase
	FROM uniconfig
...
(
	...
	regphase_C decimal(19,2),
	var_regphase_C decimal(19,2),
	regphase_R decimal(19,2),
	var_regphase_R decimal(19,2),
	...
	yvarregphase int,
	nvarregphase int,
	regphase_amount decimal(19,2),

	--regphase_amount decimal(19,2),
	available_appropriation decimal(19,2),
	available_regphase decimal(19,2),
	npayment int,
	ymovpayment int,
	nmovpayment int,
	yvarpayment int,
	...
	total_regphase_C decimal(19,2) ,
	total_var_regphase_C decimal(19,2),
	total_regphase_R decimal(19,2) ,
	total_var_regphase_R decimal(19,2),
...
SELECT 
	...
	expense.idreg,	
	E2.idexp, --IMPEGNO (in realtà è la prima fase, ossia lo stanziamento)
	E2.ymov,
	E2.nmov,  
	expenseyear.amount,

	E4.idexp, --questa è la fase intermedia
	E4.ymov,
	E4.nmov,  
	expenseyear.amount,
...
SELECT 
	...
	E4.idexp, --Questa è la seconda fase
	E4.ymov,
	E4.nmov,  
	expensevar.yvar, --EV4
	expensevar.nvar,--EV4
	expensevar.amount,--EV4
...
WHERE expense.adate between @firstday and @stop
	...
	AND expense.nphase BETWEEN @nfinphase and @nregphase --expense.nphase = @nfinphase
...
WHERE expensevar.yvar = @ayear
	...
	AND isnull(expensevar.autokind,'') <>'22'
	AND ((expensetotal.flag&1)=0) 
	AND expense.nphase BETWEEN @nfinphase and @nregphase-- = @nfinphase
	AND expensevar.adate between @firstday and @stop 
	AND (@idfin IS NULL OR  isnull(FL.idparent,expenseyear.idfin) = @idfin)	
	AND ((expense.idman  = @idman) OR (@idman IS NULL)) -- AND expense.idman is not null))	
	AND (expenseyear.idupb like @idupb)
...
WHERE expensevar.yvar = @ayear
	...
	AND expense.nphase BETWEEN @nfinphase and @nregphase 
...
AND reportname in ('rpt_partitario_spese_3f_resp_generale')
...
if (@suppressifblank = 'S') and @codelevel>2	--> se la stampa è x un livello sottostante la categoria cancella le righe
	BEGIN
		...
		ISNULL(regphase_C,0) = 0 AND
		ISNULL(var_regphase_C,0)= 0 AND
		...
		ISNULL(regphase_R,0)= 0 AND
		ISNULL(var_regphase_R,0)= 0 AND
...
DECLARE @regphase varchar(50)
DECLARE @maxphase varchar(50)

select @finphase = description from  expensephase where nphase = @nfinphase
select @regphase = description from  expensephase where nphase = @nregphase
...
SELECT 
	...
	sum(isnull(regphase_C,0)) as  regphase_C ,
	sum(isnull(var_regphase_C,0)) as  var_regphase_C ,
	...
	sum(isnull(regphase_R,0)) as  regphase_R ,
	sum(isnull(var_regphase_R,0)) as  var_regphase_R ,
...
--
	...
	yvarregphase,
	nvarregphase,
	sum(isnull(regphase_amount,0)) as regphase_amount,
	sum(isnull(available_regphase,0)) as available_regphase,
--
	npayment,
	ymovpayment,
	nmovpayment,
	...
	sum(isnull(total_var_regphase_C,0)) as total_var_regphase_C,
	...
	sum(isnull(total_regphase_R,0)) as  total_regphase_R,
	sum(isnull(total_var_regphase_R,0))as total_var_regphase_R,
...
ORDER BY manager,F.printingorder, rowkind, adate,
	nvarappropriation,nvarregphase,nvarpayment
