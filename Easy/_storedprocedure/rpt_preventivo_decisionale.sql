...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_preventivo_decisionale]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_preventivo_decisionale]
GO
...
SET ANSI_NULLS ON 
GO
--exec  rpt_preventivo_decisionale 2024, 'S', 4, '%', 'N', 'S', 'S', NULL, NULL, NULL, NULL, NULL
-- setuser 'amm'
--	exec rpt_bilprevisione 2022,'E',3,'%','S','N','N'
-- setuser 'amministrazione'
CREATE    PROCEDURE [rpt_preventivo_decisionale]
(
	@ayear smallint,
	@finpart char(1)='S',
...
(
	...
	initialprevision_prev decimal(19,2),
	mov_finphase_R_prev decimal(19,2),
...
BEGIN
	...
	FROM surplus
	WHERE ayear = @ayear --   - 1 Correzione di Francesco 24/04/2023
	...
	SELECT	
		@aa_prec = 	(SELECT ISNULL(startfloatfund, 0) FROM surplus	WHERE ayear = @ayear )  --   - 1 Correzione di Francesco 24/04/2023 - 1)
			+	
	...
	FROM surplus
	WHERE ayear = @ayear -1  --   - 1 Correzione di Francesco 24/04/2023 - 2
...
select finyear.idfin,isnull(@fixedidupb,finyear.idupb),
		case
			when @fin_kind = 2
				...
				else ISNULL(SUM(finyear.previoussecondaryprev),0)			
		end
from finyear 
	join fin f5 on finyear.idfin=f5.idfin
JOIN upb U
...
insert into #data (
	idfin,
	idupb,
	initialprevision,secondaryprevision,currentarrears
)
select finyear.idfin,isnull(@fixedidupb,finyear.idupb),
...
UPDATE #data
set #data.codefin = fin.codefin
from #data
join fin on #data.idfin = fin.idfin
...
Begin
	...
	where 	
	ISNULL(D.initialprevision_prev,0) = 0 and 	ISNULL(D.secondaryprev, 0) = 0 and
	ISNULL(D.mov_finphase_R_prev,0)  = 0 and	ISNULL(D.initialprevision, 0) = 0 and
	ISNULL(D.secondaryprevision, 0) = 0 and		ISNULL(D.currentarrears, 0) = 0
	and fin.nlevel>=5
End

update #data
set #data.idfin = fin.idfin
from #data
join fin on #data.codefin = fin.codefin
where fin.ayear = @ayear and fin.flag = @finpart_bit
...
;with raggruppa_capitoli (
 ...
 AS(
		SELECT 
			...
			ISNULL(SUM(initialprevision_prev), 0),
			ISNULL(SUM(mov_finphase_R_prev),0),
			ISNULL(SUM(secondaryprev), 0)
		FROM #data
...
SELECT 
	...
	f.nlevel,
		...
		ISNULL(SUM(initialprevision_prev),0) as prevision_comp_prev,
		ISNULL(SUM(mov_finphase_R_prev), 0) as mov_finphase_R_prev,
		ISNULL(SUM(secondaryprev), 0) as prevision_cassa,
