...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_bilconsuntivosorting_mpsiope]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_bilconsuntivosorting_mpsiope]
GO
...
--exec exp_bilconsuntivosorting_mpsiope 2019, {ts '2021-05-21 00:00:00'}, 29, 19, '00010008000600040001','N', 'N', null,null,null,null,null
CREATE  PROCEDURE [exp_bilconsuntivosorting_mpsiope]
	@ayear				int,
...
CREATE TABLE #reporttotrim (
	...
	sorkind_siope varchar(MAX),
	sorkind_missprog varchar(MAX)
)

INSERT INTO #reporttotrim
EXEC rpt_bilconsuntivosorting_mpsiope
	@ayear,
	@date,
	@idsorkind_siope,
	@idsorkind_missprog,
...
/*
SELECT DISTINCT code_mp
INTO #sorting_mp_found
FROM #data
*/

-- tabella valori estratti
/*
SELECT description AS code_mp INTO #sorting_mp_sortcodes
FROM sorting 
WHERE paridsor IN (
	SELECT idsor
	FROM sorting
