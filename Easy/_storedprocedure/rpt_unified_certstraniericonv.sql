...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_unified_certstraniericonv]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_unified_certstraniericonv]
GO
SET ANSI_NULLS ON
GO
...
CREATE     PROCEDURE [rpt_unified_certstraniericonv]
(
	@ayear smallint, 
	@idreg int, 
	@start datetime,
...
IF (@unified <> 'S')
BEGIN
        EXEC rpt_certstraniericonv @ayear, @idreg, @start, @stop, @certificatekind
        RETURN
END 
...
WHILE @@fetch_status=0 begin
	DECLARE @dip_nomesp varchar(300)
        SET @dip_nomesp = '['+@iddbdepartment+']'+ '.rpt_certstraniericonv'
