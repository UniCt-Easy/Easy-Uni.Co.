if exists (select * from dbo.sysobjects where id = object_id(N'[exp_attounderwritingupbfin]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_attounderwritingupbfin]
GO
...
SELECT @finphaseincome = assessmentphasecode FROM config WHERE ayear = @yenactment
...
SELECT @finphaseexpense = appropriationphasecode FROM config WHERE ayear = @yenactment
...
SELECT @fin_kind = fin_kind FROM config WHERE ayear = @yenactment
...
select D.idunderwriting, D.idupb, D.idfin, 
		sum( case when V.flagprevision='S' and V.idfinvarstatus<>5 then amount else 0 end ),
		sum( case when V.flagsecondaryprev='S' and V.idfinvarstatus<>5  then amount else 0 end ),
		sum( case when V.flagcredit='S' and (F.flag&1<>0 ) and V.idfinvarstatus<>5  and V.variationkind <> 5 then amount else 0 end ),
		sum( case when V.flagproceeds='S' and (F.flag&1<>0 ) and V.idfinvarstatus<>5  and V.variationkind <> 5 then amount else 0 end )	
from finvar V
join finvardetail D
	on V.yvar = D.yvar and V.nvar= D.nvar
join enactment E
	on V.idenactment = E.idenactment
join upb U
	on U.idupb = D.idupb	
join fin F
	on F.idfin= D.idfin
where V.yvar = @yenactment and V.idenactment = @idenactment
...
CREATE TABLE #PICCOLE_SPESE(idunderwriting int, idupb varchar(36),idfin int, amount decimal(19,2)	) 
...
INSERT INTO #PICCOLE_SPESE( idupb, idfin, amount )	
	...
	from #DETTAGLIO_VARIAZIONE FV
	join pettycashoperation operation
		ON FV.idfin=operation.idfin and FV.idupb=operation.idupb
	join upb 
		on upb.idupb = FV.idupb	
where  (@idsor01 IS NULL OR upb.idsor01 = @idsor01)
		AND (@idsor02 IS NULL OR upb.idsor02 = @idsor02)
		...
		AND NOT	EXISTS (SELECT * FROM pettycashoperation restoreop
							WHERE restoreop.yrestore = operation.yrestore
								AND restoreop.nrestore = operation.nrestore
								AND restoreop.idpettycash = operation.idpettycash)
		AND NOT EXISTS (select  * from pettycashoperationunderwriting U
						join pettycashoperation operation
							ON U.yoperation = operation.yoperation
							AND U.noperation = operation.noperation
							AND U.idpettycash = operation.idpettycash
						where U.idunderwriting = FV.idunderwriting AND FV.idfin=operation.idfin and FV.idupb=operation.idupb)
