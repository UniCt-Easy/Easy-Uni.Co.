...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_h_21]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_h_21]
GO
 --setuser 'amm'
 --setuser 'amministrazione'
 --declare @newProg int
 --exec exp_certificazioneunica_h_21 16071,1, 'N', @newProg out
 --select @newProg
CREATE PROCEDURE [exp_certificazioneunica_h_21]
...
where (ISNULL(p.flagexcludefromcertificate,'N') = 'S')) 
	...
	update #expense2020 set motive770 = motive770service.idmot,
		...
		where motive770service.idser = #expense2020.idser
		...
		SELECT 
			#expense2020.employtaxamount,
			#expense2020.idexp,
			#expense2020.idser,
			#expense2020.motive770,
			#expense2020.socialseccode,
			#expense2020.cfagency,
			#expense2020.titleagency 
		FROM #expense2020
		join expenselink  			on expenselink.idchild = #expense2020.idexp			and nlevel = @expensephase
	...
	BEGIN
		...
		BEGIN
		print  @idexp
			--SELECT @speseimponibilifis= SUM(amount) from profservicerefund PR
			--	join profrefund P on PR.idlinkedrefund=P.idlinkedrefund
			--	WHERE P.flagfiscaldeduction='N' AND
...
---AUX0025 = Imponibile Lordo SUM (feegross) - Imponibile Netto (SUM(taxablenet) da expensetax where (taxkind=1 ossia fiscali)
	...
	-- NEL 2021 DISTINGUIAMO I SEGUENTI CASI CHE L'ANNO SCORSO NON DISTINGUEVAMO (task 15738)
	...
	BEGIN
		SET @notsubjecttaxcode = 7
		
		IF  ISNULL(@sortcodeivakind,'') = '014' 
			SET @notsubjecttaxcode = 12 /*voce di class. convenzionale per regime forfettario*/
		...
		UPDATE  #expense2020 set notsubjecttaxcode = @notsubjecttaxcode  
		WHERE #expense2020.idexp= @idexp
	END
...
-- laddove esso non sia valorizzato per poter paginare nello stesso modulo
	...
	UPDATE #expense2020 SET notsubjecttaxcode = ( select top 1 A.notsubjecttaxcode 
	FROM #recHNonArrot A WHERE 
	A.idexp =  #expense2020.idexp AND isnull(A.notsubjecttaxcode,'') <> 0)
	WHERE isnull(#expense2020.notsubjecttaxcode,'') = 0
		...
		begin
			...
			ELSE
				BEGIN  --- creo una nuova certificazione
					...
					exec exp_certificazioneunica_d_21  @idreg, @newprogrCom, 'H',NULL, @print
				END
...
insert into #recordH (progr,modulo, quadro, riga, colonna, stringa,decimale, intero, data)
			exec exp_certificazioneunica_h_pign_21  @idreg, @newprogrCom, @modulo, @fattoprestazioni,  @inseritopignoramenti out, @newmodulo out
