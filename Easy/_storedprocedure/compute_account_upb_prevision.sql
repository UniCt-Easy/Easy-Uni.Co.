...
SELECT  
	 [@lista_idacc_idupb].idacc,
	 [@lista_idacc_idupb].idupb,
	sum(epacctotal.curramount) 
	from @lista_idacc_idupb
	JOIN epaccyear ON  [@lista_idacc_idupb].idacc LIKE SUBSTRING(epaccyear.idacc, 1, 4* @min_levelusable + 2) + '%' 
					AND  [@lista_idacc_idupb].idupb  = epaccyear.idupb  
	join epacc ON epaccyear.idepacc = epacc.idepacc
	JOIN epacctotal 
		ON epaccyear.idepacc = epacctotal.idepacc AND epacctotal.ayear = @ayear 
	WHERE epaccyear.ayear = @ayear  and epacc.nphase = 1
 group by 	[@lista_idacc_idupb].idacc,	 [@lista_idacc_idupb].idupb
...
SELECT  DISTINCT
	...
	isnull(SUM([@lista_idacc_idupb].amount),0), --0, 11587.88, 0 perchè in epexpyear non c'è il conto R10901, 6289.05 (l'altro preimpegno 295.91)
