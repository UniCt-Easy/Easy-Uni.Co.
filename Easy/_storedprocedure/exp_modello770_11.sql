...
CREATE    PROCEDURE [exp_modello770_11] (
	@recordGH varchar(2)='GH'
	)
AS BEGIN

	declare @statementyear smallint
	set @statementyear = 2011
	...
	INSERT INTO #modello770 EXEC exp_modello770_11_a

	DECLARE @progr_raggiunto int

	if @recordGH = 'GH'
	begin
		INSERT INTO #modello770 EXEC exp_modello770_11_g '770'
	end

--	INSERT INTO #modello770 EXEC exp_modello770_11_h
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Il seguente codice è la sp del reord H, ossia "exp_modello770_11_h"
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	declare @31dic10 datetime
	set @31dic10 = dateadd(yy, @annoredditi-2000, {d '2000-12-31'})
...
-- Sezione dichiarativa	
	...
	update #expense2010 set motive770 = motive770service.idmot
		from motive770service 
		where motive770service.idser = #expense2010.idser
		AND motive770service.ayear = @annodichiarazione-1

	declare @cognome varchar(60)

	DECLARE registry_crs CURSOR FOR 
		SELECT DISTINCT isnull(cf,p_iva), #expense2010.idreg, surname FROM #expense2010
		join registry on registry.idreg = #expense2010.idreg
		order by surname
	OPEN registry_crs
	...
	--9 Identificativo del produttore del software (codice fiscale)
		...
		SELECT TOP 1 
			...
			WHERE registryaddress.idreg = @idreg
				AND registryaddress.start <= @31dic10
				AND (registryaddress.stop IS NULL OR registryaddress.stop >= @31dic10)
			ORDER BY
			CASE idaddresskind
				WHEN @stand THEN 1
				WHEN @domfi THEN 2
	...
	--AU001009 Provincia (sigla)
		...
		if exists (select * from #expense2010 where idreg=@idreg and motive770='N')
		begin
	...
	--AU001017 Codice stato estero
		...
		DECLARE cursoreexpense CURSOR FOR 
			SELECT 
				#expense2010.employtaxamount,
				#expense2010.idexp,
				#expense2010.idser,
				#expense2010.motive770
			FROM #expense2010
			join expenselink 
				on expenselink.idchild = #expense2010.idexp
				and nlevel = @expensephase
			left outer join expenseprofservice
				on expenselink.idparent = expenseprofservice.idexp
			WHERE #expense2010.idreg=@idreg 
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	INSERT INTO #modello770 EXEC exp_modello770_11_b @numdichquadrog, @numdichquadroh
