...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_percipienti_g_16]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_percipienti_g_16]
GO
 
--exec [exp_certificazioneunica_percipienti_g_16]  NULL
CREATE PROCEDURE [exp_certificazioneunica_percipienti_g_16]
(
	@cf varchar(20)
 )
...
AS BEGIN
	...
	CREATE TABLE #percipienti_record_G_2015
	(
		idreg int,
	...
	INSERT INTO #percipienti_record_G_2015
		(
			idreg
		)
	...
	SELECT * FROM	#percipienti_record_G_2015
