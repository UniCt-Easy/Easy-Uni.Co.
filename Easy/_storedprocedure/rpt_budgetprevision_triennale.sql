...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_budgetprevision_triennale]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_budgetprevision_triennale]
GO


/****** Object:  StoredProcedure [amministrazione].[rpt_budgetprevision_triennale]    Script Date: 31/10/2014 16.42.45 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [rpt_budgetprevision_triennale]
(
	@ayear smallint,
	@idsorkind int,
	@idupb varchar(36)='%',
...
/*
Gianni 20 ottobre 2014
Versione modificata per dare la stampa triennale anche ai dipartimenti
includere anche prevision2 e prevision3
*/

--	exec rpt_budgetprevision 2013, 21, '%','S','S',3,null,null,null,null,null
-- exec amministrazione.rpt_budgetprevision_triennale 2014, 15, '00010001','S','N', 'S',null,null,null,null,null
...
select budgetprevision.idsor, budgetprevision.idupb,
		...
		ISNULL(SUM(budgetprevision.prevision2),0),
		ISNULL(SUM(budgetprevision.prevision3),0),
...
SELECT @lencod_lev5 = 1 --flag / 256 FROM sortinglevel WHERE nlevel = 5 and idsorkind = @idsorkind
DECLARE @startpos5 int 
SELECT @startpos5 = @startpos4 + @lencod_lev4
DECLARE @lencod_lev6 int 
SELECT @lencod_lev6 = 2 --flag / 256 FROM sortinglevel WHERE nlevel = 6 and idsorkind = @idsorkind
...
BEGIN
 SELECT 
	...
	ISNULL(SUM(prevision2),0) AS prevision2,
	ISNULL(SUM(prevision3),0) AS prevision3,
...
GROUP BY K.codesorkind, K.description,
	...
	SUBSTRING(S.sortcode, @startpos1, @lencod_lev1),S1.sortcode,
	...
	SUBSTRING(S.printingorder, @startpos5, @lencod_lev5),
	SUBSTRING(S.printingorder, @startpos6, @lencod_lev6),
	S1.printingorder, S2.printingorder,S3.printingorder,S4.printingorder,S5.printingorder,S6.printingorder,
	S1.description,S2.description,S3.description,S4.description,S5.description,S6.description,S.printingorder,
