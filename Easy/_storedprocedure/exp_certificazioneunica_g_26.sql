...
SET ANSI_NULLS ON;
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_certificazioneunica_g_26]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_certificazioneunica_g_26]
GO
--setuser 'amministrazione' 
CREATE PROCEDURE [exp_certificazioneunica_g_26]
...
--setuser 'amm'
-- setuser 'amministrazione'
-- exec exp_certificazioneunica_g_26  134981,1,'N'
...
-- Sezione dichiarativa	
	...
	--- il Soggetto ha eseguito solo Parasubordinati, solo Missioni o entrambe le prestazioni?
	CREATE TABLE #contribuente_G
	(		idreg int,
			parasubcontract char(1),
			itineration char(1),
			progrCom int 
	)
	CREATE TABLE #recordG_missioni
	(
		progr int,
		modulo int,
		quadro varchar(6),
	...
	INSERT INTO #contribuente_G EXEC [exp_certificazioneunica_percipienti_g_26]  @au1cf
	--SELECT '#contribuente_G' , * from #contribuente_G
	--- SE HA ESEGUITO SOLO MISSIONI ESEGUO ESCLUSIVAMENTE L'ALTRA STORED PROCEDURE CHE ESTRAE I DATI RELATIVI A TALI COMPENSI

	IF EXISTS (SELECT * FROM #contribuente_G WHERE ISNULL(parasubcontract,'N') = 'N' AND ISNULL(itineration,'N') = 'S' )
	BEGIN 
		INSERT INTO #recordG_missioni (progr,modulo, quadro, riga, colonna, stringa,decimale,data, intero )
		EXEC exp_certificazioneunica_d_26  @idreg, @progrCom, 'G',NULL, @print
	
		INSERT INTO #recordG_missioni (progr,modulo, quadro, riga, colonna, stringa,decimale, data, intero)
		EXEC [exp_certificazioneunica_g_missioni_26]   @idreg,  @progrCom,  @print

		IF (@print = 'S')
		BEGIN
			SELECT * FROM #recordG_missioni 
		...
		RETURN
	END 
	--- SE HA ESEGUITO ANCHE PARASUBORDINATI PROSEGUO IL CALCOLO SU TALI COMPENSI
	...
	insert into #recordG (progr,modulo, quadro, riga, colonna, stringa,decimale, data,intero)
	exec exp_certificazioneunica_d_26  @idreg, @progrCom, 'G',NULL, @print
	...
	create table #altroFiglio (
		idAltroFiglio int,
		casellaFiglio int,
		casellaFamiliareAscendente int,
		casellaFamiliareNonAscendente int,
		casellaDisabile int,
	...
	DECLARE @bonus_fiscale_integrativo DECIMAL(19,2)
	SELECT  @bonus_fiscale = - SUM(cr.employtaxgross)	
	FROM	#contratti	 			 
	JOIN 	payroll P ON #contratti.idpayroll = P.idpayroll
	JOIN 	payrolltax cr on cr.idpayroll= P.idpayroll
	JOIN	tax ON cr.taxcode = tax.taxcode  AND taxkind = 1	AND geoappliance IS NULL
	WHERE #contratti.padre IS NULL AND tax.taxref = '14_BONUS_FISCALE' -- Non dovrebbe essere presente, in quando la sua applicazione  cessata.
	...
	SELECT  @bonus_fiscale_integrativo = - SUM(cr.employtaxgross)	
	FROM	#contratti	 			 
	JOIN 	payroll P ON #contratti.idpayroll = P.idpayroll
	JOIN 	payrolltax cr on cr.idpayroll= P.idpayroll
	JOIN	tax ON cr.taxcode = tax.taxcode  AND taxkind = 1	AND geoappliance IS NULL
	WHERE #contratti.padre IS NULL AND tax.taxref = '20_BONUS_FISCALE' 
	AND ISNULL(#contratti.flagbonusappliance,'S') = 'S'
	
	-- Al primo calcolo parziale, si somma anche la ritenuta BONUS FISCALE integrativo applicata nei CUD   associati ai contratti
	SET @bonus_fiscale_integrativo =
	@bonus_fiscale_integrativo +
	ISNULL(
		(SELECT SUM(exhibitedcud.fiscalbonusapplied2020)
		FROM exhibitedcud
		JOIN #contratti
			ON #contratti.idcon = exhibitedcud.idcon
		WHERE fiscalyear = @annoredditi
	...
	SET @bonus_fiscale_integrativo   = isnull(@bonus_fiscale_integrativo,0)
	 			         							         
 
	
	-- Lo commento perch Antonio dice che i dati fiscali non vanno inseriti per le 
	...
	SET @ritenuta_addreg_irpef =
		...
		DECLARE @ritenuta_addcom_irpef_acconto_2025 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_acconto_2025 =
		ISNULL(
		...
		SET @ritenuta_addcom_irpef_acconto_2025 =
		@ritenuta_addcom_irpef_acconto_2025 +
		ISNULL(
		...
		-- Non si filtra sulla certificazione in quanto le prestazioni che non rientrano nel CUD
		-- non pagano le addizionali regionali e quindi il loro contributo  nullo
		DECLARE @ritenuta_addcom_irpef_saldo_2025 DECIMAL(19,2)
		SET @ritenuta_addcom_irpef_saldo_2025 =
		ISNULL(
			(SELECT SUM(employtax)
			FROM #cedolini
			JOIN expenselink
	...
	SET @ritenuta_addcom_irpef_saldo_2025 =
	@ritenuta_addcom_irpef_saldo_2025 +
	ISNULL(
	...
	SELECT @ritenuta_addcom_irpef_saldo_2025  = @ritenuta_addcom_irpef_saldo_2025 - @ritenuta_addcom_irpef_acconto_2025  
	...
	if @ritenuta_addcom_irpef_saldo_2025 < 0
	begin
		set @ritenuta_addcom_irpef_acconto_2025 = @ritenuta_addcom_irpef_acconto_2025 + @ritenuta_addcom_irpef_saldo_2025
		set @ritenuta_addcom_irpef_saldo_2025 = null
	end
	else
	begin
		if @ritenuta_addcom_irpef_acconto_2025 < 0
		begin
			set @ritenuta_addcom_irpef_saldo_2025 = @ritenuta_addcom_irpef_saldo_2025 + @ritenuta_addcom_irpef_acconto_2025
			set @ritenuta_addcom_irpef_acconto_2025 = null
		end
	end
...
-- Sar valorizato o il campo da trattenere o il campo da rimborsare.
...
SET @daRimborsare07_IRPEF_CAF =  ISNULL(( SELECT SUM(C.irpeftorefund) FROM #contratti JOIN cafdocument C ON #contratti.idcon = C.idcon WHERE C.ayear = @annoredditi),0)
	...
	Begin
		...
		ISNULL(
			(SELECT - SUM(expensetaxofficial.employtax) --> metto il meno perch  un rimborso, quindi la ritenuta  negativa
			FROM #cedolini
			JOIN #contratti
				ON #contratti.idcon = #cedolini.idcon
			JOIN expenselink
...
Begin
	...
	Else
	begin
		if(@Trattenuta07_ADDREGCAF >= @daTrattenere07_ADDREGCAF)-- se ho trattenuto pi del "da trattenere" del dichiarante, vuol dire che un pezzettino  del coniuge
		begin
...
Begin
	...
	Else
	begin
		if(@Rimborsato07_ADDREGCAF >= @daRimborsare07_ADDREGCAF)-- se ho rimborsato pi del "da Rimborsare" del dichiarante, vuol dire che un pezzettino  del coniuge
		begin
...
Begin
	...
	Else
	begin
		if(@Trattenuta07_ADDCOMCAF >= @daTrattenere07_ADDCOMCAF)-- se ho trattenuto pi del "da trattenere" del dichiarante, vuol dire che un pezzettino  del coniuge
		begin
...
Begin
	...
	Else
	begin
		if(@Rimborsato07_ADDCOMCAF >= @daRimborsare07_ADDCOMCAF)-- se ho rimborsato pi del "da Rimborsare" del dichiarante, vuol dire che un pezzettino  del coniuge
		begin
...
------------------------------------------------------------------------------------------------------ FINE CAF ---------------------------------------------------------------------------------------------------
	...
	--IF @workingdays = 365 
	--BEGIN
	--	SET @workingdays_1_semestre = 181 
	--	SET @workingdays_2_semestre = 184 
	--END
	--ELSE --< 365
	--BEGIN
	--	SELECT  @workingdays_1_semestre = count(*) from #workdays where worked='S' AND  giorno <= @30giu2021
	--	IF @workingdays_1_semestre>181 
	--	BEGIN
	--		SET @workingdays_1_semestre=181 
	--	END
 
	--	SELECT  @workingdays_2_semestre = @workingdays - @workingdays_1_semestre
	--END
	...
	SELECT  	@taxablegross_irpef_stranieri   = SUM (cr.taxablegross)
		FROM	#contratti	 			 
				...
				JOIN 	payrolltax cr on cr.idpayroll= P.idpayroll
				JOIN	tax ON cr.taxcode = tax.taxcode  AND taxkind = 1	AND geoappliance IS NULL
				WHERE #contratti.padre IS NULL AND tax.taxref <> '14_BONUS_FISCALE' AND tax.taxref <> '20_BONUS_FISCALE'
	...
	declare @firstChildConsidered char(1)
	set @firstChildConsidered = 'N'
	...
	SELECT
		...
		ISNULL(F.startapplication, @1gen2025_XXX),
		ISNULL(F.stopapplication, @31dic2025_XXX),
		ISNULL(F.starthandicap, @1gen2026_XXX),
		ISNULL(F.cf, ''),
	...
	begin
		...
		--declare @meseFebbraio int
		--declare @meseInizio21anni int

		--set @meseFebbraio = 2
		--set @meseInizio21anni = 3
		...
		begin
			...
			set @diffhand = datediff(m, @starthandicap, dateadd(m, @mese-1, @1gen2025_XXX))

			declare @casellaparentela char(2)
			SET @casellaparentela = 'P'

			if @idaffinity = 'C' set @casellaParentela = 'C'

			if @idaffinity = 'F' and @figlimaggiori = 0 and @diffhand < 0 and @firstChildConsidered = 'N'
				set @casellaParentela = 'F1'

			if @idaffinity = 'F' and @figlimaggiori = 0 and @diffhand >= 0
				set @casellaParentela = 'D1'

			if @idaffinity = 'F' and (@figlimaggiori > 0 or @firstChildConsidered = 'S') and @diffhand < 0
				set @casellaParentela = 'F'

			--if @idaffinity = 'A' and @diffhand < 0
			--	set @casellaParentela = 'A'
			if @idaffinity = 'G' and @diffhand < 0
				set @casellaParentela = 'G'

			if @diffhand >= 0 and (@idaffinity = 'F' and @figlimaggiori > 0)
			...
			-- Gestione della detrazione per primo figlio
			if (@idaffinity = 'F' and @figlimaggiori = 0 and @firstChildConsidered = 'N') 
			begin
				...
				begin
					update #PrimoFiglio set
						...
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2025_XXX), dateadd(yy, 3, @birthdate)) >= 0 and @mese <= @meseFebbraio THEN @valore_mese ELSE 0 end,
						maggioreDi21Anni = isnull(maggioreDi21Anni, 0) +
								case -- Maggiore o uguale a 21 anni
									WHEN DATEDIFF(m, DATEADD(yy, 21, @birthdate), DATEADD(m, @mese-1, @1gen2025_XXX)) >= 0 
									-- E minore di 30 anni (ovvero ancora sotto i 30 anni)
									AND DATEDIFF(m, DATEADD(m, @mese-1, @1gen2025_XXX), DATEADD(yy, 30, @birthdate)) > 0 /*and @mese >= @meseInizio21anni*/ 
									THEN @valore_mese ELSE 0 
								end,
						--GenFebACarico = isnull(GenFebACarico, 0) +
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2025_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--				and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2025_XXX)) < 0
						--				and @mese <= @meseFebbraio THEN @valore_mese ELSE 0 end,
					...
					insert into #PrimoFiglio values (
						...
						@cfFamigliare,
						@valore_mese,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2025_XXX), dateadd(yy, 3, @birthdate)) >= 0 and @mese <= @meseFebbraio THEN @valore_mese end,
						case -- Maggiore o uguale a 21 anni
							WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2025_XXX)) >= 0 
							-- E minore di 30 anni (ovvero ancora sotto i 30 anni)
							AND DATEDIFF(m, DATEADD(m, @mese-1, @1gen2025_XXX), DATEADD(yy, 30, @birthdate)) > 0
							/*and @mese >= @meseInizio21anni*/ 
							THEN @valore_mese 
						end,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2025_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--		and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2025_XXX)) < 0
						--		and @mese <= @meseFebbraio THEN @valore_mese end,
			...
			begin
				...
				SELECT @idAltroFiglio = idAltroFiglio
					...
					where (casellaFiglio = 1 and @casellaParentela='F' 
						...
						or casellaFamiliareAscendente = 1 and @casellaParentela = 'G'
						--or casellaFamiliareNonAscendente = 1 and @casellaParentela = 'P'
				...
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese,
						--minoreDiTreAnni   = isnull(minoreDiTreAnni,0) + 
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2025_XXX), 
						...
						--		case WHEN datediff(m, dateadd(m, @mese-1, @1gen2025_XXX), dateadd(yy, 3, @birthdate)) < 0 
						--				and datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2025_XXX)) < 0
						--				and @mese <= @meseFebbraio THEN @valore_mese 
						--			ELSE 0 
						--		end,
						percentualeDiDetrazioneSpettante = isnull(percentualeDiDetrazioneSpettante,0)
											 + isnull(@appliancepercentage,1)
					...
					insert into #AltroFiglio values (
						...
						case @casellaParentela WHEN 'G' THEN '1' end,
						case @casellaParentela WHEN 'P' THEN '1' end,
						case @casellaParentela WHEN 'D' THEN '1' end,
						@cfFamigliare,
						@valore_mese,
						--case WHEN datediff(m, dateadd(m, @mese-1, @1gen2025_XXX), dateadd(yy, 3, @birthdate)) >= 0 THEN @valore_mese end,
						case -- Maggiore o uguale a 21 anni
							WHEN datediff(m, dateadd(yy, 21, @birthdate), dateadd(m, @mese-1, @1gen2025_XXX)) >= 0 
							-- E minore di 30 anni (ovvero ancora sotto i 30 anni)
							AND DATEDIFF(m, DATEADD(m, @mese-1, @1gen2025_XXX), DATEADD(yy, 30, @birthdate)) > 0
			...
			-- Gestione Altri Familiari non ascendenti
			if @casellaParentela = 'P'
			begin
				declare @idNonAscendente int
				set @idNonAscendente = null
				SELECT @idNonAscendente = idAltroFiglio
					from #altrofiglio
					where casellaFamiliareNonAscendente = 1 and @casellaParentela = 'P'	
					and codiceFiscaleFamiliare  = @cfFamigliare
					and anno = @annoredditi

				if @idNonAscendente is not null
				begin
					update #AltroFiglio set
						numeroMesiACarico = isnull(numeroMesiACarico,0) + @valore_mese
						where idAltroFiglio = @idNonAscendente
				end ELSE begin
					SELECT @idNonAscendente = 1 + count(*) from #altrofiglio
		...
		if (select count(*) from #primoFiglio) = 1
			set @firstChildConsidered = 'S'
	...
	--DECLARE	@DB001013 INT			-- Giorni I semestre-- Dalla CU 2022 non serve pi
	--DECLARE	@DB001014 INT			-- Giorni II semestre-- Dalla CU 2022 non serve pi
	...
	BEGIN
			...
			IF (
				...
				OR
					(	-- IN FORZA AL 31 12, con data ultimo cedolino dell'anno inferiore 
						(@stopcontract > @31dic2025)  AND (@stop < @31dic2025) 
					) 
				)	
				)
				AND (@workingdays >0)		
		...
		if (	(@workingdays = 365) AND (@stopcontract > @31dic2025) /*IN FORZA AL 31 12*/ )  SET @DB001011 = NULL --- IN TAL CASO NON COMPILIAMO PERIODI PARTICOLARI
		...
		(
			...
			(	(year(@start)<@annoredditi)
				...
				OR
				(
				   (@stopcontract <= @31dic2025)  AND (@stop < @stopcontract) 
				)
			) 
		)
		OR
	...
	IF ((year(@stopcontract) = @annoredditi) AND (@stopcontract <= @31dic2025))
	BEGIN
		SET @DB001024= isnull(@ritenuta_addreg_irpef,0)
		SET @DB001028= isnull(@ritenuta_addcom_irpef_saldo_2025,0)
	END
	...
	--------------------------------------------------------------------------------------------
	-- Oneri detraibili
	-- Nei punti 341, 343, 345, 347, 349, e 351 va indicato il codice relativo allonere detraibile, per il quale spetta la detrazione
	-- dallimposta lorda nella misura del 19%, e del 26%, prelevabile dalle tabelle A e B poste in appendice alle presenti
	-- istruzioni.
	-- Nei punti 342, 344, 346, 348, 350 e 352 va indicato limporto dellonere detraibile relativo al codice riportato nei
	-- precedenti punti.
	...
	SET @DB001369= isnull(@detrazioni_per_oneri,0)
	...
	BEGIN
		...
		BEGIN
			...
			SET @DB001392= 0
		END
		-- vecchi campi della CU2021
		--INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, intero)		VALUES(@progrCom, 1, 'DB001', 1, '391', @DB001391disuso)
		--INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale)	VALUES(@progrCom, 1, 'DB001', 1, '392', @DB001392disuso)
		--INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale)	VALUES(@progrCom, 1, 'DB001', 1, '393', @DB001393disuso)
	...
	-------------------------------------- ALTRI DATI ------------------------------------------ 
	--------------------------------------------------------------------------------------------
	-- Inseriamo qui alcuni tipi di prestazioni esenti da imposizione fiscale ma non perch si tratti
	...
	BEGIN 
		...
		--non funziona se ci sono pi redditi esenti
	...
	-- tuttavia molto probabilmente si tratta di casi in cui non c' stata esplicita richiesta.
	...
	DECLARE @DB001485 DECIMAL (19,2)	-- Redditi assoggettati a ritenuta a titolo di imposta - Redditi assoggettati a ritenuta a titolo di imposta - Totale ritenute irpef sospese
	...
	-- SET @@DB001483= 0  -- non lo scriviamo
	...
	-- INSERT INTO #recordG (progr, quadro, riga, colonna, decimale)	VALUES(@progrCom, 'DB001', 1, '483', @DB001483) -- non lo scriviamo
	--------------------------------------------------------------------------------------------
	-------- Dati relativi ai conguagli in caso di Redditi erogati da altri soggetti -----------
	--------------------------------------------------------------------------------------------
	
	DECLARE @DB001531 DECIMAL(19,2)		-- Totale redditi conguagliati gi compresi nel punto 1
	DECLARE @DB001537 VARCHAR(16)	    -- Codice fiscale
	DECLARE @DB001539 DECIMAL(19,2)		-- Reddito conguagliato gi compreso nel punto 1
	DECLARE @DB001545 DECIMAL(19,2)	    -- Ritenute
	DECLARE @DB001546 DECIMAL(19,2)		-- Addizionale regionale
	DECLARE @DB001547 DECIMAL(19,2)	    -- Addizionale comunale Acconto 2021
	DECLARE @DB001548 DECIMAL(19,2)	    -- Addizionale comunale Saldo 2021
	...
	BEGIN 
		...
		SET @DB001537 = @cf_altro_sostituto					 
		SET @DB001539 = @reddito_conguagliato					 
		SET @DB001545 = @ritenute_irpef	 
		SET @DB001546 = @addizionale_regionale_irpef 
		SET @DB001547 =	@addizionale_comunale_irpef_acconto	 
		SET @DB001548 =	@addizionale_comunale_irpef_saldo
	
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, stringa)  VALUES(@progrCom,1, 'DB001', @counter, '537', @DB001537)
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale) VALUES(@progrCom,1, 'DB001', @counter, '539', @DB001539)
		...
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale) VALUES(@progrCom,1, 'DB001', @counter, '547', @DB001547)
		INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, decimale) VALUES(@progrCom,1, 'DB001', @counter, '548', @DB001548)
	...
	--UPDATE #altrofiglio SET minoreDiTreAnni = 2 where isnull(minoreDiTreAnni,0) > 2
	--UPDATE #primofiglio SET minoreDiTreAnni = 2 where isnull(minoreDiTreAnni,0) > 2

	UPDATE #altroFiglio SET maggioreDi21Anni = 12 where isnull(maggioreDi21Anni, 0) > 12
	UPDATE #primoFiglio SET maggioreDi21Anni = 12 where isnull(maggioreDi21Anni, 0) > 12

	--UPDATE #altroFiglio SET GenFebACarico = 2 where isnull(GenFebACarico, 0) > 2
	--UPDATE #primoFiglio SET GenFebACarico = 2 where isnull(GenFebACarico, 0) > 2
	...
	-- DECLARE @DB601005	INT--Mesi a carico	N2
	INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, intero) 
	SELECT @progrCom, 1, 'DB601', idconiuge, '006', 
	...
	--DECLARE @DB602009	INT--mesi a carico tra gennaio e febbraio di figli compresi tra 3 e 21 anni
	--INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, intero)
	--	SELECT @progrCom,  1,'DB60' + CONVERT(VARCHAR(3),@idprimofiglio +1),idprimofiglio, '009', 
	--	GenFebACarico from #primofiglio where GenFebACarico <> 0

	--DECLARE @DB602010	INT--numero di mesi (1-10)  a seconda dei mesi a carico tra marzo e dicembre di figli >= 21 anni
	INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, intero)
		SELECT @progrCom,  1,'DB60' + CONVERT(VARCHAR(3),@idprimofiglio +1),idprimofiglio, '010', 
		maggioreDi21Anni from #primofiglio where maggioreDi21Anni <> 0
	...
	INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, stringa)
		...
		CASE  WHEN casellaDisabile = 1 then 'D'
			  ...
			  WHEN casellaFamiliareAscendente = 1 THEN 'G'
			  WHEN casellaFamiliareNonAscendente = 1 THEN 'P'
			  else null
		END
		FROM #altrofiglio
	...
	-- DECLARE @DB603004	VARCHAR(16)--Codice fiscale	CF
	INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, stringa)
		SELECT @progrCom,  1, 'DB60'+ CONVERT(VARCHAR(3),idaltrofiglio+2), 1,'005', codiceFiscaleFamiliare
		FROM #altrofiglio

	--DECLARE @DB603005	INT--Mesi a carico	N2
	INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, intero)
		SELECT @progrCom,  1, 'DB60'+ CONVERT(VARCHAR(3),idaltrofiglio+2), 1,'006', numeroMesiACarico 
		FROM #altrofiglio
	...
	--DECLARE @DB603A07	DECIMAL(19,6)--Percentuale di detrazione spettante	PC si pu omettee se c' la percentuale
	INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, intero)
	...
	--DECLARE @DB603009	INT--mesi a carico tra gennaio e febbraio di figli compresi tra 3 e 21 anni
	--INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, intero) 
	--	SELECT @progrCom,  1, 'DB60'+ CONVERT(VARCHAR(3),idaltrofiglio+2),1, '009', GenFebACarico
	--	FROM #altrofiglio where GenFebACarico <> 0

	--DECLARE @DB603010	INT--numero di mesi (1-10)  a seconda dei mesi a carico tra marzo e dicembre di figli >= 21 anni
	INSERT INTO #recordg (progr,  modulo, quadro, riga, colonna, intero) 
		SELECT @progrCom,  1, 'DB60'+ CONVERT(VARCHAR(3),idaltrofiglio+2),1, '010', maggioreDi21Anni
		FROM #altrofiglio where maggioreDi21Anni <> 0
	...
	BEGIN 
		...
		SET @oldstop = null
		DECLARE @DC001091 CHAR(1)		-- Qualifica AN Vale B, C, D, E, F,G, H, L, M, N, P,Q, Z CREARE TABELLA DI CONFIGURAZIONE
		DECLARE @DC001092 VARCHAR(20)	-- Posizione assicurativa territoriale N11
		DECLARE @DC001093 DATETIME		-- Data inizio D4
		DECLARE @DC001094 DATETIME		-- Data fine D4
		DECLARE @DC001095 VARCHAR(10)	-- Codice comune AN .
		DECLARE @DC001096 INT			-- Personale viaggiante CB   NON LO METTIAMO 
		...
		if @startpayroll < @1gen2025_XXX set @oldstart = @1gen2025_XXX ELSE set @oldstart = @startpayroll
		if @stoppayroll > @31dic2025_XXX set @oldstop =  @31dic2025_XXX ELSE set @oldstop = @stoppayroll
		-- Inserimento dei dati assicurativi distinti per mese
		WHILE @@fetch_status = 0
		begin
			--Dati assicurativi INAIL
		
			if @startpayroll < @1gen2025_XXX set @startpayroll = @1gen2025_XXX
			if @stoppayroll > @31dic2025_XXX set @stoppayroll = @31dic2025_XXX

			if datediff(d, @oldstop, @startpayroll) > 1
			begin
				SET @modulopat = @modulopat+1
				SET @DC001091 = NULL -- Qualifica AN Vale B, C, D, E, F,G, H, L, M, N, P,Q, Z CREARE TABELLA DI CONFIGURAZIONE ASSOCIATA ALL'ANAGRAFICA
				SET @DC001092 = @oldpat	-- Posizione assicurativa territoriale N11
				SET @DC001093 =	@oldstart	-- Data inizio D4
				SET @DC001094 = @oldstop	-- Data fine D4
				SET @DC001095 = @codiceComuneEnte -- Codice comune AN .
				SET @DC001096 = NULL		-- Personale viaggiante CB   NON LO METTIAMO 
				...
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna,  intero) VALUES(@progrCom, @modulopat,  'DC001', 1, '091',  @DC001091)
				-- @DC001072 Posizione assicurativa territoriale N11
				if ((@DC001092   is not null) and (@print ='N'))
				Begin
						--  a PAT  un N11, ove Nx : Campo numerico al massimo di x cifre allineate a destra (x assume valori da 1 a 16)
						--	Quindi deve essere di x cifre, allinenata a destra, con gli spazi a sinistra
						--	DC001072      9209713813
					SET @DC001092 =  REPLICATE('0',11 - DATALENGTH(@DC001092)) + @DC001092  
				End
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, stringa) VALUES(@progrCom, @modulopat, 'DC001', @modulopat, '092', @DC001092)
				-- @DC001073 Data inizio D4
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, data) VALUES(@progrCom, @modulopat, 'DC001', @modulopat, '093', @DC001093)
				-- @DC001074 Data fine D4
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, data) VALUES(@progrCom, @modulopat, 'DC001', @modulopat, '094', @DC001094)
				-- @DC001075  Codice comune AN  
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna, stringa) VALUES(@progrCom, @modulopat, 'DC001', @modulopat, '095', @DC001095)
				--@DC001076  -- Personale viaggiante CB   NON LO METTIAMO 
				INSERT INTO #recordG (progr, modulo, quadro, riga, colonna,  intero) VALUES(@progrCom, @modulopat,  'DC001', @modulopat, '096',  @DC001096)
...
------------------------------------------------------------------------------------------------------------------
	...
	BEGIN
		...
		BEGIN
			...
			-- e il codice esenzione indicato nella casella 468  pari a 3 --
			-- Nel caso di redditi parzialmente esentati da imposizioni -----
			-- in Italia, lammontare del reddito escluso dalla tassazione --
			-- deve essere indicato nel punto 469, riportando altres -------
	...
	-- Non sono stati effettuati rimborsi dai sostituti ma bens solo ritenute -----------------
    
    --------------------------------------------------------------------------------------------
    --------------------------------------------------------------------------------------------
	-- "AN - La detrazione minima  stata ragguagliata al periodi di lavoro. -------------------
	-- Il percipiente pu fruire della detrazione per l'intero anno in sede --------------------
	-- di dichiarazione dei redditi, semprech non sia stata gi attribuita --------------------
	...
	-- allanno (inizio o cessazione del rapporto di lavoro nel corso dellanno), --------------
	-- limitatamente ai contratti in cui  stato scelto di applicare la detrazione) ------------
	--------------------------------------------------------------------------------------------
    --------------------------------------------------------------------------------------------
	...
	IF ((@DB001022 <> 0)  --@ritenuta_addreg_irpef
		 OR 
		(@DB001027 <> 0)) --@ritenuta_addcom_irpef_saldo_2021
	BEGIN
		SET @NN =
		CASE 
	...
	BEGIN
		SET @NN =
		'AN - La detrazione minima  stata ragguagliata al periodo di lavoro.' + 
		' Il percipiente pu fruire della detrazione per l''intero anno in sede' +
		' di dichiarazione dei redditi, semprech non sia stata gi attribuita' +
		' da un altro datore di lavoro e risulti effettivamente spettante.'
		INSERT INTO #recordg (progr, quadro, modulo, riga, colonna, stringa)
	...
	--  (Valorizzare quando il compenso  un parasubordinato ed  associata 
	...
	-- perch il reddito rientra nei limiti di esenzione. 
	...
	AND
	(@DB001367 =  0) --@ritenuta_addcom_irpef_saldo_2021 non applicata
	BEGIN
		SET @NN =
		'BB - Saldo dell''addizionale comunale all''IRPEF non operata in quanto' + 
		' in possesso dei requisiti reddituali per usufruire interamente della fascia' +
	...
	IF @summarybalance = 'S'
	BEGIN
		SET @NN = (SELECT 'ZZ - Il percipiente dovr procedere ad effettuare le operazioni di conguaglio in sede di dichiarazione dei redditi in quanto i redditi certificati non sono stati oggetto di conguaglio da parte del sostituto.')
		INSERT INTO #recordg (progr, quadro,modulo, riga, colonna, stringa)
...
--- SE HA ESEGUITO SIA CONTRATTI PARASUBORDINATI CHE MISSIONI ESEGUO ANCHE L'ALTRA STORED PROCEDURE CHE ESTRAE I DATI RELATIVI ALLE MISSIONI
--- QUESTE DUE FONTI DATI VANNO POI ARMONIZZATE FACENDO ALCUNE ASSUNZIONI
IF EXISTS (SELECT * FROM #contribuente_G WHERE ISNULL(parasubcontract,'N') = 'S' AND ISNULL(itineration,'N') = 'S' )
BEGIN 
	INSERT INTO #recordG_missioni 
	(	progr,
		modulo,
		quadro,
		riga ,
	...
	EXEC [exp_certificazioneunica_g_missioni_26]   @idreg,  @progrCom,  @print
	--- EFFETTUO UN MERGE DEI DATI FISCALI E PREVIDENZIALI ESTRATTI PER LE DUE TIPOLOGIE DI COMPENSI 
	--- SECONDO I SEGUENTI CRITERI

	;WITH X AS (
		SELECT progr, modulo, quadro, riga, colonna,decimale
		FROM #recordG_missioni
		WHERE 
		(quadro ='DB001' and colonna = '002'  OR  --- reddito lordo, lo sommo a quello dei cococo
		 -- quadro ='DB001' and colonna = '741'   --- essendo valore costante lo ignoro 
		 quadro ='DB001' and colonna = '752'  OR  --- reddito lordo, lo sommo a quello dei cococo come 002
		 --dato che non applico le detrazioni alle missioni,
		 --non considero date inizio e fine missioni ai fini del calcolo dei giorni lavorati
		 --prendo per buoni quelle dei compensi cococo ai fini del calcolo delle detrazioni
		 --quadro ='DB001' and colonna = '006'  OR  inizio missioni
		 --quadro ='DB001' and colonna = '008'  OR	fine missioni 
		 --quadro ='DB001' and colonna = '009'  OR  giorni lavorati missioni
		 quadro ='DB001' and colonna = '021'  OR  --- imponibile fiscale missioni
		 -- quadro ='DB001' and colonna = '390'  OR valore costante 2 (non applica bonus) non lo copio perchè tanto è uguale
		 quadro ='DC001' and colonna = '045'  OR --dati previdenziali
		 quadro ='DC001' and colonna = '046'  OR --dati previdenziali
		 quadro ='DC001' and colonna = '047'  OR --dati previdenziali
		 quadro ='DC001' and colonna = '048'   --dati previdenziali
		)
	)
	UPDATE #recordG SET #recordG.decimale =  isnull(#recordG.decimale,0) + ISNULL(x.decimale, 0) 
	FROM X
	WHERE #recordG.progr = X.progr 
	AND   #recordG.modulo = X.modulo 
	AND   #recordG.quadro = X.quadro 
	AND   #recordG.riga = X.riga 
	AND   #recordG.colonna = X.colonna 

	--- Armonizzazione dati DICHIARAZIONE EMENS tra COCOCO e MISSIONI PARASUBORDINATI GESTIONE PUBBLICA
	--- TIPO RAPPORTO EMENS,  INTERO, METTO VALORE COSTANTE 11: COLLABORAZIONI COORDINATE E CONTINUATIVE
	--- EMENS TUTTI I MESI 0/1, INTERO, LO VALORIZZERO' SUBITO DOPO IL MERGE
	--- MULTI CASELLA BARRATA, STRINGA: MESI DICHIARAZIONE EMENS, EFFETTUO UNIONE MESE PER MESE TRA LE DUE STRINGHE IN INPUT 
		;WITH X AS (
			SELECT progr, modulo, quadro, riga, colonna,stringa, intero
			FROM #recordG_missioni
			WHERE 
			( 
			 quadro ='DC001' and colonna = '037'  OR --Emens tutti i mesi 0/1
			 quadro ='DC001' and colonna = '038'  --Multi casella barrata: Mesi presentazione Emens
			)
		)
		UPDATE g SET g.intero = 
		CASE x.colonna
				WHEN '037' THEN 0 -- EMENS TUTTI I MESI: scrivo 0 (significa N, valore temporaneo che perfezionerò 
								  -- dopo il Merge tra le due fonti dati)
				ELSE g.intero
		END,
		g.stringa = 
		CASE x.colonna
				WHEN '038' THEN m.merged 
				ELSE g.stringa
		END 
		FROM #recordG g
		JOIN X x ON 1 = 1
		OUTER APPLY (
			SELECT (
				SELECT
					CASE
						WHEN SUBSTRING(ISNULL(g.stringa, '000000000000'), v.number, 1) = '1'
						  OR SUBSTRING(ISNULL(x.stringa, '000000000000'), v.number, 1) = '1'
						THEN '1'
						ELSE '0'
					END
				...
				ORDER BY v.number
				FOR XML PATH(''), TYPE
			).value('.', 'varchar(12)') AS merged
		) AS m
			WHERE g.progr = x.progr 
			AND   g.modulo = x.modulo 
			AND   g.quadro = x.quadro 
			AND   g.riga = x.riga 
			AND   g.colonna = x.colonna 

			-- EMENS TUTTI I MESI: scrivo 1 (significa S) solo se tutti i 12 mesi sono coperti dopo il Merge 
			UPDATE g SET g.intero = 1
			FROM #recordG g
			WHERE quadro = 'DC001' AND colonna = '037'  
			AND EXISTS (SELECT * FROM #recordG G1
			WHERE G1.quadro = 'DC001' AND G1.colonna = '038'   
			AND G1.stringa = REPLICATE('1', 12) --- COMPILAZIONE TUTTI I MESI
			AND g.progr = G1.progr AND g.modulo = G1.modulo)


	--- Armonizzazione dati DICHIARAZIONE UNIEMENS tra COCOCO e MISSIONI 
	--- TIPO RAPPORTO EMENS,  INTERO, METTO VALORE COSTANTE 11: COLLABORAZIONI COORDINATE E CONTINUATIVE
	--- EMENS TUTTI I MESI 0/1, INTERO, LO VALORIZZERO' SUBITO DOPO IL MERGE
	--- MULTI CASELLA BARRATA, STRINGA: MESI MANCATA PRESENTAZIONE DICHIARAZIONE UNIEMENS, EFFETTUO UNIONE MESE PER MESE TRA LE DUE STRINGHE IN INPUT 
		;WITH X AS (
			SELECT progr, modulo, quadro, riga, colonna,stringa, intero
			FROM #recordG_missioni
			WHERE 
			( 
			 quadro ='DC001' and colonna = '049'  OR --Emens tutti i mesi 0/1
			 quadro ='DC001' and colonna = '050'  OR --Multi casella barrata: Mesi presentazione Emens
			 quadro ='DC001' and colonna = '051'     --tipo rapporto Emens (11, Collaborazioni coordinate e continuative)
			)
		)
		UPDATE g SET g.intero = 
		CASE x.colonna
				WHEN '049' THEN 0 -- EMENS TUTTI I MESI: scrivo 0 (significa N, valore temporaneo che perfezionerò 
								  -- dopo il Merge tra le due fonti dati)
				WHEN '051' THEN 11 -- (costante 11, Collaborazioni coordinate e continuative)
				ELSE g.intero
		END,
		g.stringa = 
		CASE x.colonna
				WHEN '050' THEN m.merged  --- TUTTI CON ESCLUSIONE DI 
				ELSE g.stringa
		END 
		FROM #recordG g
		JOIN X x ON 1 = 1
		OUTER APPLY (
			...
			-- EMENS TUTTI I MESI: scrivo 1 (significa S) solo se tutti i 12 mesi sono coperti dopo il Merge, 
			-- ovvero non è escluso nessun mese da dichiarazione EMENS
			UPDATE g SET g.intero = 1
			FROM #recordG g
			WHERE quadro = 'DC001' AND colonna = '049'  
			AND EXISTS (SELECT * FROM #recordG G1
			WHERE G1.quadro = 'DC001' AND G1.colonna = '050'   
			AND G1.stringa = REPLICATE('0', 12) --- NON E' ESCLUSO NESSUN MESE DALLA DICHIARAZIONE UNIEMENS
			AND g.progr = G1.progr AND g.modulo = G1.modulo)
END
 

--- ESTRAGGO RECORD G DEFINITVO 
IF (@print = 'S')
BEGIN
	SELECT * FROM #recordG 
	WHERE stringa IS NOT NULL OR intero IS NOT NULL OR data IS NOT NULL OR decimale<>0
