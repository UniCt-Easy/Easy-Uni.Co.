...
CREATE                   PROCEDURE [rpt_partitario_spese_5f_resp_generale]
	@ayear int,
	@idupb	varchar(36), 
	@codelevel tinyint,
	@showupb char(1),	
...
/*
 L'ipotesi di questa stampa a 5 fasi è che 
( @residualphase < @nsecondphase) AND (@nsecondphase<@nthirdphase) AND (@nthirdphase<@nfourthphase) AND (@nfourthphase<@maxphase)

*/

-- dove fase 1 = @nfinphase, fase 4 = maxphase, fase 2 = @nfinphase+1,  fase 3 = fase2+1 fase 4 = @maxexpensephase-1
...
DECLARE @nfourthphase INT
SELECT  @nsecondphase = nphase FROM expensephase WHERE nphase = @nfinphase + 1
SELECT  @nthirdphase = nphase FROM expensephase WHERE nphase = @nsecondphase + 1
SELECT  @nfourthphase = nphase FROM expensephase  WHERE nphase = @maxexpensephase - 1 
...
(
	...
	fourthphase_C decimal(19,2),
	var_fourthphase_C decimal(19,2),
	fourthphase_R decimal(19,2),
	var_fourthphase_R decimal(19,2),
	...
	total_fourthphase_C decimal(19,2) ,
	total_var_fourthphase_C decimal(19,2),
	total_fourthphase_R decimal(19,2) ,
	total_var_fourthphase_R decimal(19,2),
...
SELECT 
	...
	E4.idexp, --questa è la quarta fase 
	E4.ymov,
	E4.nmov,  
	expenseyear.amount,
...
LEFT OUTER JOIN expenselink EL4
	ON EL4.idchild = expense.idexp  AND EL4.nlevel = @nfourthphase
...
LEFT OUTER JOIN expenselink EL5
	ON EL5.idchild = expense.idexp  AND EL5.nlevel = @maxexpensephase
...
SELECT 
	...
	expensevar.nvar,  -- EV_4
	expensevar.amount,-- EV_4
...
LEFT  OUTER JOIN expenselink EL4
	ON EL4.idchild = expensevar.idexp  AND EL4.nlevel = @nfourthphase
LEFT OUTER JOIN expenselink EL5
	ON EL5.idchild = expensevar.idexp  AND EL5.nlevel = @maxexpensephase
LEFT  OUTER JOIN finlink FL
	ON FL.idchild = expenseyear.idfin  AND FL.nlevel = @codelevel
LEFT  OUTER JOIN finlink FL1
	ON FL1.idchild = expenseyear.idfin  AND FL1.nlevel = @level_input
...
BEGIN
	...
	JOIN finvar FVAR 
		ON FD.yvar = FVAR.yvar
		AND FD.nvar = FVAR.nvar
	JOIN upb 
...
WHERE expense.adate between @firstday and @stop
	...
	AND expense.nphase  BETWEEN @nfinphase and @nfourthphase 
...
-- Disp. del Subimpegno =  Subimpegno + Var.Subimpegno alla data -  (Liquidazioni +  Var.Liquidazioni alla data )
	...
	available_thirdphase = 
		...
		JOIN  expense E					 
			ON E.idexp = expenseyear.idexp  
		JOIN expenselink elk2
			ON elk2.idchild = E.idexp AND elk2.nlevel = @nthirdphase
		where elk2.idparent = #expense.nthirdphase
			AND E.nphase = @nfourthphase
			AND expenseyear.ayear =@ayear
			and #expense.nvarthirdphase is null
		),0) 
		- ISNULL((SELECT SUM(expensevar.amount)
...
-- Disp. della Liquidazione = Liquidazione + Var. Liquidazione - (Pagamenti + Var. Pagamenti)
	available_fourthphase =		
		fourthphase_amount 
		+ ISNULL((SELECT SUM(amount)
		FROM expensevar
		join expense on expensevar.idexp = expense.idexp
		WHERE expensevar.idexp= #expense.nfourthphase
			and expensevar.adate between @firstday and @stop
			and expensevar.yvar = @ayear
			and expense.nphase  = @nfourthphase
			and #expense.nvarfourthphase is null
		),0)
		- ISNULL((SELECT SUM(expenseyear.amount)
			from expenseyear
		JOIN expense E					 
		...
		JOIN expenselink elk3
			ON elk3.idchild = E.idexp AND elk3.nlevel = @nfourthphase
		where elk3.idparent = #expense.nfourthphase
		...
		JOIN  expense E					 
			ON E.idexp = expensevar.idexp  
		JOIN expenselink elk1
			ON elk1.idchild = E.idexp AND elk1.nlevel = @nfourthphase
		WHERE elk1.idparent = #expense.nfourthphase
			and expensevar.adate between @firstday and @stop
			and expensevar.yvar = @ayear
			AND E.nphase = @maxexpensephase
			and #expense.nvarfourthphase is null
...
UPDATE #expense SET 
	...
	fourthphase_C = (SELECT SUM(ISNULL(#appropriation_C.amount,0))  
				FROM #appropriation_C
				WHERE  #appropriation_C.idfin = #expense.idfin
					and isnull(#appropriation_C.idman,0) = isnull(#expense.idman,0)
					and #appropriation_C.idupb = #expense.idupb
					and #appropriation_C.nphase = @nfourthphase
					), 
	var_fourthphase_C = (SELECT SUM(ISNULL(#var_appropriation_C.amount,0))  
				FROM #var_appropriation_C
				WHERE  #var_appropriation_C.idfin = #expense.idfin
					and isnull(#var_appropriation_C.idman,0) = isnull(#expense.idman,0)
					and #var_appropriation_C.idupb = #expense.idupb
					and #var_appropriation_C.nphase = @nfourthphase
					),
	...
	var_thirdphase_R =(SELECT SUM(ISNULL(#var_appropriation_R.amount,0))  
  	...
  	fourthphase_R =(SELECT SUM(ISNULL(#appropriation_R.amount,0))  
				FROM #appropriation_R
				WHERE  #appropriation_R.idfin = #expense.idfin
					and isnull(#appropriation_R.idman,0) = isnull(#expense.idman,0)
					and #appropriation_R.idupb = #expense.idupb
					and #appropriation_R.nphase = @nfourthphase),  
	var_fourthphase_R =(SELECT SUM(ISNULL(#var_appropriation_R.amount,0))  
				FROM #var_appropriation_R
				WHERE  #var_appropriation_R.idfin = #expense.idfin
					and isnull(#var_appropriation_R.idman,0) = isnull(#expense.idman,0)
					and #var_appropriation_R.idupb = #expense.idupb
					and #var_appropriation_R.nphase = @nfourthphase),  
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	UPDATE #expense
		...
		total_fourthphase_C = (SELECT SUM(ISNULL(expenseyear.amount, 0))
					FROM expense
					JOIN expenseyear
						ON expenseyear.idexp = expense.idexp 
						AND expenseyear.ayear = @ayear
		...
		total_var_fourthphase_C = (SELECT SUM(ISNULL(expensevar.amount, 0))
					FROM expensevar
					JOIN expenseyear
						ON expenseyear.idexp = expensevar.idexp AND expenseyear.ayear = @ayear
					JOIN expense 
		...
		total_fourthphase_R = (SELECT SUM(ISNULL(expenseyear.amount, 0))
					FROM expenseyear
					JOIN expense
						ON expense.idexp = expenseyear.idexp 
						and expenseyear.ayear = @ayear
		...
		total_var_fourthphase_R = ( SELECT SUM(ISNULL(expensevar.amount, 0))
					FROM expensevar
					JOIN expenseyear
						ON expenseyear.idexp = expensevar.idexp 
						AND expenseyear.ayear = @ayear
...
AND reportname in ('rpt_partitario_spese_5f_resp_generale')
...
if (@suppressifblank = 'S') and @codelevel>2	--> se la stampa è x un livello sottostante la categoria cancella le righe
	BEGIN
		...
		ISNULL(fourthphase_C,0) = 0 AND
		ISNULL(var_fourthphase_C,0)= 0 AND
		...
		ISNULL(fourthphase_R,0)= 0 AND
		ISNULL(var_fourthphase_R,0)= 0 AND
...
select @fourthphase = description from  expensephase where nphase = @nfourthphase
...
SELECT 
	...
	sum(isnull(fourthphase_C,0)) as  fourthphase_C ,
	sum(isnull(var_fourthphase_C,0)) as  var_fourthphase_C ,
	...
	sum(isnull(fourthphase_R,0)) as  fourthphase_R ,
	sum(isnull(var_fourthphase_R,0)) as  var_fourthphase_R ,
	...
	yvarfourthphase,
	nvarfourthphase,
	sum(isnull(fourthphase_amount,0)) as fourthphase_amount,
	sum(isnull(available_fourthphase,0)) as available_fourthphase,
	...
	sum(isnull(total_fourthphase_C,0)) as total_fourthphase_C,
	sum(isnull(total_var_fourthphase_C,0)) as total_var_fourthphase_C,
	...
	sum(isnull(total_fourthphase_R,0)) as  total_fourthphase_R,
	sum(isnull(total_var_fourthphase_R,0))as total_var_fourthphase_R,
...
GROUP BY #expense.idupb,U.codeupb,#expense.idman,MAN.title,		
	...
	nfourthphase, ymovfourthphase,nmovfourthphase,yvarfourthphase,nvarfourthphase,
...
ORDER BY manager,F.printingorder, rowkind, adate,
	nvarappropriation,nvarsecondphase,nvarthirdphase,nvarfourthphase,nvarpayment
