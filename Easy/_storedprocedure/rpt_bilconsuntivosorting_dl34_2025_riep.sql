...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_bilconsuntivosorting_dl34_2025_riep]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_bilconsuntivosorting_dl34_2025_riep]
GO
...
-- exec rpt_bilconsuntivosorting_dl34_2025_riep 2025, {ts '2025-07-16 00:00:00'}, 4, '%', 'N', 'S', 'N', NULL, NULL, NULL, NULL, NULL, 32
 
CREATE   PROCEDURE [rpt_bilconsuntivosorting_dl34_2025_riep] 
(
	@ayear int,
...
SELECT
						...
						AND (sorFin.nlevel = @levelusable
							OR (sorFin.nlevel < @levelusable
								and (select count(*) from sorting S 
									 where S.idsorkind = @idsorkind_dl34_2025 and S.paridsor = sorFin.idsor)=0
								AND (sl.flag&2)<>0)
							)
					) THEN 'S'
					ELSE 'N'
	...
	where f.nlevel=1
		...
		and upb.idupb like @idupb 	and f.sortcode like @Root and f.idsorkind = @idsorkind_dl34_2025
...
BEGIN
	...
	where 
			f.nlevel=1 	and f.sortcode like @Root  and   f.idsorkind = @idsorkind_dl34_2025
