...
IF (
	...
	JOIN restrictedfunction RF
		ON RF.idrestrictedfunction = FF.idrestrictedfunction
	WHERE FF.idflowchart = @idflowchart
		AND RF.variablename in ( 'missioni'))) > 0
BEGIN
INSERT INTO #outtable
	SELECT DISTINCT M2.edittype
	FROM menu M
...
IF (
	...
	JOIN restrictedfunction RF
		ON RF.idrestrictedfunction = FF.idrestrictedfunction
	WHERE FF.idflowchart = @idflowchart
		AND RF.variablename = 'importazioni' ) > 0)
BEGIN
	INSERT INTO #outtable
	SELECT DISTINCT M2.edittype
	FROM menu M
	JOIN menu M2
		ON M.idmenu = M2.paridmenu
	WHERE M.menucode IN ('codconsolidamento')
	AND M2.metadata = 'no_table'
	AND NOT EXISTS
		(SELECT * FROM #outtable O
		WHERE O.edittype = M2.edittype)
END
-- Gestione Diangnostica Scritture
IF (
	(SELECT COUNT(*)
	FROM flowchartrestrictedfunction FF
	JOIN restrictedfunction RF
		ON RF.idrestrictedfunction = FF.idrestrictedfunction
	WHERE FF.idflowchart = @idflowchart
		AND RF.variablename = 'diagnostica_scritture' ) > 0)
BEGIN
	INSERT INTO #outtable
	SELECT DISTINCT M.edittype
	FROM menu M
	WHERE M.menucode IN ('coddiagnosticaep')
	AND M.metadata = 'no_table'
	AND NOT EXISTS
		(SELECT * FROM #outtable O
		WHERE O.edittype = M.edittype)
END
