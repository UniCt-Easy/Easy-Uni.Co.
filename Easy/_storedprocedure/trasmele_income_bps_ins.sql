...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_income_bps_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_income_bps_ins]
GO
...
CREATE  PROCEDURE [trasmele_income_bps_ins]
(
	@y int,
	@n int
)
...
(
	...
	curramount_expense decimal(19,2), -- importo corrente della spesa accessoria
	originalamount_expense decimal(19,2), -- importo originale della spesa accessoria
	idreg int,
...
(
	yproceedstransmission, nproceedstransmission, ydoc, ndoc, idinc, curramount,
	curramount_expense,
...
-- al momento la commento in quanto BPS non ha richiesto mai il collegamento 
-- tra pagamento e incasso di pari importo su partite di giro nell'ambito della
-- generazione dei contributi C / ENTE
/*
...
WHERE   incomelast.nbill IS NOT NULL
