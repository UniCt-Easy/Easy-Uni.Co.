...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_budgeteconomicoufficiale_automatic]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_budgeteconomicoufficiale_automatic]
GO
...
--  exec rpt_budgeteconomicoufficiale_automatic 2019, '%','N','S',{ts '2019-10-09 00:00:00'}
-- A.	Budget Economico che mostra la previsione iniziale 
-- Previsione iniziale = variazioni "integrazione previsione iniziale"
CREATE      PROCEDURE [rpt_budgeteconomicoufficiale_automatic](
...
if (@prevcorrente ='S') 
	Begin
		exec rpt_budgeteconomicoufficiale_automatic_curr @ayear, @idupb,@showchildupb,	@adate, @idsor01,@idsor02,@idsor03,	@idsor04,@idsor05
		RETURN
	End
