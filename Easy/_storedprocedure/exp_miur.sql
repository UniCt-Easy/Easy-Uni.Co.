...
if exists (select * from dbo.sysobjects where id = object_id(N'[exp_miur]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_miur]
GO
...
CREATE TABLE #mov_res (idmov varchar(40), amount decimal(19,2), curramount decimal(19,2), varamount decimal(19,2))
...
--			AND ISNULL(O1.curramount,0)<>0 
			...
			AND (sorting.start IS NULL OR sorting.start <= @ayear)
			AND (sorting.stop IS NULL OR sorting.stop >= @ayear)
...
BEGIN	
	...
	SELECT 	I1.idexp, C1.idsor, O1.flag&1, @eMIUR, SUM(C1.amount),
		...
		join sorting on sorting.idsor = c1.idsor
		JOIN expenseyear I1	-- I contiene il flag relativo della competenza dell'impegno classificato MIUR
...
--			AND ISNULL(O1.curramount,0)<>0 
	...
	INSERT INTO #class (idmov, idsor, sortcode, amount, rowkind)
	SELECT #mov_res.idmov, expensesorted.idsor, sorting.sortcode, SUM(expensesorted.amount), 'P'
	FROM #mov_res
	JOIN expensesorted
	...
	GROUP BY #mov_res.idmov, expensesorted.idsor, sorting.sortcode
		
	-- Caso 2. Movimento di Spesa con variazioni
	-- Inserimento di dettagli SIOPE sul mov. principale riferito alle trattenute (calcolo per rapporto)
	INSERT INTO #class (idmov, idsor, sortcode, amount, rowkind)
	SELECT #mov_res.idmov, expensesorted.idsor, sorting.sortcode,
	CASE
		WHEN #mov_res.curramount = 0 THEN #var_res.amount
		ELSE (SUM(expensesorted.amount) / #mov_res.curramount) * #var_res.amount
	END, 
	'V'
--	(SUM(expensesorted.amount) - #var_res.amount) * #var_res.amount / #mov_res.amount, 'V'
	FROM #var_res
	JOIN #mov_res
		ON #var_res.idmov = #mov_res.idmov
	JOIN expensesorted
	...
	WHERE sorting.idsorkind = @eMIUR
		AND #mov_res.varamount <> 0
		AND expensesorted.ayear = @ayear
	GROUP BY #mov_res.idmov, sorting.sortcode, #var_res.amount, #mov_res.curramount, expensesorted.idsor
	...
	ISNULL(
		...
		FROM #class
		WHERE #class.idmov = #mov_res.idmov
			AND #class.idsor = expensesorted.idsor
			AND #class.rowkind = 'V')
	,0), 'P'
	FROM #mov_res
	JOIN expensesorted
	...
	WHERE sorting.idsorkind = @eMIUR
		AND #mov_res.varamount <> 0
		AND expensesorted.ayear = @ayear
	GROUP BY #mov_res.idmov, sorting.sortcode, expensesorted.idsor
