...
if exists (select * from dbo.sysobjects where id = object_id(N'[rebuild_epexptotal_idmov]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rebuild_epexptotal_idmov]
GO
--setuser 'amm'
--rebuild_epexptotal_idmov 2017,1
CREATE PROCEDURE rebuild_epexptotal_idmov (
	@ayear int =  NULL,
	@idmov int
)
AS BEGIN
	...
	*/
	...
	DELETE from epexptotal where ((@ayear is null)  OR ayear=@ayear) and idepexp = @idmov 
	...
	INSERT INTO epexptotal 
		...
		FROM epexpyear
		WHERE  (@ayear is null)  OR ayear=@ayear and idepexp = @idmov ;
...
(select E.paridepexp, ET.ayear, SUM(ET.curramount),SUM(ET.curramount2),SUM(ET.curramount3),
		...
		WHERE  (@ayear is null) OR ET.ayear=@ayear and E.paridepexp = @idmov
...
left outer join sumexptot SE on ET.idepexp=SE.idepexp and ET.ayear=SE.ayear 
	where ((@ayear is null)  OR ET.ayear=@ayear) and ET.idepexp = @idmov 
;
	...
	with sumcosttot(idepexp,ayear,amount) as
		(select ED.idepexp, ED.yentry, SUM(ED.amount) from 
			...
			join account A on ED.idacc=A.idacc
				WHERE     ((@ayear is null) OR ED.yentry=@ayear) and (A.flagaccountusage & 320 <>0) and ED.idepexp = @idmov 
...
;
	...
	with sumdebittot(idepexp,ayear,amount) as
		(select ED.idepexp, ED.yentry, SUM(ED.amount) from 
			entrydetail ED JOIN entry E on ED.yentry = E.yentry AND ED.nentry = E.nentry 
			join account A on ED.idacc=A.idacc
				WHERE     ((@ayear is null) OR ED.yentry=@ayear) and (A.flagaccountusage & 16 <>0) and ED.idepexp = @idmov 
						and E.identrykind NOT IN (6,10,11,12)
				group by ED.idepexp, ED.yentry
		)
		UPDATE epexptotal
...
;
	...
	with sumcredittot(idepexp,ayear,amount) as
		(select ED.idepexp, ED.yentry, SUM(ED.amount) from 
			entrydetail ED JOIN entry E on ED.yentry = E.yentry AND ED.nentry = E.nentry 
			join account A on ED.idacc=A.idacc
				WHERE     ((@ayear is null) OR ED.yentry=@ayear) and (A.flagaccountusage & 32 <>0) and ED.idepexp = @idmov 
						and E.identrykind NOT IN (6,10,11,12)
				group by ED.idepexp, ED.yentry
		)
		UPDATE epexptotal
		SET debit = isnull(debit,0)- SE.amount
		from epexptotal ET
...
;
	...
	with sumrateotot(idepexp,ayear,amount) as
		(select ED.idepexp, ED.yentry, SUM(ED.amount) from 
			entrydetail ED JOIN entry E on ED.yentry = E.yentry AND ED.nentry = E.nentry 
			join account A on ED.idacc=A.idacc
				WHERE     ((@ayear is null) OR ED.yentry=@ayear) and (A.flagaccountusage & 2 <>0) and ED.idepexp= @idmov 
						and E.identrykind NOT IN (6,10,11,12)
				group by ED.idepexp, ED.yentry
		)
		UPDATE epexptotal
