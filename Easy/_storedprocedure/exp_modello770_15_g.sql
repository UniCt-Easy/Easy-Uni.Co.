...
if exists (SELECT * from dbo.sysobjects where id = object_id(N'[exp_modello770_15_g]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [exp_modello770_15_g]
GO
...
CREATE  PROCEDURE [exp_modello770_15_g] 
AS BEGIN
-- exec exp_modello770_15_g '770'
...
--SELECT * FROM #modulocococo
	...
	CREATE TABLE #ser (idser int, codeser varchar(20), annotation varchar(400), exemptioncode int)
	...
	--SELECT @denominazione, @a1cf
		...
		if (@idnationnascita is not null) 
			begin
		...
		AND EXISTS ( SELECT *  FROM expenselast
			...
			paymenttransmission.transmissiondate <= @12gen15_XXX))
		) 
...
-- Calcolo dell'acconto all'addizionale comunale all'IRPEF
		...
		-- altrimenti si valorizza il campo del saldo impostando a NULL il campo dell'acconto (chiedere a Nino)
			...
			INSERT INTO #ser (idser, codeser, annotation, exemptioncode)
...
IF
	...
	ORDER BY startpayroll,stoppayroll

	open @cursorecedolini
	fetch next from @cursorecedolini into @patcode, @startpayroll, @stoppayroll

	declare @oldstart smalldatetime
	if @startpayroll < @1gen14_XXX set @oldstart = @1gen14_XXX ELSE set @oldstart = @startpayroll
	if @stoppayroll > @31dic14_XXX set @oldstop = @31dic14_XXX ELSE set @oldstop = @stoppayroll
	-- Inserimento dei dati assicurativi distinti per mese
	WHILE @@fetch_status = 0
	begin
		--Dati assicurativi INAIL
		if @startpayroll < @1gen14_XXX set @startpayroll = @1gen14_XXX
		if @stoppayroll > @31dic14_XXX set @stoppayroll = @31dic14_XXX
	 
	--if datediff(d, @startpayroll, @oldstop) > 1
	--begin
	-- DCXXX035 Qualifica
	...
	BEGIN
		...
		BEGIN
			...
			---------------------------------------------------------
				...
				SET @text_for_annotation = (SELECT annotation FROM #ser WHERE codeser = @codeser) 
