...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_income_carime_var_new]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_income_carime_var_new]
GO
...
CREATE        PROCEDURE [trasmele_income_carime_var_new]
(
	@y int,
	@n int
)
...
JOIN proceedstransmission tv
	ON tv.kproceedstransmission = v.kproceedstransmission
...
WHERE d.npay IN
	(SELECT DISTINCT payment.npay
		...
		WHERE proceeds.kpro IN (SELECT kpro FROM #proceedsvar) )
AND s.ymov = @y
AND i.ayear = @y
...
WHERE autokind = 10 AND
((SELECT COUNT(idpro) FROM proceeds_bank WHERE proceeds_bank.kpro = #proceedsvar.kpro) = 1)
AND
((SELECT COUNT(idinc) FROM #proceeds WHERE #proceeds.idinc = #proceedsvar.idinc AND ISNULL(#proceeds.curramount,0) <> 0) = 0)
...
WHERE I.nphase = @maxincomephase
	...
	AND NOT EXISTS (SELECT * FROM #proceedsvar PV WHERE PV.idinc = P.idinc AND ((PV.autokind = 11) OR (PV.autokind = 10 AND ISNULL(P.curramount,0) = 0 ) ))
...
SELECT
	...
	#proceeds.cupcodefin, #proceeds.cupcodeupb, #proceeds.cupcodeincome, 1,#proceeds.flagpendingincome
FROM #proceeds
JOIN incomesorted
...
-------------------------------
/*
select yproceedstransmission, nproceedstransmission, transmissiondate,
		 ydoc, ndoc, ndocformatted,fulfilled 
FROM #proceeds  -- nessun filtro sul record O, testata della reversale 
...
SELECT
	...
	ISNULL(opkind,'VAR') + 
	'R'
FROM #proceeds  -- record O, testata della reversale 
GROUP BY yproceedstransmission, nproceedstransmission, transmissiondate,
		 ydoc, ndoc, ndocformatted,fulfilled, ISNULL(opkind,'VAR')
...
SELECT
	...
	REPLICATE('0',9) +
	CASE
		WHEN ((fulfilled = 'S') AND (flagbankitaliaproceeds = 'S') AND (accountkind = 'F')) THEN '04'  -- FRUTTIFERA LIBERA ACCR. TPS
		WHEN ((fulfilled = 'S') AND (flagbankitaliaproceeds = 'S') AND (accountkind = 'I')) THEN '05'  -- INFRUTTIFERA LIBERA ACCR. TPS
	...
	'R'	
	FROM #proceeds
	WHERE (isnull(#proceeds.autokind,22) NOT IN(10,11))
	OR (isnull(#proceeds.autokind,22) = 10 AND ISNULL(#proceeds.curramount,0) <> 0)
	...
	stamp_charge,ccp_collection,ISNULL(opkind,'VAR') ,accountkind
...
SELECT
	...
	'R'
FROM #proceeds
WHERE registry_tosplit = 'S' AND ((isnull(#proceeds.autokind,22) NOT IN(10,11))
	OR (isnull(#proceeds.autokind,22)= 10 AND ISNULL(#proceeds.curramount,0) <> 0))
...
SELECT
	...
	'R'
FROM #proceeds
WHERE proceedsdescr_tosplit = 'S' AND ((isnull(#proceeds.autokind,22) NOT IN(10,11))
	OR (isnull(#proceeds.autokind,22)= 10 AND ISNULL(#proceeds.curramount,0) <> 0))
	
GROUP BY
yproceedstransmission, nproceedstransmission, ydoc, ndoc, 
ndocformatted, idpro,newproceedsdescr, ISNULL(opkind,'VAR') 
...
GROUP BY #proceeds.yproceedstransmission, #proceeds.nproceedstransmission, #proceeds.ydoc, #proceeds.ndoc,
	#proceeds.ndocformatted, #payment.ydoc, #payment.ndocformatted, #proceeds.idpro, 
	#payment.idpay, #proceeds.idexp, ISNULL(opkind,'VAR')
*/
...
SELECT
	...
	-- FILLER
	SPACE(336) +
	ISNULL(#proceeds.opkind,'VAR') + 
	'R'
FROM #proceeds JOIN #incomebill ON #proceeds.idinc = #incomebill.idinc 
WHERE  ((isnull(#proceeds.autokind,22) NOT IN(10,11))
	OR (isnull(#proceeds.autokind,22) = 10 AND ISNULL(#proceeds.curramount,0) <> 0))
GROUP BY #proceeds.yproceedstransmission, #proceeds.nproceedstransmission, #proceeds.ydoc,#proceeds.ndoc, 
#proceeds.ndocformatted, #proceeds.idpro, #incomebill.nbill, ISNULL(#proceeds.opkind,'VAR')
