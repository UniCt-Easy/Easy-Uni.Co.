...
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_expense_mps_abi36_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_expense_mps_abi36_ins]
GO
...
CREATE        PROCEDURE [trasmele_expense_mps_abi36_ins]
(
	@y int,
	@n int
)
AS BEGIN
-- setuser 'amministrazione'
--[trasmele_expense_mps_abi36ins] 2018,3
--------------------------------------------------------------
...
---- CONTROLLO N. 16. anagrafica priva di codice fiscale in un pagamento per cassa
--IF EXISTS
--(SELECT * FROM paymentcommunicated
--        join expenselast
--        	ON paymentcommunicated.idexp = expenselast.idexp
...
--WHERE ypaymenttransmission = @y
--	AND npaymenttransmission = @n
--	-- modalità di pagamento Sportello, non a regolarizzazione
--	AND 
--	(
--		(paymethod.methodbankcode  = '01'
--		AND (( expenselast.flag & 1) = 0) )
--			OR EXISTS (SELECT * FROM expenseview WHERE expenseview.idexp =  expenselast.idexp AND  netamount = 0)
--	)
...
--		(SELECT 'Al movimento n.' + CONVERT(varchar(6),paymentcommunicated.nmov) 
--		+ '/' + CONVERT(varchar(4),paymentcommunicated.ymov)
--		+ ' non è stato inserito il Codice Fiscale dell''anagrafica, obbligatorio nella modalità di pagamento per cassa. Correggere l''anagrafica e restrarre il flusso '
--	FROM paymentcommunicated
--        join expenselast
--        	ON paymentcommunicated.idexp = expenselast.idexp
--        join expense
