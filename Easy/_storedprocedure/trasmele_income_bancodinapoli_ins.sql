...
--setuser 'amm' 
if exists (select * from dbo.sysobjects where id = object_id(N'[trasmele_income_bancodinapoli_ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [trasmele_income_bancodinapoli_ins]
GO
...
CREATE  PROCEDURE [trasmele_income_bancodinapoli_ins]
(
	@y int,
	@n int
)
...
-- per gli incassi virtuali non c'è bisogno di sdoppiare le insert, in quanto abbiamo già gli incassi sdoppiati 
-- nella tabella #proceeds
INSERT INTO #siope
(
	yproceedstransmission, nproceedstransmission, ydoc, ndoc, ndocformatted, idpro,
	sortcode, descr_sorting, curramount, amount, amount_expense,cupcodefin, cupcodeupb, cupcodeincome,flagpendingincome
)
SELECT
...
(
	...
	idpaymethodEASY, idpaymethodTRS, ABI, CAB, cc, cin, 
	iddeputy, idpay,flagpendingincome
)
SELECT 
...
(
	ydoc, ndoc, idexp, curramount, idreg,
	idpaymethodEASY, idpaymethodTRS, ABI, CAB, cc, cin, iddeputy, idpay, flagpendingincome
)
SELECT 
	s.ymov, d.npay, s.idexp, i.curramount, s.idreg,
	el.idpaymethod, m.methodbankcode,
...
---- RECORD SR SOSPESA LA SCRITTTURA DEL RECORD SR PER IL MOMENTO
...
---- RECORD DM SOSPESA LA SCITTTURA DEL RECORD DM PER IL MOMENTO
--INSERT INTO #trace (y, n, ndoc, nrow, out_str)
--SELECT
--	yproceedstransmission, nproceedstransmission, ndoc,  4 * idpro + 2,
