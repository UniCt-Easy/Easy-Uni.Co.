...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_bilconsuntivo_cassa_comp_residui]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_bilconsuntivo_cassa_comp_residui]
GO
...
CREATE  PROCEDURE [rpt_bilconsuntivo_cassa_comp_residui]
(
	@ayear int,
	@date datetime,
	@finpart char(1),
...
--[rpt_bilconsuntivo_cassa_comp_residui]  2013, {ts '2013-12-31 00:00:00'},'E',3,'0001','N','S','S','S','S' 
...
GROUP BY 
	...
	showincomesurplus,f.nlevel,isnull(#data.tosuppress, @suppressifblank)	,
	 f.flag,f1.flag,f2.flag,f3.flag,f4.flag,f5.flag
...
where  ((@MostraTutteVoci = 'S'  or @suppressifblank='N') OR  (#data.idupb='0001' or isnull(#data.tosuppress, 'S')='N'))AND
	...
	f.printingorder, f1.title,f2.title,f3.title,f4.title,f5.title,
	showincomesurplus,f.nlevel,  f.flag,f1.flag,f2.flag,f3.flag,f4.flag,f5.flag,
