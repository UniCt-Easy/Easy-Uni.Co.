...
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt_budgetinvestimentiufficiale_triennale_puro]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt_budgetinvestimentiufficiale_triennale_puro]
GO
...
SET ANSI_NULLS ON 
GO
-- exec rpt_budgetinvestimentiufficiale_triennale_puro 2018, '%','S',null,null,null,null,null, 'S'
CREATE      PROCEDURE [rpt_budgetinvestimentiufficiale_triennale_puro](
...
insert into  #budget_puro (  code, sortcode, description,printingorder,
...
select  #impieghi.code, #impieghi.sortcode, #impieghi.description, #impieghi.printingorder, 
	...
	isnull(accountvardetail.amount,0),  --- totale impieghi non ripartito

	CASE WHEN accountvardetail.underwritingkind = 'C' THEN isnull(accountvardetail.amount2,0) ELSE 0 END  ,
	CASE WHEN accountvardetail.underwritingkind = 'I' THEN isnull(accountvardetail.amount2,0) ELSE 0 END  ,
	CASE WHEN accountvardetail.underwritingkind = 'p' THEN isnull(accountvardetail.amount2,0) ELSE 0 END  ,
	isnull(accountvardetail.amount2,0),  --- totale impieghi non ripartito
	
	CASE WHEN accountvardetail.underwritingkind = 'C' THEN isnull(accountvardetail.amount3,0) ELSE 0 END  ,
	CASE WHEN accountvardetail.underwritingkind = 'I' THEN isnull(accountvardetail.amount3,0) ELSE 0 END  ,
	CASE WHEN accountvardetail.underwritingkind = 'p' THEN isnull(accountvardetail.amount3,0) ELSE 0 END  ,
	isnull(accountvardetail.amount3,0)  --- totale impieghi non ripartito
FROM accountvar  
