...
namespace income_wizardinvoicedetailnoestimate {
    public partial class FrmIncome_WizardInvoiceDetailNoEstimate : Form {
            ...
            if ((oldTab == 1) && (newTab == 2)) {
                ...
                if (!verificaSeUPBUniformi(upb, upb_iva, causale)) {
                    MessageBox.Show(this,
                        "Attenzione i dettagli selezionati hanno UPB non uniformi non si puÚ andare avanti");
                    return false;
                }
                txtPerc.Text = "100";
        ...
        void AggiornaGridDettagliFattura() {
            ...
            if (causale == 1) {
                // Se Ë abilitato DOCUM significa che non ci sono contabilizzazioni diverse da ORDIN attivate,
            ...
            if (causale == 3) {
                // Se Ë abilitato IMPON significa che non ci sono contabilizzazioni ORDIN attivate,
            ...
            if (causale == 2) {
                // Se Ë abilitato IMPOS significa che non ci sono contabilizzazioni diverse da ORDIN attivate,
        ...
        private bool verificaSeUPBUniformi(object[] upb, object[] upb_iva, int causale) {
            ...
            // non ci sono righe nell'array di oggetti o se Ë una sola OK
            if ((upb_to_consider.Length == 0) || (upb_to_consider.Length == 1)) return true;
            // Se esistono pi˘ di 2 righe sicuramente ci sono UPB differenti
            if (upb_to_consider.Length > 2) return false;
            // Se ci sono 2 righe allora bisogna vedere se una delle due Ë NULL, se si ok altrimenti no
        ...
        void SetNewLinkedMov() {
            ...
            AbilitaDisabilitaSelezioneMovimento(true);
            SelezioneMovimentiEffettuata = false;
            labMsgTODO1.Text = "Sar‡ creato un nuovo movimento di entrata";
        ...
        bool CheckInfoFin() {
            ...
            if (radioNewLinkedMov.Checked) {
                ...
                // in cui non Ë prevista l'informazione del bilancio
            }
            decimal amount = 0;
            decimal ImportoDaPagare = CfgFn.GetNoNullDecimal(
                HelpForm.GetObjectFromString(typeof(decimal),
        ...
        void RecalcOperationsToDo() {
            ...
            if (ImportoDaPagare > ImportoMax) {
                MessageBox.Show("L'importo da pagare Ë superiore al totale dei dettagli selezionati");
                txtDaPagare.Text = "";
                ImportoDaPagare = 0;
                return;
            }
