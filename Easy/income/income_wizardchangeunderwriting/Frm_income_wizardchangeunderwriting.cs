...
namespace income_wizardchangeunderwriting {//spesawizardelimina//
	...
	public class Frm_income_wizardchangeunderwriting : System.Windows.Forms.Form {
		...
		bool GetUnderwriting(){
            ...
            {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Selezionare un Finanziamento per procedere");
                return false;
            }
        ...
        bool GetIncome () {
            ...
            if (txtNmovIncome.Text.Trim() == "") {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Selezionare un movimento di entrata per procedere");
                return false;
            }
        ...
        bool doAssocia () {
            ...
            {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stato selezionato un finanziamento");
                return false;
            }
            if (DS.income.Rows.Count == 0) {
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Non è stato selezionato nessun movimento finanziamento");
                return false;
            }
            ...
            if (MetaFactory.factory.getSingleton<IMessageShower>().Show(this, "Si desidera associare il finanziamento all' Accertamento e agli Incassi selezionati?", "Conferma", MessageBoxButtons.OKCancel) == DialogResult.OK) {
            ...
            if (res) MetaFactory.factory.getSingleton<IMessageShower>().Show("Operazione effettuata.");
            else
                MetaFactory.factory.getSingleton<IMessageShower>().Show("Operazione non effettuata.");
        ...
        private void btnFinanziamento_Click(object sender, EventArgs e)
        {
            ...
            if (chkFilterAvailable.Checked)
            {
                ...
                if (N == 0)
                {
                    MetaFactory.factory.getSingleton<IMessageShower>().Show("Non vi sono Finanziamenti con disponibilità sufficiente");
                    return;
                }
