			...
			try {
				...
				for (int fase = faseinizio; fase <= faseentratafine; fase++) {
					...
					foreach (DataColumn C in DSP.Tables["income"].Columns) {
						...
						if ((C.ColumnName == "flag") && (fase != faseestimate))
						{
							// azzera bit 0 del flag di income
							int flag = CfgFn.GetNoNullInt32(NewRow["flag"]);
							NewRow["flag"] = flag &= ~(1 << 0);// azzera il bit 0 di income se non è la fase di contabilizzazione"
					...
					if (fase == faseentratamax) {
						...
						if (DSP.Tables.Contains("incomelastestimatedetail")) {
							foreach (DataRow ChildR in DSP.Tables["incomelastestimatedetail"].Rows) {
								ChildR["idinc"] = NewRow["idinc"];
							}
						}
