	...
	public class Meta_incomeview : Meta_easydata {
			...
			if (R.Table.Columns["ayear"] != null) {
				if (R["ayear"].ToString() != GetSys("esercizio").ToString()) {
					MetaFactory.factory.getSingleton<IMessageShower>().Show(
						"L'entrata selezionata non è presente in questo esercizio quindi non è selezionabile.");
					return false;
				}
			}
		...
		public void CompletaCaption(DataTable T) {
			...
			foreach (DataColumn C in T.Columns) {
				if ((C.ColumnName == "idexp_lined") && ((C.Caption == "") || (C.Caption == C.ColumnName))) {
					DescribeAColumn(T, C.ColumnName, ".idexp p.giro");
					continue;
				}

				if ((C.ColumnName == "nexp_linked") && ((C.Caption == "") || (C.Caption == C.ColumnName))) {
					DescribeAColumn(T, C.ColumnName, ".n.mov.spesa p.giro");
					continue;
				}

				if ((C.ColumnName == "yexp_linked") && ((C.Caption == "") || (C.Caption == C.ColumnName))) {
					DescribeAColumn(T, C.ColumnName, ".es.mov.spesa p.giro");
					continue;
				}
				...
				if ((C.ColumnName == "cf") && ((C.Caption == "") || (C.Caption == C.ColumnName))) {
					DescribeAColumn(T, C.ColumnName, ".C.F.");
					continue;
				}

				if ((C.ColumnName == "p_iva") && ((C.Caption == "") || (C.Caption == C.ColumnName))) {
					DescribeAColumn(T, C.ColumnName, ".P.Iva");
					continue;
				}
		...
		public override void DescribeColumns(DataTable T, string ListingType) {
			...
			switch (ListingType) {
				...
				case "default": {
					...
					DescribeAColumn(T, "nexp_linked", "N.mov.spesa p.giro", npos++);
					CompletaCaption(T);
					break;
				}
			}
