	...
	public class Frm_finbalance_default : System.Windows.Forms.Form
	{
		...
		void salvaDati() {
			...
			if (Conn.RUN_SELECT_COUNT("finvar",QHS.AppAnd(
				...
				if (MetaFactory.factory.getSingleton<IMessageShower>().Show("Esiste già la variazione di assestamento dell'avanzo di cassa effettivo. Procedo comunque?",
			...
			if (AbilitaColonneCompetenza) {
                if (Conn.RUN_SELECT_COUNT("finvar", QHS.AppAnd(
                    ...
                    if (MetaFactory.factory.getSingleton<IMessageShower>().Show("Esiste già la variazione di assestamento dell'avanzo di amministrazione effettivo. Procedo comunque?",
