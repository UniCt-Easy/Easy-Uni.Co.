...
namespace meta_locationstrutturaview
{
    public class Meta_locationstrutturaview : Meta_easydata 
	{
        public Meta_locationstrutturaview(DataAccess Conn, MetaDataDispatcher Dispatcher) :
            base(Conn, Dispatcher, "locationstrutturaview") {
			...
			switch (edit_type) {
				case "struttura": {
						...
						if (R.Table.Columns.Contains("idupb") && R["idupb"].ToString().Trim() == "") {
							errmess = "Attenzione! Il campo 'Unità previsionale di base (bilancio)' è obbligatorio";
							errfield = "idupb";
							return false;
						}
						if (R.Table.Columns.Contains("idupb") && R["idupb"].ToString().Trim().Length > 36) {
							errmess = "Attenzione! Il campo 'Unità previsionale di base (bilancio)' può essere al massimo di 36 caratteri";
							errfield = "idupb";
							return false;
						}
		...
		public override void DescribeColumns(DataTable T, string ListingType) {
			...
			switch (ListingType) {
				case "struttura": {
						DescribeAColumn(T, "locationparent_description", "Struttura madre", nPos++);
