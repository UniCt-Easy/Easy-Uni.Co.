...
namespace csa_import_default {
    ...
    public class Frm_csa_import_default : System.Windows.Forms.Form {
        ...
        private void InitializeComponent() {
            ...
            this.btnSimulaEPGenera.Text = "Simula Genera Impegni di Budget";
        ...
        public void MetaData_AfterFill() {
            ...
            if ((Meta.InsertMode) || (Meta.EditMode)) {
                ...
                if (!VerificaMovFinanziari("L", false)) btnDelete.Enabled = false;
                if (!VerificaMovFinanziari("V", false)) btnDelete.Enabled = false;
                //if (!VerificaMovFinanziari("L", false)) btnElabora.Enabled = false;
                //if (!VerificaMovFinanziari("V", false)) btnElabora.Enabled = false;
                if (!VerificaScritture(false)) btnDelete.Enabled = false;
                //if (!VerificaScritture(false)) btnElabora.Enabled = false;
            }
            else {
        ...
        private bool CalcolaRighe() {
            ...
            if (Meta.EditMode) {
                btnInputRiepiloghi.Enabled = (countRiep == 0);
                btnInputVersamenti.Enabled = (countVer == 0);   
		...
		private bool VerificaMovFinanziari(string kind, bool message) {
            //if (Meta.IsEmpty) return false;
        ...
        private bool VerificaScritture(bool message) {
            //if (Meta.IsEmpty) return false;
            DataRow Curr = DS.csa_import.Rows[0];
        ...
        public void MetaData_AfterClear() {
            ...
            txtInputFile.Text = "";
            txtEsercImport.ReadOnly = true;
            txtEsercImport.Text = Meta.GetSys("esercizio").ToString();
        ...
        private void btnElabora_Click(object sender, System.EventArgs e) {
            ...
            if (DS.csa_importriep_partition.Rows.Count == 0 && DS.csa_importver_partition.Rows.Count == 0) {
                ...
                foreach (DataRow r in DS.csa_importriep.Rows) {
                    ...
                    if (DS.csa_importriep_expense.Select(QHC.CmpEq("idriep", r["idriep"])).Length == 0)
                ...
                foreach (DataRow r in DS.csa_importriep.Rows) {
                    if (r["idexp"] != DBNull.Value) continue;
                    if (r["idcsa_contract"] == DBNull.Value) continue;
                    if (DS.csa_importriep_epexp.Select(QHC.CmpEq("idriep", r["idriep"])).Length == 0)
                ...
                foreach (DataRow r in DS.csa_importver.Rows) {
                    ...
                    if (DS.csa_importver_expense.Select(QHC.CmpEq("idver", r["idver"])).Length == 0)
                ...
                foreach (DataRow r in DS.csa_importver.Rows) {
                    ...
                    if (r["idcsa_contracttax"] == DBNull.Value) continue;
                    if (DS.csa_importver_epexp.Select(QHC.CmpEq("idver", r["idver"])).Length == 0)
            ...
            Meta.SaveFormData();
            if (DS.HasChanges()) {
                MessageBox.Show(this, "Elaborazione faliita, è necessario cancellarla e rifarla.", "Errore");
                return;
            }
            //Meta.FreshForm();
        ...
        private void ImpostaColonne(string kind) {
            ...
            switch (kind) {
                case "R": {
                        mData.Columns.Add("RUOLO", typeof(string));
                        mData.Columns.Add("CAPITOLO", typeof(string));
                        mData.Columns.Add("COMPETENZA", typeof(int));
                        mData.Columns.Add("MATRICOLA", typeof(string));
                        mData.Columns.Add("LORDO", typeof(decimal));
                        break;
                    }
                case "V": {
                        mData.Columns.Add("ENTE", typeof(string));
                        mData.Columns.Add("ENTE_DESCR", typeof(string));
                        mData.Columns.Add("VOCE", typeof(string));
                        mData.Columns.Add("RUOLO", typeof(string));
                        mData.Columns.Add("CAPITOLO", typeof(string));
                        mData.Columns.Add("COMPETENZA", typeof(int));
                        mData.Columns.Add("MATRICOLA", typeof(string));
                        mData.Columns.Add("IMP_TOT", typeof(decimal));
                        break;
                    }
                case "S": {
        ...
        private bool verificaValiditaFileExcel(string task) {
            ...
            switch (task) {
                case "R": {
                    ...
                    elencoColonne.Add("MATRICOLA");
                    elencoColonne.Add("LORDO");
                    break;
                }
                case "V": {
                    elencoColonne.Add("ENTE");
                    //elencoColonne.Add("ENTE_DESCR");//opzionale
                ...
                case "S": {
                        elencoColonne.Add("DENOMINAZIONE_ANAGRAFICA");
                        elencoColonne.Add("N_SOSPESO");
                        elencoColonne.Add("IMPORTO");
                        break;
                    }
            }
            ...
            foreach (string col in elencoColonne) {
                if (!mData.Columns.Contains(col)) {
                    MessageBox.Show(this, "Nel file " + openInputFileDlg.FileName + " non esiste la colonna " + col,
                        "Errore");
                    return false;
                }
            }
        ...
        /// </summary>
        private void fillRiepiloghi() {
            if (Meta.IsEmpty) return;
            int step = mData.Rows.Count/100;
    ...
    void VisualizzaVoceCreditore(DataRow R) {
        ...
        private void btnDelete_Click(object sender, EventArgs e) {
            if (Meta.IsEmpty) return;
            if (Meta.InsertMode) return;
            if (!VerificaMovFinanziari("L", false)) return;
            if (!VerificaMovFinanziari("V", false)) return;
            if (!VerificaScritture(false)) return;
        ...
        /*
            ...
            flagclawback = 'R': idfin_income

            e poi se ha il cap. di spesa nel versamento deve avere anche l'entrata e viceversa (sempre per i recuperi)
            flagclawback = 'R': idfin_expense --> idfin_incomeclawback
            flagclawback = 'R': idfin_incomeclawback--> idfin_expense
        ...
        private void btnGeneraMovFin_Click(object sender, System.EventArgs e, string kind) {
            ...
            if (!VerificaMovFinanziari(kind, true)) return;
            if (!CalcolaRighe()) {
                MessageBox.Show(this, "Non ci sono dati da elaborare");
                return;
            }
 ...
 */
        ...
        private bool generaMovPrincipali(string IoE,bool nonPagareNettiPositivi,  Dictionary<int, Dictionary<int, decimal>> sospesi, bool lordi) {
            ...
            //        MessageBox.Show("Errore", R["kind"].ToString() + " indice:" + i.ToString() + 
                    ...
                    if (faseCorrente == fasemax) {
                        ...
                        if (IoE == "E") {
                           ...
                           if ((ModPagam == null) && (!_nuovaGestione)) {
                            ...
                            // MessageBox.Show("sospeso " + R["nbill"].ToString() + " " + "anagr." + codicecreddeb.ToString());
                            //MessageBox.Show("nuova gestione" +_nuovaGestione.ToString());
        ...
        private void btnVisualizzaScritture_Click(object sender, EventArgs e) {
            if (!VerificaScritture(false)) //esistono scritture in PD
                EditEntry();
        }
        ...
        private void btnSimulaEPGenera_Click(object sender, EventArgs e) {
            ...
            //raggruppamenti su conto, upb, lettura prev. disponibili dei conti interessati etc. 
    ...
    class Fake_EpPoster :ep_poster {
        ...
        public ProcedureMessageCollection DO_POST_SERVICE() {
        ...
        public bool saveData(DataSet d,PostData post) {
            this.d = d;
            return true;
        }
