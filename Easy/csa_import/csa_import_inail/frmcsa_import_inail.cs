...
namespace csa_import_inail {
    public partial class frmcsa_import_inail :Form {
		...
		string getHash(DataRow r, string[] listaCampi) {
			...
			string []fields = null;
			if ((r.Table.TableName.ToString() == "csa_importver_partition_expenseview") || (r.Table.TableName.ToString() == "csa_importver_partition_incomeview")){
		...
		private bool GetElencoVociCSA() {
			...
			dgrImportazioni.DataSource = null;
			dgrEntiVersamento.DataSource = null;
			string filter =  QHS.AppAnd( QHS.CmpEq("yimport", getIntSys("esercizio")), QHS.CmpEq("ayear", getIntSys("esercizio")));
        ...
        private int getIntSys(string nome) {
			 ...
			 * */
			sb.AppendLine($"exec  compute_csa_versamenti_partition {Conn.GetEsercizio()},null,@lista_idcsa_import, @lista_idreg");
		...
		private void FillMovSorted(bool azzera, bool lastphase,int index, string IoE, DataRow NewMovRow, string field_for_idsor) {
			string tMainSorted = (IoE == "I") ? "incomesorted" : "expensesorted";
			string idMovField = (IoE == "I") ? "idinc" : "idexp";
			string kind = (IoE == "I") ? "Entrata" : "Spesa";
			Dictionary<int, List < DataRow >> classDict = (IoE == "I") ? ClassEntrateLast : ClassSpeseLast;
		...
		private bool generaMovPrincipali(string IoE, bool nonPagareNettiPositivi, Dictionary<int, Dictionary<int, decimal>> sospesi, bool lordi) {
			...
			for (int ii = 0; ii < SP_Result.Rows.Count; ii++) {
				...
				for (int faseCorrente = faseIniziale; faseCorrente <= fasemax; faseCorrente++) {
					...
					//Salta la generazione dei finanziamenti dell'impegno se la riga corrente è associata ad un impegno esistente
					if (!_nuovaGestione) {
					...
					if (faseCorrente == fasemax) {
						...
						if (IoE == "E") {
							...
							if ((_nuovaGestione) && !(azzeraUltimaFase)) {
						...
						FillMovSortedFaseMAX(false,/*azzeraUltimaFase*/ NRIGA, IoE, NewMovRow); // inserisce le classificazioni 
						...
						if (azzeraUltimaFase) {
							...
							if (valore_da_azzerare > 0) {
								DRVarUltimaFase = MetaVarUltimaFase.Get_New_Row(NewMovRow, dsFinancial.Tables[tMainVar]);
					...
					FillImportMov(azzeraUltimaFase, NewImportMovRow, R, idcsa_import);
					...
					if (_nuovaGestione) {
						if (movimentiRaggruppati == null) {
							...
							FillImportMov(azzeraUltimaFase, NewImportMovPlusRow, R, idcsa_import);
							NewImportMovPlusRow[idMovField] = R["idmovimento"];
							...
							foreach (var rQuota in movimentiRaggruppati.getLinkedToMov(R)) {
								...
								FillImportMov(azzeraUltimaFase, NewImportMovPlusRow, rQuota.mov, idcsa_import);
		...
		private bool CallStoredProcedure(List<DataRow> LsImportazioni, List<DataRow> LsEnti) {
			...
			OutTable = callSp(Conn,(from r in LsImportazioni select (int)r["idcsa_import"]).ToList(),(from r in LsEnti select (int)r["idreg_agency"]).ToList());
		...
		private void btnGeneraMovFin_Click(object sender, System.EventArgs e, string kind) {
			...
			foreach (DataRow rVer in OutTable.Rows) {
                ...
                if (nuovaGestione()) {
                    if (!SP_Result.Columns.Contains("nbill"))
				...
				if (movimentiRaggruppati!=null) {
					...
					foreach (DataRow r in SP_Result.Rows) {
						object idcsa_import = r["idcsa_import"];
						if (!csaDescr.ContainsKey(idcsa_import)) continue;
						string newDescr =  csaDescr[idcsa_import];
						var linkedCsa = movimentiRaggruppati.getLinkedToMov(r);
						if (linkedCsa.Count > 1) {
							r["description"] = newDescr;
						}					 
					}
				}
        ...
        private void btnVersamenti_Click(object sender, EventArgs e) {
           ...
           // btnVersamenti.Visible = false;
			...
			if (chkAzzeraUltimaFase.Checked) message = "Si è scelto di generare i movimenti finanziari esclusa l'ultima fase?";
