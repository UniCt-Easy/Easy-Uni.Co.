...
namespace csa_import_inail {
	public partial class frmcsa_import_inail : MetaDataForm {
		MetaData Meta;
		DataAccess Conn;
		...
		static DataTable callSp(string sp_name, DataAccess Conn, List<int> idcsaimport_List, List<int> idreg_List) {
			 ...
			 * */
			sb.AppendLine(
				$"exec  {sp_name} {Conn.GetEsercizio()},null,@lista_idcsa_import, @lista_idreg");
			QueryCreator.MarkEvent(sb.ToString());
			string ErrMsg;
			return Conn.SQLRunner(sb.ToString(), 60, out ErrMsg);
		...
		private bool CallStoredProcedure(string kind,List<DataRow> LsImportazioni, List<DataRow> LsEnti) {
			string sp_name;
			if (kind == "C") // check
				{
				sp_name = "exp_csa_fin_upb_available";
				}
			else {  // generazione posticipati
				sp_name = "compute_csa_versamenti_partition";
			}
			OutTable = callSp(sp_name, Conn, 
		...
		private bool CalcolaBilancioUPBPrevisioneInsufficiente(DataTable Automatismi) {
			DataSet ds = new DataSet();
			DataTable tResult = new DataTable("tVerifiche");
		...
		private void btnGeneraMovFin_Click(object sender, System.EventArgs e, string kind) {
			...
			if ((LsEnti.Count == 0) && (LsImportazioni.Count == 0)) {
				show("E' necessario selezionare almeno un Ente e almeno un'Importazione");
				return;
			}

			if (CallStoredProcedure("V", LsImportazioni, LsEnti)) {
				...
				if ((SP_Result == null) || SP_Result.Rows.Count == 0) {
					show("Per gli Enti e le Importazioni selezionate non ci sono righe da elaborare");
					return;
				}
				else {
				...
				if (!CalcolaMovConDisponibileInsufficiente(SP_Result))  {
					show("Per gli Enti e le Importazioni selezionate saranno generati movimenti con disponibile insufficiente");
					//return;
				}
				...
				show(this, "Per gli Enti e le Importazioni selezionate non ci sono righe da elaborare ");
			}
		...
		bool success;
		private void btnVerifiche_Click(object sender, EventArgs e) {
			success = true;
			string message = "";
			lblTask.Text = "";
			DataRow[] ImportazioniSelected = GetGridSelectedRows(dgrImportazioni);
			...
			DataTable Verifiche = callSp("exp_csa_fin_upb_available_posticipati", Conn,
				(from r in LsImportazioni select (int)r["idcsa_import"]).ToList(),
				(from r in LsEnti select (int)r["idreg_agency"]).ToList());

			// Se ci sono coppie UPB Bilancio con previsione disponibile insfficiente vengono visualizzati in Excel 

			if ((Verifiche != null) && (Verifiche.Rows.Count > 0)) {
				show(this, "Esistono delle coppie UPB Bilancio con previsione insufficiente: elenco");
				exportclass.DataTableToExcel(Verifiche, true);
				success = false;
			}
		}
	}
