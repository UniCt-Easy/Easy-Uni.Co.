...
public partial class expSiopeTrasparenza_art4bis : System.Web.UI.Page {
    ...
    void GestisciDataRequest(DataAccess DepConn) {
        ...
        if (Request.Params["trimestre"] != null && Request.Params["trimestre"].ToString() != "") {
            int tt = CfgFn.GetNoNullInt32(Request.Params["trimestre"]);
            if (tt != 0)
                trimestre = tt;
        }
        DataTable Res = GetMainData(DepConn, ayear,trimestre);
    ...
    void RenderHtmlPage(DataAccess DepConn) {
        ...
        int? n_trimestre = CfgFn.GetNoNullInt32(HelpForm.GetObjectFromString(typeof(int), ddlTrimestre.SelectedValue, null));
        if (n_anno == 0) n_anno = DateTime.Now.Year;
        if (n_trimestre == 0)  n_trimestre = null;
        DataTable dt = GetMainData(DepConn, n_anno,n_trimestre);
    ...
    DataTable GetMainData(DataAccess Conn, int ayear, int? trimestre) {
        QueryHelper QHS = Conn.GetQueryHelper();
        object[] parametri = new object[] { ayear,trimestre };
        DataSet result = Conn.CallSP("compute_export_siope_art4bis", parametri, true, 3000);
        ...
        ////"http://localhost:2826/EasyWebReport_2009/expSiopeTrasparenza_art4bis.aspx?dep=amm?json=1"	
