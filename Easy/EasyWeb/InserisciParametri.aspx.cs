...
namespace EasyWebReport
{
	...
	/// </summary>
	public partial class InserisciParametri : System.Web.UI.Page
	{
        ...
        public void CreaPaginaParametri() {
            ...
            foreach (DataRow MyRowParam in Params.Rows) {
                ...
                if (FlagCombo.ToUpper() == "S") {
                    ...
                    DataTable myDtCombo;
                    //myDropList.ID = "IdParam" + CountParam.ToString();
        ...
        public void LeggiParametri() {
            ...
            foreach (DataRow drParam in Params.Rows) {
                ...
                //Myid Ë il nome dato al controllo nella webpage 
		...
		void CreatePrimaryTable(DataRow[] ReportParameters){
			...
			foreach (DataRow Param in ReportParameters){
				...
				//il campo Ë gestito se Ë flag combo = 'S' o se 
				//il campo selectioncode (manage/chhose) non Ë null
		...
		private void GetParamfromDB() {
			DataRow MR= Session["ModuleReportRow"] as DataRow;
			if (MR==null) return;
            TitoloStampa.Text = MR["Description"].ToString();
			DataAccess Conn = GetVars.GetUserConn(this);
			if (Conn==null) return;
			string filterpar= "("+
				"reportname="+QueryCreator.quotedstrvalue(MR["reportname"],true)+")";
			DataTable Params =  Conn.RUN_SELECT("reportparameter","*","number",filterpar,null,true);
			if(Params == null)return;
			...
			CreatePrimaryTable(Params.Select());
			//Aggiunge la colonna ParamValue che mi servir‡ per conservare il valore inserito dall'utente
        ...
        protected void btnVisualizzaReport_Click(object sender, EventArgs e) {
            //Response.Redirect("WebPDFView.aspx");
        ...
        protected void btnNewIndex_Click(object sender, EventArgs e) {
            Response.Redirect("IndiceReport.aspx?folder=2");
