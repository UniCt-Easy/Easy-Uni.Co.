	...
	$.fn.pgrid = function (options) {
		...
		this.filter("table").not(".ui-pgrid-table").each(function () {
			...
			pgrid.do_sort = function (column_class, loading, tbody) {
				...
				// Funzione per convertire una data italiana (gg/mm/aaaa) in un oggetto Date
				function parseItalianDate(str) {
					var parts = str.split('/');
					if (parts.length !== 3) return null;
					var day = parseInt(parts[0], 10);
					var month = parseInt(parts[1], 10) - 1; // i mesi in JS vanno da 0 a 11
					var year = parseInt(parts[2], 10);
					return new Date(year, month, day);
				}

				// Funzione per riconoscere se una stringa è una data
				var isDateLike = function (str) {
				...
				if (pgrid.pgrid_sortable) {
					...
					// NUOVO
					// Aggiungiamo una variabile sortKeyType a ogni riga, che ci dica se è una data(numero) o una stringa.
					// Nel sort, facciamo il confronto in base al tipo di chiave.

					// Calculate their sort keys and store them in their DOM objects.
					$.each(rows, function () {
						//Prende la cella della colonna specificata da pgrid.pgrid_sort_col.
						...
						if (children.innerText !== undefined)
							testoCella = children.innerText.toLowerCase();
						else if (children.textContent !== undefined)
							testoCella = children.textContent.toLowerCase();
						else
							testoCella = $(children).text().toLowerCase();

						// Controllo se il contenuto è una data

						if (isDateLike(testoCella)) {
							var date = parseItalianDate(testoCella);
							this.sortKey = date ? date.getTime() : 0; // timestamp come numero
							this.sortKeyType = 'number';
						} else {
							this.sortKey = [];
							var x = 0, y = -1, n = 0, i, j;
							while (i = (j = testoCella.charAt(x++))) {
								var m = (j === '.' || (j >= '0' && j <= '9'));
								if (m !== n) {
									this.sortKey[++y] = "";
									n = m;
							...
							this.sortKeyType = 'string';
						}
					});


					//Se la cella contiene una data(in formato gg / mm / aaaa), la riga verrà ordinata correttamente in base al timestamp.
					//Se contiene una stringa, viene usato il sorting naturale come prima.

					rows.sort(function (ae, be) {
						var a = ae.sortKey, b = be.sortKey;

						// Ordinamento numerico (es. timestamp)
						if (ae.sortKeyType === 'number' && be.sortKeyType === 'number') {
							return a - b;
						}

						// Ordinamento stringa naturale
