	...
	public class Meta_proceeds : Meta_easydata {
	    ...
	    public override DataRow Get_New_Row(DataRow ParentRow, DataTable T){
            ...
            if (T.Columns.Contains("npro_treasurer"))
            {
                ...
                RowChange.MarkAsCustomAutoincrement(T.Columns["npro_treasurer"], new RowChange.CustomCalcAutoID(CalcID));
            }
			DataRow R = base.Get_New_Row(ParentRow, T);
            if (EnableNDoc_Treasurer(R, Conn) && (!T.Columns.Contains("npro_treasurer")))
            {
                MessageBox.Show("Problemi nel calcolo del progressivo reversale per cassiere", "Attenzione");
            }
            if (flagautostampa == null) {
                flagautostampa = Conn.DO_READ_VALUE("config",
                    QHS.CmpEq("ayear", GetSys("esercizio")),
	    ...
	    static DataTable callSp(DataAccess Conn, List<int> idincList) {
	        ...
	        sb.AppendLine($"exec  trasmele_income_opisiopeplus_ins {Conn.GetEsercizio()},null,@lista");
        ...
        public static string SimulaGenerazioneOrdinativo(DataAccess myConn, DataTable Inclastview, out Dictionary<int,DataRow> admitted) {
            ...
            foreach (DataRow rExp in Inclastview.Rows) {
                if(rExp.RowState!=DataRowState.Deleted)Lincassi.Add(rExp);
            }
