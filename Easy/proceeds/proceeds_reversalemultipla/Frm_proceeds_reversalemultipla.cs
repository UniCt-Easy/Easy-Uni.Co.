...
namespace proceeds_reversalemultipla{//documentoincasso_multiplo//
	...
	/// </summary>
	public class Frm_proceeds_reversalemultipla : System.Windows.Forms.Form {
        QueryHelper QHS;
		...
		void AfterLinkBody(){
			...
			GetData.SetStaticFilter(DS.fin, QHS.AppAnd(QHS.CmpEq("ayear",Meta.GetSys("esercizio")), QHS.BitClear("flag",0)));
        ...
        public void MetaData_BeforeActivation(){
			...
			string bilanciofilter = QHS.AppAnd(QHS.CmpEq("ayear", Meta.GetSys("esercizio"))
                , QHS.BitClear("flag", 0));
			DataRow Pers = DS.config.Rows[0];
			string maxfasebil = Meta.Conn.DO_READ_VALUE("finlevel", QHS.CmpEq("ayear",
						Meta.GetSys("esercizio")), "max(nlevel)").ToString();
            ...
            if (((proceeds_flag & 2) == 2) &&
				...
				object descrfasebil = Meta.Conn.DO_READ_VALUE("finlevel", "(ayear='"+Meta.GetSys("esercizio")+"')AND"+
...
//				chbEuro.Visible=false;
        ...
        public void MetaData_AfterRowSelect(DataTable T, DataRow R) {
            ...
            if (T.TableName == "treasurer" && R != null && 
                    (Meta.DrawStateIsDone ||
                     (Meta.InsertMode && Meta.FirstFillForThisRow)
                    )
                ) {
            ...
            if (Meta.DrawStateIsDone && Meta.EditMode && T.TableName == "treasurer") {
                if (R == null) {
		...
		public void MetaData_AfterFill(){
            ...
            DataRow CurrDoc = DS.proceeds.Rows[0];
            ...
            if (Meta.IsEmpty) txtNPro_Treasurer.ReadOnly = false; else txtNPro_Treasurer.ReadOnly = true;
		}
		...
		string CalculateFilterForLinking(bool SQL){
            ...
            MyFilter = qh.AppAnd(qh.CmpEq("nphase", codicefase), qh.IsNull("kpro"), qh.CmpEq("ayear", eserccorrente));
 			if (flagcreddeb){
		...
		private void btnCollega_Click(object sender, System.EventArgs e) {
            ...
            if (UsoSiope) {
                ...
                string res = pp.Meta_proceeds.SimulaGenerazioneOrdinativo(Conn, DS.incomelastview,out admitted);
		...
		private void btnSituazione_Click_1(object sender, System.EventArgs e) {
			...
			try {
				DataRow MyRow=HelpForm.GetLastSelected(DS.proceeds);
			...
			DataSet Out = Conn.CallSP("sp_sit_docincasso",
		...
		void EliminaVariazioniDiAnnullo()   {
			...
			string filter = QHS.AppAnd(QHS.CmpEq("autokind",11),
                QHS.FieldIn("idinc",DS.incomelastview.Select(), "idinc"));
			DataAccess.RUN_SELECT_INTO_TABLE(Conn,DS.incomevar, null, 
				filter, null, true);
		...
		void AggiungiVariazioniDiAnnullo(DateTime DataAnnullo){
			...
			if (DS.incomelastview.Rows.Count==0) return;
			DataAccess Conn = Meta.Conn;
			string filter= QHS.AppAnd(QHS.AppAnd(
                QHS.CmpEq("autokind",11), 
                QHS.CmpEq("yvar", Meta.GetSys("esercizio"))),
                QHS.FieldIn("idinc", DS.incomelastview.Select(), "idinc"));
			MetaData MetaVar = MetaData.GetMetaData(this,"incomevar");
			...
			DataAccess.RUN_SELECT_INTO_TABLE(Conn,DS.incomevar, null, filter, null, true);
			DataRow Curr = DS.proceeds.Rows[0];
			...
			foreach (DataRow EntrataToAdd in DS.incomelastview.Select()){
				...
				if (VarAnnullo.Length==0){
					...
					DataRow NewVarAnnullo= MetaVar.Get_New_Row(null, DS.incomevar);
                ...
                DataAccess.RUN_SELECT_INTO_TABLE(Meta.Conn, DS.incomesorted, null, filterclass, null, true);
			}
            DataRow[] Rincomesorted = DS.incomesorted.Select();
            foreach (DataRow R in Rincomesorted)
            {
                R["amount"] = 0;
            }
        ...
        private void btnFlussoCrediti_Click(object sender, EventArgs e) {
            ...
            frmFlussoCrediti f = new frmFlussoCrediti(Conn,Meta.Dispatcher);
            if (f.errore) {
            ...
            Dictionary<int,DataRow> ex = new Dictionary<int, DataRow>();
            ...
            DataSet MyDS = (DataSet)f.grid.DataSource;
            DataTable MyTable = MyDS.Tables[f.grid.DataMember.ToString()];
            Dictionary<int, bool> selectedIdIncDic = new Dictionary<int, bool>();
            foreach (var idx in selectedIdInc) selectedIdIncDic[idx] = true;

            for (int i = 0; i < MyTable.Rows.Count; i++) {

                DataRow rGrid = MyTable.Rows[i];
                ...
                DataRow rNew = DS.incomelastview.NewRow();
                ...
                Meta.MarkTableAsNotEntityChild(DS.incomelastview);
                rNew.AcceptChanges();
                rNew["kpro"] = curr["kpro"];
        ...
        private void btnControllaFileOPI_Click(object sender, EventArgs e) {
            ...
            string res = pp.Meta_proceeds.SimulaGenerazioneOrdinativo(Conn, DS.incomelastview, out admitted);
            if (res == null) {
                MessageBox.Show("L'ordinativo ha già una dimensione accettabile.","Avviso");
                return;
            }

            int n = 0;
            Dictionary<int,DataRow> rowPerIdInc = new Dictionary<int, DataRow>();
