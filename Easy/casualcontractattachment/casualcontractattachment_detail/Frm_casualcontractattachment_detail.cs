...
namespace casualcontractattachment_detail{
	public partial class Frm_casualcontractattachment_detail : MetaDataForm{
		MetaData Meta;
        DataAccess Conn;
        CQueryHelper QHC;
        QueryHelper QHS;
        public IOpenFileDialog openFileDialog1;
		public Frm_casualcontractattachment_detail(){
			InitializeComponent();
        ...
        public void MetaData_AfterLink(){
            ...
            string filter_tipoallegato = qhs.AppAnd(qhs.CmpEq("active", "S"), qhs.BitSet("flag", 2));
            DataRow Dcasualcontract = Meta.SourceRow.GetParentRow("casualcontract_casualcontractattachment");
            if ((Dcasualcontract != null)){
                 object idser = Dcasualcontract["idser"];
                if ((idser != DBNull.Value) && (CfgFn.GetNoNullInt32(idser) != 0)){
                    filter_tipoallegato = qhs.AppAnd(filter_tipoallegato,
                        qhs.DoPar(qhs.AppOr(qhs.IsNull("idser"), qhs.CmpEq("idser", Dcasualcontract["idser"])))
                    );
                }
            }
        ...
        {
            DataRow R = DS.casualcontractattachment.Rows[0];
        ...
        private void btnAllega_Click(object sender, EventArgs e){
            if (Meta.IsEmpty) return;
            DataRow Curr = DS.casualcontractattachment.Rows[0];
