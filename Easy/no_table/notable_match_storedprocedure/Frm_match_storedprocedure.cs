    ...
    public partial class Frm_match_storedprocedure : Form{
        ...
        private void btnMatch_Click(object sender, EventArgs e){
            ...
            while ((currparam != null)&&(currparam.ToString()!=""))
            {
                ...
                if (T1 == null){
                    MessageBox.Show("La vecchia SP non esiste o presenta errori in esecuzione. La query eseguita Ë stata " + query1);
                    return;
                }
                ...
                if (T2 == null){
                    MessageBox.Show("La nuova SP non esiste o presenta errori in esecuzione. La query eseguita Ë stata " + query2);
                    return;
                }
        ...
        public bool ConfrontoconSuccesso(DataTable T1, DataTable T2, string currparam){
            ...
            if (txtFields.Text.ToString() != ""){
                // Se Ë stato specificatp un elenco di campi, confronta solo quelle colonne.
                ...
                foreach (string f in fields){
                    if ((T1.Columns.Contains(f)) && (T2.Columns.Contains(f))){
                        ...
                        if (!(T1.Columns.Contains(f)) && !(T2.Columns.Contains(f))){
                            MessageBox.Show("Con la combinazione di parametri  " + currparam +
                                " risulta che la colonna " + f + " Ë assente in entrambe le SP");
                            txtParamOld.Text = currparam;
                            txtParamNew.Text = currparam;
                            VisualizzaOut(T1, T2);
                            return false;
                        ...
                        if (!(T1.Columns.Contains(f))){
                            MessageBox.Show("Con la combinazione di parametri  " + currparam +
                                " risulta che la colonna " + f + " Ë assente nella vecchia SP");
                            txtParamOld.Text = currparam;
                            txtParamNew.Text = currparam;
                            VisualizzaOut(T1, T2);
                            return false;
                        ...
                        if (!(T2.Columns.Contains(f))){
                            MessageBox.Show("Con la combinazione di parametri  " + currparam +
                                " risulta che la colonna " + f + " Ë assente nella nuova SP");
                            txtParamOld.Text = currparam;
                            txtParamNew.Text = currparam;
                            VisualizzaOut(T1, T2);
                            return false;
                ...
                //Se non Ë stato specificato un elenco ci campi, confronta tutte le colonne
                foreach (DataColumn C in T1.Columns){
                    if (T2.Columns.Contains(C.ColumnName)){
                        ...
                        MessageBox.Show("Con la combinazione di parametri  " + currparam +
                            " risulta che la colonna " + C.ColumnName + " Ë assente nella nuova SP");
                        txtParamOld.Text = currparam;
                        txtParamNew.Text = currparam;
                        VisualizzaOut(T1, T2);
                        return false;
