	...
	public partial class Frmflussostudenti : Form {
		...
		bool creaAccertamentiDaDettagliContrattiAttivi(estimatedetailRow[] estimatedetailRows,
			...
			foreach (var rDet in estimatedetailRows) {
				...
				var aDate = getDateContrattoAttivo(rDet,
					out errore); //_conn.readValue("estimate", q.mCmp(rDet, "idestimkind", "yestim", "nestim"), "adate");
				...
				if (incomeToLink != null) {
					if ((tipoElaborazione != TipoElaborazioneIncassi.iva))
						rDet["idinc_taxable"] = incomeToLink["idinc"];
		...
		private bool fillAnnulment(object _from, object _to) {
			...
			q filtroCreditiAnnullati =
				q.eq(q.year("stop"), esercizio) & q.doPar(q.isNull("flag") | q.bitClear("flag", 0));

			if (_from != DBNull.Value) {
				filtroCreditiAnnullati &= q.ge("idflusso", _from);
			}

			if (_to != DBNull.Value) {
				filtroCreditiAnnullati &= q.le("idflusso", _to);
			}
			...
			//Legge i dett.c.attivi da annullare, collegati a c.attivi CON  flag differito
			string sqlEstimDet = $"SELECT {QueryCreator.ColumnNameList(DS.estimatedetail)} " +
			                     ...
			                     $"		{whereUPB}" +
			                     //$"		AND EK.flag&1<>0 "+ // considera gli incassi se il tipo contratto attivo prevede incassi differiti";
			                     $"     )" +
			...
			var Incassi = _conn.SQLRunner($@"
				SELECT  idinc, available
						from incometotal 
							...
							where incometotal.ayear= {esercizio} AND 
							iduniqueformcode in (
							SELECT flussocreditidetail.iduniqueformcode from flussocreditidetail {joinUPB}
								WHERE 	 {filtroCreditiAnnullati.toSql(_qhs)} 
					...
					foreach (var rCredito in dettCrediti) {
						...
						foreach (var rEstimDet in rows) {
							...
							if (info.incomeByIdInc.ContainsKey(idincInt)) {
								...
								var movYears =
									DS.incomeyear.get(_conn,
										fltmovI & q.eq("ayear",
											esercizio)); //mergeFromDb(_conn, fltmovI & q.eq("ayear", esercizio)); 
