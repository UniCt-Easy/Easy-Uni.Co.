        ...
        public virtual bool CheckAll(CopyDisplay CD) {
            ...
            foreach (Copyer C in AR) {
                if (CC.Contains(C.table)) {
                    MessageBox.Show("Errore di programmazione: il Copyer di " + C.table + " Ë stato aggiunto due volte", "Errore");
                    continue;
                }
                CC.Add(C.table);
        ...
        public bool CopyAll(CopyDisplay CD) {
            ...
            while (somethingcopied) {
                ...
                foreach (Copyer C in AR) {
                    ...
                    if (!C.CheckDependencies(Ctx, FieldToLookup)) continue;//non puÚ copiare ancora questa tabella, mancano dipendenze
                    ...
                    if (!res) {
                        ...
                        MessageBox.Show("La copia della tabella " + C.table + " non Ë stata effettuata", "Errore");
                        return false;
                    }
            ...
            foreach (Copyer C in AR) {
                if (!C.applied) {
                    MessageBox.Show("La copia della tabella " + C.table + " non Ë stata effettuata per mancanza di dipendenze", "Errore");
                }
            }
            CD.Comment("Enable Triggers");
            Dest.CallSP("enable_disable_triggers",
