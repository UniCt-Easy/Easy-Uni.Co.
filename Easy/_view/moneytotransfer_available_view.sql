...
-- CREAZIONE VISTA [moneytotransfer_available_view]
IF EXISTS(select * from sysobjects where id = object_id(N'[moneytotransfer_available_view]') and OBJECTPROPERTY(id, N'IsView') = 1)
...
CREATE    VIEW moneytotransfer_available_view 
	...
	JOIN fin fin_dest (NOLOCK)				ON fin_dest.idfin = proceedspart.idfin
	JOIN upb U_dest (NOLOCK)				ON U_dest.idupb = proceedspart.idupb
	LEFT OUTER JOIN treasurer T_dest (NOLOCK)		ON T_dest.idtreasurer = U_dest.idtreasurer	

	JOIN income (NOLOCK)					ON income.idinc = proceedspart.idinc
	JOIN incomeyear (NOLOCK)				ON incomeyear.idinc = proceedspart.idinc
	JOIN upb U_origin (NOLOCK)				ON U_origin.idupb = incomeyear.idupb
	JOIN fin fin_origin (NOLOCK)			ON fin_origin.idfin = incomeyear.idfin

	JOIN incomephase (NOLOCK)				ON incomephase.nphase = income.nphase
	LEFT OUTER JOIN underwriting (NOLOCK)	ON underwriting.idunderwriting = income.idunderwriting 	

	JOIN incomelast (NOLOCK)				ON incomelast.idinc = proceedspart.idinc	
	LEFT OUTER JOIN proceeds  (NOLOCK)		ON incomelast.kpro = proceeds.kpro
	LEFT OUTER JOIN treasurer T_origin (NOLOCK)		ON T_origin.idtreasurer = proceeds.idtreasurer	
...
UNION ALL
	...
	from finvar FV (NOLOCK)
		join finvardetail FVD_dest (NOLOCK)			ON FVD_dest.yvar=FV.yvar and FVD_dest.nvar=FV.nvar AND FVD_dest.amount > 0
		join fin f_dest (NOLOCK)					on FVD_dest.idfin=f_dest.idfin and f_dest.flag&1<>0
		join upb u_dest (NOLOCK)					on FVD_dest.idupb=u_dest.idupb 
		join treasurer t_dest (NOLOCK)				on u_dest.idtreasurer=t_dest.idtreasurer 
		left outer join underwriting uw_dest (NOLOCK)		on FVD_dest.idunderwriting=uw_dest.idunderwriting 
		join finvardetail FVD_origin (NOLOCK)		ON FVD_origin.yvar=FV.yvar and FVD_origin.nvar=FV.nvar AND FVD_origin.amount < 0
		join fin f_origin  (NOLOCK)				on FVD_origin.idfin=f_origin.idfin and f_origin.flag&1<>0
		join upb u_origin (NOLOCK)				on FVD_origin.idupb=u_origin.idupb 
		join treasurer t_origin (NOLOCK)		on u_origin.idtreasurer=t_origin.idtreasurer 
