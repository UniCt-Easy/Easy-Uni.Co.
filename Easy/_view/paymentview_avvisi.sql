...
-- CREAZIONE VISTA paymentview_avvisi
IF EXISTS(select * from sysobjects where id = object_id(N'[paymentview_avvisi]') and OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW [paymentview_avvisi]
GO
...
CREATE VIEW [paymentview_avvisi]
	(
	kpay,
	ypay,
	npay,
 ...
 --        ),0),
	...
	LEFT JOIN paymenttransmission with (nolock)
	ON paymenttransmission.kpaymenttransmission = payment.kpaymenttransmission
	LEFT JOIN treasurer with (nolock)
