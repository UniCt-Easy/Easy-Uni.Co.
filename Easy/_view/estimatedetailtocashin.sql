...
-- CREAZIONE VISTA estimatedetailtocashin
IF EXISTS(select * from sysobjects where id = object_id(N'[estimatedetailtocashin]') and OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW [estimatedetailtocashin]
GO

--setuser 'amministrazione'
--setuser 'amm'
--clear_table_info 'estimatedetailtocashin'
--select * from estimatedetailtocashin
CREATE  VIEW [estimatedetailtocashin]
(
	idestimkind,
	yestim,
	nestim,
...
AS SELECT 
	...
	ISNULL(
		(SELECT SUM(v.amount)
		  FROM incomelastestimatedetail v where 
				v.idestimkind = estimatedetail.idestimkind AND
				 v.yestim = estimatedetail.yestim AND
				 v.nestim = estimatedetail.nestim AND
				 v.rownum = estimatedetail.rownum ), 
		 0), --cashed
	ROUND(estimatedetail.taxable * estimatedetail.number * 
		  CONVERT(DECIMAL(19,6),estimate.exchangerate) *
		  (1 - CONVERT(DECIMAL(19,6),ISNULL(estimatedetail.discount, 0.0))),2
