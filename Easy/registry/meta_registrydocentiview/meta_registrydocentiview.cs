    ...
    public class Meta_registrydocentiview : Meta_easydata 
	{
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield) {
			if (!base.IsValid(R, out errmess, out errfield)) return false;
						...
						if (R.Table.Columns.Contains("matricola") && R["matricola"].ToString().Trim().Length > 50) {
							errmess = "Attenzione! Il campo 'Matricola' può essere al massimo di 50 caratteri";
							errfield = "matricola";
							return false;
						}
						if (R.Table.Columns.Contains("soggiorno") && R["soggiorno"].ToString().Trim().Length > 255) {
							errmess = "Attenzione! Il campo 'Permesso di soggiorno' può essere al massimo di 255 caratteri";
							errfield = "soggiorno";
							return false;
						}
			//$IsValid$
		...
		public override void DescribeColumns(DataTable T, string ListingType) {
			...
			switch (ListingType) {
				case "docenti": {
						...
						DescribeAColumn(T, "location_struttura_description", "Struttura di afferenza", nPos++);
						DescribeAColumn(T, "registry_istituti_title", "Istituto, Ente o Azienda", nPos++);
						DescribeAColumn(T, "classconsorsuale_title", "Classe consorsuale", nPos++);
						DescribeAColumn(T, "classconsorsuale_description", "Classe consorsuale", nPos++);
						break;
					}
				//$DescribeAColumn$
			}
		...
		public override string GetSorting(string ListingType) {
			...
			switch (ListingType) {
				case "docenti": {
						return "classconsorsuale_title asc , location_struttura_description desc, registry_istituti_title asc , sasd_title desc";
					}
				//$GetSorting$
			}
