...
namespace Segreterie.Generator {
	class Program {
		...
		//nino
		//private static string connectionString="Data Source=192.168.0.107,1434;Initial Catalog=Rettorato_Ok;Persist Security Info=True;User ID=assistenza;Password=insert a strong password";
		//bari
		private static string connectionString = "Data Source=192.168.10.201;Initial Catalog=Rettorato_Ok;Persist Security Info=True;User ID=assistenza;Password=insert a strong password";
		//private static string connectionString="Data Source=192.168.10.201;Initial Catalog=unibas_easy;Persist Security Info=True;User ID=assistenza;Password=insert a strong password";
		...
		private static int applicationID = 2; //1 VisualMDLW, 2 Segreterie, 3 INSTM utenti
		private static string currentApp = "";

		private static string queryFields =
			@"					select
		CASE WHEN EXISTS(select TABLE_NAME from information_schema.columns where TABLE_NAME = isc.TABLE_NAME + isnull(td.editlistingtype,'default') + 'view') THEN 'S' ELSE 'N' END as haveview,
					--TABELLA OGGETTO
		...
		private static string queryFields2 = @"select
					...
					CASE WHEN td.principale = 'N' and not exists(select rl.idapprelation from apprelation rl where rl.idapppages = td.idapppages
					and rl.type <> 'unique' and rl.type <> 'cerca') --relazioni che NON comportano la presenza della subpage
					THEN 'N' ELSE 'S' END AS createmetapage, forcealias, beforefillsinc,
					--CAMPO OGGETTO
		...
		static void Main(string[] args)
		{
			...
			using (var connection = new SqlConnection(connectionString))
			{
				...
				HDSGen.extConn = new SqlConnection("data source=192.168.10.201;initial catalog=Rettorato_Ok;User ID =amm;Password=insert a strong password;Application Name=HDSGene(nino);WorkStation ID =PC-Caprilli;Pooling=false;Connection Timeout=300;");
				HDSGen.extConn.Open();
				...
				//filtro finale
				var queryWhere = @" a.idapplicazione = " + applicationID + @" and (
				...
				-- OR isc.TABLE_NAME like 'graduatoriadesc'
...
--OR isc.TABLE_NAME like 'nullaosta%'
				-- 
OR 
isc.TABLE_NAME like 'registry_studenti'
				...
				foreach (var o in objects.OrderBy(x => x.Table).ThenBy(x => x.EditListingType))
				{
					...
					var principalkey = isLinkingObj ? null : GetPrincipalKey(o.Table, o.EditListingType, allfields, connection, false);
					...
					if (!allfields.Any(af => af.Table == possibleExtendingobj))
					{
						...
						foreach (var r in relationFields)
						{
							...
							if (fields.Any(f => f.Table == metadatoChild)) //ricorsivo su se stesso
								r.Name += "_alias1";

							//serve a gestire più figli dello stesso tipologia di oggetto, 
							SetMetadatoChild(fields, r, metadatoChild, ref allfields, connection);

							if (r.ForceAlias > 0)
								r.MetadatoChild = metadatoChild + "_alias" + r.ForceAlias.ToString();
							...
							if (r.RelationType == "unique")
							{
								...
								uniquefields.AddRange(allfields.Where(af => af.Table == metadatoChild && af.EditListingType == (r.EditListingType ?? "defaut")
								...
								if (string.IsNullOrWhiteSpace(sortSubQuery))
									Console.WriteLine("ERROR: ho una relazione entità (" + o.Table + ") / sub-entità (" + metadatoChild + ") uno a uno ma non ho definito un ordinamento per la sub-entità");
								...
								foreach (var fi in uniquefields.Where(f => f.Detailvisible && !f.Name.StartsWith("!")))
								{
									...
									if (HaveKeyName(fi) && (!IsPrincipalKey(fi, o.Table) || isLinkingObj))
									{
										...
										{
											hadAlias = IsDropdown(fi, fields, metadatoChild2);
										}
									}
						...
						{
							// se la generazione è parziale devo verificarlo con una query
						...
						if (createDbView || hasTxtFieldsToShow)
						{
							...
							//preparo lo script sql-------------------------------------------------------
							if (createDbView)
								WriteSQLScript(clientFolder + scriptFolder + viewName + ".sql", viewName, o.IsVoc, connection);
							//----------------------------------------------------------------------------
				...
				var arrayTablesToProtocol = ['istanza', '" + o.Table + @"'];
...
";
				...
				var arrayTablesToProtocol = ['nullaosta'" + (o.Table.Contains("_") ? ", '" + o.Table + "'" : "") + @"];
...
";
      ...
      <HintPath>..\..\..\..\..\dll\Newtonsoft.Json.dll</HintPath>
							...
							//se c'è una vista per gli elenchi o è un oggetto estendente, calcolo il get sorting per la vista e lo metto nel metadato di appoggio della vista
							var sortStringView = "";
							//if (viewFields.Any() && createDbView) {
							if (!string.IsNullOrWhiteSpace(viewObjectFile) || isExtendedObjView)
							{
								...
								if (!string.IsNullOrEmpty(sortStringView))
								{
									...
									//	//ho già inserito il valore sul metadato della vista
									//	skip = true;
								}
							}

							////nel metadato, per oridinare le GRIGLIE genero il get sorting con i campi calcolati
							////quando si tratta di un oggetto estendente, nel qual caso NON l'ho già popolato nel if precedente perchè il current è il metadato della vista
							...
							//-GETSTATICFILTER
							//lo static filter va nel metadato vista (se è estendente è in currentObjectFile, altrimenti in viewObjectFile) se è presente la vista, 
							//altrimenti nel metadato normale che è sempre in currentObjectFile
						...
						if (!skipmetapage)
						{
							...
							foreach (var f in describeColumnsFileds.Where(f => f.ListVisible).OrderBy(f => f.SortList))
							{
								...
								if (f.LookupFor != null && HaveKeyName(f) && !IsEnum(f, allfields, connection))
								{
									...
									////devo valutare che questa foreign key che fa da lookup per un campo potrebbe produrre anche più di un campo testuale di lookup
							...
							if (!string.IsNullOrEmpty(sortString))
							{
								...
								if (!string.IsNullOrEmpty(viewObjectFileJS))
									SetGetSorting(viewObjectFileJS, editListingTypes, sortString);
							}
							...
							if (o.Createmetapage)
							{
								...
								//o altre relazioni 1 a 1 
								//o campi calcolati
								//creo la beforefill con parte asincrona
								if (isExtendedObjView || relationFieldUnique.Any())
								{
									...
									"				\r\n" +
									"				//parte asincrona\r\n" +
									"				var def = appMeta.Deferred(\"beforeFill-" + o.Table + "_" + editListingTypes + "\");\r\n" +
									"				var arraydef = [];\r\n" +
									"				\r\n" +
									"				//beforeFillInside\r\n" +
									"				\r\n" +
								...
								if (isExtendedObjView)
									ReplaceStringInFile(fullPathMetaPageJS, "//beforeFillInside", "var dt = this.state.DS.tables[\"" + o.Table + "\"];\r\n" +
										"\t\t\t\tif (dt.rows.length === 0) {\r\n" +
								...
								foreach (var unique in relationFieldUnique)
								{
									...
									ReplaceStringInFile(fullPathMetaPageJS, "//beforeFillInside", "var dt" + uniqueobj + " = this.state.DS.tables[\"" + uniqueobj + "\"];\r\n" +
									...
									SetAfterlink(fullPathMetaPageJS, "appMeta.metaModel.computeRowsAs(this.state.DS.tables." + uniqueobj + ", \"" + unique.EditListingType +
									...
									var uniquefields = allfields.Where(af => af.Table == uniqueobj && af.EditListingType == (unique.EditListingType ?? "defaut") && af.Detailvisible);
								...
								foreach (var filteredrel in relationFieldfiltered)
								{
									...
									var filteredListType = filteredrel.Listtype ?? filteredrel.EditListingType;
									...
									var gridId = "grid_" + (filteredrel.MetadatoChild ?? filteredrel.Name.Substring(2)) + "_" + filteredrel.EditListingType;
									...
									if (filteredFields.Any())
									{
										...
										SetBeforeFillFilter(fullPathMetaPageJS,
								...
								if (currentfilteredFields.Any())
								{
									foreach (var filt in currentfilteredFields)
										//SetBeforeFillFilter(fullPathMetaPageJS, "this.state.currentRow." + GetJsFilter(filt) + ";\r\n");
										if (filt.Filter.StartsWith(" ="))
											SetBeforeFillFilter(fullPathMetaPageJS, "this.state.currentRow." + filt.Name + filt.Filter + ";\r\n");
								}
									...
									foreach (var f in fieldsWithDefault)
									{
										...
										else
										{
											...
											else
											{
												...
												if ((f.Type == "datetime" || f.Type == "date") && !f.Allownull)
												{
													setPageDefault = "\t\t\t\tif (this.isNullOrMinDate(" + table + "." + f.Name + "))\r\n";
													setPageDefault += "\t\t\t\t\t" + table + "." + f.Name + " = new Date();\r\n";
												}
												SetBeforeFillFilter(fullPathMetaPageJS, setPageDefault, true);
											}
										}
									}

									if (isExtendedObjView)
										SetBeforeFillFilter(fullPathMetaPageJS, "parentRow.extension = \"" + extendingObject + "\";\r\n");
								...
								if (!string.IsNullOrWhiteSpace(o.BeforeFillSinc))
									SetBeforeFillFilter(fullPathMetaPageJS, o.BeforeFillSinc);
								...
								if (relationFieldCerca.Any())
								{
									foreach (var fi in relationFieldCerca)
									{
										...
										var linkid = allfields.Where(f => f.IsKey && f.ListVisible && HaveKeyName(f)
																	&& f.Table == linktable && f.EditListingType == fi.EditListingType).FirstOrDefault();
				...
				return def.promise();
								...
								if (relationFieldCalendar.Any())
								{
									...
									{
										SetAfterlink(fullPathMetaPageJS, "$('#calendar" + fields.IndexOf(fi) + "').fullCalendar('rerenderEvents');\r\n");
									}
								}
								...
								//-aggiungo i campi che vanno affogati nella pagina perchè l'associazione è "button"
								...
								if (relationFieldButton.Any())
								{
									...
									foreach (var b in relationFieldButton)
									{
										...
										//setto a disabilitato il bottone e i callingParameters (separati perchè in caso di più bottoni nella stessa pagina la seconda parte è uguale)
										SetBeforeFillFilter(fullPathMetaPageJS, "$(\"#" + b.Name + "\").prop(\"disabled\", !this.state.isEditState());\r\n");
										SetBeforeFillFilter(fullPathMetaPageJS, callingParameters);
								...
								if (parentEntities.Any(pe => pe.RelationType == "button"))
								{
									...
									foreach (var r in parentEntities.Where(pe => pe.RelationType == "button"))
									{
										...
										if (parentkeys.Count == 1)
										{
											...
											SetBeforeFillFilter(fullPathMetaPageJS, "parentRow." + k + " = this." + k + ";\r\n");
					...
					.then(function (res) {
						alert(res);
								...
								if (!string.IsNullOrEmpty(o.CustomJavascript))
								{
									...
									//se si tratta di ovverride di eventi non devo aggiungere niente
									var pageEvents = new List<string>() { "insertClick", "beforePost" };
					...
					arrayInput = myinstance.arrayInput;");
								...
								foreach (var f in fields.Where(fi => fi.Name != "ct" && fi.Name != "lt" && fi.Name != "cu" && fi.Name != "lu").OrderBy(fi => fi.Sort))
								{
									...
									if (!f.Allownull)
									{
										...
										{
											if (f.DenyNull)
												SetAfterlink(fullPathMetaPageJS, "this.setDenyNull(\"" + f.Table + (f.ForceAlias > 0 && f.Table != o.Table ? //l'alias sulla tabella principale non ci sta
...
");
								...
								//se è un campo calcolato aggiungo la funzione per calcolarlo e al sua chiamata nella afterGetFormData
								foreach (var fi in fields)
								{
									var nametag = fi.Table + "_" + fi.EditListingType + "_" + fi.Name;
									...
									if (!skipall)
									{
										...
										if (isunique)
										{
											...
											if (fi.Name.StartsWith("XX"))
											{
												...
												formString += GetGrid(o, tab, fi, fields, allfields, fullPathMetaPageJS, string.IsNullOrWhiteSpace(fullPathTestParams) ? fullPathTest : fullPathTestParams, false, connection);
											}
										}
										//altrimenti
										if (!isgrid)
											...
											foreach (var fi in fields.Where(f => f.Tab == tab.Title && f.Detailvisible).OrderBy(f => /*f.Table).ThenBy(f =>*/ f.Sort))
											{
												...
												if (fi.RelationType != "button")
												{
													...
													else
													{
														...
														//ma se sono chiavi già lo sono per definizione quindi non serve 
														...
														{
															//------------------------------------------------------begin
															//altrimenti verifico che non sia richiesta l'unicità sulla riga
															if (fi.UniqueOnRow)
															{
																//-se destro:  
																if (!IsSinistro(position, isInverted))
																...
																if (HaveKeyName(fi) && (!IsPrincipalKey(fi, o.Table) || isLinkingObj))
																{
																	...
																	else
																	{
																		...
																		if (!string.IsNullOrEmpty(metadatoChild))
																		{
																			...
																			var metadatochildWithText = (metadatoChild != txtField.Table) ? fi.MetadatoExtendedChild : fi.MetadatoChild;

																			if (IsDropdown(fi, fields, metadatochildWithText))
																			{
																				...
																				//oggetto che userebbe una dropdowngrid ma è forzato ad usare una dropdown normale
																				...
																				{
																					//se utilizza come master una dropdowngrid ...
																					var masterField = fields.FirstOrDefault(ff => ff.MetadatoChild == fi.Master && !IsDropdown(ff, fields,
																						ff.MetadatoChild.Replace((string.IsNullOrWhiteSpace(ff.Listtype) ? "default" : ff.Listtype) + "view", "")));
																				...
																				//test
																				SetArrayInputTest(string.IsNullOrWhiteSpace(fullPathTestParams) ? fullPathTest : fullPathTestParams,
																					"{ tag: '" + datatag + searchtag.Replace("\" data-subentity ", "").Replace("\" ", "") + "', value: null, type: controlTypeEnum.select" +
																					(!string.IsNullOrEmpty(fi.Defaultvalue) ? ", default: " + fi.Defaultvalue + " " : "") + " }");
																			}
																			else
																			{
																			...
																			if (fi.IsKey && !fi.Readonlyfield)
																			{
																				...
																				SetInsertClick(fullPathMetaPageJS, "if (!$('#" + nametag + "').val() && this.children.includes(grid.dataSourceName)) {\r\n" +
																					"\t\t\t\t\treturn this.showMessageOk('Prima devi selezionare un valore per il campo " + fi.Title + "');\r\n" +
																					"\t\t\t\t}\r\n");
																				...
																				SetBottomCode(fullPathMetaPageJS, "children: ['" + string.Join("', '",
																					...
																					IsExtendingObject( f.Name.Substring(2), ref allfields,connection)? f.Name.Substring(2).Split('_')[0] : f.Name.Substring(2)
																					)) + "'],\r\n" +
																				...
																				foreach (var child in fields.Where(f => f.Name.StartsWith("XX") && fi.RelationType == "unique"))
																				{
																					...
																					SetAfterRowSelect(fullPathMetaPageJS, "if (t.name === '" + fi.MetadatoChild + "')\r\n" +
																				...
																				if (isExtendedObjView)
																				{
																					...
																					if (child != null)
																						SetAfterRowSelect(fullPathMetaPageJS, "if (t.name === '" + fi.MetadatoChild + "')\r\n" +
																						"					if (this.state.DS.tables['" + o.Table + "'].rows.length)\r\n" +
																						"						this.state.DS.tables['" + o.Table + "'].rows[0]." + child.Name + " = r." + fi.Name.Split('_')[0] + ";\r\n", null, o.Table, editListingTypes);
									...
									if (!string.IsNullOrWhiteSpace(fi.Master) && !fi.Detailvisible)
									{
										...
										if (masterField != null)
										{
											...
											SetBottomCode(fullPathMetaPageJS,
												...
												"				var def = appMeta.Deferred(\"beforeFill-manageiscrizione\");\r\n" +
												"				var self = this;\r\n" +
								...
								if (!skipmetapageDataset)
								{
									...
									//ci metto il metadato principale
									var dsMetadatoAndKey = GetDatasetTable(o.Table, false, allfields, connection, editListingTypes, fullPathMetaPageJS);
									var datasetMetadato = "<xs:choice minOccurs=\"0\" maxOccurs=\"unbounded\">\r\n";
									datasetMetadato += dsMetadatoAndKey.Split(';')[0];
									datasetMetadato += "      </xs:choice>";
									...
									if (!string.IsNullOrEmpty(extendingObject))
									{
										...
										PreparePageDataset(fullpathDatasetMetapage + ".xsd", extendedObject, o.Table, String.Join(" ", extendedObjectKey), extendingKeys, current, false,
											datasetfields, allfields, editListingTypes, "", null, connection, false, 1, "", new List<string>(), fullPathMetaPageJS, 1, null, true);
									}
									...
									if (o.Createmetapage)
									{
										...
										fullTables = Regex.Matches(fileText, @"this\.state\.DS\.tables\.(.*?)\.", RegexOptions.Singleline).Cast<Match>()
											...
											.Where(t => !t.EndsWith("view")).ToList();
									}
											...
											&& (HaveKeyName(fi) || fi.Name.StartsWith("XX")))
										{
											...
											if (fi.Name.StartsWith("XX"))
											{
												...
												if (string.IsNullOrWhiteSpace(childkey) || childkey.Split(' ').Any(k => k.StartsWith("parid")))
													childkey = GetChildKeysString(fi.Table, tablechild, editListingTypes, fi.EditListingType, key.Split(' ').ToList(), allfields, connection);
											...
											if (!string.IsNullOrEmpty(tablechild) && !string.IsNullOrEmpty(key) &&
												...
												PreparePageDataset(fullpathDatasetMetapage + ".xsd", fi.Table, tablechild, key, childkey, o.Table, minimal, datasetfields, allfields,
													editListTypeField, editListingTypes, pageid, connection, fi.IsLinkingObj, iteration, extendedTableChild, fullTables, fullPathMetaPageJS,
													sortList, fi.Master, skipLookup, fi.Title);
										}
									}
									...
									if (o.AdditionalTables.Any())
									{
										...
										foreach (var t in o.AdditionalTables)
										{
											if (!datasetFile.Contains("<xs:element name=\"" + t + "\">"))
											{
												GetMissingFields(t, null, ref allfields, connection);
												var tstring = GetDatasetTable(t, false, allfields, connection, "", fullPathMetaPageJS);
			...
			if (!skipmetadato && rebuildMetadata)
			{
				...
				var building = new Process()
				{
					StartInfo = new ProcessStartInfo
					{
						...
						FileName = builder,
						Arguments = solutionFolder + solutionFile + " /t:Build /p:configuration=debug",
					}
				};
		...
		private static string PrepareCsproj(string current, string container, string title, string editListingTypes, bool othersAppObject, List<string> objectKey = null)
		{
			...
			//lo crea se non esiste oppure lo rigenera la prima volta che lo incontra solo se non appartiene ad altre app
		...
		private static void WriteSQLScript(string file, string tableName, bool isVoc, SqlConnection connection)
		{
			if (rebuildScript && !scriptAlreadyCreated.Contains(tableName))
			{
				...
				string DNS = "EasyPay";
				string Server = "192.168.10.201";
				string Database = "Rettorato_Ok";
				string User = "assistenza";
				string Password = "insert a strong password";
				string DIPARTIMENTO = "amm";
	...
	(fullPathMetaDatoJS.EndsWith("view.js") ? "" : "				table['customObjCalculateFields'] = objCalcFieldConfig;\r\n" +
		...
		public static string GetFormatDescribeColumnJS(field fi, string colName, int? colpos = null, string foreingKeyTitle = null, bool? manyTextFields = null)
		{
			...
			return "\t\t\t\t\t\tthis.describeAColumn(table, '" + colName + "', '" + title + "', " +
				...
				(colpos ?? (fi.SortList * 10)).ToString() + ", " + len + ");\r\n"; ;
		}
		...
		/// <param name="fullPathMetaPageJS"></param>
		/// <param name="commands"></param>
		private static void SetBeforeFillFilter(string fullPathMetaPageJS, string commands, bool removeTab = false)
		{
			//aggiungo tutto l'evento se manca
			ReplaceStringInFile(fullPathMetaPageJS, "//beforeFill\r\n", "beforeFill: function () {\r\n" +
									"\t\t\t\tvar self = this;\r\n" +
		...
		private static void SetAfterFill(string fullPathMetaPageJS, string commands, string deferredName)
		{
			ReplaceStringInFile(fullPathMetaPageJS, "//afterFill\r\n", "afterFill: function () {\r\n" +
				...
				"\t\t\t},\r\n");
			ReplaceStringInFile(fullPathMetaPageJS, "//afterFillin\r\n", commands + "\t\t\t\t//afterFillin\r\n");
		...
		/// <param name="table"></param>
		/// <param name="editListingTypes"></param>
		private static void SetAfterGetFormData(string fullPathMetaPageJS, string commands, string table, string editListingTypes)
		{
...
"						return def.promise();\r\n" +
"			},\r\n");
			ReplaceStringInFile(fullPathMetaPageJS, "//afterGetFormDataInside\r\n", commands + "\t\t\t\t\t\t//afterGetFormDataInside\r\n");
		}
		...
		private static void GetMissingFields(string tablename, string tablekey, ref List<field> allfields, SqlConnection connection, bool showmessage = true)
		{
			...
			{
				var queryWhere = " (a.idapplicazione = " + applicationID + " or a.idapplicazione is null) and isc.TABLE_NAME = '" + tablename + "'";
			...
			{
				var queryWhere = " (a.idapplicazione = " + applicationID + @" or a.idapplicazione is null) and isc.TABLE_NAME in (SELECT /*TOP 1*/ cc.TABLE_NAME 
		...
		private static void SetMetadatoChild(List<field> fields, field fi, string metadatoChild, ref List<field> allfields, SqlConnection connection)
		{
			...
			if (string.IsNullOrWhiteSpace(fi.MetadatoChild)
				//|| fields.Any(f => (f.Table != fi.Table || f.Name != fi.Name) && f.MetadatoChild == metadatoChild && !IsDropdown(f, fields, metadatoChild))
				)
			{
				//verfico se non ho già costruito la relazione per lo stesso campo
				//var otherForeighKeyForSameField = fields.Where(f=> !string.IsNullOrWhiteSpace(f.MetadatoChild) && f.Table == fi.Table && f.Name == fi.Name).FirstOrDefault();
				//if (otherForeighKeyForSameField != null) {

				//	fi.MetadatoChild = otherForeighKeyForSameField.MetadatoChild;
				//	fi.MetadatoExtendedChild = otherForeighKeyForSameField.MetadatoExtendedChild;
				...
				if (IsExtendingObject(metadatoChild, ref allfields, connection))
				{
					...
					//verifico se non ci sono stati altri campi nella stessa pagina che puntano alla stessa tabella
					otherForeignKeyInTable = fields.Where(f => !string.IsNullOrWhiteSpace(f.MetadatoChild)).Count(f =>
														 (
					...
					//verifico che la foreign key non punti a tabelle dei campi stessi
					otherFieldsInDataset = fields.Any(f =>
														f.Table == extended && //padre = meta figlio
														(f.Table != fi.Table || f.Name != fi.Name) //altro campo
													);
					totalAliases = otherForeignKeyInTable + (otherFieldsInDataset ? 1 : 0);
					if (totalAliases > 0)
					{
						//devo inserire i riferimenti successivi al primo con degli alias, sia nel dataset che nell'html, indicando poi nel js quale è la vera tabella da interrogare nelle query
		...
		private static List<field> GetChildentitiesFields(string pageId, string extendedObject, List<field> allfields, SqlConnection connection)
		{
			...
			var queryRelations = @"select pt.tablename as parenttable, 
									...
									ct.icon, ct.header, ct.forcealias,
									r.type, r.idapppages, r.calendartitle, r.calendarstart, 
									r.calendarstop, r.numrowsmandatory, r.buttoninsert, r.buttonedit ,r.buttondelete, r.position,
									r.idappfielddetail_parent
									from apprelation r
				...
				foreach (DataRow relation in relations.Rows)
				{
					...
					if (allfields.Any(af => af.Table == tablename && currentObjectKey.Any(k => k.Name == af.Name.Split('_')[0])))
					{
						...
						var fi = new field
						{
							...
							ForceAlias = int.Parse(string.IsNullOrEmpty(relation["forcealias"].ToString()) ? "0" : relation["forcealias"].ToString()),
						...
						if (fi.LookupFor == null)
						{
							...
							//nel caso di subentità si parla di chiavi primarie e non di foreign ckey e in questo caso non esistono suffissi (si usano solo per le foreignkey)
							...
							if (currentObjectKey.All(cok => childkey.Any(ck => ck/*.Split('_')[0]*/ == cok.Name)))
								fi.LookupFor = string.Join(" ", childkey.Where(ck => currentObjectKey.Any(cok => ck == cok.Name)).ToList());
						}
					...
					{
						Console.WriteLine("ERROR: è stata inserita una relazione tra una pagina figlia " + tablename + " e la pagina padre " + relation["parenttable"].ToString() +
		...
		private static List<field> GetParentEntitiesFields(string pageId, string extendedObject, List<field> allfields, SqlConnection connection)
		{
			...
			var queryRelations = @"select pt.tablename as parenttable, 
									...
									CASE WHEN r.editlistingtype is null OR ltrim(rtrim(r.editlistingtype)) = '' 
									THEN ct.editlistingtype ELSE r.editlistingtype END as editlistingtype, 
									ct.icon, r.type, r.idapppages, r.calendartitle, r.calendarstart, 
									r.calendarstop, r.numrowsmandatory, ct.header
									from apprelation r
									left outer join apppages pt on pt.idapppages = r.idapppages_parent
									left outer join apppages ct on ct.idapppages = r.idapppages
									left outer join apptab t on r.idapptab = t.idapptab
		...
		public static void SetReadonly(field fi, string nametag, string fullPathMetaPageJS, obj o, string editListingTypes, bool setReadonly = true)
		{
			if (fi.Readonlyfield)
			{
				SetAfterFill(fullPathMetaPageJS, "$('#" + nametag + "').prop(\"disabled\", " + setReadonly.ToString().ToLower() + ");\r\n\t\t\t\t$('#" +
					nametag + "').prop(\"readonly\", " + setReadonly.ToString().ToLower() + ");\r\n", o.Table + "_" + editListingTypes);
			}
		}
		...
		/// <param name="fields">gli altri campi della pagina</param>
		/// <param name="table">tabella puntata dal campo</param>
		/// <returns>true/false</returns>
		public static bool IsDropdown(field fi, List<field> fields, string table)
		{
			return table.EndsWith("kind") || table.EndsWith("status") || table.EndsWith("stato") || fi.Name.StartsWith("aa") || fi.ForceDropDown;
			//|| !string.IsNullOrWhiteSpace(fi.Master) || fields.Any(f => "id" + f.Master == fi.Name);
		}
		...
		public static string GetEditListingTypeForExtended(string extended_extendingObject, string extendingEditListingType)
		{
			if (!extended_extendingObject.Contains('_'))
				return "";
			//var extendedObject = extended_extendingObject.Substring(0, extended_extendingObject.IndexOf("_"));
			//var extendingObject = extended_extendingObject.Substring(extended_extendingObject.IndexOf("_") + 1, extended_extendingObject.Length - extended_extendingObject.IndexOf("_") - 1);
			//return extendingObject + (extendingEditListingType == "default" ? "" : "_" + extendingEditListingType);
			return extended_extendingObject.Split('_')[1] + (extendingEditListingType == "default" ? "" : "_" + extendingEditListingType);
		}
		...
		private static void PreparePageDataset(string pathMetaPage, string metadatoParent, string metadatoChild, string parentKey, string childkey, string current, bool minimal,
			...
			string extendedTableChild, List<string> fullTables, string fullPathMetaPageJS, int? colpos, string masterTable = null, bool skiplookup = false, string fkTitle = null)
		{
			...
			if (metadatoParent != metadatoChild)
			{
				...
				{
					datasetMetadatoParent = GetDatasetTable(metadatoParent, false, allfields, connection, editListingTypeParent, fullPathMetaPageJS);
				...
				if (metadatoChild != current)
				{
					...
					datasetMetadatoChild = GetDatasetTable(metadatoChild, minimal && !forceFull, allfields, connection, editListingType, fullPathMetaPageJS,
					...
					if (!string.IsNullOrEmpty(datasetMetadatoChild))
					{
						...
						//e in generale tutte le subentità non generano campi di lookup sulla entità principale
						if (!skiplookup && !islinkingobject && iteration == 2 && !isSubentity)
						{
									...
									foreach (var cf in descColumnColl)
									{
										...
										if (HaveKeyName(cf))
										{
											...
											if (lookupField.Name != cf.Name || IsTextField(lookupField))
											{
												...
												SetMetadatoChild(fields, cf, lookupTable, ref allfields, connection);
										...
										else
										{
											...
											if (metadatoChildOriginal != textField.Table)
							...
							else
							{
								...
								//SOLO SE si tratta di una relazione da che parte da una foreign key e il suo campo di lookup e NON se è una relazione con una subentità
										...
										foreach (var tf in textFields)
											if (HaveKeyName(tf) && !IsEnum(tf, allfields, connection))
											{
												...
												if (newTextfields.Any(ntf => !HaveKeyName(ntf)))
												{
													...
													secondaryTableTextKey, metadatoChild, true, fields, allfields, "default", editListingType, null, connection,
													false, //gli dico se l'id fa da collegamento imponendo come se fi.table fosse una tabella di collegamento
													iteration + 1, null, fullTables, fullPathMetaPageJS, colpos, null, iteration > 2, fkTitle);
												}
											}
						...
						if (!skiplookup && !minimal)
						{
									...
									)
								{
									...
									if (!string.IsNullOrEmpty(tablechild) && !string.IsNullOrEmpty(fi.Name))
									{
										...
										if (
											...
											//se c'è un altro campo che ha generato autochoose sulla stessa tabella devo aggiungere sempre l'alias
											fields.Any(f => (f.MetadatoChild ?? "").Split(new string[] { "_alias" }, StringSplitOptions.None)[0] == tablechild
															&& !IsDropdown(f, fields, tablechild))
											)
											fields.Add(fi);
										...
										PreparePageDataset(pathMetaPage, isext ? extendedTableChild : metadatoChild, childobject, fi.Name, GetIdByForeignKey(fi.Name, fi.Table),
											...
											iteration + 1, isext ? null : extendedchildobject, fullTables, fullPathMetaPageJS, fi.SortList, null, skipLookup, fi.Title);
									}
								}
							...
							if (!string.IsNullOrEmpty(pageid))
							{
								...
								foreach (var r in relationFields)
								{
									...
									var isExtendingObject = IsExtendingObject(r.Name.Substring(2), ref allfields, connection);
									if (isExtendingObject)
									{
										//..salvo l'esteso nel campo fi.MetadatoChild con l'alias che mi serve
										SetMetadatoChild(fields, r, r.Name.Substring(2), ref allfields, connection);
										fields.Add(r);
									}
									...
									//varifico se è un figlio ricorsivo
									var kc = GetIdByTable(tc, allfields, connection, true, r.EditListingType);
									if (kc.Any(k => k.StartsWith("parid")))
									{
										ckString = GetChildKeysString(metadatoChild, tc, editListingType, r.EditListingType, mdck.Split(' ').ToList(), allfields, connection);
									...
									if (r.ForceAlias > 0)
									{
										r.MetadatoChild = tc + "_alias" + r.ForceAlias.ToString();
										fields.Add(r);
									}

									//se è figlio di una relazione 1 a 1 (ma non di una griglia con ricerca perchè gestisce la parentrow autonomamente) 
									...
									{
										var fullPathMetaDatoJS = clientFolder + metapageFolder + tc + "/meta_" + tc + ".js";
										SetMetadatoJs(fullPathMetaDatoJS, tc, tc, currentApp, GetIdFieldsByTable(tc, allfields, connection, false), isExtendingObject, allfields, connection);

										var getnewrowinside =
													"if (editType === \"" + r.EditListingType + "\") {\r\n" +
										...
										ReplaceStringInFile(fullPathMetaDatoJS, "//$getNewRowInside$\r\n", getnewrowinside);
									...
									PreparePageDataset(pathMetaPage, metadatoChild, r.MetadatoChild ?? tc, mdck, ckString.Trim(),
										...
										iteration + 1, r.MetadatoExtendedChild, fullTables, fullPathMetaPageJS, null, null, skiplookup, r.Title);
								}
							}
						}
						...
						if (!string.IsNullOrEmpty(extendedTableChild) && !metadatoChild.EndsWith("view"))
						{
							...
							PreparePageDataset(pathMetaPage, extendedTableChild, metadatoChild, keysExtendedString.Trim(), keysString.Trim(), metadatoChild, false, fields, allfields, editListingType,
								editListingTypeExtended, null, connection, false, iteration + 1, "", fullTables, fullPathMetaPageJS, 1, null, iteration > 2, fkTitle);
						}
				...
				if ((!string.IsNullOrEmpty(datasetMetadatoParent) || metadatoParent == current) && (!string.IsNullOrEmpty(datasetMetadatoChild) || metadatoChild == current))
				{
					...
					{
						//è una tendina a cascata rispetto alla tabella master
						var masterkey = (GetPrincipalKey(masterTable.Replace("defaultview", ""), "", allfields, connection, false) ?? new field()).Name;

						if (!string.IsNullOrWhiteSpace(masterkey))
						{
							//cerco la chiave interna o esterna 
							var childForeignKey = allfields.Where(af => af.Table == metadatoChild && af.EditListingType == editListingType
										&& masterkey == af.Name.Split('_')[0] || (af.Name.Contains("_") && masterkey == af.Name.Split('_')[1])).FirstOrDefault();
							if (childForeignKey == null)
							{
								Console.WriteLine("ERROR: No riesco a trovare nella tabella " + metadatoChild + " la chiave o chiave esterna corrispondente a " + masterkey + " di " + masterTable);
							}

							datasetRelation += "     <msdata:Relationship name=\"FK_" + metadatoChild + "_" + masterTable + "_" + masterkey + "\" msdata:parent=\"" +
												masterTable + "\" msdata:child=\"" + metadatoChild + "\" msdata:parentkey=\"" + masterkey + "\" msdata:childkey=\"" + childForeignKey.Name + "\" />\r\n";
						}
						else
						{
							Console.WriteLine("ERROR: " + metadatoChild + " è in una tendina in cascata con " + masterTable + " ma " + masterTable + " non ha una chiave principale da condividere.");
						}
					}
		...
		/// <param name="connection"></param>
		/// <returns></returns>
		private static string GetDatasetTable(string tablename, bool minimal, List<field> allfields, SqlConnection connection, string editListingType, string fullPathMetaPageJS,
			string parentkey = null, string masterTable = null)
		{
			...
			if (tablename.EndsWith("view")) {
				...
				if (!string.IsNullOrWhiteSpace(originalTable))
				{
					...
					if (!tableFields.Any(tf => tf.IsKey))
						if (verbose)
							Console.WriteLine("ERROR: per la vista " + tablename + " non è stato trovato il campo chiave");
				}
				else
					if (verbose)
					Console.WriteLine("ERROR: per la vista " + tablename + " non è stato trovato il campo chiave perchè non è stata trovata la tabella di partenza");
			}
			...
			if (minimal)
			{
				...
				else
				{
					...
					if (txtf != null)
						...
						if (verbose)
						Console.WriteLine("WARNING: per la vista " + tablename + " non è stato trovato il campo testuale");
				}

				//3-eventuali foreign key alla tabella master
				if (!string.IsNullOrWhiteSpace(masterTable))
				{
					var keyMaster = GetIdByTable(masterTable, allfields, connection, true);
					var keyMasterfield = tableFields.Where(af =>
							keyMaster.Any(km => km == af.Name.Split('_')[0] || (af.Name.Contains("_") && km == af.Name.Split('_')[1])) &&
							!fields.Any(f => f.Name == af.Name)
							);
					fields.AddRange(keyMasterfield);
				}
			}
			...
			return GetDatasetTable(tablename, tableAlias, minimal, fields, fullPathMetaPageJS, parentkey);
		}
		...
		/// <returns></returns>
		private static string GetDatasetTable(string tablename, string tableAlias, bool minimal, List<field> fields, string fullPathMetaPageJS, string parentkey = null)
		{
				...
				foreach (var pk in parentkey.Split(' '))
				{
					if (!fields.Any(f => f.IsKey && (f.Name.Split('_')[0] == pk || f.Name == pk)))
					{
						...
						if (fields.Any(f => f.Name == fk.Name))
						{
							...
							//devo fare in modo che la pagina ne imponga l'inserimento
							if (!fk.DenyNull)
							{
			...
			if (linkingids.Any())
			{
				foreach (var linkingid in linkingids)
				{
					...
					if (linkedtable.Contains("_"))
					{
						...
						Console.WriteLine("WARNING: L'elemento collegato dall'oggetto di collegamento " + linkingtable + " " + linkingeditlisttype + " che è " + linkedtable + " " + linkededitlisttype +
				...
				Console.WriteLine("ERROR: L'elemento collegato dall'oggetto di collegamento " + linkingtable + " " + linkingeditlisttype + " non è individuabile, verificare la visibilità nella lista di almeno uno dei campi");
		...
		private static Tuple<string, string, string, string, bool, bool> GetException(string table, string key, string linkedTable, string linkedcolumn)
		{
			...
			(ex.Item1 == table && key == null && linkedTable == null && linkedcolumn == null) || //IsExtending o IsPrincipalKey
			(ex.Item1 == table && ex.Item2 == key && linkedTable == null && linkedcolumn == null) || //isInternalAndExternalKey
			(ex.Item2 == key && table == null && linkedTable == null && linkedcolumn == null) || //getTablebyId
			(ex.Item3 == linkedTable && ex.Item4 == linkedcolumn && table == null && key == null) //ForeignKeyExceptions
			).FirstOrDefault();
		}
		...
		{
			var originalTable = "";
			if (allfields.Any(af => af.Table == tablename.Replace(editListingType + "view", "") && !af.Table.EndsWith("view")))
				originalTable = tablename.Replace(editListingType + "view", "");
			else
		...
		private static field GetPrincipalKey(string table, string edittype, List<field> allfields, SqlConnection connection, bool checkmissing = true, bool isView = false)
		{

			var realtable = table.Split(new string[] { "_alias" }, StringSplitOptions.RemoveEmptyEntries)[0];

			if (checkmissing)
				GetMissingFields(table, null, ref allfields, connection, true);
			...
			foreach (var key in keys)
				if (IsPrincipalKey(key, realtable, isView))
					return key;
			...
			obj currentObj, SqlConnection connection)
		{
			...
			if (!string.IsNullOrEmpty(extendingObjectTable))
			{
				...
				joinView += " INNER JOIN " + extendingObjectTable + " ON ";
				var addend = false;
				foreach (var key in keys)
				{
			...
			foreach (var f in fields.OrderBy(af => af.Table).ThenBy(af => af.Name))
			{
					...
					)
				{
					...
					if (!string.IsNullOrEmpty(tableChild))
					{
						...
						hadAlias = IsDropdown(f, fields, tableChild) && !(f.Table == table && (f.IsKey || f.Name == textfield.Name));
						if (!hadAlias)
							f.ViewAlias = null;
						...
						if (isLinkingObj || f.IsLinkingObj)
						{
							...
							foreach (var txtField in joinfields.Where(jf => jf.Table != table))
							{
								var alias = txtField.Table + specification + "_" + txtField.Name;
								var column = (hasAlias ? "" : schemaJoin) + txtField.Table + (txtField.Table != tableChild ? "_" + tableChild : "") + specification + "." + txtField.Name;
							...
							if (joinfields.Any(jf => jf.Table != tableChild))
							{
								...
								var extendedTableChild = joinfields.First(jf => jf.Table != tableChild).Table;
								...
								joinView += " LEFT OUTER JOIN " + schemaJoin2 + extendedTableChild + " AS " + tableAlias + specification +
											" ON " + (hasAlias ? "" : schemaJoin) + tableChild + specification + "." + idTableChild + " = " + tableAlias + specification + "." + idTableChild;
						...
						viewFields.Add(RenewField(f, f, f.SortList, f.Detailvisible, false));
						Console.Write(".");
					}
					else
					{
			...
			dropdown = " " + dropdown + " as dropdown_title";
			createView += dropdown;
			...
			if (f.Textfield > 0)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							dropdowns.Add(new textField() { Column = "isnull('" + prefix + "' + CAST( " + column + " AS NVARCHAR(64)) + ' " + f.Title.Replace("'", "''") + "' ,'')", FirstIndex = firstIndex, SecondIndex = secondIndex });
						}
						else
					...
					if (HaveKeyName(f) && !IsEnum(f, allfields, connection))
					{
						...
						foreach (var tf in textFields)
						{
							if (HaveKeyName(tf) && !IsEnum(tf, allfields, connection))
							{
								...
								{
									output.Add("!" + f.Name + "_" + secondaryTableText + "_" + ttf.Name);
								}
							}
							else
							{
		...
		private static string GetGrid(obj o, tab tab, field fi, List<field> fields, List<field> allfields, string fullPathMetaPageJS, string fullPathTest, bool mdlexcludegroup, SqlConnection connection)
		{
			...
			var gridId = "grid_" + gridtable + "_" + edittype;// + fields.IndexOf(fi);
			...
			switch (fi.RelationType)
			{
				default:
					...
					if (IsExtendingObject(gridtable, ref allfields, connection))
					{
						...
						gridtable = gridtable.Split('_')[0];// fi.MetadatoChild;
					}
				...
				case "checkbox":
					...
					var linkedtable = gridtable.Replace(o.Table, ""); //qui ci va il nome della tabella COLLEGATA (il vocabolario delle voci)

					gridFields = allfields.Where(gf => gf.Table == linkedtable && gf.EditListingType == edittype && gf.ListVisible).ToList();

					//...disegno la grid
					formString += "		<div id=\"" + gridId + "\" data-tag=\"" + linkedtable + "." + edittype + "." + edittype + "\" data-custom-control=\"checklist\" " +
						(string.IsNullOrEmpty(radiotag) ? "" : "data-mdlconditionallookup=\"" + radiotag + "\"") + " ></div>\r\n";

					//test
					SetArrayInputTest(fullPathTest, "{ tag: '" + linkedtable + "." + edittype + "." + edittype + "', value:null, " +
						(fi.Numrowsmandatory > 1 ? "rows:" + fi.Numrowsmandatory + "," : "") + " columns:" + gridFields.Count.ToString() + "," +
				...
				case "cerca": //per le tabelle di collegamento 
					...
					if (IsLinkingObject(gridtable, ref allfields, connection) || linkidManual != null)
					{
						...
						var linkid = linkidManual ?? allfields.Where(f => f.IsKey && f.ListVisible && f.Table == linktable && f.EditListingType == edittype && HaveKeyName(f)).FirstOrDefault();
						...
						if (linkid != null)
						{
							...
							//test
							{
								...
								//se è un oggetto estendente devo prendere solo i valori che si trovano su entrambe le tabelle
								var joinstring = "";
								var linkedtextfield = GetTextFieldByTable(linkedtable, allfields, connection, false, string.IsNullOrWhiteSpace(linkid.Listtype) ? "default" : linkid.Listtype).FirstOrDefault();
								...
								var schema = GetSchemaByTable(table, allfields, connection, false);
								var q = "select " + (!string.IsNullOrWhiteSpace(schema) ? schema + "." : "") + linkedtextfield.Table + "." + linkedtextfield.Name +
								...
								if (colonneTest.Rows.Count > 0)
								{
									string val = colonneTest.Rows[0][linkedtextfield.Name].ToString();
									if (string.IsNullOrWhiteSpace(val) && colonneTest.Rows.Count > 1)
										foreach (DataRow r in colonneTest.Rows)
											if (!string.IsNullOrWhiteSpace(r[linkedtextfield.Name].ToString()))
											{
												val = r[linkedtextfield.Name].ToString();
												break;
											}
									val = val.Replace("'", "\\'");
			...
			if (fi.MasterField != null)
			{
				...
				if (masterfield != null)
					SetInsertClick(fullPathMetaPageJS, "if (!$('#" + masterfield.Table + "_" + masterfield.EditListingType + "_" + masterfield.Name +
						...
						"\t\t\t\t\treturn this.showMessageOk('Prima devi selezionare un valore per il campo " + masterfield.Title + "');\r\n" +
						"\t\t\t\t}\r\n");
			}
			...
			string metadatoChild, string listtype, string metadatochildWithText, field txtField, string fullPathMetaPageJS, string fullPathTest, string fullPathTestParams, SqlConnection connection)
		{
			...
			if (fi.IsKey)
				SetAfterRowSelect(fullPathMetaPageJS, "$('#" + nametag + "').prop(\"disabled\", false);\r\n\t\t\t\t$('#" +
					nametag + "').prop(\"readonly\", false);\r\n", null, o.Table, editListingTypes);
			...
			string metadatoChild, string listtype, string metadatochildWithText, field txtField, string fullPathMetaPageJS, string fullPathTest, string fullPathTestParams, SqlConnection connection)
		{
			...
			if (string.IsNullOrWhiteSpace(fi.Testvalue))
			{
				...
				if (isView)
				{
					...
					{
						Console.WriteLine("ERROR: il dropdowngrid " + metadatoChild + " risultava avere la vista ma non è stata trovata per costruire il test");
					}
				}
				else
				{
			...
			if (!string.IsNullOrWhiteSpace(fi.Master))
			{
				...
				var masterkeys = GetPrincipalKey(masterTable, null, allfields, connection, true, true);
				var childkey = GetChildKeysString(masterTable, fi.MetadatoChild, "default", fi.EditListingType, new List<string>() { masterkeys.Name }, allfields, connection);

				SetAfterRowSelect(fullPathMetaPageJS, "if (t.name === \"" + fi.Master + "\"){\r\n" +
