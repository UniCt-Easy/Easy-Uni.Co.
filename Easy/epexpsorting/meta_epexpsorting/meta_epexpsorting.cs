...
namespace meta_epexpsorting {//meta_classtreecreddebi//
	/// <summary>
	/// MetaData for classtreecreddebi
	/// </summary>
	public class Meta_epexpsorting : Meta_easydata {
		public Meta_epexpsorting(DataAccess Conn, MetaDataDispatcher Dispatcher):
			base(Conn, Dispatcher, "epexpsorting") {
		...
		protected override Form GetForm(string FormName) {
			...
			if (FormName=="default") {
				Name = "Classificazione Impegno di Budget";
				return MetaData.GetFormByDllName("epexpsorting_default");
			}
			return null;
		}			
		
		public override void SetEntityDetail(DataRow SourceRow) {
			DS.Tables["epexpsorting"].ExtendedProperties["importototale"]= 
		...
		public override bool IsValid(DataRow R, out string errmess, out string errfield){            
            ...
            if (CfgFn.GetNoNullInt32(R["idsor"]) == 0) {
                errmess = "La selezione della classificazione Ë obbligatoria";
                errfield = "idsor";
                return false;
            }
			if (R["kind"].ToString().ToUpper()=="N" && CfgFn.GetNoNullDecimal(R["amount"])< 0){
				errmess="Attenzione! L'importo di una classificazione di tipo Normale non puÚ essere negativo";
				errfield="amount";
				return false;
			}
			return true;
		...
		public override DataRow Get_New_Row(DataRow ParentRow, DataTable T) {
			RowChange.MarkAsAutoincrement(
				...
				7,
				false);
            RowChange.SetMySelector(T.Columns["rownum"],"idepexp",0);
